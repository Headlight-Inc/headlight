...
async function runStream(
	...
	if (!Stream) throw new Error(`${mode === "compress" ? "CompressionStream" : "DecompressionStream"} is not available.`);

	const stream = new ReadableStream<Uint8Array>({
		start(controller) { controller.enqueue(data); controller.close(); }
	});
	const transformed = stream.pipeThrough(new Stream(format) as any) as ReadableStream<Uint8Array>;
...
export const gunzip = (data: Uint8Array) => runStream(data, "gzip", "decompress");
export const deflate = (data: Uint8Array) => runStream(data, "deflate", "compress");
export const inflate = (data: Uint8Array) => runStream(data, "deflate", "decompress");
