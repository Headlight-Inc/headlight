...
const normalizeProject = (row) => ({
	...
	lastCrawlScore:
		row.last_crawl_score === null || row.last_crawl_score === undefined
			? null
			: Number(row.last_crawl_score),
...
const matchesPageFilter = (page, query) => {
	if (
		query.status &&
		Number(page.statusCode || page.fullData?.statusCode || 0) !==
			Number(query.status)
	)
		return false;
...
async function authenticateApiKey(turso, req, scopes = []) {
	...
	if (scopes.some((scope) => !keyScopes.includes(scope))) {
		return {
			error: {
				status: 403,
				body: { error: "API key does not have the required scope." },
			},
		};
	}
	...
	if (tracker.count > Number(row.rate_limit_per_minute || 100)) {
		return {
			error: { status: 429, body: { error: "API rate limit exceeded." } },
		};
	}
...
export async function notifyProjectWebhooks(
	turso,
	projectId,
	eventName,
	payload,
	...
	const matchingHooks = result.rows.filter((row) =>
		jsonParse(row.events_json, []).includes(eventName),
	);
	await Promise.allSettled(
		matchingHooks.map((row) => sendWebhook(turso, row, eventName, payload)),
	);
}

export async function registerPhaseERoutes(app, turso) {
	console.log(
		`[PhaseEApi] Initializing routes. Agents in registry: ${AGENT_REGISTRY.size}`,
	);
	...
	app.post("/api/ai/chat", async (req, res) => {
		...
		try {
			const aiResponse = await completeAI({
				...
				systemPrompt:
					systemPrompt ||
					"You are Headlight, an SEO and GEO assistant. Be concise, practical, and data-aware.",
				maxTokens: Number(maxTokens || 600),
				format,
			});
			return res.json({
				text: aiResponse.text || "No AI providers were available.",
			});
		} catch (error) {
			return res
				.status(500)
				.json({ error: error.message || "AI chat failed." });
		}
	});
	...
	app.post("/api/internal/projects/:projectId/api-keys", async (req, res) => {
		...
		const normalizedScopes =
			Array.isArray(scopes) && scopes.length ? scopes : ["read"];
		await turso.execute({
	...
	app.post("/api/internal/projects/:projectId/webhooks", async (req, res) => {
		...
		await turso.execute({
			...
			args: [
				...
				String(url),
				JSON.stringify(
					Array.isArray(events) && events.length ? events : ["crawl.completed"],
				),
	...
	const withApiAuth =
		...
		async (req, res, next) => {
			try {
				...
				res
					.status(500)
					.json({ error: error.message || "API authentication failed." });
			}
		};
	...
	app.get("/api/v1/projects/:id", withApiAuth(["read"]), async (req, res) => {
		if (req.params.id !== req.apiKey.projectId) {
			return res
				.status(403)
				.json({ error: "API key cannot access this project." });
		}
		const result = await turso.execute({
			sql: `SELECT * FROM projects WHERE id = ? LIMIT 1`,
			args: [req.params.id],
		...
		return res.json({
			data: result.rows[0] ? normalizeProject(result.rows[0]) : null,
		});
	});

	app.get(
		"/api/v1/projects/:id/crawls",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
			...
			return res.json({
				data: result.rows.map(normalizeRun),
				meta: {
					total: result.rows.length,
					offset: 0,
					limit: result.rows.length,
	...
	app.get(
		"/api/v1/projects/:id/crawls/latest",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
			...
			return res.json({
				data: result.rows[0] ? normalizeRun(result.rows[0]) : null,
			});
		},
	);

	app.get(
		"/api/v1/projects/:id/crawls/:crawlId",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
	...
	app.get(
		"/api/v1/projects/:id/crawls/:crawlId/pages",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
			...
			const sortDirection =
				String(req.query.order || "desc") === "asc" ? 1 : -1;
			const result = await turso.execute({
			...
			let pages = result.rows
				...
				.filter((page) => matchesPageFilter(page, req.query));
			...
			return res.json({
				data: paginated,
				meta: toPaginationMeta(pages.length, offset, limit),
			});
		},
	);

	app.get(
		"/api/v1/projects/:id/crawls/:crawlId/pages/:encodedUrl",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
			...
			return res.json({
				data: result.rows[0] ? normalizeInsight(result.rows[0]) : null,
			});
		},
	);

	app.get(
		"/api/v1/projects/:id/crawls/:crawlId/issues",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
			...
			return res.json({
				data: result.rows.map(normalizeIssueCluster),
				meta: {
					total: result.rows.length,
					offset: 0,
					limit: result.rows.length,
	...
	app.get(
		"/api/v1/projects/:id/crawls/:crawlId/compare/:otherCrawlId",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.id !== req.apiKey.projectId) {
				return res
			...
			const [leftRun, rightRun] = await Promise.all([
				turso.execute({
					sql: `SELECT * FROM crawl_runs WHERE id = ? LIMIT 1`,
					args: [req.params.crawlId],
				}),
				turso.execute({
					sql: `SELECT * FROM crawl_runs WHERE id = ? LIMIT 1`,
					args: [req.params.otherCrawlId],
				}),
			]);
			...
			return res.json({
				data: {
					...
					previous,
					scoreDelta:
						(current?.summary?.healthScore || 0) -
						(previous?.summary?.healthScore || 0),
					issueDelta:
						(current?.issueOverview?.length || 0) -
						(previous?.issueOverview?.length || 0),
				},
			});
		},
	);
	...
	app.get(
		...
		async (req, res) => {
			...
			return res.json({
				data: result.rows.map(normalizeTask),
				meta: {
					total: result.rows.length,
					offset: 0,
					limit: result.rows.length,
		...
		async (req, res) => {
			...
			await turso.execute({
				...
				args: [
					...
					JSON.stringify(
						Array.isArray(req.body?.affectedUrls) ? req.body.affectedUrls : [],
					),
			...
			const created = {
				...
				source: "api",
				affectedUrls: Array.isArray(req.body?.affectedUrls)
					? req.body.affectedUrls
					: [],
				createdAt: now,
				updatedAt: now,
			};
	...
	app.get("/api/internal/projects/:projectId/agents", async (req, res) => {
		...
		try {
			...
			const configs = new Map(
				configsRes.rows.map((r) => [String(r.agent_id), r]),
			);
			const lastRuns = new Map(
				lastRunsRes.rows.map((r) => [String(r.agent_id), r]),
			);
	...
	app.get(
		"/api/internal/projects/:projectId/agents/:agentId/runs",
		async (req, res) => {
	...
	app.patch(
		"/api/internal/projects/:projectId/agents/:agentId",
		async (req, res) => {
			const { projectId, agentId } = req.params;
	...
	app.post(
		"/api/internal/projects/:projectId/agents/:agentId/run",
		async (req, res) => {
			const { projectId, agentId } = req.params;
	...
	app.get(
		...
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.projectId !== req.apiKey.projectId)
	...
	app.get(
		"/api/v1/projects/:projectId/agents/:agentId/runs",
		withApiAuth(["read"]),
		async (req, res) => {
			if (req.params.projectId !== req.apiKey.projectId)
				return res.status(403).json({ error: "Access denied" });
	...
	app.post(
		"/api/v1/projects/:projectId/agents/:agentId/run",
		withApiAuth(["write"]),
		async (req, res) => {
			if (req.params.projectId !== req.apiKey.projectId)
				return res.status(403).json({ error: "Access denied" });
			...
			try {
				...
				executeAgent(
					...
					console.error(`[AgentManualRun:${agent.id}] Error:`, err),
				);
