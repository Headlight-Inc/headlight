...
export const IssueFixerAgent = {
	...
	execute: async (context) => {
		...
		if (issues.length === 0) {
			return {
				status: "success",
				summary: "No critical or high issues found. Great job!",
				findings: [],
			};
		}
		...
		for (const issue of issues) {
			...
			const affectedUrls = JSON.parse(
				String(issue.affected_urls_json || "[]"),
			).slice(0, 3);
			const prompt = `
			...
			try {
				...
				console.error(
					`[IssueFixerAgent] Failed to parse AI fix for ${issue.id}:`,
					err,
				);
				continue;
			}
