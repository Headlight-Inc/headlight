...
export const RankGuardAgent = {
    ...
    execute: async (context) => {
        ...
        if (sessionsRes.rows.length < 2) {
            return { status: 'partial', summary: 'Need at least 2 crawl runs with GSC data to monitor rank shifts.' };
        }
        ...
        const currentPagesRes = await turso.execute({ sql: pagesQuery, args: [currentSession] });
        const previousPagesRes = await turso.execute({ sql: pagesQuery, args: [previousSession] });
        ...
        for (const page of currentPages) {
            ...
            const clickDropPct = prevClicks > 0 ? (prevClicks - currClicks) / prevClicks : 0;

            if (posDrop >= 5 || clickDropPct > 0.5) {
                // Correlate with crawl changes
                const correlations = [];
                if (String(page.content_hash) !== String(prev.content_hash)) correlations.push('Content changed');
                if (page.status_code !== prev.status_code) correlations.push(`Status code changed: ${prev.status_code} -> ${page.status_code}`);
                if (String(page.canonical) !== String(prev.canonical)) correlations.push('Canonical URL changed');
                if (String(page.meta_robots) !== String(prev.meta_robots)) correlations.push('Meta robots changed');
                if ((Number(prev.health_score || 100) - Number(page.health_score || 100)) > 20) correlations.push('Health score dropped significantly');
        ...
        if (drops.length === 0) {
            return { status: 'success', summary: 'Search rankings are stable. No significant drops detected.', findings: [] };
        }
        ...
        for (const drop of topDrops) {
            ...
            let analysis = { diagnosis: 'Rank fluctuation.', recoverySteps: ['Monitor position and content quality.'], urgency: 'High' };
            ...
            await turso.execute({
                ...
                args: [
                    ...
                    description,
                    'todo',
                    String(analysis.urgency).toLowerCase() === 'critical' ? 'critical' : 'high',
                    'seo',
                    'agent:rank-guard',
