...
export const LinkWatcherAgent = {
    ...
    execute: async (context) => {
        ...
        if (sessionsRes.rows.length < 2) {
            return { status: 'partial', summary: 'Need at least 2 crawl runs with link data to monitor changes.' };
        }
        ...
        const currentPagesRes = await turso.execute({ sql: linkQuery, args: [currentSession] });
        const previousPagesRes = await turso.execute({ sql: linkQuery, args: [previousSession] });
        ...
        const opportunities = currentPages
            .filter(p => Number(p.health_score || 0) > 80 && Number(p.internal_pagerank || 0) < 0.05)
            .sort((a, b) => Number(b.health_score) - Number(a.health_score))
            .slice(0, 5);

        if (shifts.length === 0 && opportunities.length === 0) {
            return { status: 'success', summary: 'Link profile is stable.', findings: [] };
        }
        ...
        for (const shift of shifts.slice(0, 3)) {
            ...
            let analysis = { assessment: 'Site structure changed.', actions: ['Add more internal links to this page.'] };
