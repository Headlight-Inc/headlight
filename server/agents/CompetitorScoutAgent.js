...
export const CompetitorScoutAgent = {
	...
	execute: async (context) => {
		...
		if (competitors.length === 0) {
			return {
				status: "partial",
				summary: "No competitors configured for this project.",
			};
		}
		...
		for (const competitor of competitors.slice(0, 3)) {
			...
			const prevResults =
				context.previousRun?.resultJson?.competitorSnapshots?.[
		...
		return {
			...
			resultExtra: {
				competitorSnapshots: Object.fromEntries(
					findings.map((f) => [f.data.competitorId, f.data.newUrls]),
				),
			},
		};
	},
