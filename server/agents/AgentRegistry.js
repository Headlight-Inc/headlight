...
function shouldRunAgent(agentDef, projectId, lastRun) {
    ...
    if (schedule === 'weekly') {
        ...
        if (lastRunDate && (now.getTime() - lastRunDate.getTime() < 6 * 24 * 60 * 60 * 1000)) return false;
...
export async function runDueAgents(turso) {
    ...
    try {
        ...
        const projectsRes = await turso.execute('SELECT id FROM projects WHERE auto_crawl_enabled = 1');
        ...
        for (const project of projects) {
            ...
            for (const [agentId, agentDef] of AGENT_REGISTRY.entries()) {
                ...
                if (shouldRunAgent(agentDef, projectId, lastRun)) {
                    console.log(`[AgentRegistry] Agent ${agentId} is due for project ${projectId}`);
...
export async function runAgentByEvent(turso, eventType, eventData) {
    ...
    console.log(`[AgentRegistry] Checking event-based agents for "${eventType}" on project ${projectId}`);

    for (const [agentId, agentDef] of AGENT_REGISTRY.entries()) {
        if (agentDef.trigger === 'event' && agentDef.event === eventType) {
            console.log(`[AgentRegistry] Triggering agent ${agentId} due to event ${eventType}`);
            await executeAgent(agentDef, projectId, turso, completeAI, aiBatch);
        }
    }
}
