...
export const TrafficAnalystAgent = {
    ...
    execute: async (context) => {
        ...
        if (sessionsRes.rows.length < 2) {
            return { status: 'partial', summary: 'Need at least 2 sessions with traffic data to analyze trends.' };
        }
        ...
        const currentPagesRes = await turso.execute({ sql: trafficQuery, args: [currentSession] });
        const previousPagesRes = await turso.execute({ sql: trafficQuery, args: [previousSession] });
        ...
        for (const page of currentPages) {
            ...
            const sessionDropPct = prevSessions > 20 ? (prevSessions - currSessions) / prevSessions : 0;

            if (sessionDropPct > 0.5) {
                // Correlate with status codes/health
                const issues = [];
                if (page.status_code !== 200) issues.push(`Status code is ${page.status_code}`);
                if (Number(page.health_score || 100) < 60) issues.push('Low health score');
        ...
        const siteWideDrop = totalPrevSessions > 100 ? (totalPrevSessions - totalCurrentSessions) / totalPrevSessions : 0;
        if (siteWideDrop > 0.2) {
            anomalies.push({
                type: 'site_wide_drop',
        ...
        if (anomalies.length === 0) {
            return { status: 'success', summary: 'Traffic levels are healthy and stable.', findings: [] };
        }
        ...
        for (const anomaly of anomalies.filter(a => a.type === 'page_traffic_drop').slice(0, 3)) {
            const taskId = randomUUID();
            await turso.execute({
                sql: `INSERT INTO crawl_tasks (
                    id, project_id, title, description, status, priority, 
