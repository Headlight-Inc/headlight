	...
	try {
		...
		for (const wordlistPath of candidates) {
			...
			if (Array.isArray(words) && words.length > 0) {
				return new Set([
					...fallback,
					...words.map(normalizeDictionaryWord).filter((w) => w.length >= 2),
				]);
			}
		}
	} catch (e) {
		console.error(
			"[loadDictionary] Error loading bundled wordlist:",
			e.message,
		);
	}
...
function maybeBaseForms(word) {
	...
	if (word.endsWith("ed") && word.length > 4)
		forms.push(word.slice(0, -2), `${word.slice(0, -2)}e`);
	return forms;
}
...
const GENERIC_LINK_TEXT_PATTERN =
	/^(click here|read more|learn more|more|here|link|this|download|submit)$/i;
...
parentPort.on("message", (task) => {
	const { html, staticHtml, url, depth, baseHostname, config, robotsRules } =
		task;
	...
	try {
		...
		if ($static) {
			...
			const textDiffPercent = Math.round(
				(diffLen / Math.max(staticText.length, 1)) * 100,
			);

			const jsOnlyLinks = [...renderedLinks].filter(
				(l) => !staticLinks.has(l),
			).length;
			...
			const addedWords = renderedWords
				.filter((word) => !staticWords.includes(word))
				.slice(0, 40)
				.join(" ");
			const removedWords = staticWords
				.filter((word) => !renderedWords.includes(word))
				.slice(0, 40)
				.join(" ");
			...
			jsRenderDiff = {
				...
				criticalContentJsOnly:
					staticText.length < 200 && renderedText.length > 500,
				hydrationMismatch:
					Math.abs(renderedWords.length - staticWords.length) > 150 &&
					staticWords.length > 0,
		...
		const metaRobotsTags = $('meta[name="robots"]')
		...
		const sentenceCount =
			textContent.split(/[.!?]+/).filter((s) => s.trim().length > 0).length ||
			1;
		...
		const flesch =
			wordCount > 0
				...
				: 0;
		const readability =
			flesch > 80 ? "Easy" : flesch > 60 ? "Standard" : "Difficult";
		const textRatio =
			html.length > 0
				? ((textContent.length / html.length) * 100).toFixed(2)
				: 0;
		...
		const hasArticleSchema = schemaTypes.some((type) =>
		...
		const hasSkipLink =
			$(
				'a[href="#main-content"], a[href="#content"], a[href="#main"], a.skip-link, a.skip-nav',
			).length > 0;
		...
		const viewportMaxScale1 = /maximum-scale\s*=\s*1(\.0)?/i.test(
			viewportContent,
		);
		...
		$("script[src]").each((_, el) => {
			const src = $(el).attr("src") || "";
			try {
				const scriptHost = new URL(src, url).hostname
					.replace(/^www\./i, "")
					.toLowerCase();
		...
		const exposedEmails = Array.from(
			new Set(
				(textContent.match(emailPattern) || []).filter(
					(email) =>
						!email.includes("example.com") && !email.includes("@schema.org"),
				),
			),
		);
		...
		const privacyPageLinked =
			$('a[href*="privacy"], a[href*="privacy-policy"]').length > 0;
		const termsPageLinked =
			$(
				'a[href*="terms"], a[href*="terms-of-service"], a[href*="terms-and-conditions"]',
			).length > 0;
		const hasCookieBanner =
			...
			$(
				'script[src*="cookiebot"], script[src*="onetrust"], script[src*="cookieconsent"], script[src*="trustarc"], script[src*="quantcast"]',
			).length > 0;
		...
		const hasSessionId = /[?&](sid|session|phpsessid|jsessionid|token)=/i.test(
			parsedUrl.search,
		);
		...
		const genericAnchorCount = anchorTexts.filter((text) =>
			GENERIC_LINK_TEXT_PATTERN.test(text),
		).length;
		const anchorTextDiversity = new Set(anchorTexts).size;

		const isSoft404 =
			wordCount < 50 &&
			/(page not found|404|not found|does not exist|no longer available|couldn't find)/i.test(
				textContent,
			);
		const hasFavicon =
			$(
		...
		const hasCharset =
			Boolean(charsetValue) || $('meta[http-equiv="Content-Type"]').length > 0;
		const hasRssFeed =
			$('link[type="application/rss+xml"], link[type="application/atom+xml"]')
				.length > 0;
		const hasServiceWorker =
			/navigator\.serviceWorker\.register/i.test(pageSource) ||
			$("script")
				.toArray()
				.some((el) =>
					/navigator\.serviceWorker\.register/i.test($(el).html() || ""),
				);
		...
		const llmsTxtStatus = hasLlmsTxt
			? llmsTxt?.summary
				? "present_with_guidance"
				: "present"
			: "missing";
		const aiBotAccessSummary =
			Object.entries(aiBotAccess)
		...
		const hasListAfterHeading =
			$("h2 + ul, h2 + ol, h3 + ul, h3 + ol").length > 0;
		const hasDefinitionPattern =
			/^(what is|who is|definition of|refer to)\s/i.test(
		...
		const hasComparisonTable =
			$("table").filter((_, el) => {
				...
				return (
					headers.includes("vs") ||
					headers.includes("compare") ||
					headers.includes("difference")
				);
			}).length > 0;
		...
		const questionHeadings = $("h2, h3").filter((_, el) =>
			/^(what|how|why|when|where|who|can|does|is)\s.*\?$/i.test(
				$(el).text().trim(),
			),
		);
		const hasQuestionFormat = questionHeadings.length > 0;
		const answerBoxReady =
			questionHeadings.filter((_, el) => {
		...
		const conversationalPronouns = (
			textContent.match(/\b(you|your|we|our|I|my)\b/gi) || []
		).length;
		...
		if (
			["allow", "partial"].includes(aiBotAccess.gptBot) ||
			["allow", "partial"].includes(aiBotAccess.claudeBot)
		)
			geoScore += 10;
		...
		// ─── Business Signals (t4-*) ────────────────────────
		const hasPricingPage = /\/(pricing|plans|packages|cost|rates)(\/|$)/i.test(
			url,
		);
		const phonePattern = /(\+?1?\s?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4})/g;
		const phoneNumbers = Array.from(
			new Set(textContent.match(phonePattern) || []),
		).slice(0, 5);
		const addressPattern =
			/\d+\s+[\w\s]{5,}(?:street|st|avenue|ave|road|rd|blvd|drive|dr|lane|ln|way|court|ct|circle|cir|trail|trl|parkway|pkwy)[\s,]+[\w\s]+,?\s*[A-Z]{2}\s*\d{5}/gi;
		...
		const hasStickyBar =
			/position\s*:\s*fixed/i.test(html) && /top\s*:\s*0/i.test(html);
		const hasEmbeddedMap =
			$(
				'iframe[src*="google.com/maps"], iframe[src*="mapbox.com"], iframe[src*="apple.com/maps"], .map-container, #map',
			).length > 0;

		const accessibilityStatementLinked =
			$('a[href*="accessibility"], a[href*="a11y-statement"]').length > 0;
		...
		if (!cmsType) {
			if (
				$('meta[name="generator"][content*="Webflow"]').length ||
				html.includes("webflow.com")
			)
				cmsType = "Webflow";
			else if (
				html.includes("static.wixstatic.com") ||
				$('meta[name="generator"][content*="Wix"]').length
			)
				cmsType = "Wix";
			else if (
				...
				cmsType = "Squarespace";
			else if ($('meta[name="generator"][content*="Joomla"]').length)
				cmsType = "Joomla";
			else if (
				$('meta[name="generator"][content*="Ghost"]').length ||
		...
		const hasTrustBadges =
			$(
				'[class*="trust"], [class*="badge"], [class*="certification"], [alt*="secure"], [alt*="certified"]',
			).length > 0;
		const hasTestimonials =
			$('[class*="testimonial"], [class*="review"], [class*="quote"]').length >
			0;
		const hasCaseStudies =
			$('a[href*="case-stud"], a[href*="success-stor"]').length > 0;
		const hasCustomerLogos =
			$('[class*="logo-wall"], [class*="client-logo"], [class*="trusted-by"]')
				.length > 0;

		const ctaButtons = $(
			'a[class*="cta"], a[class*="btn"], button[class*="cta"], [class*="call-to-action"]',
		);
		const ctaTexts = ctaButtons
			.map((_, el) => $(el).text().trim())
			.get()
			.filter(Boolean);
		...
		const adPlatforms = {
			googleAds:
				$('script[src*="googleads"], script[src*="adservices.google"]').length >
				0,
			metaPixel:
				$(
					'script[src*="facebook.net/en_US/fbevents"], script[src*="connect.facebook.net"]',
				).length > 0,
		...
		const forms = $("form");
		const hasFormsWithAutocomplete =
			forms.find("input[autocomplete]").length > 0;

		// ─── Industry Specific (t4-*) ───────────────────────
		// ─── Ecommerce fingerprints ─────────────────────────────
		const currencyMatches = (
			pageSource.match(/[$€£¥₹₽₺₴₦₨R$]\s?\d|USD\s?\d|EUR\s?\d|BAM\s?\d/g) || []
		).length;
		const currencySymbolDensity =
			textContent.length > 0 ? currencyMatches / textContent.length : 0;
		const hasAddToCartButton =
			/(\b|id=|class=)["']?[^"']*(add[-_ ]?to[-_ ]?cart|add[-_ ]?to[-_ ]?basket|buy[-_ ]?now|dodaj[-_ ]?u[-_ ]?kos|in[-_ ]?den[-_ ]?warenkorb|ajouter[-_ ]?au[-_ ]?panier|aggiungi[-_ ]?al[-_ ]?carrello)/i.test(
				pageSource,
			);
		const hasCartEndpoint =
			$(
				'a[href*="/cart"], a[href*="/korpa"], a[href*="/panier"], a[href*="/warenkorb"], a[href*="/checkout"]',
			).length > 0;
		const priceVisible =
			/(\b\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})\b)\s?(€|\$|£|USD|EUR|BAM|GBP|KM)\b|(€|\$|£|USD|EUR|BAM|GBP|KM)\s?\b\d/i.test(
				textContent,
			);
		const hasStockStatus =
			/(in[-_ ]?stock|out[-_ ]?of[-_ ]?stock|na[-_ ]?stanju|nema[-_ ]?na[-_ ]?stanju|auf[-_ ]?lager|ausverkauft|en[-_ ]?stock|agotado)/i.test(
				textContent,
			);
		...
		const headings = $("h2, h3")
			.map((_, el) => $(el).text().trim())
			.get();
		const questionLike = headings.filter(
			(t) =>
				/\?$/.test(t) ||
				/^(what|why|how|when|where|who|can|is|do|does|zašto|kako|šta|warum|wie|was|qué|comment|pourquoi)\b/i.test(
					t,
				),
		);
		...
		const industrySignals = {
			...
			hasBreadcrumbUI:
				$('[class*="breadcrumb"], nav[aria-label*="bread" i]').length > 0,
			...
			hasLocalBusinessSchema:
				/"@type"\s*:\s*"(LocalBusiness|Restaurant|Store|Dentist|[A-Za-z]+Service)"/i.test(
					pageSource,
				),
			hasNapOnPage: phoneMatches && phoneMatches.length > 0 && hasPostalAddress,
			hasEmbeddedMap:
				$(
					'iframe[src*="google.com/maps"], iframe[src*="maps.google"], [class*="map"]',
				).length > 0,
			hasReservationLink:
				$('a[href*="opentable"], a[href*="resy"], a[href*="reserv"]').length >
				0,
			hasMenuLink: $('a[href*="menu"]').length > 0,
			hasPriceRange:
				/"priceRange"\s*:/i.test(pageSource) ||
				/\$+\s?to\s?\$+/i.test(textContent),
			// SaaS
			hasPricingPage: $('a[href*="/pricing"], a[href*="/prices"]').length > 0,
			hasTrialCta:
				/(free[-_ ]?trial|start[-_ ]?free|try[-_ ]?free|get[-_ ]?started[-_ ]?free)/i.test(
					pageSource,
				),
			hasPricingLink: $('a[href*="/pricing"]').length > 0,
			hasIntegrationsPage: $('a[href*="/integration"]').length > 0,
			// Healthcare / Finance
			hasMedicalAuthor: /medically[-_ ]?reviewed|md\b|m\.d\.|physician/i.test(
				textContent,
			),
			hasMedicalReviewer:
				/reviewed[-_ ]?by[^<.]{0,80}(md|m\.d\.|physician|dr\.)/i.test(
					textContent,
				),
			hasMedicalDisclaimer:
				/(not[-_ ]?medical[-_ ]?advice|educational[-_ ]?purposes[-_ ]?only|consult[-_ ]?a[-_ ]?doctor)/i.test(
					textContent,
				),
			hasFinancialDisclaimer:
				/(not[-_ ]?financial[-_ ]?advice|for[-_ ]?informational[-_ ]?purposes[-_ ]?only|past[-_ ]?performance)/i.test(
					textContent,
				),
			hasAuthorCredentials:
				/(cfa\b|cfp\b|phd\b|ph\.d\.|md\b|m\.d\.|jd\b)/i.test(textContent),
			// Q&A
			...
			for (const rule of config.customExtractionRules) {
				try {
					const globToRegex = (glob) =>
						new RegExp("^" + String(glob || "*").replace(/\*/g, ".*") + "$");
					const pathname = new URL(url).pathname;
					const isMatch =
						rule.pages === "*" || globToRegex(rule.pages).test(pathname);
					...
					if (rule.condition === "exists") passed = $el.length > 0;
						...
						passed = $el.length > 0 && $el.text().trim() === "";
		...
		parentPort.postMessage({
			...
			data: {
				...
				scriptsWithoutSri,
				exposedApiKeys,
				exposedEmails: exposedEmails.slice(0, 10),
				privacyPageLinked,
				termsPageLinked,
				hasCookieBanner,
				// URL structure
				...
				hasEmbeddedMap,
				detectedLibraries,
				accessibilityStatementLinked,
				cmsType,
				// ─── NAP (for WQA location_page + homepage compare) ──────
				napSnapshot: {
					phones: Array.from(
						new Set(
							(phoneNumbers || []).map((p) => String(p).replace(/[^\d+]/g, "")),
						),
					),
					address: hasPostalAddress
						? (
								(textContent.match(
									/\d+\s+\w[\w\s]+(?:street|st|avenue|ave|road|rd|blvd|drive|dr|lane|ln|way|court|ct|circle|cir|trail|trl|parkway|pkwy)[\s,]+[\w\s]+,?\s*[A-Z]{2}\s*\d{5}/i,
								) || [])[0] || ""
							).trim()
						: "",
