...
app.post("/api/integrations/google/exchange", async (req, res) => {
	...
	if (!code || !redirectUri) {
		...
		return res
			.status(400)
			.json({ error: "code and redirectUri are required." });
	}
	...
	try {
		...
		if (response.statusCode !== 200) {
			...
			return res
				...
				.json({ error: "Google exchange failed", details: payload });
		}

		debugLog("Fetching user info (email)...");
		// Get user info (email)
		const userRes = await undiciRequest(
		...
		try {
			...
			if (data.type === "START_CRAWL") {
				...
				const googleAccountEmail =
					googleConfig.email || googleConfig.accountLabel || null;
				...
				if (googleAccountEmail && !googleConfig.accessToken) {
					try {
						...
						if (tokenRes.rows.length > 0) {
							...
							console.log(
								`Resolved server-side tokens for ${googleAccountEmail}`,
							);
						}
					} catch (err) {
				...
				const normalizedConfig = {
					...
					gscRefreshToken:
						rawConfig.gscRefreshToken || googleConfig.refreshToken || "",
					gscSiteUrl:
						rawConfig.gscSiteUrl ||
						googleConfig.gscSiteUrl ||
						googleConfig.selection?.siteUrl ||
						"",
					ga4AccessToken:
						rawConfig.ga4AccessToken || googleConfig.accessToken || "",
					ga4PropertyId:
						rawConfig.ga4PropertyId ||
						googleConfig.ga4PropertyId ||
						googleConfig.selection?.propertyId ||
						"",
					bingAccessToken:
						rawConfig.bingAccessToken || bingConfig.accessToken || "",
				};
				...
				if (payload.projectId) {
					notifyProjectWebhooks(turso, payload.projectId, "crawl.started", {
						...
						sessionId,
						url:
							normalizedConfig.startUrls?.[0] ||
							normalizedConfig.gscSiteUrl ||
							payload.url ||
							"",
						config: normalizedConfig,
					}).catch((err) =>
						console.error("crawl.started webhook failed:", err),
					);
				}
						...
						try {
							...
							if (latestRun.rows.length > 0) {
								...
								if (topPages.rows.length > 0) {
									normalizedConfig.startUrls = topPages.rows.map((r) =>
										String(r.url),
									);
					...
					async (event, payload) => {
						...
						if (
							...
							try {
								...
								console.log(
									`Updated server-side token in Turso for ${googleAccountEmail}`,
								);
							} catch (err) {
								console.error(
									"Failed to update Turso token from crawler event:",
									err,
								);
							}
						}
						...
						if (event === "CRAWL_FINISHED" && sessionId) {
								...
								notifyProjectWebhooks(
									...
									"crawl.completed",
									{
										...
										score:
											...
											null,
										issueCount:
											payload.issueOverview?.length ??
											payload.summary?.issueOverview?.length ??
											0,
									},
								).catch((err) =>
									console.error("crawl.completed webhook failed:", err),
								);
							}
							...
							AgentRegistry.runAgentByEvent(turso, "crawl.completed", {
								...
								console.error(
									"[AgentEvent:crawl.completed] Error:",
									err.message,
								),
							);
						}
						...
						if (event === "ERROR" && payload.projectId && !payload.url) {
							notifyProjectWebhooks(turso, payload.projectId, "crawl.failed", {
								...
								console.error("crawl.failed webhook failed:", err),
							);
						}
				...
				sessionTimeout = setTimeout(async () => {
					if (crawlerInstance) {
						console.log(
							`Session timeout reached, stopping crawl for session ${sessionId}`,
						);
						await crawlerInstance.stop();
						crawlerInstance = null;
					}
			...
			try {
				client.send(
					JSON.stringify({
						...
						payload: {
							message:
								"Server is shutting down. Your scan progress has been saved.",
						},
					}),
				);
			} catch {}
