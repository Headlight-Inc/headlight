...
const PROVIDERS = [
  ...
  },
  {
    ...
    parseUsage: (data) => (data.usage?.input_tokens || 0) + (data.usage?.output_tokens || 0)
  },
  {
    name: 'huggingface',
    envKey: 'HF_TOKEN',
...
async function getQuota(providerName, turso) {
  ...
  try {
    ...
    return { provider: providerName, requests_today: 0, tokens_today: 0, requests_this_minute: 0, last_reset_minute: Date.now(), last_reset_day: today };
  } catch (err) {
...
export async function completeAI({ prompt, systemPrompt, maxTokens, format }, turso) {
