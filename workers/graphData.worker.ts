...
function computeGraphData(analysisPages: any[]): {
	nodes: GraphNode[];
	links: GraphLink[];
} {
	...
	const pageByUrl = new Map<string, any>(
		analysisPages.map((page) => [page.url, page]),
	);
	...
	analysisPages.forEach((page) => {
		...
		try {
			...
			nodeName =
				parsedUrl.pathname === "/"
					? parsedUrl.hostname || page.url
					: parsedUrl.pathname || page.url;
			dirPath =
				segments.length > 0 ? `/${segments.slice(0, 1).join("/")}` : "/";
	...
	nodes.forEach((node) => {
		...
		const bucketIndex = Math.max(
			0,
			bucket.findIndex((entry) => entry.id === node.id),
		);
		...
		page.outlinksList.forEach((targetUrl: string) => {
			...
			const isStructural =
				...
				(targetDepth === sourceDepth + 1 ||
					(targetDepth === sourceDepth &&
						targetPage?.url.startsWith(page.url.replace(/\/$/, ""))));
			...
			links.push({
				...
				sameSection: Boolean(
					sourceNode &&
					targetNode &&
					sourceNode.sectionKey === targetNode.sectionKey,
				),
			});
		});
	});
