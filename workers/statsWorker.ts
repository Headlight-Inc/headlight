...
self.onmessage = (e) => {
	...
	if (type === "CALCULATE_STATS") {
		...
		const slowPages = pages.filter(
			(p) => p.loadTime && p.loadTime > 1500,
		).length;
		const largePages = pages.filter(
			(p) => p.sizeBytes && p.sizeBytes > 2 * 1024 * 1024,
		).length;
		...
		const missingHreflang = pages.filter(
			(p) => !Array.isArray(p.hreflang) || p.hreflang.length === 0,
		).length;
		const poorLCP = pages.filter((p) => p.lcp && p.lcp > 2500).length;
		const mixedContent = pages.filter((p) => p.mixedContent === true).length;
		const multipleH1s = pages.filter(
			(p) => p.multipleH1s === true || p.h1_2 !== undefined,
		).length;
		const duplicateTitlePages = pages.filter(
			(p) =>
				p.title &&
				duplicateTitleSet?.has(
					p.title.trim().replace(/\s+/g, " ").toLowerCase(),
				),
		).length;
		const duplicateMetaDescPages = pages.filter(
			(p) =>
				p.metaDesc &&
				duplicateMetaDescSet?.has(
					p.metaDesc.trim().replace(/\s+/g, " ").toLowerCase(),
				),
		).length;

		const referringDomains = pages.reduce(
			(acc: number, p: any) => acc + (p.referringDomains || 0),
			0,
		);
		const urSum = pages.reduce(
			(acc: number, p: any) => acc + (p.urlRating || 0),
			0,
		);
		const urCount = pages.filter((p: any) => p.urlRating > 0).length;
		const avgUr = urCount > 0 ? Math.round((urSum / urCount) * 10) / 10 : 0;
		const maxUr = pages.reduce(
			(acc: number, p: any) => Math.max(acc, p.urlRating || 0),
			0,
		);
