...
export default function GoogleOAuthCallback() {
	useEffect(() => {
		...
		console.log("[GoogleOAuthCallback] Handled redirect query:", {
			code: !!code,
			error,
		});
		...
		if (window.opener) {
			console.log(
				"[GoogleOAuthCallback] window.opener found, sending postMessage",
			);
			window.opener.postMessage(result, window.location.origin);
		} else {
			console.warn(
				"[GoogleOAuthCallback] No window.opener found. Relying on localStorage fallback.",
			);
		}
