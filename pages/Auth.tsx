...
const isTimeoutError = (message: string) =>
	message.toLowerCase().includes("timed out after 15 seconds");
...
const Auth: React.FC = () => {
			...
			return await Promise.race([
				...
				new Promise<T>((_, reject) => {
					timer = setTimeout(() => {
						reject(
							new Error(
								`${action} timed out after 15 seconds at ${getCurrentOrigin()}. Check your Clerk key, allowed origin, and network connection, then try again.`,
							),
						);
	...
	const handleSignupVerify = async (e: React.FormEvent) => {
		...
		try {
			...
			setError(
				`Verification result: ${result.status}. Please check your email.`,
			);
		} catch (err: any) {
	...
	const handleSocial = async (
		strategy: "oauth_google" | "oauth_apple" | "oauth_x",
	) => {
		if (!signInLoaded || !signIn) {
			setError("Authentication is still loading. Try again in a moment.");
			return;
...
const normalizeError = (err: any) => {
	...
	if (typeof message === "string") {
		if (message.toLowerCase().includes("data breach"))
			return "Please pick a safer password.";
		if (message.toLowerCase().includes("already been verified"))
			return "This account is already verified.";
		if (isTimeoutError(message)) {
			const originMatch = message.match(
				/at (https?:\/\/[^.\s/]+(?::\d+)?|https?:\/\/[^\s]+)/i,
			);
