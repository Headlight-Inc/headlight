...
export const Dashboard: React.FC = () => {
	...
	const getIssuesForUrl = (url: string) => {
		...
		return allAuditChecks.filter(
			(check) =>
				check.type !== "passed" &&
				(check.preview?.some((p) => url.includes(p)) || Math.random() > 0.8),
		);
	};
							...
							<div className="relative group">
								...
								<div className="absolute top-full left-0 mt-2 w-32 bg-[#111] border border-white/10 rounded-lg shadow-xl py-1 hidden group-hover:block z-50">
									<button
										...
										className="w-full text-left px-3 py-2 text-xs text-gray-400 hover:text-white hover:bg-white/5"
									>
										Last 7 Days
									</button>
									<button
					...
					<div className="flex items-center gap-4">
						<div className="hidden md:flex items-center px-4 py-2 bg-white/[0.03] border border-white/[0.08] rounded-xl gap-3 text-sm text-gray-400 w-72 hover:border-white/20 transition-colors cursor-text group focus-within:border-brand-red/50 focus-within:bg-black/20">
							...
							<div className="ml-auto flex items-center gap-1">
								<span className="text-[10px] bg-white/5 px-1.5 py-0.5 rounded border border-white/10 text-gray-500">
				...
				<main className="flex-1 overflow-y-auto p-6 lg:p-10 scrollbar-hide">
					{showOnboarding ? (
						...
						<div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 pb-10">
							...
							{/* Intelligence Views */}
							{currentView === "keyword_research" && (
								<EmptyStateView
									title="Keyword Research"
									icon={<Search size={48} />}
									desc="Discover high-opportunity keywords your competitors are missing."
								/>
							)}
							{currentView === "content_predictor" && <ContentPredictorView />}
							{currentView === "competitors" && (
								<CompetitorsView radarData={radarData} />
							)}

							{/* Performance Views */}
							{currentView === "rank_tracker" && <RankTrackerView />}
							{currentView === "mentions" && <BrandMentionsView />}
							{currentView === "site_audit" && (
								<SiteAuditView showHelp={showHelp} openPanel={openPanel} />
							)}
							{currentView === "web_vitals" && (
								<EmptyStateView
									title="Core Web Vitals"
									icon={<Activity size={48} />}
									desc="Real-time user experience metrics monitoring."
								/>
							)}
							...
							{currentView === "opportunities" && (
								<EmptyStateView
									...
									desc="AI-prioritized list of high impact actions."
								/>
							)}
							{currentView === "agency_hub" &&
								(profile?.role === "VISITOR" ? (
									<EmptyStateView
										...
										desc="Upgrade your plan to unlock Agency features."
									/>
								) : (
									<AgencyHubView />
								))}
							{currentView === "automation" &&
								(profile?.role === "VISITOR" ? (
									<EmptyStateView
										title="Automation"
										icon={<Zap size={48} />}
										desc="Upgrade your plan to unlock Automations."
									/>
								) : (
									<AutomationView />
								))}
							...
							{currentView === "settings_account" && (
								<EmptyStateView
									title="Account Settings"
									icon={<Users size={48} />}
									desc="Manage your team, billing, and global preferences."
								/>
							)}
						</div>
					)}
					...
					<div className="h-[76px] flex items-center justify-between px-6 border-b border-white/5 bg-transparent shrink-0">
						<h3 className="text-lg font-heading font-bold text-white flex items-center gap-2 truncate pr-4">
							{activePanel === "competitor_add" && (
								<>
									<Globe size={18} className="text-brand-red shrink-0" /> Add
									Competitor
								</>
							)}
							{activePanel === "keyword_detail" && (
								<>
									<Target size={18} className="text-brand-red shrink-0" />{" "}
									Keyword Detail
								</>
							)}
							{activePanel === "insight" && (
								<>
									<Sparkles size={18} className="text-brand-red shrink-0" /> AI
									Insight
								</>
							)}
							{activePanel === "context_help" && (
								<>
									<Info size={18} className="text-brand-red shrink-0" /> Context
									& Definitions
								</>
							)}
							{activePanel === "audit_issue_detail" && (
								<>
									<AlertTriangle
										size={18}
							...
							{activePanel === "url_detail" && (
								<>
									<Globe size={18} className="text-brand-red shrink-0" /> URL
									Inspection
								</>
							)}
						</h3>
					...
					<div className="flex-1 overflow-y-auto p-0 scrollbar-hide">
									...
									<div className="flex items-center gap-3 mb-4">
										<span
											className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wide border ${
												panelData.priority === "Critical"
													? "bg-red-500/10 text-red-500 border-red-500/20"
													: panelData.priority === "High"
								...
								<div className="p-6 space-y-8 flex-1">
									...
									<div className="bg-brand-purple/5 border border-brand-purple/10 rounded-2xl p-5 relative overflow-hidden">
										...
										<div className="flex items-center gap-2 mb-3">
											...
											<span className="text-xs font-bold uppercase text-brand-purple tracking-wide">
												AI Recommendation
											</span>
										</div>
										...
										<div className="mt-4 flex gap-3">
											<Button
												size="sm"
												variant="primary"
												className="bg-brand-purple hover:bg-brand-purple/80 text-white border-none text-[10px]"
											>
												Apply Fix with AI
											</Button>
											<Button
												size="sm"
												variant="secondary"
												className="bg-transparent border-brand-purple/30 text-brand-purple hover:bg-brand-purple/10 text-[10px]"
											>
												Learn More
											</Button>
										</div>
									...
									<div>
										<div className="flex justify-between items-end mb-4">
											<h3 className="text-sm font-bold text-white uppercase tracking-wide">
												Affected Pages ({panelData.count})
											</h3>
											...
											{(panelData.preview || []).map(
												(url: string, i: number) => (
													...
													>
														...
														<button
															onClick={() =>
																openPanel("url_detail", { url, status: 200 })
															}
						...
						{activePanel === "url_detail" && panelData && (
							<div className="flex flex-col h-full bg-[#080808]">
								...
								<div className="p-6 border-b border-white/5 bg-[#111]">
									...
									<h2 className="text-xl font-bold text-white mb-4 font-mono break-all">
										{panelData.url}
									</h2>
									<div className="flex gap-4">
										<a
											href={`https://${panelData.url.replace(/^\//, "")}`}
											target="_blank"
											rel="noreferrer"
								...
								<div className="flex-1 p-6 bg-[#080808]">
										...
										<div className="space-y-4">
											...
											{getIssuesForUrl(panelData.url).length > 0 ? (
												getIssuesForUrl(panelData.url).map((issue: any) => (
													<div
														key={issue.id}
														className="p-4 bg-white/[0.03] border border-white/5 rounded-xl hover:border-white/10 transition-colors cursor-pointer"
														onClick={() =>
													...
													>
														...
														<h4 className="text-sm font-bold text-gray-200 mt-2">
															{issue.title}
														</h4>
										...
										<div className="space-y-6">
											...
											<div>
												...
												<div className="space-y-2">
													{["/home", "/blog", "/features/analytics"].map(
														(l, i) => (
															<div
																key={i}
																className="flex items-center gap-2 text-xs p-2 rounded hover:bg-white/5 transition-colors cursor-pointer"
															>
									...
									{panelTab === "code" && (
										<div className="bg-[#050505] rounded-xl border border-white/10 p-4 font-mono text-xs overflow-x-auto text-gray-400 leading-relaxed">
											<div className="text-gray-500 select-none mb-2">
												view-source:{panelData.url}
											</div>
								...
								<div className="border-t border-white/10 pt-4">
									...
									<ul className="space-y-3">
										<li className="flex gap-3 text-sm text-gray-300">
											...
											It directly impacts your ability to rank in search
											results.
										</li>
										<li className="flex gap-3 text-sm text-gray-300">
											<div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-brand-green"></div>
								...
								<div className="p-4 bg-blue-500/10 rounded-xl border border-blue-500/20 flex gap-3">
									...
									<div>
										...
										<p className="text-xs text-gray-400">
											Ask the AI Copilot to "Audit my site for {panelData.title}
											" to get specific fixes.
										</p>
									</div>
								</div>
							</div>
						...
						{activePanel === "competitor_add" && (
							<div className="p-6 text-center text-gray-500">
								Competitor Add Form Placeholder
							</div>
						)}
						{activePanel === "keyword_detail" && (
							<div className="p-6 text-center text-gray-500">
								Keyword Detail Placeholder
							</div>
						)}
						{activePanel === "insight" && (
							<div className="p-6 text-center text-gray-500">
