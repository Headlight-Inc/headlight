...
export const PublicReport: React.FC = () => {
	...
	async function loadReport() {
		try {
			...
			if (
				reportConfig.expires_at &&
				new Date(reportConfig.expires_at) < new Date()
			) {
	...
	if (loading)
		return (
			<div className="h-screen bg-[#080808] flex items-center justify-center text-white">
				Loading report...
			</div>
		);
	if (error)
		return (
			<div className="h-screen bg-[#080808] flex items-center justify-center text-red-500">
				{error}
			</div>
		);
			...
			<div className="h-screen bg-[#080808] flex items-center justify-center p-4">
				<div className="max-w-md w-full bg-[#111] border border-[#222] rounded-xl p-8">
					...
					<p className="text-gray-400 text-sm mb-6">
						This report is private. Please enter the password to view it.
					</p>
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
							...
							<div className="bg-[#111] border border-[#222] rounded-xl p-6">
								...
								<div className="text-4xl font-bold text-white">
									{summary?.summary?.totalCrawled || 0}
								</div>
							</div>
							<div className="bg-[#111] border border-[#222] rounded-xl p-6">
								<span className="text-sm text-gray-400 block mb-1">
								...
								<div className="text-4xl font-bold text-red-500">
									{summary?.issueOverview?.critical || 0}
								</div>
							</div>
						</div>
					...
					<section className="mb-16">
						...
						<div className="space-y-4">
							{issues?.map((issue: any) => (
								<div
									key={issue.id}
									className="bg-[#111] border border-[#222] rounded-xl p-6 flex items-start justify-between"
								>
									<div>
									...
									<div key={issue.id} className="flex gap-4">
										...
										<div>
											...
											<p className="text-sm text-gray-400 leading-relaxed">
												{issue.aiFix ||
													`Review and resolve the ${issue.count} occurrences of this issue to improve your site's health score.`}
											</p>
				...
				{!report?.white_label && (
					<div className="flex flex-col items-center gap-4">
						...
						<p className="text-gray-500 text-xs">
							Modern SEO Intelligence & Technical Audits
						</p>
					</div>
				)}
			</footer>
