...
class SeoCrawlerErrorBoundary extends Component<
	ErrorBoundaryProps,
	ErrorBoundaryState
> {
			...
			return (
				<div className="h-screen bg-[var(--bg-main)] text-[var(--text-secondary)] flex items-center justify-center p-6">
					<div className="max-w-xl w-full rounded-lg border border-[#2a2a2a] bg-[#111] p-5">
						<div className="text-[11px] uppercase tracking-widest text-[#888] mb-2">
							Crawler Error
						</div>
						<div className="text-[16px] text-white font-semibold mb-2">
							The crawler UI hit a runtime error.
						</div>
...
function SeoCrawlerLayout() {
	...
	const shouldShowEmptyState =
		!projectsLoading && !isSetup && !activeProject && projects.length === 0;
	...
	return (
		<div className="flex flex-col h-screen bg-[var(--bg-main)] text-[var(--text-secondary)] font-sans overflow-hidden">
			...
			<div className="flex-1 flex min-h-0 relative overflow-hidden">
				{!isCompactLayout && activeViewType !== "competitor_matrix" && (
					<PanelErrorBoundary
						name="Sidebar"
						fallback={
							<div className="m-3 rounded border border-[#2b2b2f] bg-[#111] p-3 text-[12px] text-[#999]">
					...
					<div className="flex-1 flex min-h-0 relative overflow-hidden">
						...
						{!isCompactLayout && (
							<PanelErrorBoundary
								...
								fallback={
									<div className="m-3 rounded border border-[#2b2b2f] bg-[#111] p-3 text-[12px] text-[#999]">
										Audit panel failed to load.
									</div>
								}
							>
								{isWqaMode ? (
									<WqaSidebarRouter />
								) : auditFilter.modes.includes("competitor_gap") ? (
									<CompSidebarRouter />
								) : (
									<AuditSidebar />
								)}
			...
			<PanelErrorBoundary
				name="Status Bar"
				fallback={
					<div className="h-[36px] border-t border-[#2b2b2f] bg-[#0f0f12]" />
				}
			>
				<StatusBar />
			</PanelErrorBoundary>
			...
			{showComparisonView && (
				<ComparisonView onClose={() => setShowComparisonView(false)} />
			)}
			{showExportDialog && (
			...
			{!shouldShowEmptyState && isCompactLayout && (
				<>
					...
					<MobileBottomSheet
						isOpen={showMobileExplorer}
						onClose={() => setShowMobileExplorer(false)}
						title={isWqaMode ? "Filters" : "Site Explorer"}
						defaultHeight={82}
					>
					...
					<MobileBottomSheet
						isOpen={showMobileAudit}
						onClose={() => setShowMobileAudit(false)}
						title="Audit Panel"
						defaultHeight={82}
					>
						{isWqaMode ? (
							<WqaSidebarRouter embedded />
						) : (
