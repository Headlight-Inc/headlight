...
export class BacklinkClientService {
	...
	static async fetchAhrefsMetrics(
		...
		for (let i = 0; i < urls.length; i += CHUNK_SIZE) {
			...
			onProgress?.(
				`Ahrefs: Fetching batch ${Math.floor(i / CHUNK_SIZE) + 1}...`,
			);

			try {
				const response = await fetch(
					"https://api.ahrefs.com/v3/statistics/metrics",
					{
						method: "POST",
						headers: {
				...
				if (!response.ok) {
					...
					console.warn(
						`[Ahrefs] Batch failed: ${err.error || response.statusText}`,
					);
					continue;
				}
	...
	static async enrichSession(
		...
		if (integrations.ahrefsToken) {
			...
			const ahrefsData = await this.fetchAhrefsMetrics(
				urls,
				integrations.ahrefsToken,
				onProgress,
			);
		...
		if (integrations.semrushApiKey) {
			...
			const semrushData = await this.fetchSemrushMetrics(
				rootDomain,
				integrations.semrushApiKey,
			);
