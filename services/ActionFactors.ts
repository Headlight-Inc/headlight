...
export function deriveActionFactors(
	page: any,
	ctx: {
		industry: DetectedIndustry;
		availability: DataAvailability;
	...
	const keywordHealth = !ctx.availability.gsc
		? "unknown"
		: impressions === 0
			? "none"
	...
	const backlinkTier = !ctx.availability.backlinks
		? "unknown"
		: referringDomains >= 25
			? "strong"
			: referringDomains >= 5
	...
	return {
		impressions,
		clicks: num(page.gscClicks),
		position,
		ctr,
		ctrGap: num(page.ctrGap),
		ctrTrend,
		trafficTrend,
		keywordHealth,
		...
		backlinkTier,
		referringDomains,
		inlinks: num(page.inlinks),
		ipr: num(page.internalPageRank),
		freshness,
		decayRisk: num(page.contentDecayRisk),
