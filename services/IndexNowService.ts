...
export async function submitToIndexNow(
	...
	if (!config.apiKey || urls.length === 0)
	...
	const body = JSON.stringify({
		...
		key: config.apiKey,
		keyLocation:
			config.keyLocation || `https://${config.host}/${config.apiKey}.txt`,
		urlList: batch,
	});
	...
	const promises = INDEXNOW_ENDPOINTS.map(async (endpoint) => {
		try {
			onProgress?.(
				`Submitting ${batch.length} URLs to ${new URL(endpoint).hostname}...`,
			);
...
export async function submitFixedPages(
	...
	const currentPages = await crawlDb.pages
		.where("crawlId")
		.equals(currentSessionId)
		.toArray();
	const previousPages = await crawlDb.pages
		.where("crawlId")
		.equals(previousSessionId)
		.toArray();
	...
	onProgress?.(
		`Found ${uniqueUrls.length} fixed/updated pages to submit to IndexNow`,
	);
