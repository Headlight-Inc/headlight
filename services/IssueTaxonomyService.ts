...
import { getAuditIssues, getAuditHistory, getLatestAuditResult } from './CrawlPersistenceService';
...
export interface ClassifiedIssue {
    ...
    aiFix: string;
    trend: 'new' | 'recurring' | 'improving' | 'worsening' | 'fixed' | 'regressed';
    trendDetail?: string;
}
...
export async function trackIssueTrends(
    ...
    const previousIssueMap = new Map(previousIssues.map((i: any) => [i.title, i]));
...
function estimateTrafficImpact(issue: any): number {
    ...
    if (issue.title?.includes('Title') || issue.title?.includes('Meta Description')) {
        return Math.round(count * avgVisitsPerPage * 0.15 * multiplier); // 15% CTR improvement
    }

    // For broken pages, estimate lost traffic
    if (issue.title?.includes('Broken') || issue.title?.includes('Server Error')) {
...
export interface CannibalizationGroup {
    keyword: string;
    pages: { url: string; title: string; gscPosition: number; gscClicks: number }[];
    impact: string;
    suggestion: string;
}
...
export function detectContentDecay(
    ...
    for (const page of currentPages) {
        ...
        const dropPercentage = ((previousClicks - currentClicks) / previousClicks) * 100;
