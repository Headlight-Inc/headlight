...
export function extractKeywordsFromPages(pages: any[]): DiscoveredKeyword[] {
	const keywordFrequency = new Map<
		string,
		{ count: number; gsc: boolean; position?: number; volume?: number }
	>();
	...
	for (const page of pages) {
		...
		if (titleKeyword) {
			const entry = keywordFrequency.get(titleKeyword) || {
				count: 0,
				gsc: false,
			};
			entry.count++;
	...
	return [...keywordFrequency.entries()]
		.filter(
			([keyword, data]) => keyword.length > 3 && keyword.split(" ").length <= 7,
		)
...
function guessIntent(keyword: string): string {
	const lower = keyword.toLowerCase();
	if (/\b(buy|price|cheap|discount|deal|order|shop)\b/.test(lower))
		return "Transactional";
	if (/\b(best|top|review|compare|vs|alternative)\b/.test(lower))
		return "Commercial";
	if (/\b(how|what|why|when|where|guide|tutorial|learn)\b/.test(lower))
