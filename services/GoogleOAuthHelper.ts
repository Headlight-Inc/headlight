...
export async function getGoogleTokenStatus(
	...
	try {
		const res = await fetch(
			`/api/integrations/google/status?email=${encodeURIComponent(email)}`,
			{
	...
	return new Promise((resolve) => {
		...
		const readStoredResult = (): GoogleOAuthCallbackPayload | null => {
			try {
				...
				console.error(
					"[GoogleOAuthHelper] Failed to parse stored OAuth result",
					e,
				);
				return null;
			}
		...
		const resolveFromPayload = (payload: GoogleOAuthCallbackPayload | null) => {
			...
			if (
				payload.type !== "GOOGLE_OAUTH_CALLBACK" &&
				payload.type !== "headlight-oauth-callback"
			) {
				return false;
			}
