...
export function extractVisibleDate(input: ExtractInput): string | null {
	...
	const cjk = /(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日/.exec(
		textContent,
	);
	...
	for (const lang of Object.keys(MONTHS)) {
		...
		for (const name of months) {
			...
			const rx1 = new RegExp(
				`\\b(\\d{1,2})[\\.\\s,]+(?:de\\s+)?${escapeRegex(name)}[\\.\\s,]*(?:de\\s+|del\\s+)?(\\d{4})\\b`,
				"i",
			);
			...
			const rx2 = new RegExp(
				`\\b${escapeRegex(name)}[\\.\\s,]+(\\d{1,2})(?:st|nd|rd|th)?[\\.\\s,]+(\\d{4})\\b`,
				"i",
			);
