...
export class Ga4ClientService {
	 ...
	 */
	private static async backoffFetch(
		fn: () => Promise<Response>,
		retries = 3,
	): Promise<Response> {
		let delay = 1000;
	...
	private static async fetchPaginatedGa4(
		...
		while (offset < maxRows) {
			...
			const response = await this.backoffFetch(() =>
				fetch(`${this.API_BASE}/${propertyId}:runReport`, {
					method: "POST",
					headers: {
			...
			if (response.status === 401 && googleEmail) {
				const refreshedAccessToken = await refreshWithLock(
					googleEmail,
					refreshGoogleToken,
				);
				if (
			...
			if (!response.ok) {
				...
				throw new Error(
					`GA4 API Error: ${error.error?.message || response.statusText}`,
				);
			}
	...
	static async enrichSession(
		...
		const targetCanonicalSet = new Set(
			(options.targetUrls || htmlPages.map((page) => page.url)).map((url) =>
				UrlNormalization.toCanonical(url),
			),
		);
		const targetPages = htmlPages.filter((page) =>
			targetCanonicalSet.has(UrlNormalization.toCanonical(page.url)),
		);
		...
		onProgress?.(
			`Fetching GA4 Current Period (up to ${maxRows.toLocaleString()} rows)...`,
		);
		...
		onProgress?.(
			`Fetching GA4 Previous Period (up to ${maxRows.toLocaleString()} rows)...`,
		);
		...
		const normalizePath = (p: string) => {
			...
			return UrlNormalization.toCanonical(
				`${baseDomain}${p.startsWith("/") ? "" : "/"}${p}`,
			);
		};
		...
		for (const page of targetPages) {
			...
			const candidates = [page.url, pageCanonical, page.finalUrl].filter(
				Boolean,
			) as string[];
			for (const candidate of candidates) {
				...
				if (cur) {
					const result = UrlNormalization.getMatchResult(
						page.url,
						cur.rawPath.startsWith("http")
							? cur.rawPath
							: `${baseDomain}${cur.rawPath}`,
			...
			if (bestCur) {
				enrichedCount++;
				const sessionsDeltaAbsolute =
					bestCur.sessions - (bestPrev?.sessions || 0);
				const sessionsDeltaPct = bestPrev?.sessions
					? sessionsDeltaAbsolute / bestPrev.sessions
					: null;
				const engagementTimePerPage =
					bestCur.sessions > 0 ? bestCur.engagementTime / bestCur.sessions : 0;
				const conversionRate =
					bestCur.sessions > 0 ? bestCur.conversions / bestCur.sessions : 0;
