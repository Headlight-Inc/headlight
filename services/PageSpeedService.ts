...
export async function fetchPageSpeedInsights(
  ...
  return {
    ...
    fieldCLS: cruxMetrics.CUMULATIVE_LAYOUT_SHIFT_SCORE?.percentile ? 
      cruxMetrics.CUMULATIVE_LAYOUT_SHIFT_SCORE.percentile / 100 : null,
    ...
    lcpElement: lighthouse?.audits?.['largest-contentful-paint-element']?.details?.items?.[0]?.node?.snippet || null,
