...
export class CompetitorProfileBuilder {
	...
	static fromCrawlPages(
		...
		const productPages = pages.filter(
			(p) => productPattern.test(p.url) && p.crawlDepth > 0,
		);
		...
		if (contentPages.length > 0) {
			...
			profile.blogPostsPerMonth =
				Math.round((recentCount / 12) * 10) / 10 ||
				(contentPages.length > 5 ? 2 : 0);
		}

		// 4. avgImagesPerArticle
		if (blogPages.length > 0) {
			const totalImages = blogPages.reduce(
				(sum, p) => sum + (p.imageCount || 0),
				0,
			);
		...
		profile.embedsVideoInArticles = blogPages.some(
			(p) => p.hasEmbeddedVideo || p.schemaTypes?.includes("VideoObject"),
		);

		// 6. avgWordsPerArticle
		const wordyArticles = blogPages.filter((p) => p.wordCount >= 100);
		if (wordyArticles.length > 0) {
			const totalWords = wordyArticles.reduce(
				(sum, p) => sum + (p.wordCount || 0),
				0,
			);
			profile.avgWordsPerArticle = Math.round(
				totalWords / wordyArticles.length,
			);
		}
		...
		if (productPages.length > 0) {
			const totalWords = productPages.reduce(
				(sum, p) => sum + (p.wordCount || 0),
				0,
			);
			profile.productPageAvgWordCount = Math.round(
				totalWords / productPages.length,
			);
		}
		...
		profile.hasSchemaOnProducts = pages.some(
			(p) => p.hasProductSchema || p.schemaTypes?.includes("Product"),
		);

		// 13. hasTargetedLandingPages
		profile.hasTargetedLandingPages = pages.some((p) =>
			landingPattern.test(p.url),
		);

		// 14. hasEmailOptIn
		profile.hasEmailOptIn = !!(
			rootPage?.hasEmailOptIn || rootPage?.industrySignals?.hasEmailOptIn
		);

		// 15. hasOptimizedLocalPages
		profile.hasOptimizedLocalPages = pages.some((p) =>
			localPattern.test(p.url),
		);
		...
		// 18. overallSeoScore
		const scores = htmlPages
			.map((p) => p.techHealthScore)
			.filter((s) => s !== null) as number[];
		if (scores.length > 0) {
			profile.overallSeoScore = Math.round(
				scores.reduce((a, b) => a + b, 0) / scores.length,
			);
		}

		// 19. pagesIndexed
		profile.pagesIndexed = pages.filter(
			(p) => p.indexable !== false && p.statusCode === 200,
		).length;

		// 20. Authority markers
		profile.referringDomains = Math.max(
			...pages.map((p) => p.referringDomains || 0),
		);
		...
		if (rootPage?.socialLinks) {
			...
			if (rootPage.socialLinks.facebook)
				profile.facebookUrl = `https://facebook.com/${domain}`;
			if (rootPage.socialLinks.twitter || rootPage.socialLinks.x)
				profile.twitterUrl = `https://twitter.com/${domain}`;
			if (rootPage.socialLinks.instagram)
				profile.instagramUrl = `https://instagram.com/${domain}`;
			if (rootPage.socialLinks.youtube)
				profile.youtubeUrl = `https://youtube.com/${domain}`;
		}
		...
		profile.socialGrowthRate =
			socialActivity > 0 ? Math.min(100, socialActivity * 2) : null;
		...
		// ── Search Visibility ──
		const allGscClicks = htmlPages.reduce(
			(sum, p) => sum + (p.gscClicks || 0),
			0,
		);
		const allGscImpressions = htmlPages.reduce(
			(sum, p) => sum + (p.gscImpressions || 0),
			0,
		);
		profile.estimatedOrganicTraffic = allGscClicks || null;

		const pagesWithPosition = htmlPages.filter(
			(p) => p.gscPosition && p.gscPosition > 0,
		);
		if (pagesWithPosition.length > 0) {
			const posSum = pagesWithPosition.reduce(
				(sum, p) => sum + (p.gscPosition || 0),
				0,
			);
			profile.avgOrganicPosition =
				Math.round((posSum / pagesWithPosition.length) * 10) / 10;
			profile.keywordsInTop3 = pagesWithPosition.filter(
				(p) => (p.gscPosition || 100) <= 3,
			).length;
			profile.keywordsInTop10 = pagesWithPosition.filter(
				(p) => (p.gscPosition || 100) <= 10,
			).length;
			profile.keywordsInTop20 = pagesWithPosition.filter(
				(p) => (p.gscPosition || 100) <= 20,
			).length;
		...
		if (pagesWithPosition.length > 0) {
			const brandedClicks = brandedPages.reduce(
				(s, p) => s + (p.gscClicks || 0),
				0,
			);
			profile.brandedTrafficPct =
				allGscClicks > 0
		...
		if (profile.keywordsInTop10 && pages.length > 0) {
			profile.shareOfVoice = Math.min(
				100,
				Math.round((profile.keywordsInTop10 / pages.length) * 100),
			);
		}
		profile.keywordOverlapPct = profile.totalRankingKeywords
			? Math.round(
					((profile.keywordsInTop10 || 0) /
						Math.max(1, profile.totalRankingKeywords)) *
						100,
				)
			: null;
		...
		if (indexablePages.length > 0) {
			const totalWords = indexablePages.reduce(
				(s, p) => s + (p.wordCount || 0),
				0,
			);
		...
		const clusters = new Set(
			indexablePages.map((p) => p.topicCluster).filter(Boolean),
		);
		profile.topicCoverageBreadth = clusters.size || null;

		// Content efficiency
		if (
			profile.totalIndexablePages &&
			profile.totalIndexablePages > 0 &&
			allGscClicks > 0
		) {
			profile.contentEfficiency = Math.round(
				allGscClicks / profile.totalIndexablePages,
			);
		}

		// Duplicate %
		const duplicates = indexablePages.filter(
			(p) => p.exactDuplicate || p.nearDuplicateMatch,
		);
		profile.duplicateContentPct =
			indexablePages.length > 0
		...
		profile.thinContentPct =
			indexablePages.length > 0
				? Math.round((thinPages.length / indexablePages.length) * 100)
				: null;

		// Schema coverage %
		const schemaPages = indexablePages.filter(
			(p) => p.schemaTypes && p.schemaTypes.length > 0,
		);
		profile.schemaCoveragePct =
			indexablePages.length > 0
				? Math.round((schemaPages.length / indexablePages.length) * 100)
				: null;

		// FAQ/HowTo count
		profile.faqHowToCount =
			indexablePages.filter(
				(p) =>
		...
		profile.recentNewPages =
			indexablePages.filter((p) => {
				const ts = Date.parse(p.visibleDate || "");
				return (
					Number.isFinite(ts) && ts >= Date.now() - 30 * 24 * 60 * 60 * 1000
				);
			}).length || null;
		...
		profile.averagePageAge =
			pageAges.length > 0
				? Math.round(
						pageAges.reduce((sum, age) => sum + age, 0) / pageAges.length,
					)
				: null;
		if (indexablePages.length > 0) {
			const recent90 = indexablePages.filter((p) => {
				const ts = Date.parse(p.visibleDate || p.lastModified || "");
				return (
					Number.isFinite(ts) && ts >= Date.now() - 90 * 24 * 60 * 60 * 1000
				);
			}).length;
			...
			if (older90 > 0) {
				profile.contentVelocityTrend = Math.round(
					((recent90 - older90) / older90) * 100,
				);
			}
		}

		// ── Technical Health ──
		const techScores = indexablePages
			.map((p) => p.techHealthScore)
			.filter((s): s is number => s !== null && s !== undefined);
		profile.techHealthScore =
			techScores.length > 0
				? Math.round(techScores.reduce((a, b) => a + b, 0) / techScores.length)
				: null;

		// CWV pass rate
		const cwvPages = indexablePages.filter(
			(p) => p.lcp !== null && p.lcp !== undefined,
		);
		const cwvPassing = cwvPages.filter(
			(p) => (p.lcp || 0) <= 2500 && (p.cls || 0) <= 0.1 && (p.inp || 0) <= 200,
		);
		profile.cwvPassRate =
			cwvPages.length > 0
		...
		const mobileLhScores = indexablePages
			...
			.filter((s): s is number => s !== null && s !== undefined);
		profile.mobileFriendlinessScore =
			mobileLhScores.length > 0
				? Math.round(
						mobileLhScores.reduce((a, b) => a + b, 0) / mobileLhScores.length,
					)
				: null;
		...
		if (rootPage) {
			...
			const secScore =
				...
				(hasHsts ? 25 : 0) +
				(hasCsp ? 20 : 0) +
				((rootPage as any).sslGrade === "A"
					? 15
					: (rootPage as any).sslGrade === "B"
						? 10
						: 5);
			profile.securityGrade =
				secScore >= 85
					? "A"
					: secScore >= 65
						? "B"
		...
		const depthOk = indexablePages.filter(
			(p) => (p.crawlDepth || 0) <= 3,
		).length;
		profile.crawlabilityScore =
			indexablePages.length > 0
				? Math.round(
						((robotsOk + inSitemap + depthOk) / (indexablePages.length * 3)) *
							100,
					)
				: null;
		...
		const citationScores = indexablePages
			.map((p) => p.citationWorthiness)
			.filter((s): s is number => s !== null && s !== undefined);
		profile.avgCitationWorthiness =
		...
		if (rootPage && (rootPage as any).aiBotAccess) {
			...
			const blocked = Object.values(access).every(
				(v) => v === "blocked" || v === "disallow",
			);
			const open = Object.values(access).every((v) => v === "allow");
			profile.aiBotAccessPolicy = blocked
				? "blocked"
				: open
					? "open"
					: "partial";
		...
		const passageReady = indexablePages.filter(
			(p) => (p.passageReadiness || 0) >= 60,
		);
		profile.passageReadyPct =
			indexablePages.length > 0
				? Math.round((passageReady.length / indexablePages.length) * 100)
				: null;

		const snippetReady = indexablePages.filter(
			(p) => p.hasFeaturedSnippetPatterns || p.answerBoxReady,
		);
		profile.featuredSnippetReadyPct =
			indexablePages.length > 0
				? Math.round((snippetReady.length / indexablePages.length) * 100)
				: null;

		// ── User Experience & Conversion ──
		const bouncePages = indexablePages.filter(
			(p) => p.ga4BounceRate !== null && p.ga4BounceRate !== undefined,
		);
		if (bouncePages.length > 0) {
			profile.avgBounceRate = Math.round(
				(bouncePages.reduce((s, p) => s + (p.ga4BounceRate || 0), 0) /
					bouncePages.length) *
					100,
			);
		}

		const sessionPages = indexablePages.filter(
			(p) => p.ga4EngagementTimePerPage || p.ga4AvgSessionDuration,
		);
		if (sessionPages.length > 0) {
			profile.avgSessionDuration = Math.round(
				sessionPages.reduce(
					(s, p) =>
						s + (p.ga4EngagementTimePerPage || p.ga4AvgSessionDuration || 0),
					0,
				) / sessionPages.length,
			);
		}
		...
		const conversionPatterns =
			/(signup|register|checkout|contact|demo|trial|pricing|quote|get-started|book)/i;
		profile.conversionPathCount =
			indexablePages.filter((p) => conversionPatterns.test(p.url)).length ||
			null;

		// CTA density
		const ctaPages = indexablePages.filter(
			(p) => p.ctaTexts && p.ctaTexts.length > 0,
		);
		profile.ctaDensityScore =
			indexablePages.length > 0
				? Math.min(
						100,
						Math.round((ctaPages.length / indexablePages.length) * 100),
					)
				: null;
		...
		if (rootPage) {
			...
			profile.emailOptInQuality =
				hasOptIn && hasOffer ? "Strong" : hasOptIn ? "Basic" : "None";
		}
...
Return JSON with these fields (use null if not determinable):
{
  ...
  "pricingComparison": "Brief pricing description or null"
	...
	static fromOwnCrawlSession(
		...
		return this.merge(createEmptyProfile(domain), crawlProfile, {
...
Return JSON with these fields (use null if not determinable):
{
  ...
  "pricingComparison": "Brief pricing description or null"
			...
			if (data.valueProposition && !profile.valueProposition)
				profile.valueProposition = data.valueProposition;
			if (data.employeeCountEstimate && !profile.employeeCountEstimate)
				profile.employeeCountEstimate = data.employeeCountEstimate;
			if (data.contentQualityAssessment && !profile.contentQualityAssessment)
				profile.contentQualityAssessment = data.contentQualityAssessment;
			if (data.pricingModel && !profile.pricingModel)
				profile.pricingModel = data.pricingModel;
			if (data.optInOffer && !profile.optInOffer)
				profile.optInOffer = data.optInOffer;
			if (data.shippingOffers && !profile.shippingOffers)
				profile.shippingOffers = data.shippingOffers;
			if (data.topContentTypeByShares && !profile.topContentTypeByShares)
				profile.topContentTypeByShares = data.topContentTypeByShares;
			if (data.emailOptInQuality && !profile.emailOptInQuality)
				profile.emailOptInQuality = data.emailOptInQuality;
			if (
				data.offersSameProducts !== undefined &&
				profile.offersSameProducts === null
			)
				profile.offersSameProducts = data.offersSameProducts;
			if (data.pricingComparison && !profile.pricingComparison)
				profile.pricingComparison = data.pricingComparison;
	...
	static mergeEnrichment(
		...
		if (enrichmentData.dataSourcesUsed) {
			...
			for (const src of enrichmentData.dataSourcesUsed) {
				if (!existing.dataSourcesUsed.includes(src))
					existing.dataSourcesUsed.push(src);
			}
		}
...
Return a JSON object with this exact structure:
{
  ...
  "competitivePosition": "1st" | "2nd" | "3rd" | "4th" | "5th+"
		...
		try {
			...
			return {
				executiveSummary:
					"Unable to generate brief. Run a crawl with AI enabled and try again.",
				competitorAnalyses: [],
				topAdvantages: [],
				topVulnerabilities: [],
				recommendedActions: [],
