...
export class KeywordUploadMerger {
	private static chooseStrategicRow(
		rows: KeywordUploadRow[],
	): KeywordUploadRow {
		return [...rows].sort((left, right) => {
			if ((right.traffic || 0) !== (left.traffic || 0))
				return (right.traffic || 0) - (left.traffic || 0);
			if ((right.volume || 0) !== (left.volume || 0))
				return (right.volume || 0) - (left.volume || 0);
			const leftPos =
				left.position > 0 ? left.position : Number.POSITIVE_INFINITY;
			const rightPos =
				right.position > 0 ? right.position : Number.POSITIVE_INFINITY;
			return leftPos - rightPos;
		})[0];
	}

	private static chooseBestRankingRow(
		rows: KeywordUploadRow[],
	): KeywordUploadRow {
		return [...rows].sort((left, right) => {
			const leftPos =
		...
		for (const [canonical, matchedRows] of rowsByCanonical.entries()) {
			...
			if (page) {
				updates.push({
					...
					bestKeywordSource: bestRow?.keyword
						? "upload"
						: page.bestKeywordSource,
