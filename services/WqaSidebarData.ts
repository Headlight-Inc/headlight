...
export interface WqaTrendingPage {
  url: string;
  pagePath: string;
  pageCategory: string;
  sessions: number;
  ...
  clicks: number;
  ctr: number;
  valueRank: number; // impressions + sessions combined rank
}

export interface WqaPositionBand {
  band: '1-3' | '4-10' | '11-20' | '21-50' | '51+';
  count: number;
}

export interface WqaTrendsData {
  growing: WqaTrendingPage[];
  declining: WqaTrendingPage[];
  positionBands: WqaPositionBand[];
  noImpressions: number;  // pages with 0 impressions
  noTraffic: number;      // pages with 0 ga4Sessions
  newlyRanking: number;   // pages that started ranking since last crawl (gscImpressions > 0 but no prior data)
}

function bandForPosition(pos: number): WqaPositionBand['band'] {
  if (pos <= 3) return '1-3';
...
export function computeWqaTrendsData(pages: any[]): WqaTrendsData {
  const htmlPages = pages.filter((p) => p.isHtmlPage && asNum(p.statusCode) === 200);

  // Build trending page objects for pages with meaningful data
  const trending: WqaTrendingPage[] = htmlPages
    .filter((p) => asNum(p.gscImpressions) > 50 || asNum(p.ga4Sessions) > 10)
    .map((p) => ({
      url: String(p.url || ''),
      pagePath: String(p.pagePath || p.url || ''),
      pageCategory: String(p.pageCategory || 'other'),
      sessions: asNum(p.ga4Sessions),
      sessionsDeltaPct: asNum(p.sessionsDeltaPct),
      impressions: asNum(p.gscImpressions),
      impressionsDeltaPct: p.gscImpressionsDelta !== null && p.gscImpressionsDelta !== undefined
        ? asNum(p.gscImpressionsDelta)
        : null,
      position: asNum(p.gscPosition),
      clicks: asNum(p.gscClicks),
      ctr: asNum(p.gscCtr),
      // Combined rank: pages with more sessions + impressions rank higher
      valueRank: asNum(p.ga4Sessions) * 2 + asNum(p.gscImpressions) * 0.01,
    }));

  // Top growing: sorted by sessionsDeltaPct descending, only pages actively declining/growing
  const growing = trending
    .filter((p) => p.sessionsDeltaPct > 0.1 && p.sessions > 10)
    .sort((a, b) => b.sessionsDeltaPct - a.sessionsDeltaPct)
    .slice(0, 15);

  // Top declining: sorted by sessionsDeltaPct ascending
  const declining = trending
    .filter((p) => p.sessionsDeltaPct < -0.1 && p.sessions > 10)
    .sort((a, b) => a.sessionsDeltaPct - b.sessionsDeltaPct)
    .slice(0, 15);

  // Position band distribution
  const bandCounts: Record<WqaPositionBand['band'], number> = {
    '1-3': 0, '4-10': 0, '11-20': 0, '21-50': 0, '51+': 0,
  };
  for (const p of htmlPages) {
    const pos = asNum(p.gscPosition);
    if (pos > 0) {
      bandCounts[bandForPosition(pos)] += 1;
    }
  }
  const positionBands: WqaPositionBand[] = (
    ['1-3', '4-10', '11-20', '21-50', '51+'] as WqaPositionBand['band'][]
  ).map((band) => ({ band, count: bandCounts[band] }));

  // Zero-signal counts
  const noImpressions = htmlPages.filter((p) => asNum(p.gscImpressions) === 0).length;
  const noTraffic = htmlPages.filter((p) => asNum(p.ga4Sessions) === 0).length;

  // Newly ranking: has impressions but sessionsDeltaPct is null or very high
  // (proxy: pages with impressions > 0 but no prior sessions data)
  const newlyRanking = htmlPages.filter(
    (p) =>
      asNum(p.gscImpressions) > 0 &&
      asNum(p.ga4Sessions) === 0 &&
      p.sessionsDeltaPct === null
  ).length;
