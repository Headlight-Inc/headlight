...
export const FALLBACK_FORMULAS: Record<string, FallbackFn> = {
  ...
  brandedTrafficPct: (p) => p.hasKnowledgePanel ? 40 : (p.domainAge || 0) > 5 ? 25 : 15,
  trafficTrend30d: (p) => {
  ...
  serpFeatureCount: (p) => {
    ...
    const pages = p.totalIndexablePages || 0;
    return schema > 0 && pages > 0 ? Math.round((pages * (schema / 100)) / 10) : null;
  },
  domainAuthority: (p) => {
  ...
  ahrefsDR: (p) => p.mozDA ? Math.round(p.mozDA * 0.95) : p.domainAuthority || null,
  mozDA: (p) => p.ahrefsDR ? Math.round(p.ahrefsDR * 1.05) : p.domainAuthority || null,
  mozPA: (p) => {
  ...
  mobileFriendlinessScore: (p) => {
    const cms = (p.cmsType || '').toLowerCase();
    if (cms.includes('shopify') || cms.includes('squarespace') || cms.includes('wix')) return 85;
    if (cms.includes('wordpress')) return 75;
    return 65;
  },
  jsRenderDependencyPct: (p) => {
    const stack = (p.techStackSignals || []).map((s) => s.toLowerCase());
    if (stack.some((s) => /react|angular|vue|next|nuxt|svelte/.test(s))) return 65;
  ...
  socialTotalFollowers: (p) =>
    (p.facebookFans || 0) + (p.twitterFollowers || 0) + (p.youtubeSubscribers || 0) +
      (p.instagramFollowers || 0) + (p.linkedinFollowers || 0) + (p.tiktokFollowers || 0) || null,
  socialGrowthRate: (p) => {
    const activity = (p.facebookUpdatesPerMonth || 0) + (p.twitterUpdatesPerMonth || 0) + (p.youtubeUpdatesPerMonth || 0);
  ...
  aggregateReviewScore: (p) => {
    ...
    if (p.trustpilotScore && p.trustpilotReviewCount) scores.push({ s: p.trustpilotScore, c: p.trustpilotReviewCount });
    if (p.g2Rating && p.g2ReviewCount) scores.push({ s: p.g2Rating, c: p.g2ReviewCount });
    if (p.capterraRating && p.capterraReviewCount) scores.push({ s: p.capterraRating, c: p.capterraReviewCount });
    if (p.googleReviewScore && p.googleReviewCount) scores.push({ s: p.googleReviewScore, c: p.googleReviewCount });
  ...
  aggregateReviewCount: (p) =>
    (p.trustpilotReviewCount || 0) + (p.g2ReviewCount || 0) + (p.capterraReviewCount || 0) + (p.googleReviewCount || 0) || null,
  ...
  contentThreatScore: (p) => {
    ...
    const q = p.contentQualityAssessment === 'Excellent' ? 90 :
      p.contentQualityAssessment === 'Good' ? 70 :
      p.contentQualityAssessment === 'Average' ? 40 : 20;
