...
import { scrapeG2, scrapeCapterra, scrapeGoogleReviews } from './scrapers/ReviewScrapers';
import { scrapeYouTube, scrapeTwitterViaNitter, scrapeInstagram, scrapeTikTok } from './scrapers/SocialScrapers';
import { scrapeGoogleSiteCount, checkKnowledgePanel, checkPressCoverage, checkDirectoryCitations } from './scrapers/GoogleSerpScraper';
...
export async function runPhaseA(ctx: EnrichmentContext): Promise<void> {
  ...
  results.forEach((r, i) => {
    if (r.status === 'fulfilled' && r.value) mergeInto(profile, r.value, names[i]);
  });
  ...
  try {
    ...
    if (robotsResp.ok) {
      ...
      const blocked = ['GPTBot', 'Google-Extended', 'CCBot', 'anthropic-ai', 'ClaudeBot']
        .filter((bot) => robotsTxt.toLowerCase().includes(`user-agent: ${bot.toLowerCase()}`) && robotsTxt.toLowerCase().includes('disallow: /'));
  ...
  try {
    const llmsResp = await fetch(`https://${domain}/llms.txt`, { signal: AbortSignal.timeout(3000) });
...
export async function runPhaseB(ctx: EnrichmentContext): Promise<void> {
  ...
  try {
    mergeInto(profile, await scrapeGoogleSiteCount(domain, fetchFn), 'google-site');
  } catch {
    // ignore
  }
  ...
  if (profile.businessName) {
    try {
      mergeInto(profile, await checkKnowledgePanel(profile.businessName, fetchFn), 'google-kp');
    } catch {
      // ignore
    }
  }
...
export async function runPhaseC(ctx: EnrichmentContext): Promise<void> {
  ...
  const sources = [
    ...
    { name: 'Majestic', fn: () => scrapeMajestic(domain, fetchFn), delay: 8000 },
    { name: 'SimilarWeb', fn: () => scrapeSimilarWeb(domain, fetchFn), delay: 6000 },
    { name: 'SEMrush', fn: () => scrapeSemrush(domain, fetchFn), delay: 5000 },
    { name: 'Ubersuggest', fn: () => scrapeUbersuggest(domain, fetchFn), delay: 5000 },
  ];

  const businessName = profile.businessName || domain.split('.')[0];
  const reviewSources = [
    { name: 'G2', fn: () => scrapeG2(businessName, fetchFn), delay: 5000 },
    { name: 'Capterra', fn: () => scrapeCapterra(businessName, fetchFn), delay: 5000 },
    { name: 'Google Reviews', fn: () => scrapeGoogleReviews(businessName, fetchFn), delay: 3000 },
  ];

  const socialSources = [
    profile.youtubeUrl ? { name: 'YouTube', fn: () => scrapeYouTube(profile.youtubeUrl!, fetchFn), delay: 3000 } : null,
    profile.twitterUrl ? { name: 'Twitter', fn: () => scrapeTwitterViaNitter(profile.twitterUrl!, fetchFn), delay: 3000 } : null,
    profile.instagramUrl ? { name: 'Instagram', fn: () => scrapeInstagram(profile.instagramUrl!, fetchFn), delay: 3000 } : null,
    profile.tiktokUrl ? { name: 'TikTok', fn: () => scrapeTikTok(profile.tiktokUrl!, fetchFn), delay: 3000 } : null,
  ].filter(Boolean) as Array<{ name: string; fn: () => Promise<Partial<CompetitorProfile>>; delay: number }>;

  const localSources = [
    { name: 'Press Coverage', fn: () => checkPressCoverage(businessName, fetchFn), delay: 2000 },
    { name: 'Directory Citations', fn: () => checkDirectoryCitations(businessName, fetchFn), delay: 6000 },
  ];

  const allSources = [...sources, ...reviewSources, ...socialSources, ...localSources];
  ...
  for (let i = 0; i < allSources.length; i++) {
    ...
    try {
      ...
      if (data.estimatedOrganicTraffic) trafficEstimates.set(source.name, data.estimatedOrganicTraffic);
      if (data.totalRankingKeywords) kwEstimates.set(source.name, data.totalRankingKeywords);
      if (data.referringDomains) rdEstimates.set(source.name, data.referringDomains);
  ...
  if (trafficEstimates.size >= 2) {
    ...
    profile.estimatedOrganicTraffic = Math.round(values.reduce((a, b) => a + b, 0) / values.length);
  }
  if (kwEstimates.size >= 2) {
    const values = [...kwEstimates.values()];
    profile.totalRankingKeywords = Math.round(values.reduce((a, b) => a + b, 0) / values.length);
  }
  if (rdEstimates.size >= 2) {
    const values = [...rdEstimates.values()];
    profile.referringDomains = Math.round(values.reduce((a, b) => a + b, 0) / values.length);
  }

  profile.lastEnrichmentPhase = 'C';
  profile.dataConfidence = (profile.dataSourcesUsed?.length || 0) >= 5 ? 'medium' : 'low';
  onProgress?.('C', 100, 'Phase C complete');
}

export async function runPhaseD(ctx: EnrichmentContext, crawlPages: any[]): Promise<void> {
  const { profile, onProgress } = ctx;
  onProgress?.('D', 0, 'Processing crawl results...');

  const { CompetitorProfileBuilder } = await import('../CompetitorProfileBuilder');
  const crawlProfile = CompetitorProfileBuilder.fromCrawlPages(ctx.domain, crawlPages);
...
export async function runFullEnrichment(
  ...
  const ctx: EnrichmentContext = { domain, profile, fetchFn, userApiKeys, onProgress };
...
function parseHomepageQuick(html: string, _domain: string): Partial<CompetitorProfile> {
  const result: Partial<CompetitorProfile> = {};
  const lower = html.toLowerCase();

  const socialPatterns: Array<{ key: keyof CompetitorProfile; pattern: RegExp }> = [
    { key: 'facebookUrl', pattern: /href="(https?:\/\/(?:www\.)?facebook\.com\/[^"]+)"/i },
    { key: 'twitterUrl', pattern: /href="(https?:\/\/(?:www\.)?(?:twitter|x)\.com\/[^"]+)"/i },
    { key: 'instagramUrl', pattern: /href="(https?:\/\/(?:www\.)?instagram\.com\/[^"]+)"/i },
    { key: 'youtubeUrl', pattern: /href="(https?:\/\/(?:www\.)?youtube\.com\/(?:c(?:hannel)?\/|@)[^"]+)"/i },
    { key: 'linkedinUrl', pattern: /href="(https?:\/\/(?:www\.)?linkedin\.com\/company\/[^"]+)"/i },
    { key: 'tiktokUrl', pattern: /href="(https?:\/\/(?:www\.)?tiktok\.com\/@[^"]+)"/i },
  ];
  ...
  if (lower.includes('wp-content') || lower.includes('wp-includes')) result.cmsType = 'WordPress';
  ...
  if (lower.includes('gtag') || lower.includes('google-analytics') || lower.includes('googletagmanager')) stack.push('Google Analytics');
  ...
  result.hasLiveChat = lower.includes('intercom') || lower.includes('drift') || lower.includes('zendesk') || lower.includes('crisp') || lower.includes('tidio') || lower.includes('livechat') || lower.includes('tawk');
  result.hasEmailOptIn = lower.includes('type="email"') && lower.includes('subscribe');
  result.hasFreeTrial = lower.includes('free trial') || lower.includes('start free') || lower.includes('try free') || lower.includes('freemium');

  const adPlatforms: string[] = [];
  if (lower.includes('googleads') || lower.includes('google_conversion') || lower.includes("gtag('event',")) adPlatforms.push('Google Ads');
  if (lower.includes('fbevents') || lower.includes('facebook.com/tr') || lower.includes('fbq(')) adPlatforms.push('Meta Pixel');
  if (lower.includes('linkedin.com/px') || lower.includes('snap.licdn.com')) adPlatforms.push('LinkedIn Insight');
  if (lower.includes('tiktok.com/i18n/pixel')) adPlatforms.push('TikTok Pixel');
  if (lower.includes('ads.twitter.com') || lower.includes('twq(')) adPlatforms.push('Twitter Pixel');
  if (adPlatforms.length > 0) result.adPlatformsDetected = adPlatforms;

  result.hasConversionTracking = lower.includes('conversion') && (lower.includes('gtag') || lower.includes('fbq'));
  result.hasRemarketingTags = lower.includes('remarketing') || lower.includes('retargeting') || lower.includes('adroll') || lower.includes('criteo');

  const adDivs = (html.match(/googlesyndication|adsense|ad-slot|data-ad/gi) || []).length;
  ...
  for (const block of jsonLds) {
    try {
      ...
      if (json['@type'] === 'Organization' && json.name) result.businessName = json.name;
      if (Array.isArray(json.sameAs)) {
        for (const url of json.sameAs) {
          if (url.includes('facebook.com') && !result.facebookUrl) result.facebookUrl = url;
          if ((url.includes('twitter.com') || url.includes('x.com')) && !result.twitterUrl) result.twitterUrl = url;
          if (url.includes('instagram.com') && !result.instagramUrl) result.instagramUrl = url;
          if (url.includes('youtube.com') && !result.youtubeUrl) result.youtubeUrl = url;
          if (url.includes('linkedin.com') && !result.linkedinUrl) result.linkedinUrl = url;
          if (url.includes('tiktok.com') && !result.tiktokUrl) result.tiktokUrl = url;
        }
      }
    } catch {
