...
export async function scrapeTrustpilot(
	...
	try {
		...
		if (jsonLdMatches) {
			for (const match of jsonLdMatches) {
				try {
					...
					const rating =
						data.aggregateRating || data?.mainEntity?.aggregateRating;
					if (rating) {
						result.trustpilotScore = parseFloat(rating.ratingValue) || null;
						result.trustpilotReviewCount =
							parseInt(rating.reviewCount, 10) || null;
						break;
					}
				} catch {
		...
		if (!result.trustpilotScore) {
			...
			if (countMatch)
				result.trustpilotReviewCount = parseInt(countMatch[1], 10);
		}
	} catch {
		// ignore failures
	}
