...
export async function scrapeSpyFu(
	...
	try {
		...
		result.estimatedOrganicTraffic =
			extractNumber(text, "Monthly SEO Clicks") ??
			extractNumber(text, "Organic Monthly Clicks");
		result.totalRankingKeywords =
			extractNumber(text, "SEO Keywords") ??
			extractNumber(text, "Organic Keywords");
		result.adsTraffic =
			extractNumber(text, "Monthly PPC Clicks") ??
			extractNumber(text, "Paid Monthly Clicks");
		result.adsTrafficCost =
			extractNumber(text, "Monthly PPC Budget") ??
			extractNumber(text, "Monthly Ad Budget");
		result.ppcKeywordsCount =
			extractNumber(text, "PPC Keywords") ??
			extractNumber(text, "Paid Keywords");
		...
		if (keywordData.length > 0) {
			...
			result.keywordsInTop10 = keywordData.filter(
				(k) => k.position <= 10,
			).length;
			result.keywordsInTop20 = keywordData.filter(
				(k) => k.position <= 20,
			).length;
			const posSum = keywordData.reduce((s, k) => s + k.position, 0);
			result.avgOrganicPosition =
				Math.round((posSum / keywordData.length) * 10) / 10;

			const domainRoot = domain.split(".")[0].toLowerCase();
			const branded = keywordData.filter((k) =>
				k.keyword.toLowerCase().includes(domainRoot),
			);
			result.brandedTrafficPct = Math.round(
				(branded.length / keywordData.length) * 100,
			);
		}

		const seCostMatch = text.match(
			/Organic.*?Value[\s\n:]*\$?([\d,\.]+[KMB]?)/i,
		);
		if (seCostMatch)
			result.seTrafficCost = parseAbbreviatedNumber(seCostMatch[1]);
	} catch (err) {
...
function extractKeywordTable(
	html: string,
): Array<{ keyword: string; position: number; volume: number }> {
	const results: Array<{ keyword: string; position: number; volume: number }> =
		[];
	const rowPattern =
		/<tr[^>]*>[\s\S]*?<td[^>]*>([^<]+)<\/td>[\s\S]*?<td[^>]*>(\d+)<\/td>[\s\S]*?<td[^>]*>([\d,]+)<\/td>/g;
