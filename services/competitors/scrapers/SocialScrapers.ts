...
export async function scrapeYouTube(
  ...
  try {
    ...
    if (subMatch) result.youtubeSubscribers = parseAbbreviatedNumber(subMatch[1]);

    const videoMatch = html.match(/([\.\d,]+)\s*videos?/i);
    if (videoMatch) result.youtubeVideoCount = parseInt(videoMatch[1].replace(/,/g, ''), 10);

    const channelIdMatch = html.match(/"channelId"\s*:\s*"([^"]+)"/);
    if (channelIdMatch) {
      try {
        const rssHtml = await fetchFn(`https://www.youtube.com/feeds/videos.xml?channel_id=${channelIdMatch[1]}`);
        const publishDates = [...rssHtml.matchAll(/<published>([^<]+)<\/published>/g)]
          .map((m) => Date.parse(m[1]))
          .filter((d) => !Number.isNaN(d));
        if (publishDates.length >= 2) {
          const thirtyDaysAgo = Date.now() - 30 * 86400000;
          result.youtubeUpdatesPerMonth = publishDates.filter((d) => d >= thirtyDaysAgo).length;
        }
      } catch {
...
export async function scrapeTwitterViaNitter(
  ...
  const nitterInstances = ['nitter.privacydev.net', 'nitter.poast.org', 'nitter.1d4.us'];
...
export async function scrapeInstagram(
  ...
  try {
    ...
    if (metaMatch) result.instagramFollowers = parseAbbreviatedNumber(metaMatch[1]);
...
export async function scrapeTikTok(
  ...
  try {
    ...
    const followerMatch = html.match(/"followerCount"\s*:\s*(\d+)/) || html.match(/([\.\d,]+[KMB]?)\s*Followers/i);
    if (followerMatch) result.tiktokFollowers = parseAbbreviatedNumber(followerMatch[1]);
  } catch (err) {
