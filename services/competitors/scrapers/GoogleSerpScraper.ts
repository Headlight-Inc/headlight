...
export async function scrapeGoogleSiteCount(
	...
	try {
		...
		const countMatch =
			html.match(/About ([\d,]+) results/i) || html.match(/([\d,]+) results/i);
		if (countMatch)
			result.googleIndexedPages = parseInt(countMatch[1].replace(/,/g, ""), 10);
	} catch {
		// ignore
	}
...
export async function checkKnowledgePanel(
	...
	try {
		...
		result.hasKnowledgePanel =
			...
			html.includes("kno-result") ||
			html.includes('"knowledgePanelData"');
	} catch {
		// ignore
	}
	return result;
...
export async function checkLocalRanking(
	...
	try {
		const html = await fetchFn(
			`https://www.google.com/search?q=${encodeURIComponent(`${businessType} ${city}`)}`,
		);
...
export async function checkPressCoverage(
	...
	try {
		const html = await fetchFn(
			`https://www.google.com/search?q=${encodeURIComponent(`"${businessName}"`)}&tbm=nws`,
		);
		result.localPressCoverage =
