...
export async function scrapeSimilarWeb(
  ...
  try {
    ...
    if (nextDataMatch) {
      try {
        ...
        if (overview) {
          ...
          result.avgBounceRate = overview.bounceRate ? Math.round(overview.bounceRate * 100) : null;
          result.avgSessionDuration = overview.avgVisitDuration || null;
          result.pagesPerVisit = overview.pagesPerVisit ? Math.round(overview.pagesPerVisit * 10) / 10 : null;
        }
      } catch {
        // ignore parse issues
      }
    ...
    if (!result.estimatedOrganicTraffic) {
      ...
      if (visitsMatch) result.estimatedOrganicTraffic = parseAbbreviatedNumber(visitsMatch[1]);
    }
    if (!result.avgBounceRate) {
      const bounceMatch = text.match(/Bounce Rate[\s:]*([\.\d]+)%/i);
      if (bounceMatch) result.avgBounceRate = Math.round(parseFloat(bounceMatch[1]));
    }
    if (!result.avgSessionDuration) {
      const durationMatch = text.match(/(\d{2}):(\d{2}):(\d{2})/);
      if (durationMatch) {
        result.avgSessionDuration = parseInt(durationMatch[1], 10) * 3600 + parseInt(durationMatch[2], 10) * 60 + parseInt(durationMatch[3], 10);
      }
    }
    ...
    while ((siteMatch = similarPattern.exec(html)) !== null) {
      ...
      if (site !== domain.toLowerCase() && !site.includes('similarweb')) similarSites.add(site);
    }
    if (similarSites.size > 0) result.similarSites = [...similarSites].slice(0, 10);
