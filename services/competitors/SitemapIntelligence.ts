...
export async function analyzeSitemap(
	...
	const blogPattern =
		/\/(blog|news|articles?|insights?|resources?|learn|posts?)\//i;
	...
	const otherUrls =
		urls.length - blogUrls.length - productUrls.length - landingUrls.length;
	...
	result.contentTypeBreakdown =
		[
			blogUrls.length > 0
				? `Blog ${Math.round((blogUrls.length / total) * 100)}%`
				: null,
			productUrls.length > 0
				? `Product ${Math.round((productUrls.length / total) * 100)}%`
				: null,
			landingUrls.length > 0
				? `Landing ${Math.round((landingUrls.length / total) * 100)}%`
				: null,
		]
			.filter(Boolean)
			.join(", ") || "General";
	...
	const blogWithDates = blogUrls.filter(
		(u) => u.lastmod && !Number.isNaN(Date.parse(u.lastmod)),
	);
	const recentBlog30d = blogWithDates.filter(
		(u) => Date.parse(u.lastmod as string) >= thirtyDaysAgo,
	);
	const recentBlog90d = blogWithDates.filter(
		(u) => Date.parse(u.lastmod as string) >= ninetyDaysAgo,
	);
	const recentBlogYear = blogWithDates.filter(
		(u) => Date.parse(u.lastmod as string) >= yearAgo,
	);
	...
	if (recentBlogYear.length > 0) {
		result.blogPostsPerMonth =
			Math.round((recentBlogYear.length / 12) * 10) / 10;
	}

	const allWithDates = urls.filter(
		(u) => u.lastmod && !Number.isNaN(Date.parse(u.lastmod)),
	);
	const freshPages = allWithDates.filter(
		(u) => Date.parse(u.lastmod as string) >= sixMonthsAgo,
	);
	if (allWithDates.length > 0) {
		result.contentFreshnessScore = Math.round(
			(freshPages.length / allWithDates.length) * 100,
		);
	}

	const ages = allWithDates.map(
		(u) => (now - Date.parse(u.lastmod as string)) / (30 * 86400000),
	);
	if (ages.length > 0) {
		result.averagePageAge = Math.round(
	...
	if (prev90d.length > 0) {
		const recent90dAll = allWithDates.filter(
			(u) => Date.parse(u.lastmod as string) >= ninetyDaysAgo,
		);
		result.contentVelocityTrend = Math.round(
			((recent90dAll.length - prev90d.length) / prev90d.length) * 100,
		);
	}
	...
	for (const sitemapUrl of sitemapUrls) {
		try {
			...
			if (xml.includes("<sitemapindex")) {
				const childUrls = [...xml.matchAll(/<loc>([^<]+)<\/loc>/g)].map(
					(m) => m[1],
				);
				for (const childUrl of childUrls.slice(0, 10)) {
					try {
						const childResp = await fetch(childUrl, {
							headers: {
								"User-Agent": "Mozilla/5.0 (compatible; HeadlightBot/1.0)",
							},
							signal: AbortSignal.timeout(8000),
						});
