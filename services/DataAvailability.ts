...
export function detectDataAvailability(pages: any[]): DataAvailability {
  if (pages.length === 0) return { gsc: false, ga4: false, backlinks: false, cwv: false };

  const gsc = pages.some((p) => p.gscImpressions !== undefined && p.gscImpressions !== null);
  const ga4 = pages.some((p) => p.ga4Sessions !== undefined && p.ga4Sessions !== null);
  const backlinks = pages.some((p) => Number(p.referringDomains || 0) > 0 || Number(p.backlinks || 0) > 0);
