...
class TechDebtService {
    static calculate(pages: any[]): TechDebtBreakdown {
        ...
        const factors: Array<{ label: string; impact: number; description: string; type: 'critical' | 'moderate' | 'low' }> = [];
        let totalImpact = 0;

        // 1. Legacy Image Formats
        const legacyImages = pages.reduce((acc, p) => acc + (p.legacyFormatImages || 0), 0);
        if (legacyImages > 0) {
            const impact = Math.min(15, Math.round(legacyImages / pages.length * 10));
            factors.push({
        ...
        if (highDomPages > 0) {
            const impact = Math.min(20, Math.round(highDomPages / pages.length * 50));
            factors.push({
        ...
        const hydrationIssues = pages.filter(p => p.jsRenderDiff?.criticalContentJsOnly).length;
        if (hydrationIssues > 0) {
            const impact = Math.min(25, Math.round(hydrationIssues / pages.length * 100));
            factors.push({
        ...
        if (missingSecurity > 0) {
            const impact = Math.min(10, Math.round(missingSecurity / pages.length * 10));
            factors.push({
