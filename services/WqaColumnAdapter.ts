...
import {
  WQA_METRICS, isMetricVisible, type CatalogContext,
...
export function getWqaColumns(ctx: WqaColumnContext): string[] {
  ...
  return WQA_METRICS
    .filter((m) => m.surfaces.includes('grid'))
...
export function getWqaDefaultVisibleColumns(ctx: WqaColumnContext): string[] {
  ...
  return WQA_METRICS
    .filter((m) => m.surfaces.includes('grid') && m.isDefault && all.has(m.key))
    .map((m) => m.key);
}

// ─── Legacy-compat shims (so AuditModeConfig's current signature keeps working) ───
export function getWqaColumnsLegacy(
  industry: DetectedIndustry, language = 'en', cms: string | null = null
): string[] {
  return getWqaColumns({
    industry, language, cms,
