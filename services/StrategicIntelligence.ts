...
export function calculateInternalPageRank(
	...
	for (let i = 0; i < iterations; i++) {
		...
		for (const url in nodes) {
			const newRank =
				baseRank + sinkRedistribution + damping * (nextRanks[url] || 0);
	...
	for (const url in nodes) {
		normalized[url] =
			maxRank > 0 ? Number(((nodes[url].rank / maxRank) * 100).toFixed(2)) : 0;
	}
		...
		for (const key in groups) {
			if (groups[key].length > 1) {
				groups[key].forEach((url) => {
					...
					cannibalized[url] = Array.from(
						new Set([...(cannibalized[url] || []), ...others]),
					);
				});
			}
		}
...
export function calculatePredictiveTrafficImpact(page: any): number {
	...
	if (!page.title) potentialCtrBoost += 0.2;
...
export function calculatePredictiveScore(page: any): number {
	...
	if (page.internalPageRank !== undefined) {
		if (page.internalPageRank < 5)
			score -= 10; // Low-flow pages
...
export function calculateAuthorityScore(page: any): number {
	...
	const score =
		Math.min(100, referringDomains * 2) * 0.3 +
		urlRating * 0.4 +
...
export function calculateBusinessValueScore(page: any): number {
	...
	const engagementVal = Math.min(
		100,
		Math.max(0, 1 - bounceRate) * 50 + Math.min(300, avgDuration) / 6,
	);
...
export function calculateOpportunityScore(page: any): number {
	...
	const techPenalty =
		page.statusCode >= 400 ? 50 : page.loadTime > 2000 ? 15 : 0;
 ...
 */
export function getTrafficPerformanceStatus(
	page: any,
): "growing" | "losing" | "steady" {
...
export function getRecommendedAction(page: any): ActionResult {
	...
	if (engagement > 50 && business > 40 && tech < 55) {
		return {
			...
			reason:
				"This page drives traffic and conversions but has technical problems that could hurt it.",
	...
	if (sessions > 100 && business < 20 && engagement > 40) {
		return {
			...
			reason:
				"Users are engaging with the content but not converting. Review CTAs and offer relevance.",
	...
	if (impressions > 500 && ctr < 0.02 && content > 30) {
		return {
			...
			reason:
				"Google shows this page often, but almost nobody clicks. The title and meta description need work.",
	...
	if (business > 45 && authority < 30) {
		return {
			...
			reason:
				"This page has business value but very few links pointing to it and needs more internal support.",
	...
	if (authority > 45 && position > 12) {
		return {
			...
			reason:
				"Has backlinks and authority but still ranks poorly, which usually points to technical or intent problems.",
	...
	if (content < 35 && visibility > 20) {
		return {
			...
			reason:
				"Google is showing this page but the content is thin or weak. Expanding it could improve rankings.",
	...
	if (sessions > 50 && bounceRate > 0.7) {
		return {
			...
			reason:
				"Users land here but leave quickly, so the content or UX likely does not match their expectations.",
	...
	if (content < 25 && engagement < 10 && visibility < 10 && authority < 10) {
		return {
			...
			reason:
				"No traffic, no links, and thin content. Merge it into a stronger page or remove it.",
			factors: [
				"no engagement",
				"no visibility",
				"weak content",
...
export function calculatePageValue(
...
): { score: number; tier: PageValueTier } {
	const sessions = Number(page.ga4Sessions || 0);
	const clicks = Number(page.gscClicks || 0);
	...
	if (industry === "ecommerce") {
		...
		conversionValue =
			Number(page.ga4Conversions || page.ga4GoalCompletions || 0) * 50;
...
export function classifyContentAge(
	visibleDate: string | null | undefined,
): "fresh" | "aging" | "stale" | "ancient" | "unknown" {
...
export function checkIntentMatch(
	...
	if (
		(p === "transactional" && k === "commercial") ||
		(p === "commercial" && k === "transactional")
	)
		return "aligned";
