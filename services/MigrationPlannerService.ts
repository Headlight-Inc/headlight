...
class MigrationPlannerService {
	 ...
	 */
	static async generateAIMappings(
		sourceUrls: string[],
		targetUrls: string[],
	...
	static exportHtaccess(mappings: MigrationMapping[]) {
		return mappings
			...
			.map(
				(mapping) =>
					`Redirect ${mapping.redirectType} ${this.getPath(mapping.sourceUrl)} ${mapping.targetUrl}`,
			)
			.join("\n");
	}
	...
	static exportNginx(mappings: MigrationMapping[]) {
		return mappings
			...
			.map(
				(mapping) =>
					`rewrite ^${this.getPath(mapping.sourceUrl)}$ ${mapping.targetUrl} permanent;`,
			)
			.join("\n");
	}
