...
export async function logActivity(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[ActivityService] Failed to sync activity to cloud:",
				error,
			);
		}
	}
...
export async function getActivityFeed(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[ActivityService] Failed to fetch activity from cloud:",
				error,
			);
		}
	}
...
export async function createNotification(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[ActivityService] Failed to sync notification to cloud:",
				error,
			);
		}
	}
...
export async function markRead(notificationId: string): Promise<void> {
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[ActivityService] Failed to sync notification read status:",
				error,
			);
		}
	}
...
export async function markAllRead(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[ActivityService] Failed to sync markAllRead to cloud:",
				error,
			);
		}
	}
