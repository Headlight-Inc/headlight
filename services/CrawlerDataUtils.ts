...
export const normalizeCrawlerPage = (page: any): any | null => {
	...
	return {
		...
		indexabilityStatus:
			typeof page.indexabilityStatus === "string"
		...
		topicCluster:
			typeof page.topicCluster === "string" ? page.topicCluster : "",
		funnelStage: typeof page.funnelStage === "string" ? page.funnelStage : "",
		searchIntent:
			typeof page.searchIntent === "string" ? page.searchIntent : "",
		...
		headingHierarchy: Array.isArray(page.headingHierarchy)
			? page.headingHierarchy
			: [],
		schemaTypes: Array.isArray(page.schemaTypes) ? page.schemaTypes : [],
		fontDisplayValues: Array.isArray(page.fontDisplayValues)
			? page.fontDisplayValues
			: [],
		uniqueThirdPartyDomains: Array.isArray(page.uniqueThirdPartyDomains)
			? page.uniqueThirdPartyDomains
			: [],
		...
		hasHsts:
			typeof page.hasHsts === "boolean"
				? page.hasHsts
				: typeof page.hstsMissing === "boolean"
					? !page.hstsMissing
					: undefined,
		hasCsp:
			typeof page.hasCsp === "boolean"
				? page.hasCsp
				: typeof page.cspPresent === "boolean"
					? page.cspPresent
					: undefined,
		hasXFrameOptions:
			typeof page.hasXFrameOptions === "boolean"
				? page.hasXFrameOptions
				: typeof page.xFrameMissing === "boolean"
					? !page.xFrameMissing
					: undefined,
		hasXContentTypeOptions:
			typeof page.hasXContentTypeOptions === "boolean"
				? page.hasXContentTypeOptions
				: typeof page.xContentTypeNoSniff === "boolean"
					? page.xContentTypeNoSniff
					: undefined,
		hasCacheControl:
			typeof page.hasCacheControl === "boolean"
				? page.hasCacheControl
				: undefined,
		hasEtag: typeof page.hasEtag === "boolean" ? page.hasEtag : undefined,
		hasLastModified:
			typeof page.hasLastModified === "boolean"
				? page.hasLastModified
				: undefined,
		hasExpires:
			typeof page.hasExpires === "boolean" ? page.hasExpires : undefined,
		hasViewportMeta:
			typeof page.hasViewportMeta === "boolean"
				? page.hasViewportMeta
				: undefined,
		viewportWidth:
			typeof page.viewportWidth === "boolean" ? page.viewportWidth : undefined,
		...
		mainKwVolume: Number(
			page.mainKwSearchVolume || page.mainKwEstimatedVolume || 0,
		),
		mainKwVolumeSource: page.mainKwSearchVolume
			? "db"
			: page.mainKwEstimatedVolume
				? "gsc"
				: "none",

		// Consolidated Volume Metrics (Best)
		bestKwVolume: Number(
			page.bestKwSearchVolume || page.bestKwEstimatedVolume || 0,
		),
		bestKwVolumeSource: page.bestKwSearchVolume
			? "db"
			: page.bestKwEstimatedVolume
				? "gsc"
				: "none",
	};
};
...
import {
	classifyPageCategory,
	classifyPageCategoryRich,
	learnSiteSegments,
} from "./PageCategoryClassifier";
import {
 ...
 */
export const runPostCrawlScoring = (
	completedPages: any[],
): { pages: any[]; siteType: SiteTypeResult } => {
	...
	try {
		const firstUrl =
			completedPages.find((p) => p.crawlDepth === 0)?.url ||
			completedPages[0]?.url;
		if (firstUrl)
			siteCtx.rootHostname = new URL(firstUrl).hostname.replace(/^www\./, "");
	} catch {
		// noop
	}
	...
	const pass1Pages = enrichedPages.map((p) => {
		...
		return {
			...p,
			pageCategory: cat.category,
			pageCategoryConfidence: cat.confidence,
			pageCategorySignals: cat.signals,
		};
	});
	...
	const pages = pass1Pages.map((p) => {
		...
		if (homepageNap && p.napSnapshot && p.crawlDepth > 0) {
			const samePhone = p.napSnapshot.phones.some((f: string) =>
				homepageNap.phones.includes(f),
			);
			const sameAddr = p.napSnapshot.address === homepageNap.address;
			napMatchWithHomepage = samePhone || sameAddr;
			napHasDistinctAddress =
				p.napSnapshot.address !== "" &&
				homepageNap.address !== "" &&
				p.napSnapshot.address !== homepageNap.address;
		}
		...
		const contentAge = classifyContentAge(
			p.visibleDate || p.wpPublishDate || p.lastModified,
		);
		...
		const { score: pageValueScore, tier: pageValueTier } = calculatePageValue(
			updatedPage,
			siteType.industry,
		);
		...
		updatedPage.estimatedImpact = all.reduce(
			(s, a) => s + a.estimatedImpact,
			0,
		);
 ...
 */
export function detectWwwInconsistency(pages: any[]): {
	...
	return {
		hasInconsistency: wwwUrls > 0 && nonWwwUrls > 0,
		wwwUrls,
		nonWwwUrls,
	};
}
...
export const exportPagesAsCSV = (pages: any[]): void => {
	...
	const rows = pages.map((page) =>
		ALL_COLUMNS.map((col) => {
			const val =
				page[col.key] === null || page[col.key] === undefined
					? ""
					: page[col.key];
			const strVal =
	...
	link.setAttribute(
		"download",
		`headlight_scan_${new Date().toISOString().split("T")[0]}.csv`,
	);
 ...
 */
export const exportRawSessionData = async (
	sessionId: string,
	...
	try {
		...
		link.setAttribute(
			"download",
			`headlight_raw_dump_${new Date().toISOString().split("T")[0]}.json`,
		);
		document.body.appendChild(link);
		link.click();
		document.body.removeChild(link);
