...
export async function validateHtml(
	url: string,
): Promise<{ errors: number; warnings: number; messages: any[] }> {
	try {
		...
		const errors =
			data.messages?.filter((m: any) => m.type === "error").length || 0;
		const warnings =
			data.messages?.filter(
				(m: any) => m.type === "warning" || m.type === "info",
			).length || 0;
...
export async function scanSecurityHeaders(
	hostname: string,
): Promise<{ grade: string; score: number }> {
	try {
		// Start scan
...
export async function getWaybackInfo(url: string): Promise<{
	...
	try {
		const [firstResp, lastResp, countResp] = await Promise.all([
			fetchWithTimeout(
				`https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(url)}&output=json&fl=timestamp&limit=1&sort=asc`,
			),
			fetchWithTimeout(
				`https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(url)}&output=json&fl=timestamp&limit=1&sort=desc`,
			),
			fetchWithTimeout(
				`https://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(url)}&output=json&fl=timestamp&showNumPages=true`,
			),
		]);
...
// ─── SSL Labs ───────────────────────────────────────
export async function getSSLGrade(
	hostname: string,
): Promise<{ grade: string; protocol: string }> {
	try {
		const resp = await fetchWithTimeout(
