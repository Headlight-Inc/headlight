...
export function deriveTrafficStatus(page: any): TrafficStatus {
	if (page.sessionsDeltaPct === null || page.sessionsDeltaPct === undefined)
		return "none";
...
export function matchesIndustryFilter(page: any, value: string): boolean {
	switch (value) {
		...
		case "in_stock":
			return (
				page.pageCategory === "product" && !page.industrySignals?.outOfStock
			);
		case "out_of_stock":
			return (
				page.pageCategory === "product" &&
				page.industrySignals?.outOfStock === true &&
				page.indexable !== false
			);
		case "no_product_schema":
			return (
				page.pageCategory === "product" &&
				!(page.schemaTypes || []).includes("Product")
			);
		case "has_author":
			return Boolean(page.industrySignals?.hasAuthorAttribution);
		case "no_author":
			return (
				page.isHtmlPage === true && !page.industrySignals?.hasAuthorAttribution
			);
		case "location_pages":
			return page.pageCategory === "location_page";
		case "no_local_schema":
			return (
				page.isHtmlPage === true &&
...
export function filterWqaPages(pages: any[], filter: WqaFilterState): any[] {
	return pages.filter((p) => {
		if (filter.pageCategory !== "all" && p.pageCategory !== filter.pageCategory)
			return false;
		if (
			filter.technicalAction !== "all" &&
			p.technicalAction !== filter.technicalAction
		)
			return false;
		if (
			filter.contentAction !== "all" &&
			p.contentAction !== filter.contentAction
		)
			return false;

		if (
			filter.priorityLevel > 0 &&
			derivePriorityBucket(p) !== filter.priorityLevel
		)
			return false;
		if (filter.valueTier !== "all" && deriveValueTier(p) !== filter.valueTier)
			return false;
		if (
			filter.trafficStatus !== "all" &&
			deriveTrafficStatus(p) !== filter.trafficStatus
		)
			return false;
		if (
			filter.searchStatus !== "all" &&
			deriveSearchStatus(p) !== filter.searchStatus
		)
			return false;
		if (
			filter.contentAge !== "all" &&
			deriveContentAge(p) !== filter.contentAge
		)
			return false;
		if (
			filter.indexability !== "all" &&
			deriveIndexStatus(p) !== filter.indexability
		)
			return false;

		if (filter.funnelStage !== "all" && p.funnelStage !== filter.funnelStage)
			return false;
		if (
			filter.industryFilter !== "all" &&
			!matchesIndustryFilter(p, filter.industryFilter)
		)
			return false;
