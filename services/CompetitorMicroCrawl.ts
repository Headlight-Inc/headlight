...
export async function runCompetitorMicroCrawl(
	...
	try {
		...
		onProgress({
			...
			pagesCrawled: 0,
			totalDiscovered: 0,
			message: `Starting crawl for ${domain}...`,
		});
		...
		onProgress({
			...
			message: "Building competitive profile...",
		});
		...
		if (options?.aiEnrich !== false && options?.aiComplete) {
			...
			const textToAnalyze =
				(homepage as any).textContent ||
				`${homepage.title} ${homepage.metaDesc} ${homepage.h1_1}`;
			...
			if (textToAnalyze) {
				onProgress({
					...
					pagesCrawled: pages.length,
					totalDiscovered: pages.length,
					message: "AI analyzing competitor...",
				});
		...
		onProgress({
			...
			pagesCrawled: pages.length,
			totalDiscovered: pages.length,
			message: `Profile complete for ${domain}`,
		});
