...
export class GhostCrawler {
	...
	private initAiWorker() {
		if (typeof window !== "undefined" && window.Worker) {
			this.aiWorker = new Worker(
				new URL("../workers/ai-scoring.worker.ts", import.meta.url),
				{ type: "module" },
			);
			...
			this.aiWorker.onmessage = (e) => {
				if (e.data.status === "ready") {
					this.emit(
						"log",
						"Local AI Content Scoring Model Loaded (WebGPU/WASM)",
						"success",
					);
	...
	private async fetchText(url: string): Promise<string | null> {
		...
		try {
			const response = await fetch(bridgeTarget, {
				mode: "cors",
				headers: {
					"User-Agent": this.config.userAgent || "Headlight-Ghost/1.0",
				},
			...
			if (isUsingBridge) {
				this.emit(
					"log",
					`Ghost Bridge error (${response.status}) fetching ${url}. Attempting direct fallback...`,
					"warning",
				);
			...
			if (isUsingBridge) {
				this.emit(
					"log",
					`Ghost Bridge failed: ${err.message}. Attempting direct fallback...`,
					"warning",
				);
				try {
					const fallbackRes = await fetch(url, {
	...
	private async fetchSitemapUrls(
		...
		try {
			...
			const parseSitemap = async (sitemapUrl: string) => {
				...
				if (!xmlText) {
					console.log(
						`[GhostCrawler] Failed to fetch sitemap text: ${sitemapUrl}`,
					);
					return;
				}
				...
				if (nestedSitemaps.length > 0) {
					console.log(
						`[GhostCrawler] Detected sitemap index: ${sitemapUrl} (${nestedSitemaps.length} nested)`,
					);
				...
				console.log(
					`[GhostCrawler] Found ${urls.length} URL entries in sitemap: ${sitemapUrl}`,
				);
				...
				if (urls.length === 0) {
					const locMatches = Array.from(
						xmlText.matchAll(/<loc>([\s\S]*?)<\/loc>/gi),
					)
						.map((match) =>
							String(match[1] || "")
								.replace(/<!\[CDATA\[|\]\]>/g, "")
					...
					console.log(
						`[GhostCrawler] Fallback regex found ${locMatches.length} loc matches: ${sitemapUrl}`,
					);
			...
			console.log(
				`[GhostCrawler] Finished sitemap parsing. Total unique URLs: ${collected.size}, coverageParsed: ${coverageParsed}`,
			);
		...
		try {
			...
			if (!response.ok) {
				...
				if (!bridgeConfig && response.status === 0) {
					throw new Error(
						"CORS Blocked. Please enable the Ghost Bridge or use a CORS extension.",
					);
				}
	...
	private parseHtml(
		...
		if (generator.includes("wordpress")) cmsType = "wordpress";
			...
			html.includes("wix-first-interactive")
		)
			cmsType = "wix";
		else if (
		...
		else if (html.includes("next.js") || html.includes("__NEXT_DATA__"))
			cmsType = "nextjs";
		...
		return {
			...
			resources: this.config.crawlResources
				? ([
						...
						...Array.from(doc.querySelectorAll("script")).map((s) =>
							s.getAttribute("src"),
						),
	...
	private queueForPersistence(page: any) {
		...
		const dbPage: CrawledPage = {
			...
			inSitemap:
				page.inSitemap ??
				(this.sitemapUrls
					? this.sitemapUrls.has(
							UrlNormalization.toCanonical(page.finalUrl || page.url),
						)
					: null),
