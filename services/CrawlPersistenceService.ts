...
import {
	buildIssuePriorityRequest,
	buildFixSuggestionRequest,
} from "./ai/tasks/prompts";
...
function detectIssuesFromPages(pages: any[]): DetectedIssue[] {
	...
	for (const page of pages) {
		...
		if (!page.metaDesc || page.metaDesc.trim() === "")
			missingMetaDescs.push(url);
		else {
			if (page.metaDescLength > 160) longMetaDescs.push(url);
			if (page.metaDescLength < 70 && page.metaDescLength > 0)
				shortMetaDescs.push(url);
		...
		if (page.isThinContent || (page.wordCount && page.wordCount < 100))
			thinContent.push(url);
		...
		if (
			(page.inlinks === 0 || page.inlinks === undefined) &&
			page.statusCode === 200 &&
			page.contentType?.includes("text/html")
		)
			orphanedPages.push(url);
		if (
			!page.ogTitle &&
			page.statusCode === 200 &&
			page.contentType?.includes("text/html")
		)
			missingOgTags.push(url);
		if (
			(!page.schemaTypes || page.schemaTypes?.length === 0) &&
			page.statusCode === 200 &&
			page.contentType?.includes("text/html")
		)
			missingSchema.push(url);
	...
	const dupesTitleUrls = Array.from(duplicateTitles.values())
		.filter((urls) => urls.length > 1)
		.flat();
	const dupesDescUrls = Array.from(duplicateMetaDescs.values())
		.filter((urls) => urls.length > 1)
		.flat();
	...
	pages.forEach((p) => {
		if (detectContentDecay(p)) {
			addIssue(
				"Strategic",
				"Strategic Content Decay Detected",
				`Page has high impressions (${p.gscImpressions}) but very low clicks (${p.gscClicks}), suggesting content is losing relevance.`,
				[p.url],
				"Critical",
				"error",
				"Medium",
				15,
				"Refresh the page against current SERP leaders and update the promise, headers, and core advice.",
			);
		}
		if (cannibalizedGroups[p.url]) {
			addIssue(
				...
				`This page shares a highly similar title/H1 with: ${cannibalizedGroups[p.url].join(", ")}.`,
				[p.url],
				"High",
				"warning",
				"Medium",
				10,
				"Consolidate, canonicalize, or differentiate the intent between overlapping pages.",
			);
		}
		if (
			(p.link_equity || p.linkEquity || 0) > 50 &&
			(p.outlinksList?.length || 0) < 3
		) {
			addIssue(
				"Strategic",
				"Authority Sink (High PageRank, Low Outlinks)",
				"This page receives significant internal authority but does not distribute it to other sections of the site.",
				[p.url],
				"High",
				"warning",
				"Low",
				8,
				"Add 2-3 strategic internal links from this page to priority destinations.",
			);
		}
	});
	...
	addIssue(
		"technical",
		"Server Errors (5xx)",
		"Pages returning server error codes.",
		serverErrors,
		"Critical",
		"error",
		"High",
		15,
		"Investigate the application and upstream dependencies causing the 5xx responses.",
	);
	addIssue(
		"technical",
		"Broken Pages (4xx)",
		"Pages returning client error codes.",
		brokenPages,
		"Critical",
		"error",
		"Low",
		10,
		"Restore content or redirect these URLs to the most relevant destination.",
	);
	addIssue(
		"technical",
		"Redirect Loops",
		"Pages caught in redirect loops.",
		redirectLoops,
		"Critical",
		"error",
		"Medium",
		10,
		"Break the loop and point directly to the final destination.",
	);
	addIssue(
		"content",
		"Lorem Ipsum Content",
		"Pages containing placeholder text.",
		loremIpsum,
		"Critical",
		"error",
		"Low",
		15,
		"Replace placeholder content or noindex unfinished pages.",
	);
	addIssue(
		"content",
		"Missing Page Titles",
	...
	addIssue(
		...
		"Low",
		5,
		"Add concise, compelling meta descriptions.",
	);
	addIssue(
		"content",
		"Missing H1 Tags",
	...
	addIssue(
		...
		"Medium",
		8,
		"Expand, merge, or deindex low-value thin pages.",
	);
	addIssue(
		"performance",
		"Very Slow Pages (>5s)",
		...
		"High",
		8,
		"Reduce server latency and large render-blocking assets.",
	);
	addIssue(
		"content",
		"Duplicate Content",
		...
		"Medium",
		8,
		"Use canonicals or consolidate duplicated content.",
	);
	addIssue(
		"links",
		"Orphaned Pages",
	...
	addIssue(
		"content",
		"Duplicate Titles",
		"Multiple pages sharing the same title.",
		dupesTitleUrls,
		"Medium",
		"warning",
		"Low",
	...
	addIssue(
		...
		"Low",
		1,
		"Trim titles to 50-60 characters.",
	);
	addIssue(
		"content",
		...
		"Low",
		1,
		"Expand titles to better describe page intent.",
	);
	addIssue(
		"content",
	...
	addIssue(
		...
		"Very short meta descriptions.",
		shortMetaDescs,
		"Medium",
		"notice",
		"Low",
		1,
		"Expand descriptions with clearer context.",
	);
	addIssue(
		"content",
		"Multiple H1 Tags",
	...
	addIssue(
		...
		"Medium",
		3,
		"Optimize heavy assets and server response time.",
	);
	addIssue(
		"technical",
		"Large Pages (>2MB)",
		"Pages larger than 2MB.",
		largePages,
		"Medium",
		"warning",
		"Medium",
		3,
		"Reduce payload size and move heavy assets off the critical path.",
	);
	addIssue(
		"technical",
		...
		"Low",
		3,
		"Add self-referencing canonicals for indexable pages.",
	);
	addIssue(
		"content",
		"Missing Image Alt Text",
		"Pages with images lacking alt text.",
		missingAltText,
		"Medium",
		"warning",
		"Low",
	...
	addIssue(
		...
		"Low",
		5,
		"Update insecure asset URLs to HTTPS.",
	);
	addIssue(
		"content",
	...
	addIssue(
		...
		"Pages overusing keywords unnaturally.",
		keywordStuffing,
		"Medium",
		"warning",
		"Medium",
		5,
		"Rewrite the content for clarity and natural phrasing.",
	);
	addIssue(
		"content",
		"Low Text-to-HTML Ratio",
		"Pages with less than 10% visible text.",
		lowTextRatio,
		"Medium",
		"notice",
		"Medium",
		2,
		"Reduce markup bloat or add meaningful content.",
	);
	addIssue(
		"technical",
		"Insecure Forms",
		"Pages with forms submitting over HTTP.",
		insecureForms,
		"Medium",
		"warning",
		"Low",
	...
	addIssue(
		"technical",
		"Redirected Pages",
		"Pages returning 3xx status codes.",
		redirectPages,
		"Low",
		"notice",
		"Low",
	...
	addIssue(
		...
		"Pages skipping heading levels.",
		incorrectHeadingOrder,
		"Low",
		"notice",
		"Low",
		1,
		"Restructure headings hierarchically.",
	);
	addIssue(
		"performance",
		"Poor LCP (>2.5s)",
		"Pages with poor Largest Contentful Paint.",
		badLcp,
		"Medium",
		"warning",
		"High",
		5,
		"Optimize the largest element and reduce blocking resources.",
	);
	addIssue(
		"performance",
		"Poor CLS (>0.1)",
		"Pages with poor layout stability.",
		badCls,
		"Medium",
		"warning",
		"Medium",
	...
	addIssue(
		"technical",
		"Not in Sitemap",
		"Indexable pages not included in the XML sitemap.",
		notInSitemap,
		"Low",
		"notice",
		"Low",
		1,
		"Add these indexable pages to the sitemap.",
	);
	addIssue(
		"technical",
		"Broken Hreflang References",
		"Pages with hreflang tags pointing to broken URLs.",
		brokenHreflang,
		"Medium",
		"warning",
		"Medium",
		3,
		"Repair hreflang targets and reciprocity.",
	);

	return issues;
}

function calculateSiteHealthScore(
	pages: any[],
	issues: DetectedIssue[],
): number {
	if (pages.length === 0) return 0;
	let score = 100;
	const totalHtmlPages = pages.filter(
		(p) => p.contentType?.includes("text/html") && p.statusCode < 400,
	).length;
...
function calculateThematicScores(pages: any[], issues: DetectedIssue[]) {
	const totalPages =
		pages.filter((p) => p.statusCode && p.statusCode < 400).length || 1;
	const categoryImpact = (cat: string) => {
		const catIssues = issues.filter(
			(i) => i.category === cat && i.issue_type !== "passed",
		);
		let totalImpact = 0;
	...
	const avgLoadTime =
		pages.reduce((sum, p) => sum + (p.loadTime || 0), 0) / (pages.length || 1);
	const speedScore =
		avgLoadTime < 1000
			? 100
			: avgLoadTime < 2000
	...
	const securityScore = Math.round(
		(httpsPages / totalPages) * 100 - (mixedContentCount / totalPages) * 30,
	);
...
const getEvidenceSourcesForPage = (page: any) => {
	const sources = ["crawl"];
	if ((page.gscClicks || 0) > 0 || (page.gscImpressions || 0) > 0)
		sources.push("gsc");
	if ((page.ga4Sessions || 0) > 0 || (page.ga4Users || 0) > 0)
		sources.push("ga4");
	if ((page.referringDomains || 0) > 0 || (page.urlRating || 0) > 0)
		sources.push("ahrefs");
	return sources;
};
...
const buildTopPages = (pages: any[]) => {
	return [...pages]
		.sort((a, b) => {
			const left = Number(
				b.opportunityScore || b.businessValueScore || b.gscImpressions || 0,
			);
			const right = Number(
				a.opportunityScore || a.businessValueScore || a.gscImpressions || 0,
			);
			return left - right;
		})
		.slice(0, 15)
...
const buildLeanPageSummary = (page: any) => ({
	...
	page_category_signals: page.pageCategorySignals
		? JSON.stringify(page.pageCategorySignals)
		: null,
...
const getPreviousRunSummaries = async (projectId: string) => {
	...
	return new Map(
		result.rows.map((row) => [
			String(row.url),
			safeJsonParse<any>(row.summary_json, {}),
		]),
	);
};
...
export async function persistCrawlResults(params: {
	...
	try {
		...
		const evidenceSources = Array.from(
			new Set(pages.flatMap((page) => getEvidenceSourcesForPage(page))),
		);
			...
			if (detectedIssues.length > 3) {
				...
				if (rankingResponse && rankingResponse.ranked) {
					for (const item of rankingResponse.ranked) {
						...
						if (issue) {
							issue.score_impact =
								item.estimatedTrafficImpact === "high"
									? 90
									: item.estimatedTrafficImpact === "medium"
										? 60
										: 30;
							...
							try {
								...
								if (fixResponse && fixResponse.fix) {
									...
									if (fixResponse.effort) {
										issue.effort =
											fixResponse.effort === "low"
												? "Low"
												: fixResponse.effort === "medium"
													? "Medium"
													: "High";
									}
								}
								...
								console.warn(
									`[CrawlPersistence] AI fix failed for ${issue.title}:`,
									aiErr,
								);
							}
						}),
		...
		if (isCloudSyncEnabled) {
			...
			const trendMetrics = {
				...
				linkEquity: pages.reduce(
					(sum, page) => sum + Number(page.linkEquity || 0),
					0,
				),
				contentFreshnessRisk: detectedIssues.filter((issue) =>
					issue.title.includes("Decay"),
				).length,
			};
			...
			await client.batch([
				{
					...
					args: [
						...
						policy,
						retentionPolicy,
						JSON.stringify([urlCrawled]),
						JSON.stringify({ maxPages: pages.length, maxDepth: maxDepthSeen }),
						createdAt,
					],
				},
				{
				...
				},
				{
					sql: `INSERT OR REPLACE INTO trend_snapshots (id, project_id, run_id, snapshot_at, metrics_json)
                          VALUES (?, ?, ?, ?, ?)`,
					args: [
						buildDbId("trend", runId),
						projectId,
						runId,
						createdAt,
						JSON.stringify(trendMetrics),
					],
				},
			]);
				...
				await client.batch(
					detectedIssues.map((issue, index) => ({
						...
						args: [
							...
							"new",
							JSON.stringify({
								sources: issue.title.startsWith("Strategic")
									? ["crawl", "gsc", "ga4"]
									: ["crawl"],
							}),
							createdAt,
...
export async function persistEnrichmentStatus(params: {
	...
	try {
		...
		console.error(
			"[CrawlPersistence] Failed to persist enrichment status:",
			err,
		);
	}
}
...
const mapRunRowToAudit = (row: any): AuditResult => {
	...
	return {
		...
		status: (summary.status ||
			row.status ||
			"completed") as AuditResult["status"],
...
export async function getAuditAggregatedMetrics(auditId: string) {
	if (!isCloudSyncEnabled)
		return { gscClicks: 0, gscImpressions: 0, ga4Sessions: 0, ga4Views: 0 };
	await ensureSchema();
	const result = await turso().execute({
...
export async function persistCompetitorProfile(
	...
	await client.execute({
		...
		args: [
			...
			profile._meta?.crawledAt
				? new Date(profile._meta.crawledAt).toISOString()
				: null,
		],
	});
}
