...
export type CrawlerIntegrationAuthType =
	| "oauth"
	| "token"
	| "property"
	| "upload";
export type CrawlerIntegrationOwnership = "anonymous" | "project";
export type CrawlerIntegrationStatus =
	| "connected"
	| "configured"
	| "degraded"
	| "error";
export type CrawlerIntegrationSyncStatus =
	| "idle"
	| "syncing"
	| "success"
	| "partial"
...
const PROJECT_STORAGE_KEY_PREFIX =
	"headlight:seo-crawler-integrations:project:";
...
const mergeScopes = (...scopeLists: Array<string[] | null | undefined>) =>
	Array.from(
		new Set(
			scopeLists.flatMap((scopes) =>
				Array.isArray(scopes) ? scopes.filter(Boolean) : [],
			),
		),
	);
...
const sanitizeConnection = (
	connection: CrawlerIntegrationConnection,
...
const normalizeConnectionMap = (
	...
	if (legacyGsc || legacyGa4) {
		...
		next.google = {
			...
			status:
				currentGoogle.status ||
				legacyGsc?.status ||
				legacyGa4?.status ||
				"connected",
			authType: "oauth",
			ownership:
				currentGoogle.ownership ||
				legacyGsc?.ownership ||
				legacyGa4?.ownership ||
				"project",
			connectedAt:
				currentGoogle.connectedAt ||
				legacyGsc?.connectedAt ||
				legacyGa4?.connectedAt ||
				Date.now(),
			accountLabel:
				currentGoogle.accountLabel ||
			...
			hasCredentials: Boolean(
				currentGoogle.hasCredentials ||
				legacyGsc?.hasCredentials ||
				legacyGa4?.hasCredentials,
			),
			metadata: {
				...(legacyGsc?.metadata || {}),
				...(legacyGa4?.metadata || {}),
			...
			selection: {
				siteUrl:
					currentGoogle.selection?.siteUrl ||
					currentGoogle.sync?.siteUrl ||
					legacyGsc?.selection?.siteUrl ||
					legacyGsc?.metadata?.siteUrl,
				propertyId:
					currentGoogle.selection?.propertyId ||
					currentGoogle.sync?.propertyId ||
					legacyGa4?.selection?.propertyId ||
					legacyGa4?.metadata?.propertyId,
			},
			sync: {
				status:
					currentGoogle.sync?.status ||
					legacyGsc?.sync?.status ||
					legacyGa4?.sync?.status ||
					"idle",
				lastSyncedAt:
					currentGoogle.sync?.lastSyncedAt ||
					legacyGsc?.sync?.lastSyncedAt ||
					legacyGa4?.sync?.lastSyncedAt,
				lastAttemptedAt:
					currentGoogle.sync?.lastAttemptedAt ||
					legacyGsc?.sync?.lastAttemptedAt ||
					legacyGa4?.sync?.lastAttemptedAt,
				siteUrl:
					currentGoogle.sync?.siteUrl ||
					legacyGsc?.sync?.siteUrl ||
					legacyGsc?.selection?.siteUrl ||
					legacyGsc?.metadata?.siteUrl,
				propertyId:
					currentGoogle.sync?.propertyId ||
					legacyGa4?.sync?.propertyId ||
					legacyGa4?.selection?.propertyId ||
					legacyGa4?.metadata?.propertyId,
			},
		};
...
const writeStorage = (
	...
	try {
		window.localStorage.setItem(
			key,
			JSON.stringify(sanitizeConnectionMap(connections)),
		);
	} catch (error) {
...
const toConnectionMap = (
	rows: IntegrationRecord[] | null | undefined,
): Partial<
	Record<CrawlerIntegrationProvider, CrawlerIntegrationConnection>
> => {
	const mapped = (rows || []).reduce(
		(acc, row) => {
			acc[row.provider] = {
				...
				credentials: {},
				hasCredentials:
					Boolean(row.credentials && Object.keys(row.credentials).length > 0) ||
					(row.provider === "google" && !!row.account_label),
...
const mapDbRows = (rows: any[]): IntegrationRecord[] => {
	return rows.map((row) => ({
		...
		ownership: "project",
		connected_at: Number(
			row.updated_at ? new Date(String(row.updated_at)).getTime() : Date.now(),
		),
		...
		sync: parseJson<CrawlerIntegrationSyncState>(row.sync_json, {
			status: "idle",
		}),
	}));
};

export const getAnonymousCrawlerIntegrations = () =>
	readStorage(ANON_STORAGE_KEY);
...
export const saveAnonymousCrawlerIntegrations = (
	...
	const normalized = sanitizeConnectionMap(
		Object.fromEntries(
			Object.entries(connections).map(([provider, connection]) => [
				provider,
				connection
					? { ...connection, ownership: "anonymous" as const }
					: connection,
			]),
		) as Partial<
			Record<CrawlerIntegrationProvider, CrawlerIntegrationConnection>
...
export const getProjectCachedCrawlerIntegrations = (projectId: string) =>
	readStorage(projectStorageKey(projectId));
	...
	const normalized = sanitizeConnectionMap(
		Object.fromEntries(
			Object.entries(connections).map(([provider, connection]) => [
				provider,
				connection
					? { ...connection, ownership: "project" as const }
					: connection,
			]),
		) as Partial<
			Record<CrawlerIntegrationProvider, CrawlerIntegrationConnection>
...
export const fetchProjectCrawlerIntegrations = async (
	projectId: string,
): Promise<CrawlerIntegrationsLoadResult> => {
...
export const upsertProjectCrawlerIntegration = async (
	projectId: string,
	connection: CrawlerIntegrationConnection,
) => {
...
export const removeProjectCrawlerIntegration = async (
	projectId: string,
	provider: CrawlerIntegrationProvider,
) => {
	if (isCloudSyncEnabled) {
...
export const replaceProjectCrawlerIntegrations = async (
	...
	const rows = Object.values(sanitizeConnectionMap(connections))
		.filter((connection): connection is CrawlerIntegrationConnection =>
			Boolean(connection),
		)
	...
	saveProjectCachedCrawlerIntegrations(
		projectId,
		sanitizeConnectionMap(connections),
	);
};

export const promoteAnonymousCrawlerIntegrationsToProject = async (
	projectId: string,
) => {
