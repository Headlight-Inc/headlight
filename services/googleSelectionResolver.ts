...
export class GoogleSelectionResolver {
	...
	static async resolveEffectiveGoogleSelection(args: {
		...
		let gscSource: EffectiveGoogleSelection["source"] =
			existingSelection?.source || "detected";
		...
		let ga4Source: EffectiveGoogleSelection["source"] =
			existingSelection?.source || "detected";

		if (!ga4PropertyId || ga4Confidence < 90) {
			const ga4Match = await this.detectGa4Property(
				accessToken,
				crawlTarget,
				history,
			);
		...
		return {
			...
			ga4Confidence,
			source:
				gscSource === "manual" || ga4Source === "manual"
					? "manual"
					: "detected",
		};
	}
			...
			for (const account of accounts) {
				for (const prop of account.propertySummaries || []) {
					...
					for (const stream of streams) {
						...
						const score = this.scoreGa4Stream(
							streamHost,
							crawl.hostname,
							crawl.registrableDomain,
						);
						if (score > propertyScore) propertyScore = score;
					}

					// Historical hints
					if (history?.lastUsedGa4ForProject === propertyId)
						propertyScore += 25;
