...
export function getExpectedCtr(position: number): number {
	...
	if (rounded <= 50)
		return Math.max(0.003, 0.008 - ((rounded - 20) / 30) * 0.005);
...
export function estimateCtrImprovementClicks(
	...
	return Math.round(
		Math.max(0, Number(impressions || 0) * (expected - actualCtr)),
	);
}
...
export function estimatePositionImprovementClicks(
	...
	return Math.round(
		Math.max(
			0,
			Number(impressions || 0) * (newExpectedCtr - currentExpectedCtr),
		),
	);
}
