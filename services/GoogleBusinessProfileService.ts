...
export class GoogleBusinessProfileService {
    private static BIZ_INFO_BASE = 'https://mybusinessbusinessinformation.googleapis.com/v1';
    ...
    static async listLocations(accessToken: string): Promise<GbpLocation[]> {
        try {
            ...
            for (const account of accounts) {
                const locRes = await fetch(`${this.BIZ_INFO_BASE}/${account.name}/locations?readMask=name,title,storefrontAddress,primaryPhone,websiteUri,regularHours,categories,metadata`, {
                    headers: { 'Authorization': `Bearer ${accessToken}` }
                });
                ...
                locations.forEach((loc: any) => {
                    allLocations.push({
                        ...
                        categories: loc.categories?.additionalCategories?.map((c: any) => c.displayName) || 
     ...
     */
    static matchLocationToSite(locations: GbpLocation[], crawlDomain: string): GbpLocation | null {
        return locations.find(loc => {
            if (!loc.websiteUri) return false;
            try {
                const locDomain = new URL(loc.websiteUri).hostname.replace(/^www\./, '');
     ...
     */
    static async getReviews(accountId: string, locationId: string, accessToken: string): Promise<GbpReview[]> {
        try {
            const response = await fetch(`${this.MY_BIZ_BASE}/${accountId}/locations/${locationId}/reviews?pageSize=50`, {
                headers: { 'Authorization': `Bearer ${accessToken}` }
            });
     ...
     */
    static async enrichSession(sessionId: string, accessToken: string, crawlDomain: string, onProgress?: (msg: string) => void): Promise<{ enriched: boolean; location?: GbpLocation }> {
