...
export function discoverFromLinkNeighborhood(
	...
	return [...domainStats.entries()]
		...
		.map(([domain, stats]) => ({
			...
			estimatedAuthority: Math.min(
				100,
				Math.round(stats.count * 3 + stats.pages.size * 5),
			),
			confidence:
				stats.count >= 5
					? ("high" as const)
...
export function analyzeCompetitorOverlap(
	...
	try {
		competitorDomain = new URL(competitorPages[0]?.url || "").hostname.replace(
			/^www\./,
			"",
		);
	} catch {
...
export function computeThreatScores(
	...
	const yourQuality =
		({ Excellent: 4, Good: 3, Average: 2, Poor: 1 } as any)[
			yourProfile.contentQualityAssessment || "Average"
		] || 2;
	const compQuality =
		({ Excellent: 4, Good: 3, Average: 2, Poor: 1 } as any)[
			competitorProfile.contentQualityAssessment || "Average"
		] || 2;
	...
	const avgThreat =
		(contentThreatScore + authorityThreatScore + innovationThreatScore) / 3;
	...
	return {
		threatLevel,
		contentThreatScore,
		authorityThreatScore,
		innovationThreatScore,
		opportunityAgainstThem,
	};
}
