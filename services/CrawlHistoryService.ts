...
const buildSummaryMetrics = (pages: any[], session?: CrawlSession) => {
	...
	const totalClicks = pages.reduce(
		(sum, p) => sum + (Number(p.gscClicks) || 0),
		0,
	);
	const totalSessions = pages.reduce(
		(sum, p) => sum + (Number(p.ga4Sessions) || 0),
		0,
	);
	const avgGeoScore = average(
		pages.map((p) => Number(p.geoScore || 0)).filter((v) => v > 0),
	);
	const avgContentQuality = average(
		pages.map((p) => Number(p.contentQualityScore || 0)).filter((v) => v > 0),
	);
	const googlebotVisits = pages.reduce(
		(sum, p) => sum + (Number(p.googlebotVisits30d) || 0),
		0,
	);
	...
	return {
		...
		schemaCoverage:
			pages.length > 0 ? Math.round((pagesWithSchema / pages.length) * 100) : 0,
		notFoundPages: pages.filter((page) => Number(page?.statusCode || 0) === 404)
			.length,
		totalClicks,
		totalSessions,
...
const buildSummaryDelta = (
	oldPages: any[],
	newPages: any[],
	oldSession?: CrawlSession,
	newSession?: CrawlSession,
) => {
		...
		req.onupgradeneeded = (e) => {
			...
			if (oldVersion < 1) {
				if (!db.objectStoreNames.contains(SESSIONS_STORE)) {
					const sessStore = db.createObjectStore(SESSIONS_STORE, {
						keyPath: "id",
					});
					...
					sessStore.createIndex("lastActivityAt", "lastActivityAt", {
						unique: false,
					});
				}
				if (!db.objectStoreNames.contains(PAGES_STORE)) {
					const pageStore = db.createObjectStore(PAGES_STORE, {
						keyPath: ["sessionId", "url"],
					});
			...
			if (oldVersion < 2) {
				...
				if (!sessStore.indexNames.contains("url"))
					sessStore.createIndex("url", "url", { unique: false });
				if (!sessStore.indexNames.contains("startedAt"))
					sessStore.createIndex("startedAt", "startedAt", { unique: false });
				if (!sessStore.indexNames.contains("status"))
					sessStore.createIndex("status", "status", { unique: false });
				if (!sessStore.indexNames.contains("lastActivityAt"))
					sessStore.createIndex("lastActivityAt", "lastActivityAt", {
						unique: false,
					});
				...
				if (db.objectStoreNames.contains(PAGES_STORE)) {
					...
					if (!pageStore.indexNames.contains("sessionId"))
						pageStore.createIndex("sessionId", "sessionId", { unique: false });
				}
			}
...
export async function getSessions(limit = 50): Promise<CrawlSession[]> {
	...
	const merged = new Map<string, CrawlSession>(
	...
	if (isCloudSyncEnabled) {
		try {
			const cloudResult = await turso().execute(
				`SELECT metadata FROM crawl_sessions ORDER BY created_at DESC LIMIT ${limit}`,
			);
			...
			console.warn(
				"Failed to fetch sessions from cloud, using local sessions:",
				err,
			);
		}
	}
...
export async function getSession(
	id: string,
): Promise<CrawlSession | undefined> {
	const db = await openDB();
	return new Promise((resolve, reject) => {
		const tx = db.transaction(SESSIONS_STORE, "readonly");
...
export function diffSessions(
	...
	if (oldPages.length === 0 && newPages.length === 0) {
		return {
			...
			issuesFixed: [],
			newIssues: [],
			summaryDelta: buildSummaryDelta(
				oldPages,
				newPages,
				oldSession,
				newSession,
	...
	for (const [url, newP] of newMap.entries()) {
		...
		const fieldChanges = COMPARISON_FIELDS.filter(
			(field) => !valuesEqual(oldP[field], newP[field]),
		).map((field) => ({
			field,
		...
		const oldIssueMap = new Map(
			collectPageIssues(oldP).map((issue) => [issue.id, issue]),
		);
		const newIssueMap = new Map(
			collectPageIssues(newP).map((issue) => [issue.id, issue]),
		);
