...
const commonRegex = /^(\S+) \S+ \S+ \[([^\]]+)\] "(\S+) (\S+) \S+" (\d+) (\d+|-)/;
...
export class LogFileAnalysisService {
        ...
        if (line.startsWith('{')) {
          try {
            ...
            return [{
              ...
              method: String(json.ClientRequestMethod || json.method || 'GET'),
              url: String(json.ClientRequestURI || json.url || ''),
              status: Number(json.EdgeResponseStatus || json.status || 0),
              userAgent: String(json.ClientRequestUserAgent || json.userAgent || '')
            }];
          } catch {
            return [];
          }
  ...
  static aggregate(entries: ParsedLogEntry[]) {
    ...
    entries.forEach((entry) => {
      ...
      if (['gptbot', 'claudebot', 'perplexity', 'bytespider'].includes(bot)) totals.aiBots += 1;
      ...
      if (bot === 'googlebot') {
        ...
        current.googlebotLastVisit = current.googlebotLastVisit && current.googlebotLastVisit > entry.timestamp
    ...
    byUrl.forEach((metrics) => {
      metrics.botCrawlBudgetShare = totals.totalBotRequests
        ? Number((metrics.googlebotVisits30d + metrics.aiBotVisits30d) / totals.totalBotRequests)
        : 0;
    });
  ...
  static applyToPages(pages: any[], entries: ParsedLogEntry[]) {
    ...
    const updatedPages = pages.map((page) => {
      ...
      const fallbackMatch = !direct && path
        ? Array.from(byUrl.entries()).find(([entryUrl]) => this.safePath(entryUrl) === path)?.[1]
        : null;
