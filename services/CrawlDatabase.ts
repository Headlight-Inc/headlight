...
class CrawlDB extends Dexie {
	...
	constructor() {
				...
				return tx
					...
					.modify((page) => {
						...
						page.recommendedActionFactors =
							page.recommendedActionFactors || null;
						...
						page.finalUrl =
							page.finalUrl || page.redirectUrl || page.url || null;
					});
			});
		...
		this.version(6)
			...
			.upgrade((tx) => {
				return tx
					...
					.modify((page) => {
						page.hasHsts =
							page.hasHsts ?? (page.hstsMissing === false ? true : false);
						page.hasCsp =
							page.hasCsp ?? (page.cspPresent === true ? true : false);
						page.hasXFrameOptions =
							page.hasXFrameOptions ??
							(page.xFrameMissing === false ? true : false);
						page.hasXContentTypeOptions =
							page.hasXContentTypeOptions ?? page.xContentTypeNoSniff === true;
		...
		this.version(7).stores({
			...
			tasks:
				"id, projectId, sessionId, status, priority, assigneeId, createdAt",
		...
		this.version(10).stores({
			...
			rules: "id, projectId, enabled",
			notifications:
				"id, userId, projectId, read, createdAt, [userId+projectId+read]",
		});
			...
			.upgrade((tx) => {
				return tx
					...
					.modify((page) => {
						...
						page.sitemapLastmodMismatchCount =
							page.sitemapLastmodMismatchCount ?? 0;
						page.sitemapValidationSampleSize =
							page.sitemapValidationSampleSize ?? 0;
						page.sitemapValidationTruncated =
							page.sitemapValidationTruncated ?? false;
					});
			});
