...
function getNextRunTime(config: CrawlerConfig): Date | null {
	...
	const now = new Date();
	const [hours, minutes] = (config.scheduleTime || "02:00")
		.split(":")
		.map(Number);
...
function shouldRunNow(config: CrawlerConfig): ScheduleCheckResult {
	...
	const now = new Date();
	const [targetHours, targetMinutes] = (config.scheduleTime || "02:00")
		.split(":")
		.map(Number);
	...
	if (Math.abs(nowMinutes - targetTotalMinutes) > 30)
	...
	return {
		shouldRun: true,
		reason: `Scheduled ${config.scheduleFrequency} crawl`,
	};
}
