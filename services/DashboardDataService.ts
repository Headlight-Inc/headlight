...
const storageKey = (collection: CollectionName, projectId: string) =>
	`headlight:data:${collection}:${projectId}`;
...
const readCollection = <T>(
	...
	try {
		return JSON.parse(
			window.localStorage.getItem(storageKey(collection, projectId)) || "[]",
		);
	} catch {
		return [];
	}
...
const writeCollection = <T>(
	...
	window.localStorage.setItem(
		storageKey(collection, projectId),
...
export const listKeywords = async (
	...
	return readCollection<KeywordRecord>("keywords", projectId).sort((a, b) =>
...
export const refreshKeywordRanks = async (projectId: string) => {
	const keywords = readCollection<KeywordRecord>("keywords", projectId).map(
		(kw) => {
			const nextPosition = Math.max(
				1,
				(kw.position || rand(5, 40)) + rand(-4, 4),
			);
	...
	const histories = readCollection<RankHistoryRecord>(
		"rank_history",
		projectId,
	);
...
export const listCompetitors = async (
	...
	return readCollection<CompetitorRecord>("competitors", projectId).sort(
...
export const deleteCompetitor = async (projectId: string, id: string) => {
	const items = readCollection<CompetitorRecord>(
		"competitors",
		projectId,
...
export const listAutomationRules = async (
	...
	return readCollection<AutomationRuleRecord>(
		"automation_rules",
		projectId,
...
export const addAutomationRule = async (
	...
	const items = readCollection<AutomationRuleRecord>(
		"automation_rules",
		projectId,
	);
...
export const updateAutomationRule = async (
	projectId: string,
	id: string,
	updates: Partial<AutomationRuleRecord>,
) => {
	const items = readCollection<AutomationRuleRecord>(
		"automation_rules",
		projectId,
...
export const listBrandMentions = async (
	projectId: string,
): Promise<BrandMentionRecord[]> => {
	return readCollection<BrandMentionRecord>("brand_mentions", projectId).sort(
		(a, b) => b.detected_at.localeCompare(a.detected_at),
	);
};

export const seedBrandMentions = async (
	projectId: string,
	brandName: string,
) => {
	const existing = readCollection<BrandMentionRecord>(
		"brand_mentions",
		projectId,
	);
	...
	const avgPosition =
		keywords.length > 0
			? keywords
					...
					.reduce((sum, kw) => sum + Number(kw.position || 0), 0) /
				Math.max(1, keywords.filter((kw) => kw.position !== null).length)
			: null;
	return {
		keywords,
		mentions,
...
export const syncFromCrawl = async (
	...
	const existingKeywordSet = new Set(
		existingKeywords.map((kw) => kw.keyword.toLowerCase().trim()),
	);

	// 1. Auto-import keywords from GSC data
	const gscPages = crawlPages.filter(
		(p) => p.gscClicks > 0 || p.gscImpressions > 100,
	);
	...
	const nonGscPages = crawlPages.filter(
		(p) => !p.gscClicks && p.h1_1 && p.contentType?.includes("html"),
	);
	for (const page of nonGscPages.slice(0, 30)) {
		...
		if (
			keyword &&
			keyword.length > 3 &&
			keyword.split(" ").length <= 6 &&
			!existingKeywordSet.has(keyword.toLowerCase())
		) {
			await addKeyword(projectId, keyword, {
	...
	let rootHostname = "";
	try {
		rootHostname = new URL(crawlPages[0]?.url || "").hostname.replace(
			/^www\./,
			"",
		);
	} catch {
	...
	for (const page of crawlPages) {
		...
		for (const link of links) {
			try {
				...
				externalDomainCounts.set(
					domain,
					(externalDomainCounts.get(domain) || 0) + 1,
				);
			} catch {
				/* ignore bad URLs */
			}
	...
	if (projectName) {
		...
		const existingMentionSources = new Set(
			existingMentions.map((m) => m.source),
		);
		...
		for (const page of crawlPages) {
			if (!page.url?.includes(rootHostname)) {
				...
				const pageText =
					...
					(page.metaDesc || "");
				if (
					pageText.toLowerCase().includes(brandLower) &&
					!existingMentionSources.has(page.url)
				) {
					const mentions = readCollection<BrandMentionRecord>(
						"brand_mentions",
						projectId,
					);
