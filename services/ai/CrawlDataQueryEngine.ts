...
const hasAny = (message: string, phrases: string[]) =>
...
export class CrawlDataQueryEngine {
	static detectIntent(message: string): ChatQueryType {
		...
		if (hasAny(query, ["open settings", "go to settings", "show settings"]))
			return "navigate";
		if (hasAny(query, ["show me", "find pages", "pages with", "which pages"]))
			return "filter";
		...
		if (hasAny(query, ["what should i fix", "priority", "best roi"]))
			return "suggest";
		if (hasAny(query, ["write meta", "generate meta", "generate content"]))
			return "generate";
		return "report";
	}
	...
	static execute(
		message: string,
		pages: any[],
		context?: { crawlHistory?: any[]; healthScore?: number },
	): QueryResult {
		const query = lower(message);
		...
		if (intent === "navigate") {
			return {
				...
				handledLocally: true,
				response:
					"Settings is the right place for crawl configuration, API keys, storage, and monitoring preferences.",
		...
		if (hasAny(query, ["googlebot", "bot visits"])) {
			const matches = [...pages]
				...
				.sort(
					(a, b) => (b.googlebotVisits30d || 0) - (a.googlebotVisits30d || 0),
				)
				.slice(0, 10);
			return {
				type: "data",
		...
		if (hasAny(query, ["ai search", "geo", "optimized for ai"])) {
			...
			return {
				...
				matches,
				actions: [
					{ label: "Open GEO Panel", type: "open_audit_tab", payload: "geo" },
				],
			};
		}
		...
		if (scoreMatch) {
			...
			let matches = pages.filter(
				(page) =>
					Number(page.healthScore || page.priorityScore || 0) < threshold,
			);
			const impressionsMatch = query.match(/(\d+)\s+impressions/);
			if (impressionsMatch) {
				const minImpressions = Number(impressionsMatch[1]);
				matches = matches.filter(
					(page) => Number(page.gscImpressions || 0) > minImpressions,
				);
			}
			return this.pageListResult(
		...
		if (intent === "suggest") {
			const matches = [...pages]
				.sort(
					(a, b) =>
						Number(b.opportunityScore || b.geoScore || 0) -
						Number(a.opportunityScore || a.geoScore || 0),
				)
				.slice(0, 5);
			return {
				type: "suggest",
	...
	private static pageListResult(
		...
		return {
			...
			handledLocally: true,
			response: matches.length
				? `${label}: ${matches.length} matches. Top results: ${matches
						.slice(0, 8)
						.map((page) => page.url)
						.join("; ")}`
