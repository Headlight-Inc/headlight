...
export function createGeminiAdapter(apiKey: string): AIProviderAdapter {
  ...
  return {
    ...
    async complete(request: AIRequest): Promise<AIResponse> {
      ...
      const res = await fetchWithRetry(`${BASE}:generateContent?key=${apiKey}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    async *completeStream(request: AIRequest): AsyncGenerator<string, AIResponse> {
      ...
      const body = {
        contents: [
          ...(systemParts.length ? [{ role: 'model', parts: systemParts }] : []),
          { role: 'user', parts: [{ text: request.prompt }] },
        ],
        generationConfig: {
          maxOutputTokens: request.maxTokens || 512,
      ...
      const res = await fetchWithRetry(`${BASE}:streamGenerateContent?key=${apiKey}&alt=sse`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
        timeout: 30000,
