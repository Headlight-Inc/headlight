	...
	return {
		...
		async complete(request: AIRequest): Promise<AIResponse> {
			...
			switch (request.taskType) {
				...
				default:
					text = JSON.stringify({
						error: "Task not supported locally",
						fallback: true,
					});
			}
...
function localClassify(text: string): string {
	...
	const transactional =
		/(buy|price|pricing|checkout|order|purchase|deal|discount|coupon|shop|cart|subscribe)/.test(
			lower,
		);
	const commercial =
		/(best|top|review|compare|vs|versus|alternative|recommend)/.test(lower);
	const navigational =
		/(login|sign.?in|account|dashboard|contact|about|support)/.test(lower);
	...
	const posWords = (
		lower.match(
			/\b(great|excellent|amazing|love|best|good|wonderful|fantastic|easy|beautiful|helpful)\b/g,
		) || []
	).length;
	const negWords = (
		lower.match(
			/\b(bad|terrible|awful|worst|hate|poor|broken|difficult|ugly|frustrating|annoying)\b/g,
		) || []
	).length;
	const sentiment =
		posWords > negWords + 2
...
function localScore(text: string): string {
	...
	return JSON.stringify({
		quality: Math.min(100, Math.max(0, score)),
		method: "heuristic",
	});
}
