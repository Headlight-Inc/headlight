...
Critical Issues: ${currentCrawlSummary?.topIssues?.join(", ") || "None identified"}
		...
		const samplePages = pages
			...
			.map(
				(p) =>
					`- [${p.url}]: Title: "${p.title}", Status: ${p.statusCode}, Impressions: ${p.gscImpressions || 0}, GEO: ${p.geoScore || 0}, Tech: ${p.techHealthScore || 0}`,
			)
			.join("\n");

		// Distribution summaries
		const statusCodeDist = pages.reduce(
			(acc, p) => {
		...
		const geoDist = pages.reduce(
			(acc, p) => {
				const bucket = Math.floor((p.geoScore || 0) / 20) * 20;
				acc[`${bucket}-${bucket + 20}`] =
					(acc[`${bucket}-${bucket + 20}`] || 0) + 1;
				return acc;
			},
