...
const DEFAULT_QUOTAS: Record<
	AIProvider,
	{ rpm: number; rpd: number; tpd: number }
> = {
...
export class AIRouter {
	...
	async complete(request: AIRequest): Promise<AIResponse> {
		...
		const chain =
			TASK_PROVIDER_CHAIN[request.taskType] || TASK_PROVIDER_CHAIN.classify;
		...
		for (const providerName of chain) {
			...
			try {
				...
				const ttl =
					request.taskType === "classify" || request.taskType === "score"
						? 30 * 60 * 1000
						: 5 * 60 * 1000;
		...
		throw new Error(
			`All AI providers failed for ${request.taskType}: ${errors.join("; ")}`,
		);
	}
