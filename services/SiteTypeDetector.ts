...
function scoreByCms(
	pages: PageSignals[],
): Partial<Record<DetectedIndustry, number>> {
	...
	if (
		...
		const hasWoo = pages.some(
			(p) =>
				p.industrySignals?.hasProductSchema || p.industrySignals?.priceVisible,
		);
...
function scoreBySchema(
	...
	const count = (types: string[]) =>
		types.reduce((sum, t) => sum + (typeCounts[t] || 0), 0);
	...
	if (
		...
		scores.local = Math.min(
			95,
			70 + count(["LocalBusiness", "Restaurant", "Store"]) * 5,
		);
	}
	...
	if (has(["MedicalWebPage", "MedicalCondition", "Drug", "MedicalClinic"])) {
		scores.healthcare = Math.min(
			90,
			60 + count(["MedicalWebPage", "MedicalCondition"]) * 5,
		);
	}
...
function scoreByContentPatterns(
	pages: PageSignals[],
): Partial<Record<DetectedIndustry, number>> {
	const scores: Partial<Record<DetectedIndustry, number>> = {};
	...
	const pricePages = htmlPages.filter(
		(p) => p.industrySignals?.priceVisible,
	).length;
	if (pricePages / total > 0.15) {
		scores.ecommerce = Math.min(
			85,
			30 + Math.round((pricePages / total) * 100),
		);
	}

	const longContentPages = htmlPages.filter(
		(p) => Number(p.wordCount || 0) > 500,
	).length;
	...
	const localSignalPages = htmlPages.filter(
		(p) =>
			p.hasPostalAddress ||
			p.hasEmbeddedMap ||
	...
	if (localSignalPages / total > 0.1) {
		scores.local = Math.min(
			75,
			25 + Math.round((localSignalPages / total) * 100),
		);
	}
	...
	const medicalPages = htmlPages.filter(
		(p) => p.industrySignals?.hasMedicalAuthor,
	).length;
	if (medicalPages > 3) {
	...
	if (longPages.length >= 5) {
		const withByline = longPages.filter(
			(p) =>
		...
		if (bylineRatio >= 0.4) {
			...
			scores.blog = Math.max(
				scores.blog || 0,
				Math.min(75, 35 + Math.round(bylineRatio * 50)),
			);
			// Bias toward news if publishing cadence suggests it (many long pages + dates)
			if (longPages.length >= 20 && bylineRatio >= 0.6) {
				scores.news = Math.max(
					scores.news || 0,
					Math.min(70, 30 + longPages.length),
				);
			}
		}
	}
	...
	const locationSlugPages = htmlPages.filter((p) => {
		try {
			return /\/(location|locations|service-area|areas-we-serve|branches|office|lokacija|standort|ubicacion|emplacement)/i.test(
	...
	const napPages = htmlPages.filter(
		(p) => p.hasPostalAddress && (p.phoneNumbers || []).length > 0,
	).length;
...
function scoreByUrlStructure(
	...
	const deepPages = htmlPages.filter(
		(p) => Number(p.crawlDepth || 0) >= 3,
	).length;
	if (deepPages / total > 0.4) {
		scores.ecommerce = Math.max(
			scores.ecommerce || 0,
			Math.min(50, 15 + Math.round((deepPages / total) * 50)),
		);
	}

	const shallowPages = htmlPages.filter(
		(p) => Number(p.crawlDepth || 0) <= 1,
	).length;
...
function scoreByGscQueries(
	...
	const ecomKw = keywords.filter((k) =>
		/\b(buy|price|shop|cheap|order|deliver|sale|discount|coupon|free shipping)\b/i.test(
			k,
		),
	).length;
	...
	const localKw = keywords.filter((k) =>
		/\b(near me|near|in \w+|city|town|location|hours|directions|open now)\b/i.test(
			k,
		),
	).length;
	...
	const saasKw = keywords.filter((k) =>
		/\b(software|tool|app|platform|alternative|vs\b|comparison|pricing|free trial)\b/i.test(
			k,
		),
	).length;
	...
	const healthKw = keywords.filter((k) =>
		/\b(symptom|treatment|doctor|medical|health|diagnosis|medicine|therapy|clinic)\b/i.test(
			k,
		),
	).length;
	if (healthKw / totalKw > 0.1) {
		scores.healthcare = Math.min(
			60,
			20 + Math.round((healthKw / totalKw) * 100),
		);
	}
...
function detectLanguages(pages: PageSignals[]): {
	primary: string;
	all: Array<{ code: string; percentage: number }>;
	isMulti: boolean;
} {
...
function scoreByExtraEvidence(
	...
	const inNewsSitemapCount = html.filter(
		(p) => p.industrySignals?.inNewsSitemap,
	).length;
	const timeTagDensity =
		html.filter((p) => p.industrySignals?.hasTimeTag).length / html.length;
	...
	const currencyDensity =
		html.filter((p) => p.industrySignals?.currencySymbolDensity > 0.002)
			.length / html.length;
	const hasCart = html.some(
		(p) =>
			p.industrySignals?.hasAddToCartButton ||
			p.industrySignals?.hasCartEndpoint,
	);
	if (currencyDensity > 0.3)
		scores.ecommerce = Math.max(scores.ecommerce || 0, 70);
	if (hasCart) scores.ecommerce = Math.max(scores.ecommerce || 0, 80);

	// LOCAL: multi-location NAP
	const locationPages = html.filter(
		(p) => (p.phoneNumbers || []).length > 0 && p.hasPostalAddress,
	);
	...
	if (saasMarkers >= 2)
		scores.saas = Math.max(scores.saas || 0, 40 + saasMarkers * 10);

	// JOB BOARD: JobPosting density
	const jobCount = html.filter((p) =>
		(p.schemaTypes || []).includes("JobPosting"),
	).length;
	if (jobCount >= 5)
		scores.job_board = Math.max(
			scores.job_board || 0,
			60 + Math.min(35, jobCount),
		);
...
export function detectSiteType(rawPages: PageSignals[]): SiteTypeResult {
	...
	const sorted = (
		Object.entries(finalScores) as Array<[DetectedIndustry, number]>
	).sort((a, b) => b[1] - a[1]);

	const top = sorted[0] || ["general", 0];
	const primary: DetectedIndustry =
		top[1] >= INDUSTRY_CONFIDENCE_MIN ? top[0] : "general";
	const confidence =
		top[1] >= INDUSTRY_CONFIDENCE_MIN ? Math.min(100, top[1]) : top[1];
	const isLowConfidence = top[1] < INDUSTRY_CONFIDENCE_MIN;

	const second = sorted[1];
	const secondaryIndustry =
		second && top[1] - second[1] <= SECONDARY_MIN_DELTA && second[1] >= 40
			? second[0]
			: null;
