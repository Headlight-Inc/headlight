...
export async function exportReportsPDF({
	...
	doc.setFont("helvetica", "bold");
	doc.setFontSize(22);
	doc.text("Website Quality Report", margin, y);
	y += 26;
	doc.setFont("helvetica", "normal");
	doc.setFontSize(10);
	doc.setTextColor(170, 170, 170);
	doc.text(
		`Pages: ${pages.length.toLocaleString()}  ·  Industry: ${industry}  ·  Generated ${new Date().toLocaleDateString()}`,
		margin,
		y,
	);
	y += 22;
	...
	autoTable(doc, {
		...
		body: [
			...
			[
				...
				Math.round(stats.totalEstimatedImpact).toLocaleString(),
			],
		],
		styles: {
			fillColor: [17, 17, 17],
	...
	Object.entries(facets.searchStatuses).forEach(([k, v]) =>
		dist.push(["Search position", k, v]),
	);
	Object.entries(facets.trafficStatuses).forEach(([k, v]) =>
		dist.push(["Traffic trend", k, v]),
	);
	Object.entries(facets.valueTiers).forEach(([k, v]) =>
		dist.push(["Value tier", k, v]),
	);
	Object.entries(facets.contentAges).forEach(([k, v]) =>
		dist.push(["Content age", k, v]),
	);
	Object.entries(facets.indexabilities).forEach(([k, v]) =>
		dist.push(["Indexability", k, v]),
	);
	autoTable(doc, {
		startY: y,
		theme: "grid",
	...
	autoTable(doc, {
		...
		body: sorted.map((a) => [
			...
			a.action,
			String(a.pageCount),
			Math.round(a.totalEstimatedImpact).toLocaleString(),
			a.effort,
		]),
		styles: {
			fillColor: [17, 17, 17],
