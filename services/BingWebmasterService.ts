...
export class BingWebmasterService {
	 ...
	 */
	private static async fetchBing(
		method: string,
		...
		const response = await fetch(
			`${this.API_BASE}/${method}?${queryParams.toString()}`,
			{
				headers: {
		...
		if (!response.ok) {
			...
			throw new Error(
				`Bing API Error (${response.status}): ${errorText || response.statusText}`,
			);
		}
	 ...
	 */
	static async getUrlTrafficInfo(
		siteUrl: string,
		accessToken: string,
		onProgress?: (msg: string) => void,
	): Promise<Record<string, BingTrafficMetrics>> {
	 ...
	 */
	static async getCrawlErrors(
		siteUrl: string,
		accessToken: string,
	): Promise<BingCrawlError[]> {
	...
	static async enrichSession(
		...
		try {
			// Fetch traffic data
			const trafficData = await this.getUrlTrafficInfo(
				siteUrl,
				accessToken,
				onProgress,
			);
