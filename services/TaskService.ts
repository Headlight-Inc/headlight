...
export async function createTask(
	...
	const task: CrawlTask = {
		...
		affected_urls_json: data.affectedUrls
			? JSON.stringify(data.affectedUrls)
			: null,
		...
		await createNotification(projectId, task.assignee_id, {
			...
			body:
				task.priority === "critical"
					? "High priority task requires attention."
					: null,
...
export async function updateTask(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[TaskService] Failed to sync task update to cloud:",
				error,
			);
		}
	}
...
export async function toggleSubtask(
	...
	await crawlDb.subtasks.update(subtaskId, {
		completed,
...
function mapRowToTask(row: any): CrawlTask {
	return {
		...
		affected_urls_json: row.affected_urls_json
			? String(row.affected_urls_json)
			: null,
