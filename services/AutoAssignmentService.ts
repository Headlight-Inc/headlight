...
export async function getRules(projectId: string): Promise<AssignmentRule[]> {
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[AutoAssignmentService] Failed to fetch rules from cloud:",
				error,
			);
		}
	}
...
export async function createRule(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[AutoAssignmentService] Failed to sync rule to cloud:",
				error,
			);
		}
	}
...
export async function seedDefaultRules(projectId: string): Promise<void> {
	...
	const defaultRules: Array<
		Omit<AssignmentRule, "id" | "project_id" | "created_at">
	> = [
		{
			rule_name: "Auto-create tasks for critical issues",
			trigger_type: "issue_severity",
			trigger_condition_json: JSON.stringify({
				severity: ["Critical"],
				minAffectedPages: 1,
			}),
	...
	for (const issue of issues) {
		for (const rule of enabledRules) {
			if (isMatch(rule, issue)) {
				...
				const alreadyExists = existingTasks.find(
					(t) => t.linked_issue_id === issue.id,
				);
	...
	try {
		...
		const affectedCount =
			...
			(issue.affected_urls_json
				? JSON.parse(issue.affected_urls_json).length
				: 0);
...
export async function reconcileTasksWithIssues(
	...
	const crawlerTasks = tasks.filter(
		(t) => t.source === "crawler" && t.linked_issue_id,
	);
	...
	const currentIssueTitles = new Set(
		result.rows.map((r) => String(r.title).toLowerCase()),
	);
