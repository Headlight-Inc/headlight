...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	const localStorageKey = user
		? `headlight:projects:${source}:${user.id}`
		: null;
	const activeIdKey = user
		? `headlight:projects:${source}:${user.id}:active`
		: null;
	...
	const cacheLocally = (
		...
		nextActive: Project | null,
	) => {
		if (typeof window === "undefined" || !localStorageKey || !activeIdKey)
			return;
	...
	const fetchProjects = useCallback(async () => {
		...
		try {
			...
			if (cloudProjects.length > 0) {
				...
				const active =
					cloudProjects.find((p) => p.id === savedActiveId) || cloudProjects[0];
				...
				if (localProjects.length > 0 && !migrationRunRef.current) {
					...
					console.log(
						`[Projects] No cloud projects found, but local projects exist. Attempting migration...`,
					);
					...
					if (migrated > 0) {
						console.log(
							`[Projects] Migrated ${migrated} local projects to cloud.`,
						);
						...
						if (fresh.length > 0) {
							...
							const savedActiveId = readLocalActiveId();
							const active =
								fresh.find((p) => p.id === savedActiveId) || fresh[0];
							setActiveProject(active);
						...
						const savedActiveId = readLocalActiveId();
						const active =
							localProjects.find((p) => p.id === savedActiveId) ||
							localProjects[0];
						setActiveProject(active);
					}
				} else {
			...
			if (localProjects.length > 0) {
				setActiveProject(
					localProjects.find((p) => p.id === savedActiveId) || localProjects[0],
				);
			} else {
				setActiveProject(null);
			}
	...
	useEffect(() => {
		...
		const handleStorageChange = (e: StorageEvent) => {
			...
			if (e.key === localStorageKey) {
				console.log(
					"[Projects] Storage changed in another tab, re-fetching...",
				);
				fetchProjects();
			}
		};
