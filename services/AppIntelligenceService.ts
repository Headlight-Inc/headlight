...
const fallbackInsights = () => ({
    ...
    thematicScores: null,
    issueOverview: null,
    insights: [{ title: 'Run your first crawl', detail: 'Connect your site to get actionable insights.' }]
});
...
export const generateContentPrediction = async (topic: string) => {
    try {
        ...
        return {
            ...
            difficulty: 0,
            outline: [],
            recommendations: ['AI prediction unavailable. Re-run after connecting an AI provider.']
        };
    }
};
    ...
    try {
        ...
        if (isCloudSyncEnabled) {
            ...
            if (result.rows.length > 0) {
                ...
                const scores = JSON.parse(String(result.rows[0].thematic_scores_json || '{}'));
                return {
                    visibility: {
                        overallScore: summary.healthScore || 0,
                        trend: summary.scoreDelta > 0 ? 'up' : summary.scoreDelta < 0 ? 'down' : 'neutral',
        ...
        const latest = sessions.find((session) => session.projectId === projectId) || sessions[0];
        if (!latest) return fallbackInsights();
        const pages = await getPages(latest.id);
        const previous = sessions.find((session) => session.id !== latest.id && session.projectId === latest.projectId);
        const overallScore = Math.round(
            pages.reduce((sum, page) => sum + Number(page.healthScore || 0), 0) / Math.max(1, pages.length)
        );
        const scoreDelta = previous ? overallScore - Number(previous.healthScore || 0) : 0;
        return {
            visibility: {
                overallScore,
            ...
            insights: [
                {
                    ...
                    detail: 'Cloud summary is unavailable, showing insights from your latest local crawl.'
                }
            ]
        };
    } catch (error) {
