...
export class GscClientService {
	...
	private static getDateRanges(days: number = 30) {
		...
		const overallStart = new Date(
			overallEnd.getTime() - days * 24 * 60 * 60 * 1000,
		);

		let currentEnd = new Date(overallEnd);
		while (currentEnd > overallStart) {
			let currentStart = new Date(
				currentEnd.getTime() - 6 * 24 * 60 * 60 * 1000,
			);
	...
	private static async fetchPaginated(
		...
		for (const range of ranges) {
			...
			while (startRow < 50000) {
				// GSC API hard limit per request is 50k
	...
	private static isBetterBestKeyword(
		candidate: GscMetricRow,
	...
	static async enrichSession(
		...
		onProgress?.(
			`Fetching GSC Page Summary (up to ${maxPageRows.toLocaleString()} rows)...`,
		);
		const pageRows = await this.fetchPaginated(
			siteUrl,
			accessToken,
			["page"],
			options.days || 30,
		...
		onProgress?.(
			`Fetching GSC Page+Query Details (up to ${maxQueryRows.toLocaleString()} rows)...`,
		);
		const queryRows = await this.fetchPaginated(
			siteUrl,
			accessToken,
			["page", "query"],
			options.days || 30,
		...
		const targetPageUrlByCanonical = new Map<string, string>(
			targetPages.map((page) => [
				UrlNormalization.toCanonical(page.url),
				page.url,
			]),
		);
		...
		for (const page of targetPages) {
			...
			const candidates = [page.url, canonical, page.finalUrl].filter(
				Boolean,
			) as string[];
			for (const candidate of candidates) {
				const row = gscCanonicalMap.get(
					UrlNormalization.toCanonical(candidate),
				);
				if (row) {
