...
export async function createCloudProject(
	...
	try {
		console.log(
			`[Projects] Attempting to create project: ${project.id} (${project.name})`,
		);
		await turso().execute({
		...
		console.log(
			`[Projects] Successfully created project in cloud: ${project.id}`,
		);
		...
		console.error(
			`[Projects] Failed to create project in cloud: ${project.id}`,
			error,
		);
		throw error;
	}
...
export async function deleteCloudProject(id: string): Promise<void> {
	...
	try {
		...
		const sessionIds = sessionsRes.rows
		...
		queries.push({
			sql: `DELETE FROM crawler_configs WHERE id = ?`,
			args: [id],
		});
		...
		if (sessionIds.length > 0) {
			sessionIds.forEach((sid) => {
				...
				queries.push({
					sql: "DELETE FROM crawl_sessions WHERE id = ?",
					args: [sid],
				});
			});
		}
		...
		console.log(
			`[Projects] Batch deletion completed successfully for project: ${id}`,
		);
	} catch (error: any) {
		console.error(
			`[Projects] Critical failure during project deletion: ${id}`,
			error.message,
		);
		throw error;
	}
...
function mapRowToProject(row: any): ProjectRecord {
	return {
		...
		last_crawl_score:
			row.last_crawl_score != null ? Number(row.last_crawl_score) : undefined,
		last_crawl_grade: row.last_crawl_grade
			? String(row.last_crawl_grade)
			: undefined,
		...
		auto_crawl_interval: (row.auto_crawl_interval ||
			"weekly") as ProjectRecord["auto_crawl_interval"],
		notification_email: row.notification_email
			? String(row.notification_email)
			: undefined,
	};
}
