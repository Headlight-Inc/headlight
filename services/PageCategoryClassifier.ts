...
export function classifyPageCategoryRich(
	...
	if (page.statusCode >= 400)
		return done("other", 100, [`status ${page.statusCode}`]);
	...
	if (hasPaginationSignals(page, path))
		return done("pagination", 95, ["rel=next/prev or ?page=N"]);
	if (isSearchResultsPage(page.url))
		return done("search_results", 95, ["search query param or slug"]);
	...
	if (
		...
		page.industrySignals?.hasProductSchema
	) {
	...
	if (isContactPage(page, path))
		return done("contact", 85, ["contact slug or form+phone"]);
	...
	if (hasCategorySlug(path))
		return done("category", 60, ["category-slug fallback"]);
	if (hasBlogSlug(path) && !page.visibleDate)
		return done("blog_index", 55, ["blog-slug, no date"]);
	...
	try {
		...
		if (
			[...parsed.searchParams.keys()].some((k) =>
				/^(q|s|search|query|suche|pretraga|busqueda|recherche)$/i.test(k),
			)
		)
			return true;
...
function locationSignals(
	...
	let score = 0;
	if (page.hasEmbeddedMap) {
		score++;
		signals.push("embedded map");
	}
	if (page.hasPostalAddress) {
		score++;
		signals.push("postal address");
	}
	if ((page.phoneNumbers || []).length > 0) {
		score++;
		signals.push("phone");
	}
	if (page.industrySignals?.hasLocalBusinessSchema) {
		score += 2;
		signals.push("LocalBusiness schema");
	}
	if (hasLocationSlug(path)) {
		score++;
		signals.push("location slug");
	}
...
function articleLikeSignals(
	...
	// Classic fallback
	if (hasDate && isLong && isDeep)
		return { isArticle: true, confidence: 80, signals };

	// Site-learned blog segment: e.g. /vijesti/naslov — no date found but site pattern says it's news
	const firstSeg = (
		page.firstPathSegment || getFirstSegment(path)
	).toLowerCase();
	if (
		firstSeg &&
...
function isBlogIndexPage(
	...
	if (generic && seg && siteCtx.learnedBlogSegments?.has(seg.toLowerCase()))
		return true;
...
function isResourcePage(path: string): boolean {
	return /\/(resource|download|whitepaper|ebook|guide|webinar|template|tool)\b/i.test(
		path,
	);
}
...
export function learnSiteSegments(
	...
	for (const p of pages) {
		...
		if (Number(p.wordCount || 0) > 400)
			longCountBySeg[seg] = (longCountBySeg[seg] || 0) + 1;
	}
	...
	return {
		...
		learnedBlogSegments: learnedBlog,
		learnedCategorySegments: learnedCat,
	};
}
