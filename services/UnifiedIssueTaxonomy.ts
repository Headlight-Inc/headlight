import { CHECK_REGISTRY, type AuditMode, type IndustryFilter } from './CheckRegistry';
...
const RAW_SEO_ISSUES_TAXONOMY = [
    ...
    },
    {
        ...
        issues: [
            { id: 'js_dependent_content', checkId: 't1-js-required', label: 'Critical Content Requires JavaScript', type: 'warning', condition: (p: any) => p.jsRenderDiff?.criticalContentJsOnly === true },
            { id: 'js_hidden_links', checkId: 't1-dynamic-links', label: 'Internal Links Hidden Behind JavaScript', type: 'warning', condition: (p: any) => p.jsRenderDiff?.jsOnlyLinks > 0 },
            { id: 'js_hidden_images', checkId: 't1-js-required', label: 'Images Hidden Behind JavaScript', type: 'notice', condition: (p: any) => p.jsRenderDiff?.jsOnlyImages > 0 },
            { id: 'js_hidden_schema', checkId: 't1-js-required', label: 'Structured Data Only via JavaScript', type: 'notice', condition: (p: any) => p.jsRenderDiff?.jsOnlySchema === true },
            { id: 'high_js_diff', checkId: 't1-js-required', label: 'High HTML vs Rendered Diff (>50%)', type: 'notice', condition: (p: any) => p.jsRenderDiff?.textDiffPercent > 50 },
...
export const UNIFIED_ISSUE_TAXONOMY: UnifiedIssueGroup[] = RAW_SEO_ISSUES_TAXONOMY.map(group => ({
...
export function getIssuesForMode(modes: AuditMode[], industry: IndustryFilter): UnifiedIssueGroup[] {
