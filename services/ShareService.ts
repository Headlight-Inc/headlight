...
export async function createSharedReport(
	...
	await client.execute({
		...
		args: [
			...
			report.white_label ? 1 : 0,
			report.custom_logo_url,
			report.custom_company_name,
			JSON.stringify(report.include_sections),
...
export async function getSharedReport(
	...
	return {
		...
		custom_company_name: row.custom_company_name
			? String(row.custom_company_name)
			: null,
