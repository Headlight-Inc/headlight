...
const readProjectMentions = (projectId?: string) => {
	...
	return safeParseJson(
		window.localStorage.getItem(`headlight:data:brand_mentions:${projectId}`),
	);
};
...
const extractAggregateRating = (page: any) => {
	...
	for (const block of schemaBlocks) {
		const aggregate =
			...
			(block?.["@type"] === "AggregateRating" ? block : null);
		if (!aggregate) continue;
		const ratingValue = Number(aggregate.ratingValue || aggregate.rating || 0);
		const reviewCount = Number(
			aggregate.reviewCount || aggregate.ratingCount || 0,
		);
		return {
			ratingValue:
...
export const checkCitationNAP: CheckEvaluator = (page, ctx) => {
	...
	const phones = Array.isArray(page?.phoneNumbers)
		? page.phoneNumbers.map(normalizePhone).filter(Boolean)
		: [];
	...
	const napSignals = [
		hasName,
		hasAddress,
		Boolean(phones.length || gbpPhone),
	].filter(Boolean).length;
	const phoneConsistent = gbpPhone
		? phones.includes(gbpPhone) || phones.length === 0
		: null;
	const napConsistent = gbpPhone
		? Boolean(phoneConsistent && hasAddress)
		: napSignals >= 2;
	const severity = napConsistent
		? "pass"
		: napSignals >= 1
			? "info"
			: "warning";
	...
	return {
		...
		expected:
			"Name, address, and phone should be present and consistent with business listings",
...
export const checkReviewScore: CheckEvaluator = (page) => {
	...
	return {
		...
		auditModes: ["full", "local_seo", "website_quality"],
		industries: ["all"],
	};
};
...
export const checkReviewVolume: CheckEvaluator = (page) => {
	...
	const aggregate = extractAggregateRating(page);
	const reviewCount = Number(
		page?.gbpReviewCount || aggregate.reviewCount || 0,
	);
	const severity =
		reviewCount >= 25
			? "pass"
...
export const checkReviewRecency: CheckEvaluator = (page) => {
	if (!rootOnly(page)) return null;
	const reviewCount = Number(
		page?.gbpReviewCount || extractAggregateRating(page).reviewCount || 0,
	);
	const freshestSignal = Number(page?.gbpEnrichedAt || 0);
	const ageDays =
		freshestSignal > 0
			? Math.floor((Date.now() - freshestSignal) / (24 * 60 * 60 * 1000))
			: null;
	const severity =
		reviewCount === 0
			? "info"
	...
	return {
		...
		fixSuggestion:
			"Store actual review timestamps when GBP reviews are ingested to upgrade this from a proxy signal.",
		auditModes: ["full", "local_seo"],
		industries: ["all"],
	};
};
...
export const checkBrandMentions: CheckEvaluator = (page, ctx) => {
	...
	const severity =
		linkable.length > 0 ? "pass" : mentions.length > 0 ? "info" : "info";
