...
export const extractRecentContentCount = (pages: any[] = [], days = 90) => {
	...
	return pages.filter((page) => {
		...
		const isContentLike =
			Number(page?.wordCount || 0) >= 300 ||
			/(blog|news|article|guide|resources|learn|insights)/i.test(
				String(page?.url || ""),
			);
...
const getPrimaryCompetitorDomain = (pages: any[] = []) =>
	normalizeHost(pages[0]?.url);

const summarizeKeywordGaps = (ownPages: any[], competitorPages: any[]) =>
	findKeywordGaps(ownPages, competitorPages)
...
export const checkCompKeywordGap: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			"t4-comp-keyword-gap",
			"competitor",
			"Competitor Keyword Gap",
			"No competitor crawl data available for keyword-gap analysis.",
		);
	}
...
export const checkCompContentGap: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			...
			"No competitor crawl data available for content-gap analysis.",
		);
	}
...
export const checkCompBacklinkGap: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			"t4-comp-backlink-gap",
			"competitor",
			"Competitor Backlink Gap",
			"No competitor crawl data available for backlink-gap analysis.",
		);
	}
...
export const checkCompTechStack: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			...
			"No competitor crawl data available for tech-stack comparison.",
		);
	}
...
export const checkCompPricing: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			...
			"No competitor crawl data available for pricing comparison.",
		);
	}
	...
	const severity =
		competitorHasPricing && !ownHasPricing
			? "warning"
	...
	return {
		...
		value: {
			ownHasPricing,
			competitorHasPricing,
			competitorDomain: getPrimaryCompetitorDomain(competitorPages),
		},
...
export const checkCompSocial: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			"t4-comp-social",
			"competitor",
			"Competitor Social Footprint",
			"No competitor crawl data available for social-footprint comparison.",
		);
	}
	...
	const severity =
		competitorCount > ownCount ? "warning" : ownCount > 0 ? "pass" : "info";

	return {
		checkId: "t4-comp-social",
		tier: 4,
		category: "competitor",
		name: "Competitor Social Footprint",
		severity,
		value: { ownCount, competitorCount },
		expected:
			"Own social footprint should match or exceed competitor visibility",
		message:
			competitorCount > ownCount
...
export const checkCompContentFreq: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			"t4-comp-content-freq",
			"competitor",
			"Competitor Content Frequency",
			"No competitor crawl data available for publishing-frequency comparison.",
		);
	}
	...
	const severity =
		competitorRecent > Math.max(ownRecent * 1.5, ownRecent + 2)
			? "warning"
			: competitorRecent > 0 || ownRecent > 0
				? "info"
				: "pass";
	...
	return {
		...
		tier: 4,
		category: "competitor",
		name: "Competitor Content Frequency",
		severity,
		value: {
			ownRecentPages90d: ownRecent,
			competitorRecentPages90d: competitorRecent,
		},
		expected:
			"Publishing frequency should be competitive over the last 90 days",
		message:
			competitorRecent > ownRecent
...
export const checkCompLinkVelocity: CheckEvaluator = (page, ctx) => {
	...
	if (competitorPages.length === 0) {
		return infoNoCompetitorData(
			"t4-comp-link-velocity",
			"competitor",
			"Competitor Link Velocity Proxy",
			"No competitor crawl data available for link-velocity comparison.",
		);
	}
	...
	const severity =
		competitorVelocityProxy > ownVelocityProxy * 1.25
			? "warning"
			: competitorVelocityProxy > ownVelocityProxy
				? "info"
				: "pass";
	...
	return {
		...
		tier: 4,
		category: "competitor",
		name: "Competitor Link Velocity Proxy",
		severity,
		value: {
			ownVelocityProxy,
			competitorVelocityProxy,
			yourRefDomains,
			competitorRefDomains,
			ownRecent,
			competitorRecent,
		},
		...
		fixSuggestion:
			"Replace this proxy with true historical backlink snapshots when backlink history is available.",
		auditModes: ["full", "competitor_gap"],
		industries: ["all"],
	};
};
