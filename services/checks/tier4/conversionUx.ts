...
export const checkSocialProof: CheckEvaluator = (page) => {
  ...
  return {
    ...
    tier: 4, category: 'conversion_ux', name: 'Social Proof Signals',
    severity,
...
export const checkFormQuality: CheckEvaluator = (page) => {
  ...
  const hasForms = page.hasFormsWithAutocomplete !== undefined || (page.formsWithoutLabels || 0) > 0;
  ...
  const severity = labelsMissing ? 'warning' : hasAutocomplete ? 'pass' : 'info';
  
  return {
    checkId: 't4-form-quality',
    tier: 4, category: 'conversion_ux', name: 'Form Quality',
    severity,
...
export const checkExitIntent: CheckEvaluator = (page) => {
  ...
  return {
    ...
    tier: 4, category: 'conversion_ux', name: 'Exit Intent Detection',
    severity: 'info',
