...
export const checkHealthAuthor: CheckEvaluator = (page) => {
	...
	const isMedicalContent =
		page.textContent?.toLowerCase().includes("treatment") ||
		page.textContent?.toLowerCase().includes("diagnosis");
	...
	return {
		...
		tier: 4,
		category: "healthcare",
		name: "Medical Author Attribution",
		severity: hasMedicalAuthor ? "pass" : "warning",
		value: { hasMedicalAuthor },
		expected: "Verified medical author attribution",
		message: hasMedicalAuthor
			? "Medical author attribution found."
			: "Medical content detected without clear author credentials. Critical for E-E-A-T.",
		auditModes: ["full"],
...
export const checkHealthDisclaimer: CheckEvaluator = (page) => {
	...
	return {
		...
		message: hasDisclaimer
			? "Medical disclaimer found."
			: "No medical disclaimer detected on the site.",
		auditModes: ["full"],
		industries: ["healthcare"],
	};
};
...
export const checkHealthReviewed: CheckEvaluator = (page) => {
	...
	return {
		...
		message: hasMedicalReview
			? "Medical review language detected."
			: "No medical review signal detected on health-related content.",
		auditModes: ["full"],
		industries: ["healthcare"],
	};
};
...
export const checkHealthSchema: CheckEvaluator = (page) => {
	...
	return {
		...
		expected:
			"MedicalWebPage, MedicalCondition, Drug, or Physician schema on health content",
		message: hasMedicalSchema
			? "Medical schema detected."
			: "No medical schema detected on health-related content.",
		auditModes: ["full"],
		industries: ["healthcare"],
	};
};
