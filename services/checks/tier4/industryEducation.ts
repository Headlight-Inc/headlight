...
export const checkEduCourseSchema: CheckEvaluator = (page, ctx) => {
	...
	return {
		...
		message: hasCourseSchema
			? "Education/course schema detected."
			: "No course-related structured data detected.",
		auditModes: ["full"],
...
export const checkEduAccreditation: CheckEvaluator = (page, ctx) => {
	if (ctx?.industry !== "education") return null;
	if (page.crawlDepth !== 0 && !page.industrySignals?.hasAccreditation)
		return null;
	...
	return {
		...
		message: hasAccreditation
			? "Accreditation signal detected."
			: "No clear accreditation signal detected.",
		auditModes: ["full"],
		industries: ["education"],
	};
};
...
export const checkEduSyllabus: CheckEvaluator = (page, ctx) => {
	...
	return {
		...
		message: hasSyllabus
			? "Syllabus or curriculum structure detected."
			: "No syllabus-style structure detected.",
