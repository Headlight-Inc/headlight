...
export const checkFinanceDisclaimer: CheckEvaluator = (page, ctx) => {
	...
	const isFinanceLike =
		/invest|stock|portfolio|retirement|crypto|mortgage|loan|tax/i.test(
	...
	return {
		...
		value: { hasDisclaimer },
		expected:
			"Clear financial disclaimer on advisory or informational finance pages",
		message: hasDisclaimer
			? "Financial disclaimer detected."
			: "No financial disclaimer detected on finance-related content.",
		auditModes: ["full"],
		industries: ["finance"],
	};
};
...
export const checkFinanceFreshness: CheckEvaluator = (page, ctx) => {
	...
	return {
		...
		tier: 4,
		category: "finance",
		name: "Financial Content Freshness",
		...
		message: isFresh
			? `Finance content appears fresh (${dateValue}).`
			: `Finance content may be stale (${dateValue}).`,
		auditModes: ["full", "content"],
		industries: ["finance"],
	};
};
