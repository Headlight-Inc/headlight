...
export const checkCarbonFootprint: CheckEvaluator = (page) => {
	...
	const severity = ["A", "B"].includes(rating)
		? "pass"
		: rating === "C"
			? "info"
			: "warning";
...
export const checkCookieCompliance: CheckEvaluator = (page) => {
	...
	return {
		...
		message: hasBanner
			...
			: "No cookie consent banner detected. Required for GDPR/CCPA.",
		auditModes: ["full"],
		industries: ["all"],
	};
};
...
export const checkAccessibilityStatement: CheckEvaluator = (page, ctx) => {
	if (page.crawlDepth !== 0) return null;
	const hasA11y = ctx?.allPages.some((p) =>
		/(accessibility|a11y-statement)/i.test(p.url),
	);
	...
	return {
		...
		message: hasA11y
			...
			: "No accessibility statement found on site.",
		auditModes: ["full", "accessibility"],
		industries: ["all"],
	};
};
