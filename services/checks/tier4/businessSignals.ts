...
export const checkPricingPage: CheckEvaluator = (page, ctx) => {
	...
	return {
		...
		name: "Pricing Page Detection",
		severity: hasPricing ? "pass" : "info",
		value: hasPricing,
		expected: true,
		message: hasPricing
			? "Pricing page detected."
			: "No pricing page found. Consider adding one for conversion.",
...
export const checkTrustSignals: CheckEvaluator = (page) => {
	...
	return {
		...
		severity,
		value: {
			privacyPolicy: page.privacyPageLinked,
			terms: page.termsPageLinked,
			badges: page.hasTrustBadges,
			socialProof: page.hasTestimonials || page.hasCaseStudies,
		},
		expected: "At least 3 trust signals present",
		message:
			severity === "pass"
				? `${count}/4 trust signals found.`
				: `Only ${count}/4 trust signals found. Missing elements may reduce user confidence.`,
...
export const checkConversionPaths: CheckEvaluator = (page, ctx) => {
	...
	const conversionPages =
		ctx?.allPages.filter((p) =>
			/(signup|sign-up|register|checkout|cart|contact|demo|trial|get-started|book)/i.test(
				p.url,
			),
		) || [];
	...
	return {
		checkId: "t4-conversion-paths",
		tier: 4,
		category: "business_signals",
		name: "Conversion Path Detection",
		severity: count > 0 ? "pass" : "info",
		value: { count, urls: conversionPages.map((p) => p.url).slice(0, 10) },
		expected: "At least 1 conversion path",
		message:
			count > 0
				? `${count} conversion paths detected (signup, demo, checkout, etc.)`
				: "No clear conversion paths found.",
...
export const checkUSPDetection: CheckEvaluator = (page) => {
	...
	return {
		checkId: "t4-usp-detection",
		tier: 4,
		category: "business_signals",
		name: "USP Detection",
		severity: hasUSP ? "pass" : "info",
		value: { detected, count: detected.length },
		expected: "Unique Selling Proposition patterns detected in content",
		message: hasUSP
			? `Detected ${detected.length} USP-related keywords.`
			: "No clear USP patterns detected on homepage.",
...
export const checkTeamPage: CheckEvaluator = (page, ctx) => {
	if (page.crawlDepth !== 0) return null;
	const hasTeam = ctx?.allPages.some((p) =>
		/(team|staff|our-people|leadership)/i.test(p.url),
	);

	return {
		checkId: "t4-team-page",
		tier: 4,
		category: "business_signals",
		name: "Team Page Analysis",
		severity: hasTeam ? "pass" : "info",
		value: hasTeam,
		expected: "Existence of a team or leadership page",
		message: hasTeam
			? "Team page detected."
			: "No dedicated team or leadership page found.",
		auditModes: ["full"],
		industries: ["all"],
	};
};
...
export const checkBusinessAge: CheckEvaluator = (page) => {
	if (page.crawlDepth !== 0) return null;
	return {
		checkId: "t4-business-age",
		tier: 4,
		category: "business_signals",
		name: "Business Age Estimation",
		severity: "info",
