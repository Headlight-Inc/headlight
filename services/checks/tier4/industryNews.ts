...
export const checkNewsArticleSchema: CheckEvaluator = (page, ctx) => {
	...
	const hasNewsSchema = (page.schemaTypes || []).some((t: string) =>
	...
	return {
		...
		message: hasNewsSchema
			? "Content schema detected."
			: "No article-related schema found on this content page.",
		auditModes: ["full", "news_editorial"],
...
export const checkNewsPubDate: CheckEvaluator = (page) => {
	...
	return {
		...
		message: hasDate
			? `Publication date detected: ${page.visibleDate}`
			: "No visible publication date found on this article.",
		auditModes: ["full", "news_editorial"],
		industries: ["news", "blog"],
	};
};
...
export const checkNewsAuthor: CheckEvaluator = (page) => {
	...
	return {
		...
		message: hasAuthor
			...
			: "No author attribution found. Important for E-E-A-T.",
		auditModes: ["full", "news_editorial"],
		industries: ["news", "blog"],
	};
};
