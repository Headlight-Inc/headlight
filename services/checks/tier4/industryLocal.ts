...
export const checkLocalSchema: CheckEvaluator = (page, ctx) => {
	...
	const hasLocalSchema = (page.schemaTypes || []).some((t: string) =>
		localTypes.includes(t),
	);
...
export const checkLocalMap: CheckEvaluator = (page, ctx) => {
	...
	return {
		...
		message: hasMap
			...
			: "No embedded map found. Useful for local user experience.",
		auditModes: ["full", "local_seo"],
		industries: ["local"],
	};
};
...
export const checkLocalHours: CheckEvaluator = (page, ctx) => {
	...
	const hasHours =
		schemaStr.includes("openingHours") ||
		schemaStr.includes("openingHoursSpecification");

	return {
		checkId: "t4-local-hours",
		tier: 4,
		category: "local",
		name: "Business Hours Markup",
		severity: hasHours ? "pass" : "info",
		value: { hasHours },
		expected: "Business hours specified in schema",
		message: hasHours
			? "Business hours markup found."
			: "No business hours detected in structured data.",
		auditModes: ["full", "local_seo"],
		industries: ["local"],
	};
};
