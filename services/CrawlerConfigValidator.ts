...
export function validateCrawlConfig(
	...
	if (!firstUrl.trim()) {
		...
		try {
			const normalized = firstUrl.startsWith("http")
				? firstUrl
				: `https://${firstUrl}`;
			new URL(normalized);
		} catch {
	...
	if (Number.isFinite(maxPages) && maxPages < 0)
		errors.push("Max pages cannot be negative.");
	if (Number.isFinite(maxPages) && maxPages > 1_000_000)
		warnings.push("Crawling over 1M pages may take a very long time.");
	if (!Number.isFinite(maxDepth) || maxDepth < 0)
		errors.push("Max depth cannot be negative.");
	if (!Number.isFinite(threads) || threads < 1 || threads > 200)
		errors.push("Concurrency must be between 1 and 200.");
	if (Number.isFinite(crawlDelay) && crawlDelay < 0)
		errors.push("Crawl delay cannot be negative.");
	...
	for (const line of String(config.customHeaders || "")
		.split(/\r?\n/)
		.map((entry) => entry.trim())
		.filter(Boolean)) {
		if (!line.includes(":")) {
...
export class CrawlerConfigValidator {
	...
	static validate(config: Partial<CrawlerConfig>): CrawlerConfig {
		...
		if (
			!sanitized.startUrls ||
			!Array.isArray(sanitized.startUrls) ||
			sanitized.startUrls.length === 0
		) {
		...
		if (sanitized.startUrls.length === 0) {
			throw new Error(
				"No valid Start URLs provided (must include http/https).",
			);
		}

		// 2. Numerical clamping
		sanitized.limit = String(
			Math.max(1, Math.min(100000, parseInt(sanitized.limit || "100"))),
		);
		sanitized.maxDepth = String(
			Math.max(1, Math.min(50, parseInt(sanitized.maxDepth || "10"))),
		);
		...
		sanitized.requestTimeout = Math.max(
			1,
			Math.min(120, sanitized.requestTimeout || 30),
		);

		// 3. String sanitization
		sanitized.userAgent = (sanitized.userAgent || "HeadlightCrawler/1.0")
			.trim()
			.substring(0, 255);
		...
		sanitized.aiBatchSize = Math.max(
			1,
			Math.min(100, sanitized.aiBatchSize || 20),
		);

		// 7. Security: Remove sensitive fields if they are just placeholders or whitespace
		if (sanitized.proxyPass && sanitized.proxyPass.trim() === "")
			sanitized.proxyPass = "";
		if (sanitized.authPass && sanitized.authPass.trim() === "")
			sanitized.authPass = "";
