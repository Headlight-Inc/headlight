...
export async function createComment(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[CollaborationService] Failed to sync comment to cloud:",
				error,
			);
		}
	}
...
export async function getComments(
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[CollaborationService] Failed to fetch comments from cloud:",
				error,
			);
		}
	}
...
export async function resolveComment(
	...
	await crawlDb.comments.update(commentId, {
		resolved: true,
		resolved_by: userId,
		resolved_at,
	});
	...
	if (isCloudSyncEnabled) {
		try {
			...
			console.error(
				"[CollaborationService] Failed to sync comment resolution:",
				error,
			);
		}
	}
...
export async function addReaction(
	...
	const reactions = comment.reactions_json
		? JSON.parse(comment.reactions_json)
		: {};
