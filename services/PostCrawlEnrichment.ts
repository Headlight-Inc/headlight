...
import {
	...
	getWaybackInfo,
	getSSLGrade,
} from "./ExternalEnrichmentService";
...
export class PostCrawlEnrichment {
	...
	static async runUnifiedEnrichment(
		...
		if (totalHtmlPages === 0) {
			onProgress?.(
				"No HTML pages found in database. Check crawl settings or isHtmlPage logic.",
			);
		...
		onProgress?.(
			`Starting enrichment: ${totalHtmlPages.toLocaleString()} HTML pages found, targeting top ${targetPages.length.toLocaleString()} for deep data.`,
		);
		console.log(
			`[Enrichment] Session ${sessionId}: Total HTML=${totalHtmlPages}, Targeted=${targetPages.length}`,
		);

		// 1. CSV Data Merging (Highest Priority Overrides)
		if (config.keywordCsvData?.length) {
			onProgress?.(
				`Merging ${config.keywordCsvData.length} manual keywords...`,
			);
			await KeywordUploadMerger.mergeFromCsv(sessionId, config.keywordCsvData);
		}
		if (config.backlinkCsvData?.length) {
			onProgress?.(
				`Merging ${config.backlinkCsvData.length} manual backlinks...`,
			);
			await BacklinkUploadMerger.mergeFromCsv(
				sessionId,
				config.backlinkCsvData,
			);
		}
		...
		if (googleAccessToken && config.gscSiteUrl) {
			try {
				onProgress?.(
					`Connecting to GSC: ${config.gscSiteUrl} (using identity: ${config.googleEmail || "unknown"})...`,
				);
				await GscClientService.enrichSession(
					sessionId,
					config.gscSiteUrl,
					googleAccessToken,
					onProgress,
				...
				onProgress?.(
					`GSC Error: ${err.message || "Unknown error"}${isAuthError ? " - Try reconnecting Google in Integrations." : ""}`,
				);
			}
		} else {
			const reason = !googleAccessToken
		...
		if (config.bingAccessToken) {
			try {
				...
				if (bingSiteUrl) {
					await BingWebmasterService.enrichSession(
						sessionId,
						bingSiteUrl,
						config.bingAccessToken,
						onProgress,
						{ targetUrls },
					);
				}
		...
		if (googleAccessToken && config.ga4PropertyId) {
			try {
				onProgress?.(
					`Connecting to GA4: ${config.ga4PropertyId} (using identity: ${config.googleEmail || "unknown"})...`,
				);
				...
				const isAuthError =
					...
					err.message?.toLowerCase().includes("credentials");
				onProgress?.(
					`GA4 Error: ${err.message || "Unknown error"}${isAuthError ? " - Try reconnecting Google in Integrations." : ""}`,
				);
			}
		} else {
			const reason = !googleAccessToken
				? "No access token provided."
				: "No GA4 property ID selected.";
		...
		if (googleAccessToken && config.industry === "local") {
			try {
				...
				const rootDomain = new URL(
					targetUrls[0] || targetPages[0]?.url || "",
				).hostname.replace(/^www\./, "");
				await GoogleBusinessProfileService.enrichSession(
					sessionId,
					googleAccessToken,
					rootDomain,
					onProgress,
		...
		if (config.ahrefsToken || config.semrushApiKey) {
			try {
				...
				await BacklinkClientService.enrichSession(
					sessionId,
					{
						ahrefsToken: config.ahrefsToken,
		...
		const hasPaidBacklinks = Boolean(
			config.ahrefsToken || config.semrushApiKey,
		);
		if (!hasPaidBacklinks) {
			try {
				...
				if (sampleUrl) {
					...
					await CommonCrawlService.enrichSession(
						sessionId,
						rootDomain,
						onProgress,
					);
				...
				onProgress?.(
					`Common Crawl: ${err.message || "Skipped (service unavailable)"}`,
				);
			}
		}
		...
		if (config.psiApiKey) {
			...
			for (let i = 0; i < topPages.length; i += batchSize) {
				...
				onProgress?.(
					`PSI: Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(topPages.length / batchSize)}...`,
				);

				await Promise.all(
					batch.map(async (page) => {
						try {
							const psi = await fetchPageSpeedInsights(
								page.url,
								config.psiApiKey,
							);
		...
		try {
			const cmsResult = await CMSService.enrichSession(
				config.sessionId,
				session.startUrl,
				onProgress,
			);
			if (cmsResult.cms) {
				onProgress?.(
					`Enriched ${cmsResult.enriched} pages with ${cmsResult.cms} metadata.`,
				);
			}
		} catch (err) {
				...
				if (session) {
					...
					if (lastSession) {
						await submitFixedPages(
							{
								apiKey: config.indexNowApiKey,
								host: new URL(session.startUrl).hostname,
							},
							sessionId,
							lastSession.id,
							onProgress,
	...
	static async enrichSelectedPages(
		...
		const targetPages = await crawlDb.pages.where("url").anyOf(urls).toArray();
	...
	private static async enrichGEO(
		...
		for (let i = 0; i < pages.length; i += batchSize) {
			...
			onProgress?.(
				`GEO: Analyzing content reachability for batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(pages.length / batchSize)}...`,
			);
			...
			await Promise.all(
				updates.map((update) =>
					crawlDb.pages.update(update.url, update as any),
				),
			);
		}
	}

	private static calculateHeuristicCitationWorthiness(
		page: CrawledPage,
	): number {
		let score = 50;
	...
	private static calculateHeuristicExtractionReady(page: CrawledPage): number {
		...
		if (page.jsRenderDiff && page.jsRenderDiff.textDiffPercent < 10)
			score += 20;
		return Math.min(100, score);
	}
}
