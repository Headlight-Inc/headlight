...
export class UrlNormalization {
	...
	static getMatchResult(
		...
		if (pageCanon === dataCanon)
			return { joinType: "canonical", confidence: 98 };

		// 3. Redirect / Final URL Match (High Confidence)
		if (finalUrl) {
			const finalCanon = this.toCanonical(finalUrl);
			if (finalCanon === dataCanon)
				return { joinType: "redirect", confidence: 95 };
		}
		...
		const getPath = (u: string) => {
			try {
				const urlObj = new URL(
					u.startsWith("http")
						? u
						: `https://example.com/${u.startsWith("/") ? u.slice(1) : u}`,
				);
