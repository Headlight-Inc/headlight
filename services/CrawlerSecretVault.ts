...
const buildKey = (scope: string, provider: CrawlerIntegrationProvider) =>
	`${scope}:${provider}`;
...
const persistVault = () => {
	...
	try {
		window.localStorage.setItem(
			STORAGE_KEY,
			JSON.stringify(Object.fromEntries(secretVault.entries())),
		);
	} catch (error) {
...
export const getCrawlerSecretScope = (projectId?: string | null) =>
	projectId || "anonymous";
...
export const clearCrawlerIntegrationSecret = (
	scope: string,
	provider: CrawlerIntegrationProvider,
) => {
	hydrateVault();
