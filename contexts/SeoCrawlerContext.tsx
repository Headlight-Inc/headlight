...
import {
	AUDIT_MODES,
	getWqaDefaultVisibleColumns as getWqaDefaultVisibleColumnsBase,
} from "../services/AuditModeConfig";
...
} from "../services/StrategicIntelligence";
import {
	persistCrawlResults,
	syncCrawlStatus,
	persistEnrichmentStatus,
	deleteCompetitorProfile,
	loadCompetitorProfiles as loadCompetitorProfilesCloud,
} from "../services/CrawlPersistenceService";
import {
	syncFromCrawl,
	listCompetitors,
	deleteCompetitor as removeCompetitorRecord,
...
import {
	GoogleSelectionResolver,
	EffectiveGoogleSelection,
} from "../services/googleSelectionResolver";
...
import {
	computeShareOfVoice,
	computeThreatScores,
} from "../services/CompetitorDiscoveryService";
import {
	detectSiteType,
	type DetectedIndustry,
	type SiteTypeResult,
} from "../services/SiteTypeDetector";
import {
	DEFAULT_WQA_STATE,
	getEffectiveIndustry,
	getEffectiveLanguage,
	type WebsiteQualityState,
	type WqaViewMode,
} from "../services/WebsiteQualityModeTypes";
import {
	computeWqaActionGroups,
	computeWqaSiteStats,
	deriveWqaScore,
} from "../services/WqaSidebarData";
...
export interface CrawlerContextType {
	...
	setActiveCategories: React.Dispatch<
		React.SetStateAction<Array<{ group: string; sub: string }>>
	>;
	...
	setColumnWidths: (
		w:
			...
			| ((p: Record<string, number>) => Record<string, number>),
	) => void;
	...
	integrationConnections: Partial<
		Record<CrawlerIntegrationProvider, CrawlerIntegrationConnection>
	>;
	...
	saveIntegrationConnection: (
		...
		connection: Omit<
			CrawlerIntegrationConnection,
			"provider" | "connectedAt" | "ownership"
		>,
	) => void;
	...
	setCollabOverlayTarget: (
		t: { type: CommentTargetType; id: string; title: string } | null,
	) => void;
	activeCommentTarget: { type: CommentTargetType; id: string } | null;
	setActiveCommentTarget: (
		t: { type: CommentTargetType; id: string } | null,
	) => void;
	exportSubset: (category: {
		group: string;
		sub: string;
		condition?: (p: any) => boolean;
	}) => void;
	createTaskForCategory: (category: {
		group: string;
		sub: string;
		condition?: (p: any) => boolean;
	}) => Promise<void>;
	bulkAIAnalyzeCategory: (category: {
		group: string;
		sub: string;
		condition?: (p: any) => boolean;
	}) => Promise<void>;
	...
	setCrawlingCompetitorDomain: React.Dispatch<
...
export const SeoCrawlerContext = createContext<CrawlerContextType | undefined>(
	undefined,
);
...
const getScopedCrawlerSessionStorageKey = (
	projectId: string | null | undefined,
) =>
	projectId
		? `${CRAWLER_LAST_SESSION_STORAGE_KEY}:${projectId}`
		: CRAWLER_LAST_SESSION_STORAGE_KEY;

const getScopedCrawlerDraftStorageKey = (
	projectId: string | null | undefined,
) =>
	projectId
		? `${CRAWLER_DRAFT_STORAGE_KEY}:${projectId}`
		: CRAWLER_DRAFT_STORAGE_KEY;
...
const replaceHashRouteSearchParams = (
...
const derivePageIntelligence = (page: any) => {
	...
	const avgSessionDuration = Number(
		page.ga4EngagementTimePerPage || page.ga4AvgSessionDuration || 0,
	);
	...
	const authorityScore = Number(
		page.authorityScore ?? calculateAuthorityScore(page),
	);
	const businessValueScore = Number(
		page.businessValueScore ?? calculateBusinessValueScore(page),
	);
	const opportunityScore = Number(
		page.opportunityScore ?? calculateOpportunityScore(page),
	);

	// Engagement/quality metrics (lightweight, no duplicate in services)
	const engagementRisk = clampScore(
		bounceRate * 100 - Math.min(40, avgSessionDuration / 5),
	);
	const trafficQuality = clampScore(
		businessValueScore * 0.65 + Math.max(0, 1 - bounceRate) * 35,
	);
	...
	const coverage =
		coverageParts.length > 0
			? Math.round(
					(coverageParts.reduce((sum, item) => sum + item, 0) /
...
export function SeoCrawlerProvider({ children }: { children: ReactNode }) {
	...
	const routeProjectId =
		routeProjectParam ||
		routeSearchParams.get("projectId") ||
		routeSearchParams.get("project") ||
		null;
	...
	const [crawlingMode, setCrawlingMode] = useState<
		"spider" | "list" | "sitemap"
	>("spider");
	...
	const [activeCategories, setActiveCategories] = useState<
		Array<{ group: string; sub: string }>
	>([{ group: "internal", sub: "All" }]);
	const [auditFilter, setAuditFilter] =
		useState<AuditFilterState>(DEFAULT_FILTER_STATE);
	const [customPresets, setCustomPresets] = useState<CustomAuditPreset[]>(() =>
		getLocalPresets(),
	);
	...
	const [wqaInspectorTab, setWqaInspectorTab] =
		useState<WqaInspectorTab>("summary");
	...
	const [wqaSidebarTab, setWqaSidebarTab] =
		useState<WqaSidebarTab>("wqa_overview");
	...
	const [leftSidebarPreset, setLeftSidebarPreset] = useState<string | null>(
		null,
	);
	...
	const [detectedGa4Property, setDetectedGa4Property] = useState<string | null>(
		null,
	);
	const [tier4Results, setTier4Results] = useState<Map<string, any[]>>(
		new Map(),
	);
	...
	>("dark");
	const [workerFilteredPages, setWorkerFilteredPages] = useState<any[] | null>(
		null,
	);
	const [integrationConnections, setIntegrationConnections] = useState<
		Partial<Record<CrawlerIntegrationProvider, CrawlerIntegrationConnection>>
	>({});
	...
	const [integrationsSource, setIntegrationsSource] = useState<
		"anonymous" | "project" | "project-cache" | "none"
	>("none");
	const [crawlRuntime, setCrawlRuntime] = useState<
		CrawlerContextType["crawlRuntime"]
	>({
		stage: "idle",
		queued: 0,
		activeWorkers: 0,
	...
	const [wqaState, setWqaState] =
		useState<WebsiteQualityState>(DEFAULT_WQA_STATE);
	...
	const [savedWqaViews, setSavedWqaViews] = useState<WqaSavedView[]>(() =>
		getLocalWqaViews(),
	);
	const [activeWqaViewId, setActiveWqaViewId] = useState<string | null>(null);
	const [activeWqaQuickFilterId, setActiveWqaQuickFilterId] = useState<
		string | null
	>(null);
	...
	const deleteWqaView = useCallback(
		(id: string) => {
	...
	const applyWqaView = useCallback(
		(id: string) => {
	...
	const applyWqaQuickFilter = useCallback(
		(id: string) => {
	...
	const activeSidebarSections = useMemo(() => {
		const primaryMode = auditFilter.modes[0] || "full";
		return (
			AUDIT_MODES[primaryMode]?.sidebarSections || [
				"overview",
				"issues",
				"details",
			]
	...
	const setWqaIndustryOverride = useCallback(
	...
	useEffect(() => {
		...
		const onKey = (e: KeyboardEvent) => {
			...
			if (e.key === "k") {
				e.preventDefault();
				document.getElementById("wqa-search")?.focus();
			}
		...
		if (saved) {
			try {
				...
				setConfig((prev) => {
					...
					if (parsed.aiTasks)
						next.aiTasks = { ...prev.aiTasks, ...parsed.aiTasks };
					if (parsed.alertChannels)
						next.alertChannels = {
							...prev.alertChannels,
							...parsed.alertChannels,
						};
					if (parsed.aiCustomKeys)
						next.aiCustomKeys = {
							...prev.aiCustomKeys,
							...parsed.aiCustomKeys,
						};
	...
	const scopedLastSessionStorageKey =
		getScopedCrawlerSessionStorageKey(scopedProjectId);
	const scopedDraftStorageKey =
		getScopedCrawlerDraftStorageKey(scopedProjectId);

	const activeProjectHostname = useMemo(() => {
		return getNormalizedHostname(
			activeProject?.domain || activeProject?.url || null,
		);
	}, [activeProject?.domain, activeProject?.url]);

	const sessionMatchesProjectScope = useCallback(
	...
	const buildSessionSignature = useCallback(
		(mode: "spider" | "list" | "sitemap", urls: string[]) => {
	...
	useEffect(() => {
		...
		if (isCrawling && pages.length >= 50 && !earlyIndustryLocked) {
			...
			addLog(
				`Industry locked to ${siteType.industry} (data stable at ${pages.length} pages).`,
				"info",
			);
			return;
		}
	...
	useEffect(() => {
		...
		const industryToSync =
			auditFilter.industry === "all"
				? null
				: (auditFilter.industry as DetectedIndustry);
		...
		isWqaMode,
		auditFilter.industry,
		wqaState.industryOverride,
		hasHydrated,
		setWqaIndustryOverride,
	]);
	...
	useEffect(() => {
		...
		try {
			...
			if (typeof saved.leftSidebarWidth === "number") {
				setLeftSidebarWidth(
					Math.min(420, Math.max(180, saved.leftSidebarWidth)),
				);
			}
			if (typeof saved.auditSidebarWidth === "number") {
				setAuditSidebarWidth(
					Math.min(640, Math.max(260, saved.auditSidebarWidth)),
				);
			}
		...
		const hydrateIntegrations = async () => {
			...
			try {
				if (isAuthenticated && integrationProjectId) {
					const promotion =
						await promoteAnonymousCrawlerIntegrationsToProject(
							integrationProjectId,
						);
					if (cancelled) return;
					...
					const result =
						await fetchProjectCrawlerIntegrations(integrationProjectId);
					if (cancelled) return;
					...
					setIntegrationsSource(
						Object.keys(anonymousConnections).length > 0 ? "anonymous" : "none",
					);
				}
			} catch (error) {
	...
	useEffect(() => {
		const googleSecrets = getCrawlerIntegrationSecret(
			integrationSecretScope,
			"google",
		);
		const bingSecrets = getCrawlerIntegrationSecret(
			integrationSecretScope,
			"bingWebmaster",
		);
		...
		setConfig((prev: any) => ({
			...prev,
			gscApiKey:
				googleSecrets.accessToken ||
				googleSecrets.access_token ||
				prev.gscApiKey ||
				"",
			gscRefreshToken:
				googleSecrets.refreshToken ||
				googleSecrets.refresh_token ||
				prev.gscRefreshToken ||
				"",
			gscSiteUrl:
				integrationConnections.google?.sync?.siteUrl ||
				integrationConnections.google?.selection?.siteUrl ||
				prev.gscSiteUrl ||
				"",
			ga4PropertyId:
				integrationConnections.google?.sync?.propertyId ||
				integrationConnections.google?.selection?.propertyId ||
				prev.ga4PropertyId ||
				"",
	...
	useEffect(() => {
		...
		setIntegrationConnections((prev: any) => {
			...
			const googleConnection: any = next.google || {
				...
				accountLabel:
					legacyGsc?.accountLabel ||
					legacyGa4?.accountLabel ||
					"Migrated Account",
				sync: { status: "idle" },
			};

			if (legacyGsc?.selection?.siteUrl) {
				googleConnection.sync = {
					...googleConnection.sync,
					siteUrl: legacyGsc.selection.siteUrl,
				};
			}
			if (legacyGa4?.selection?.propertyId) {
				googleConnection.sync = {
					...googleConnection.sync,
					propertyId: legacyGa4.selection.propertyId,
				};
			}
			...
			// Migrate secrets too
			const gscSecrets = getCrawlerIntegrationSecret(
				integrationSecretScope,
				"googleSearchConsole" as any,
			);
			const ga4Secrets = getCrawlerIntegrationSecret(
				integrationSecretScope,
				"googleAnalytics" as any,
			);
			const accessToken =
				gscSecrets?.accessToken ||
				gscSecrets?.access_token ||
				ga4Secrets?.accessToken ||
				ga4Secrets?.access_token;
			const refreshToken =
				gscSecrets?.refreshToken ||
				gscSecrets?.refresh_token ||
				ga4Secrets?.refreshToken ||
				ga4Secrets?.refresh_token;
		...
		if (integrationProjectId) {
			const migratedConnections = {
				...
				google: {
					provider: "google",
					label:
						legacyGsc?.label || legacyGa4?.label || "Google Search & Analytics",
					status: "connected",
					authType: "oauth",
					...
					selection: {
						siteUrl:
							legacyGsc?.selection?.siteUrl ||
							integrationConnections.google?.selection?.siteUrl,
						propertyId:
							legacyGa4?.selection?.propertyId ||
							integrationConnections.google?.selection?.propertyId,
					},
					sync: {
					...
					hasCredentials: Boolean(
						getCrawlerIntegrationSecret(integrationSecretScope, "google" as any)
							.accessToken ||
						getCrawlerIntegrationSecret(integrationSecretScope, "google" as any)
							.access_token ||
						getCrawlerIntegrationSecret(
	...
	useEffect(() => {
		if (autoRestoreAttemptedRef.current || !hasHydrated || isLoadingHistory)
			return;
		if (isAuthenticated && integrationProjectId) {
			saveProjectCachedCrawlerIntegrations(
				integrationProjectId,
	...
	useEffect(() => {
		...
		const formatElapsed = () => {
			const totalSeconds = Math.max(
				0,
				Math.floor((Date.now() - crawlStartTime) / 1000),
			);
	...
	useEffect(() => {
		if (!isDraggingLeftSidebar && !isDraggingSidebar && !isDraggingDetails)
			return;
		...
		const handleMouseMove = (event: MouseEvent) => {
			...
			if (isDraggingSidebar) {
				const nextWidth = Math.min(
					640,
					Math.max(260, window.innerWidth - event.clientX),
				);
			...
			if (isDraggingDetails) {
				const nextHeight = Math.min(
					520,
					Math.max(180, window.innerHeight - event.clientY),
				);
		...
		document.body.style.cursor =
			isDraggingLeftSidebar || isDraggingSidebar ? "ew-resize" : "ns-resize";
	...
	const loadCrawlHistory = useCallback(async () => {
		...
		try {
			...
			addLog(`Failed to load scan history: ${err.message}`, "error", {
				source: "history",
			});
			setCrawlHistory([]);
		} finally {
	...
	const handleStartPauseRef = useRef<
		(force?: boolean | React.MouseEvent | React.KeyboardEvent) => void
	>(() => {});
	const saveCrawlSessionRef = useRef<
		(status?: "completed" | "paused" | "failed") => Promise<void>
	>(async () => {});
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "Escape") {
				...
				if (showAutoFixModal) {
					if (!isFixing) setShowAutoFixModal(false);
					return;
				}
				if (showListModal) {
	...
	const flushPendingPageUpdates = useCallback(() => {
		...
		const mergedUpdates = updates.map((payload) => {
			...
			const mergedPage = {
				...
				gscClicks: hasOwn(payload, "gscClicks")
					? payload.gscClicks
					: (existingPage.gscClicks ?? null),
				gscImpressions: hasOwn(payload, "gscImpressions")
					? payload.gscImpressions
					: (existingPage.gscImpressions ?? null),
				gscCtr: hasOwn(payload, "gscCtr")
					? payload.gscCtr
					: (existingPage.gscCtr ?? null),
				gscPosition: hasOwn(payload, "gscPosition")
					? payload.gscPosition
					: (existingPage.gscPosition ?? null),
				mainKeyword: hasOwn(payload, "mainKeyword")
					? payload.mainKeyword
					: (existingPage.mainKeyword ?? null),
				mainKeywordSource: hasOwn(payload, "mainKeywordSource")
					? payload.mainKeywordSource
					: (existingPage.mainKeywordSource ?? null),
				mainKwVolume: hasOwn(payload, "mainKwVolume")
					? payload.mainKwVolume
					: (existingPage.mainKwVolume ?? null),
				mainKwSearchVolume: hasOwn(payload, "mainKwSearchVolume")
					? payload.mainKwSearchVolume
					: (existingPage.mainKwSearchVolume ?? null),
				mainKwEstimatedVolume: hasOwn(payload, "mainKwEstimatedVolume")
					? payload.mainKwEstimatedVolume
					: (existingPage.mainKwEstimatedVolume ?? null),
				mainKwPosition: hasOwn(payload, "mainKwPosition")
					? payload.mainKwPosition
					: (existingPage.mainKwPosition ?? null),
				bestKeyword: hasOwn(payload, "bestKeyword")
					? payload.bestKeyword
					: (existingPage.bestKeyword ?? null),
				bestKeywordSource: hasOwn(payload, "bestKeywordSource")
					? payload.bestKeywordSource
					: (existingPage.bestKeywordSource ?? null),
				bestKwVolume: hasOwn(payload, "bestKwVolume")
					? payload.bestKwVolume
					: (existingPage.bestKwVolume ?? null),
				bestKwSearchVolume: hasOwn(payload, "bestKwSearchVolume")
					? payload.bestKwSearchVolume
					: (existingPage.bestKwSearchVolume ?? null),
				bestKwEstimatedVolume: hasOwn(payload, "bestKwEstimatedVolume")
					? payload.bestKwEstimatedVolume
					: (existingPage.bestKwEstimatedVolume ?? null),
				bestKwPosition: hasOwn(payload, "bestKwPosition")
					? payload.bestKwPosition
					: (existingPage.bestKwPosition ?? null),
				ga4Views: hasOwn(payload, "ga4Views")
					? payload.ga4Views
					: (existingPage.ga4Views ?? null),
				ga4Sessions: hasOwn(payload, "ga4Sessions")
					? payload.ga4Sessions
					: (existingPage.ga4Sessions ?? null),
				ga4Users: hasOwn(payload, "ga4Users")
					? payload.ga4Users
					: (existingPage.ga4Users ?? null),
				ga4BounceRate: hasOwn(payload, "ga4BounceRate")
					? payload.ga4BounceRate
					: (existingPage.ga4BounceRate ?? null),
				ga4EngagementTimePerPage: hasOwn(payload, "ga4EngagementTimePerPage")
					? payload.ga4EngagementTimePerPage
					: (existingPage.ga4EngagementTimePerPage ?? null),
				ga4EngagementRate: hasOwn(payload, "ga4EngagementRate")
					? payload.ga4EngagementRate
					: (existingPage.ga4EngagementRate ?? null),
				ga4AvgSessionDuration: hasOwn(payload, "ga4AvgSessionDuration")
					? payload.ga4AvgSessionDuration
					: (existingPage.ga4AvgSessionDuration ?? null),
				ga4Conversions: hasOwn(payload, "ga4Conversions")
					? payload.ga4Conversions
					: (existingPage.ga4Conversions ?? null),
				ga4ConversionRate: hasOwn(payload, "ga4ConversionRate")
					? payload.ga4ConversionRate
					: (existingPage.ga4ConversionRate ?? null),
				ga4Revenue: hasOwn(payload, "ga4Revenue")
					? payload.ga4Revenue
					: (existingPage.ga4Revenue ?? null),
				sessionsDelta: hasOwn(payload, "sessionsDelta")
					? payload.sessionsDelta
					: (existingPage.sessionsDelta ?? null),
				sessionsDeltaAbsolute: hasOwn(payload, "sessionsDeltaAbsolute")
					? payload.sessionsDeltaAbsolute
					: (existingPage.sessionsDeltaAbsolute ?? null),
				sessionsDeltaPct: hasOwn(payload, "sessionsDeltaPct")
					? payload.sessionsDeltaPct
					: (existingPage.sessionsDeltaPct ?? null),
				isLosingTraffic: hasOwn(payload, "isLosingTraffic")
					? payload.isLosingTraffic
					: (existingPage.isLosingTraffic ?? null),
				urlRating: hasOwn(payload, "urlRating")
					? payload.urlRating
					: (existingPage.urlRating ?? null),
				referringDomains: hasOwn(payload, "referringDomains")
					? payload.referringDomains
					: (existingPage.referringDomains ?? null),
				backlinks: hasOwn(payload, "backlinks")
					? payload.backlinks
					: (existingPage.backlinks ?? null),
				...
				opportunityScore: hasOwn(payload, "opportunityScore")
					? payload.opportunityScore
					: (existingPage.opportunityScore ?? null),
				isHtmlPage: hasOwn(payload, "isHtmlPage")
					? payload.isHtmlPage
					: (existingPage.isHtmlPage ?? false),
				timestamp: Date.now(),
			};
		...
		setSelectedPage((prev) => {
			if (!prev) return prev;
			const selectedUpdate = mergedUpdates.find(
				(update) => update.url === prev.url,
			);
	...
	const queuePageUpdate = useCallback(
		(payload: any) => {
			...
			pendingPageUpdatesRef.current.set(payload.url, {
				...existing,
				...payload,
			});
	...
	const runTier4Checks = useCallback(
		(pages: any[]) => {
	...
	const handleStartPause = (
		forceResumeParam?: boolean | React.MouseEvent | React.KeyboardEvent,
	) => {
		...
		if (!urlsToScan.length || !urlsToScan[0]) {
			addLog(
				`Please provide a valid ${crawlingMode === "list" ? "list of URLs" : "web address"}.`,
				"error",
				{ source: "system" },
			);
			return;
		...
		const validation = validateCrawlConfig({
			...config,
			startUrls: urlsToScan,
		});
		if (!validation.valid) {
			validation.errors.forEach((message) =>
				addLog(message, "error", { source: "system" }),
			);
			return;
		}
		validation.warnings.forEach((message) =>
			addLog(message, "warn", { source: "system" }),
		);
		...
		if (googleConn && gscToken && !isResume) {
			addLog(`Resolving Google properties for ${urlsToScan[0]}...`, "info", {
				source: "system",
			});
			...
			GoogleSelectionResolver.resolveEffectiveGoogleSelection({
					...
					if (resolution.siteUrl || resolution.propertyId) {
						const updates: any = {
							selection: {
								...
								propertyId:
									resolution.propertyId || googleConn.selection?.propertyId,
						...
						if (
							resolution.siteUrl !== googleConn.selection?.siteUrl &&
							resolution.siteUrl
						) {
							addLog(
								`Auto-detected GSC property: ${resolution.siteUrl} (Confidence: ${resolution.gscConfidence}%)`,
								"success",
								{ source: "system" },
							);
						...
						if (
							...
							addLog(
								`Auto-detected GA4 property: ${resolution.propertyId} (Confidence: ${resolution.ga4Confidence}%)`,
								"success",
								{ source: "system" },
							);
						...
						if (targetProjectId) {
							upsertProjectCrawlerIntegration(targetProjectId, {
								...googleConn,
								...updates,
							});
						} else {
				...
				.catch((err) => {
					...
					addLog(
						"Google property resolution failed. Crawl will continue without search data.",
						"warn",
						{ source: "system" },
					);
				});
		...
		const shouldAutoUseGhost =
			!config.useGhostEngine && !configuredWsUrl && !isLocalhost;
		...
		if (!useGhostMode) {
			...
			addLog(
				"No remote scanner configured. Using Ghost Engine (Local-Only).",
				"info",
				{ source: "system" },
			);
		}
		...
		if (sessionId) {
			...
			saveSession(sessionDraft)
				...
				.catch((error) =>
					console.error("Failed to create initial crawl draft:", error),
				);
		}
		...
		if (useGhostMode) {
			...
			const ghost = new GhostCrawler({
				maxConcurrent:
					config.concurrent || parseInt(String(config.threads), 10) || 5,
				...
				(payload: {
					...
					if (payload.urls && pagesRef.current.length > 0) {
						...
						pagesRef.current.forEach((page) => {
							...
							const canonical = UrlNormalization.toCanonical(
						...
						if (pagesToUpdate.length > 0) {
							addLog(
								`Sitemap parse complete: Back-filled status for ${pagesToUpdate.length} pages.`,
								"info",
							);
			...
			ghost.on("complete", () => {
				...
				if (completedPages.length > 0) {
					...
					const { pages: updated, siteType: detectedSiteType } =
						runPostCrawlScoring(completedPages);
					...
					if (activeProject?.id) {
						const crawlDuration = crawlStartTime
						...
						persistCrawlResults({
							...
							.then((result) => {
								if (result) {
									addLog(
										`Dashboard synced — Health Score: ${result.score}/100, ${result.issues.length} issues detected.`,
										"success",
										{ source: "system" },
									);
									...
									syncFromCrawl(
										...
										.then(async (sync) => {
											if (
												sync.keywordsImported > 0 ||
												sync.competitorsFound > 0
											) {
												addLog(
													`Auto-discovered: ${sync.keywordsImported} keywords, ${sync.competitorsFound} competitors.`,
													"info",
													{ source: "analysis" },
												);
											}
											...
											if (autoTasks.length > 0) {
												addLog(
													`Auto-created ${autoTasks.length} tasks from crawler issues.`,
													"info",
													{ source: "system" },
												);
							...
							.catch((err) => {
								console.error(
									"[CrawlPersistence] Ghost crawl dashboard sync failed:",
									err,
								);
							});
					}
		...
		try {
			...
			ws.onopen = async () => {
				...
				const getOrRefreshGoogleToken = async () => {
					const email =
						...
						googleConnection?.sync?.email ||
						(googleConnection?.metadata?.email as string);
					const secrets = googleConnection
						? getCrawlerIntegrationSecret(integrationSecretScope, "google")
						: null;
					let token = secrets?.accessToken || secrets?.access_token;
					const refreshToken = secrets?.refreshToken || secrets?.refresh_token;

					if (!token && email) {
						addLog("Refreshing Google access token...", "info", {
							source: "system",
						});
				...
				} = await getOrRefreshGoogleToken();
				let currentGsc =
					config.gscSiteUrl || googleConnection?.selection?.siteUrl;
				let currentGa4 =
					config.ga4PropertyId || googleConnection?.selection?.propertyId;
				...
				if (googleConnection && !googleAccessToken && !googleEmail) {
					addLog(
						"Google connection is inactive (no valid session/email). Please reconnect in the Integrations tab.",
						"warn",
						{ source: "system" },
					);
				}
				...
				if (googleAccessToken && (!currentGsc || !currentGa4)) {
					...
					const resolution =
						await GoogleSelectionResolver.resolveEffectiveGoogleSelection({
					...
					if (resolution.siteUrl && !currentGsc) {
						...
						addLog(
							`Auto-detected GSC property: ${currentGsc} (Confidence: ${resolution.gscConfidence}%)`,
							"success",
						);
					}
					if (resolution.propertyId && !currentGa4) {
						...
						addLog(
							`Auto-detected GA4 property: ${currentGa4} (Confidence: ${resolution.ga4Confidence}%)`,
							"success",
						);
					}

					if (
						(resolution.siteUrl && !config.gscSiteUrl) ||
						(resolution.propertyId && !config.ga4PropertyId)
					) {
				...
				if (googleAccessToken && !currentGsc) {
					addLog(
						"GSC sync will be skipped: No Search Console property selected.",
						"warn",
						{ source: "system" },
					);
				}
				if (googleAccessToken && !currentGa4) {
					addLog(
						"GA4 sync will be skipped: No Analytics property ID selected.",
						"warn",
						{ source: "system" },
					);
				}
					...
					JSON.stringify({
						...
						config: {
							...
							google: {
								...
								refreshToken:
									googleRefreshToken || (config as any).gscRefreshToken,
							...
							bing: {
								accessToken:
									getCrawlerIntegrationSecret(
										integrationSecretScope,
										"bingWebmaster",
									).accessToken || config.bingAccessToken,
							},
							uploads: {
								...
								contentInventory:
									integrationConnections.contentInventory?.uploadData,
							},
						},
					}),
				);
			...
			ws.onmessage = (e) => {
				...
				if (data.type === "FETCHING") {
					...
					const next = {
						...
						concurrency:
							payload.concurrency || parseInt(String(config.threads), 10) || 5,
					...
					if (activeProject?.id && now - lastSyncTimeRef.current > 2000) {
						...
						const progress =
							next.discovered > 0 ? (next.crawled / next.discovered) * 100 : 0;
						syncCrawlStatus({
							projectId: activeProject.id,
							status: "running",
							progress,
					...
					setSitemapData(
						(prev) =>
							prev ?? buildSitemapState(0, data.payload.sitemaps, false),
					);
				} else if (data.type === "LOG") {
					addLog(
						data.payload?.message || data.msg,
						data.payload?.type || data.logType || "info",
						{ source: "crawler" },
					);
					...
					if (crawlerPayload.isHtmlPage === undefined) {
						crawlerPayload.isHtmlPage = Boolean(
							crawlerPayload.contentType?.includes("text/html") ||
							crawlerPayload.contentType?.includes("application/xhtml"),
						);
					}

					const pendingSize = pendingPageUpdatesRef.current.has(
						crawlerPayload.url,
					)
						? 0
						: 1;
					if (
						...
						pagesRef.current.length +
							pendingPageUpdatesRef.current.size +
							pendingSize >
							trialPagesLimit
					) {
						wsRef.current?.send(JSON.stringify({ type: "STOP_CRAWL" }));
						flushPendingPageUpdates();
						addLog(
							`Trial limit reached (${trialPagesLimit} pages). Sign in for unlimited scanning.`,
							"info",
							{ source: "system" },
						);
					...
					if (provider !== "google") {
						mergeCrawlerIntegrationSecret(
							integrationSecretScope,
							provider as CrawlerIntegrationProvider,
							{ accessToken },
						);
					}
					setConfig((prev: any) => {
						if (provider === "google")
							return { ...prev, gscApiKey: accessToken };
						if (provider === "bingWebmaster")
							return { ...prev, bingAccessToken: accessToken };
						return prev;
					});
				} else if (data.type === "ERROR") {
					const errMsg = data.payload.message || "Error encountered";
					if (errMsg === "Crawler stopped" || errMsg.includes("aborted"))
						return;
					...
					const successCount = Number(
						data.payload?.successfulPages ??
							data.payload?.payloadPages ??
							pagesRef.current.length,
					);
					const foundCount = Number(data.payload?.totalPages ?? successCount);
					addLog(
						`Scan complete. Found ${foundCount} URLs; captured ${successCount}.`,
						"success",
						{ source: "crawler" },
					);
					...
					const runPostCrawlFlow = async () => {
						if (currentSessionIdRef.current) {
							...
							const completedPages = [...pagesRef.current];
							addLog(
								"Calculating Strategic PageRank & Health Scores...",
								"info",
								{ source: "analysis" },
							);
							const { pages: updated, siteType: detectedSiteType } =
							...
							// Run Tier 4 checks
							addLog("Running Tier 4 deep evaluation...", "info", {
								source: "analysis",
							});
							...
							if (activeProject?.id && updated.length > 0) {
								...
								if (result) {
									addLog(
										`Dashboard synced — Health Score: ${result.score}/100`,
										"success",
										{ source: "system" },
									);
									if (updateProject) {
									...
									try {
										addLog("Reconciling tasks with latest issues...", "info", {
											source: "system",
										});
										const { resolved, reopened } =
											await reconcileTasksWithIssues(
												activeProject.id,
												result.auditId, // the run ID from persistCrawlResults
											);
										if (resolved > 0)
											addLog(
												`${resolved} tasks auto-resolved (issues fixed)`,
												"success",
												{ source: "system" },
											);
										if (reopened > 0)
											addLog(
												`${reopened} tasks reopened (issues returned)`,
												"warn",
												{ source: "system" },
											);

										addLog("Executing auto-assignment rules...", "info", {
											source: "system",
										});
										...
										const createdTasks = await executeRules(
											activeProject.id,
										...
										if (createdTasks.length > 0) {
											addLog(
												`Auto-created ${createdTasks.length} tasks from detected issues`,
												"success",
												{ source: "system" },
											);
										...
										console.error(
											"[SeoCrawlerContext] Auto-assignment failed:",
											assignErr,
										);
										addLog(
											"Auto-assignment pipeline failed, but crawl was saved.",
											"warn",
											{ source: "system" },
										);
									}
			...
			ws.onclose = () => {
				...
				if (!wsIntentionalCloseRef.current && isCrawlActiveRef.current) {
					...
					addLog(
						"Crawler connection dropped. Attempting to reconnect may require resume.",
						"warn",
						{ source: "system" },
					);
				}
			...
			addLog(
				`Failed to start crawl: ${error.message || "Unknown connection error"}`,
				"error",
				{ source: "system" },
			);
			setIsCrawling(false);
	...
	const activeCheckIds = useMemo(
		() => getActiveCheckIds(auditFilter),
		[auditFilter],
	);
	const activeCheckCategories = useMemo(
		() => getActiveCategoryTreeIds(auditFilter),
		[auditFilter],
	);
	...
	const applyAuditMode = useCallback(
		(modes: AuditMode[], industry: IndustryFilter) => {
	...
	const saveCustomPreset = useCallback(
		(name: string, modes: AuditMode[], industry: IndustryFilter) => {
	...
	useEffect(() => {
		...
		const exists = filteredIssuePages.some((group) =>
			group.issues.some((issue: any) => issue.id === activeMacro),
		);
		if (!exists) {
			setActiveMacro(null);
		}
	...
	const pagesWithDerivedSignals = useMemo(() => {
		...
		return pages.map((page) => {
			...
			return {
				...
				isDuplicateMetaDesc:
					Boolean(metaKey) && duplicateMetaDescSet.has(metaKey),
	...
	const categoryCounts = useMemo(() => {
		if (pagesWithDerivedSignals.length === 0)
		...
		const allCats = [
			...
			...(dynamicClusters.length > 0
				? [
						{
							id: "ai-clusters",
							label: "AI Topic Clusters",
							icon: null,
							sub: ["All", ...dynamicClusters],
						},
					]
				: []),
		];
		...
		if (activeMacro) {
			if (activeMacro !== "all" && MACRO_FILTERS[activeMacro]) {
				...
				for (const issueGroup of filteredIssuePages) {
					const issue = issueGroup.issues.find(
						(entry) => entry.id === activeMacro,
					);
					if (issue) {
		...
		const sanitizedSelections = activeCategories
			...
			.filter(
				(selection, index, arr) =>
					arr.findIndex(
						(entry) =>
							entry.group === selection.group && entry.sub === selection.sub,
					) === index,
			);

		const hasEffectiveSelection = sanitizedSelections.some(
			(selection) =>
				!(selection.group === "internal" && selection.sub === "All"),
		);

		if (!isWqaMode && hasEffectiveSelection) {
			list = list.filter((page) =>
				sanitizedSelections.some((selection) =>
					matchesCategoryFilter(selection.group, selection.sub, page, {
						rootHostname,
					}),
				),
			);
	...
	const wqaForecast = useMemo(() => {
		...
		return ForecastService.computeForecast(
			pagesWithDerivedSignals,
	...
	const hasOnlyDefaultCategory =
		...
		activeCategories.every(
			(entry) => entry.group === "internal" && entry.sub === "All",
		);
	const canUseWorkerFiltering =
		pagesWithDerivedSignals.length > 5000 &&
	...
	const handleImport = async (file: File) => {
		try {
			...
			await upsertPages(
				data.sessionId,
				data.pages.map((p: any) => ({ ...p, crawlId: data.sessionId })),
			);

			await loadCrawlHistory();
			await loadSession(data.sessionId);

			addLog(`Successfully imported ${data.pages.length} pages.`, "success", {
				source: "history",
			});
		} catch (err: any) {
			console.error("Import failed:", err);
			addLog(`Failed to import crawl data: ${err.message}`, "error", {
				source: "history",
			});
		}
	};
	...
	const healthScore = useMemo(() => {
		...
		const brokenPenalty =
			isIssueEnabled("404") || isIssueEnabled("500") ? stats.broken * 5 : 0;
		const serverPenalty = isIssueEnabled("500") ? stats.serverErrors * 10 : 0;
		const titlePenalty = isIssueEnabled("title_missing")
			? stats.missingTitles * 2
			: 0;
		const metaPenalty = isIssueEnabled("meta_missing")
			? stats.missingMetaDesc * 1
			: 0;
		const speedPenalty = isIssueEnabled("slow_response")
			? stats.slowPages * 3
			: 0;
		const indexPenalty =
			isIssueEnabled("noindex") || isIssueEnabled("blocked_robots")
				? stats.nonIndexable * 1
				: 0;
	...
	const persistSessionCheckpoint = useCallback(
		async (
			...
			if (!sessionId) return;
			const existingSessionProjectId = crawlHistory.find(
				(session) => session.id === sessionId,
			)?.projectId;

			const resolvedEntryUrls =
				crawlingMode === "list"
					? listUrls
							.split("\n")
							.map((u) => u.trim())
			...
			const resolvedStatus =
				...
				(isCrawling
					? "running"
					: crawlRuntime.stage === "completed"
						? "completed"
						: crawlRuntime.stage === "error"
							? "failed"
							: "paused");
	...
	useEffect(() => {
		...
		sessionCheckpointTimeoutRef.current = window.setTimeout(
			() => {
				const now = Date.now();
				const shouldIncludePages =
					isCrawling && now - lastPagesCheckpointRef.current > 30000;
	...
	const strategicOpportunities = useMemo(() => {
		...
		return [...pagesWithDerivedSignals]
			.filter(
				(page) =>
					page.recommendedAction && page.recommendedAction !== "Monitor",
			)
			.sort((a, b) => {
				const scoreA =
					Number(a.opportunityScore || 0) + Number(a.businessValueScore || 0);
				const scoreB =
					Number(b.opportunityScore || 0) + Number(b.businessValueScore || 0);
	...
	// AI Layer States
	const [aiResults, setAiResults] = useState<Map<string, PageAIResult>>(
		new Map(),
	);
	...
	const [collabOverlayTarget, setCollabOverlayTarget] = useState<{
		type: CommentTargetType;
		id: string;
		title: string;
	} | null>(null);
	...
	useEffect(() => {
		...
		const loadCollabData = async () => {
			try {
				...
				console.error(
					"[SeoCrawlerContext] Failed to load collaboration data:",
					err,
				);
			}
		};
	...
	useEffect(() => {
		auditInsightsRef.current = auditInsights;
	}, [auditInsights]);

	const runAIAnalysis = useCallback(
		async (pagesToAnalyze?: any[]) => {
			const targetPages =
				pagesToAnalyze ||
			...
			addLog(
				`Starting AI Analysis for ${targetPages.length} pages...`,
				"info",
				{ source: "analysis" },
			);
			...
			try {
				...
				const results = await engine.analyzePages(
					...
					(done, total, url) => {
						...
						if (
							analysisRuntimeRef.current.isAnalyzing &&
							analysisRuntimeRef.current.stage === "ai-analysis"
						) {
				...
				const updatedPages = targetPages.map((p) => {
					...
					return {
						...
						funnelStage:
							ai.searchIntent === "transactional"
								? "Transactional"
								: ai.searchIntent === "commercial"
									? "Commercial"
									: ai.searchIntent === "navigational"
										? "Navigational"
										: "Informational",
						contentQualityScore:
							ai.contentQualityScore ?? p.contentQualityScore,
						...
						grammarErrors:
							ai.grammarSource === "ai"
								? (ai.grammarAIErrors ?? p.grammarErrors)
								: p.grammarErrors,
						...
						strategicPriority:
							ai.contentQualityScore != null
								? ai.contentQualityScore < 40
									? "High"
									: ai.contentQualityScore < 70
										? "Medium"
										: "Low"
								: p.strategicPriority,
						recommendedAction:
							ai.fixSuggestions?.[0]?.fix || p.recommendedAction,
						recommendedActionReason:
							ai.contentWeaknesses?.[0] || p.recommendedActionReason,
				...
				addLog(`AI analysis error: ${(err as Error).message}`, "error", {
					source: "analysis",
				});
			} finally {
	...
	const checkAndDispatchAlerts = useCallback(
		async (currentScore: number, freshPages: any[]) => {
			if (!activeProject || !config.changeDetection) return;

			const alerts: AlertPayload[] = [];
			const previousSession = crawlHistory.find(
				(s) =>
			...
			if (config.alertOnNewIssues) {
				const totalIssues = freshPages.reduce(
					(sum, p) => sum + getPageIssues(p).length,
					0,
				);
				if (totalIssues > 50) {
					alerts.push({
	...
	const triggerGoogleDriveBackup = useCallback(async () => {
		...
		if (!googleEmail) {
			...
			addLog(
				"Connect Google account in Integrations to enable Drive backups.",
				"warn",
			);
			return;
		}
		...
		try {
			...
			const accessToken = await refreshWithLock(
				googleEmail,
				refreshGoogleToken,
			);
			if (!accessToken) {
				addLog(
					"Failed to authorize Google Drive access. Please reconnect your account.",
					"error",
				);
				return;
			}
			...
			const sessionPages = await crawlDb.pages
				.where("crawlId")
				.equals(sessionId)
				.toArray();
			...
			addLog(
				`Snapshot synced to Google Drive (Headlight Backups).`,
				"success",
				{ source: "session" },
			);
		} catch (err: any) {
	...
	const saveCrawlSession = useCallback(
		async (status: "completed" | "paused" | "failed" = "completed") => {
			if (!currentSessionIdRef.current) return;
			try {
				...
				await loadCrawlHistory();
				addLog(
					`Session saved locally (${pagesRef.current.length} pages).`,
					"success",
					{ source: "session" },
				);
			...
			try {
				...
				if (sess) {
					if (scopedProjectId && !sessionMatchesProjectScope(sess)) {
						addLog(
							"Skipped loading a session from a different project scope.",
							"warn",
							{ source: "history" },
						);
						return;
					...
					if (normalizedSavedPages.length > 0) {
						await crawlDb.pages.bulkPut(normalizedSavedPages);
					}
					...
					setCrawlingMode(
						effectiveSession.crawlingMode ||
							effectiveSession.config?.crawlingMode ||
							"spider",
					);
					const entryUrls = effectiveSession.entryUrls || [];
					if (
						(effectiveSession.crawlingMode ||
							effectiveSession.config?.crawlingMode) === "list"
					) {
					...
					setAuditFilter({
						modes:
							Array.isArray(effectiveSession.auditModes) &&
							effectiveSession.auditModes.length > 0
								? (effectiveSession.auditModes as AuditMode[])
								: ["full"],
						industry:
							(effectiveSession.industryFilter as IndustryFilter) || "all",
					});
					...
					setSitemapData(
						buildSitemapState(
							effectiveSession.sitemapData?.totalUrls ??
								normalizedSavedPages.filter((page: any) => page.inSitemap)
									.length,
							effectiveSession.sitemapData?.sources ??
								effectiveSession.robotsTxt?.sitemaps,
							!!effectiveSession.sitemapData,
						),
					);
					setCrawlStartTime(effectiveSession.startedAt || null);
					setCrawlRuntime(
						effectiveSession.runtime || {
							stage:
							...
							maxDepthSeen: Math.max(
								0,
								...normalizedSavedPages.map(
									(page: any) => page.crawlDepth || 0,
								),
							),
							concurrency:
								parseInt(String(effectiveSession.config?.threads), 10) || 5,
							mode:
								effectiveSession.crawlingMode ||
								effectiveSession.config?.crawlingMode ||
								"spider",
					...
					addLog(
						`Loaded session with ${normalizedSavedPages.length} pages.`,
						"success",
						{ source: "history" },
					);
				}
	...
	useEffect(() => {
		initializeDatabase().catch((err) =>
			console.warn("Failed to initialize Turso DB:", err),
		);
	}, []);
	...
	useEffect(() => {
		...
		const preferredSessionId =
			sessionIdFromUrl ||
			window.localStorage.getItem(scopedLastSessionStorageKey);
		...
		if (draftRaw && restoredDraftScopeRef.current !== scopedDraftStorageKey) {
			try {
				...
				if (!urlInput && typeof draft.urlInput === "string")
					setUrlInput(draft.urlInput);
				if (!listUrls && typeof draft.listUrls === "string")
					setListUrls(draft.listUrls);
				if (
					draft.crawlingMode &&
					["spider", "list", "sitemap"].includes(draft.crawlingMode)
				) {
		...
		const restoreTarget =
			projectScopedHistory.find((s) => s.id === preferredSessionId)?.id ??
	...
	const resumeCrawlSession = useCallback(
		async (sessionId: string) => {
			await loadSession(sessionId);
			addLog(
				"Session restored. Restarting crawl with saved configuration...",
				"info",
				{ source: "session" },
			);
	...
	const compareSessions = useCallback(
		async (oldSessionId: string, newSessionId: string) => {
			try {
				...
				addLog(
					`Compared sessions: ${oldPages.length} pages vs ${newPages.length} pages.`,
					"success",
				);
			} catch (err) {
	...
	const deleteCrawlSession = useCallback(
		async (sessionId: string) => {
			try {
	...
	const saveIntegrationConnection = useCallback(
		(
			...
			const nextConnection: CrawlerIntegrationConnection = {
				...
				connectedAt: Date.now(),
				ownership:
					isAuthenticated && integrationProjectId ? "project" : "anonymous",
				...
				hasCredentials:
					...
					(provider === "google" && !!connection.accountLabel),
			};
			...
			setIntegrationConnections((prev) => {
				...
				if (isAuthenticated && integrationProjectId) {
					upsertProjectCrawlerIntegration(
						...
						console.error(
							"Failed to persist project crawler integration:",
							error,
						);
					});
				} else {
	...
	const removeIntegrationConnection = useCallback(
		(provider: CrawlerIntegrationProvider) => {
			setIntegrationConnections((prev) => {
				const next = { ...prev };
				...
				if (isAuthenticated && integrationProjectId) {
					removeProjectCrawlerIntegration(integrationProjectId, provider).catch(
						(error) => {
							console.error(
								"Failed to remove project crawler integration:",
								error,
							);
						},
					);
	...
	const runFullEnrichment = useCallback(async () => {
		...
		// Providers
		const ahrefsSecrets = getCrawlerIntegrationSecret(
			integrationSecretScope,
			"ahrefs" as any,
		);
		const semrushSecrets = getCrawlerIntegrationSecret(
			integrationSecretScope,
			"semrush" as any,
		);
		const bingSecrets = getCrawlerIntegrationSecret(
			integrationSecretScope,
			"bingWebmaster" as any,
		);
		const ahrefsToken = ahrefsSecrets?.api_key || null;
		const semrushApiKey = semrushSecrets?.api_key || null;
		const bingAccessToken =
			bingSecrets?.accessToken || config.bingAccessToken || null;

		// Auto-detect targets / manual overrides
		let gscSiteUrl =
			config.gscSiteUrl || googleConnection?.selection?.siteUrl || urlInput;
		let ga4PropertyId =
		...
		try {
			...
			if (googleEmail) {
				...
				googleAccessToken =
					(await refreshGoogleToken(googleEmail)) || undefined;
				if (!googleAccessToken) {
					addLog(
						"Google connection metadata is present, but no stored access token was found. Reconnect Google to sync GSC/GA4.",
						"warn",
					);
				} else {
					...
					if (!gscSiteUrl || !ga4PropertyId) {
						...
						try {
							...
							if (resolution.siteUrl && !gscSiteUrl) {
								...
								addLog(
									`Auto-detected GSC property for enrichment: ${gscSiteUrl} (Confidence: ${resolution.gscConfidence}%)`,
									"success",
								);
							}
							if (resolution.propertyId && !ga4PropertyId) {
								...
								addLog(
									`Auto-detected GA4 property for enrichment: ${ga4PropertyId} (Confidence: ${resolution.ga4Confidence}%)`,
									"success",
								);
							}
						} catch (e) {
			...
			if (googleAccessToken && !gscSiteUrl) {
				addLog(
					"GSC sync will be skipped: no Search Console property found for this domain.",
					"warn",
					{ source: "system" },
				);
			}
			if (googleAccessToken && !ga4PropertyId) {
				addLog(
					"GA4 sync will be skipped: no Analytics property found for this domain.",
					"warn",
					{ source: "system" },
				);
			}
			...
			await PostCrawlEnrichment.runUnifiedEnrichment(
				...
				(msg) => {
					...
					if (
						analysisRuntimeRef.current.isAnalyzing &&
						analysisRuntimeRef.current.stage === "strategic-audit"
					) {
			...
			const enrichedPages = await crawlDb.pages
				.where("crawlId")
				.equals(currentSessionId)
				.toArray();
			const htmlPages = enrichedPages.filter((page) => page.isHtmlPage);
			const gscMatched = htmlPages.filter(
				(page) => page.gscEnrichedAt !== null,
			).length;
			const ga4Matched = htmlPages.filter(
				(page) => page.ga4EnrichedAt !== null,
			).length;
			const backlinkMatched = htmlPages.filter(
				(page) =>
			...
			await persistEnrichmentStatus({
				...
				gsc: {
					...
					status: gscMatched > 0 ? "success" : "partial",
				},
				ga4: {
					matched: ga4Matched,
					total: htmlPages.length,
					status: ga4Matched > 0 ? "success" : "partial",
				},
				backlinks: {
					matched: backlinkMatched,
					total: htmlPages.length,
					status: backlinkMatched > 0 ? "success" : "partial",
				},
			});
			...
			addLog(
				"Enrichment complete. Strategic opportunities updated.",
				"success",
			);
		} catch (error: any) {
	...
	const runIncrementalEnrichment = useCallback(async () => {
		...
		try {
			...
			if (googleEmail) {
				googleAccessToken =
					(await refreshWithLock(googleEmail, refreshGoogleToken)) || undefined;
			}
			...
			await PostCrawlEnrichment.runIncrementalEnrichment(
				{
					...
					gscSiteUrl:
						config.gscSiteUrl ||
						googleConnection?.selection?.siteUrl ||
						urlInput,
					ga4PropertyId:
						config.ga4PropertyId || googleConnection?.selection?.propertyId,
	...
	const runSelectedEnrichment = useCallback(
		async (urls: string[]) => {
			...
			try {
				...
				if (googleEmail) {
					googleAccessToken =
						(await refreshWithLock(googleEmail, refreshGoogleToken)) ||
						undefined;
				}
	...
	const exportSubset = useCallback(
		(category: any) => {
			const filtered = pagesWithDerivedSignals.filter((page) =>
				matchesCategoryFilter(category.group, category.sub, page, {
					rootHostname,
				}),
			);
			const headers = visibleColumns
				.map((key) => ALL_COLUMNS.find((c) => c.key === key)?.label || key)
				.join(",");
	...
	const createTaskForCategory = useCallback(
		async (category: any) => {
			if (!activeProject?.id) {
			...
			try {
				...
				addLog(
					`Created task for "${category.sub}" (${filtered.length} pages)`,
					"success",
	...
	const bulkAIAnalyzeCategory = useCallback(
		async (category: any) => {
			const filtered = pagesWithDerivedSignals.filter(
				(p) =>
					p.isHtmlPage &&
					p.statusCode === 200 &&
					matchesCategoryFilter(category.group, category.sub, p, {
						rootHostname,
					}),
			);
			if (filtered.length === 0) {
				addLog(
					"No suitable pages found for AI analysis in this category.",
					"info",
				);
				return;
			}
			addLog(
				`Starting AI analysis for "${category.sub}" (${filtered.length} pages)...`,
				"info",
				{ source: "analysis" },
			);
	...
	const runCompleteAnalysis = useCallback(async () => {
		if (analysisRuntime.isAnalyzing || pagesWithDerivedSignals.length === 0)
			return;
		...
		try {
			...
			const htmlPages = await crawlDb.pages
				.where("crawlId")
				.equals(currentSessionIdRef.current || "")
				.toArray();
			const totalHtml = htmlPages.filter((p) => p.isHtmlPage).length;
			const enrichedCount = htmlPages.filter(
				(p) => p.gscEnrichedAt || p.ga4EnrichedAt || p.backlinkEnrichedAt,
			).length;

			// Only auto-continue if there's a significant gap (e.g., > 500 pages or large % missing)
			if (totalHtml > 500 && enrichedCount < totalHtml) {
				addLog(
					`Auto-continuing enrichment for remaining ${totalHtml - enrichedCount} pages...`,
					"info",
					{ source: "analysis" },
				);
			...
			addLog("Complete Analysis finished successfully.", "success", {
				source: "analysis",
			});
			...
			addLog(`Complete Analysis failed: ${err.message}`, "error", {
				source: "analysis",
			});
		}
	}, [
	...
	useEffect(() => {
		if (
			crawlRuntime.stage === "completed" &&
			config.autoBackupDestination === "google-drive" &&
			pages.length > 0
		) {
	...
	const [crawlingCompetitorDomain, setCrawlingCompetitorDomain] = useState<
		string | null
	>(null);

	// ─── Competitive Mode View ───
	const [competitiveViewMode, setCompetitiveViewMode] =
		useState<CompetitiveViewMode>("matrix");

	const [competitiveState, setCompetitiveState] =
		useState<CompetitiveModeState>(DEFAULT_COMPETITIVE_STATE);
	...
	const refreshAllCompetitors = useCallback(async () => {
		...
		for (const domain of domains) {
			...
			try {
				...
				const profile = await runCompetitorMicroCrawl(
					domain,
					activeProject.id,
					{
						maxPages: 30,
	...
	const buildOwnProfile = useCallback(() => {
		...
		for (const page of pages) {
			...
			if (!rawUrl) continue;
			try {
				ownDomain = new URL(String(rawUrl)).hostname
					.replace(/^www\./, "")
					.toLowerCase();
				if (ownDomain) break;
			} catch {
		...
		if (!ownDomain) {
			try {
				if (activeProject?.domain) {
					ownDomain = String(activeProject.domain)
						.replace(/^www\./, "")
						.toLowerCase();
				} else if (activeProject?.url) {
					ownDomain = new URL(String(activeProject.url)).hostname
						.replace(/^www\./, "")
						.toLowerCase();
				}
			} catch {
	...
	const refreshCompetitorScores = useCallback(
		(targetDomain?: string) => {
			setCompetitiveState((prev) => {
				...
				const domainsToRefresh = targetDomain
					? [targetDomain]
					: [...prev.competitorProfiles.keys()];
	...
	const addCompetitorAndCrawl = useCallback(
		async (competitorUrl: string) => {
			let domain: string;
			try {
			...
			setCompetitiveState((prev) => ({
				...prev,
				isCrawlingCompetitor: domain,
			}));
			addLog(`Starting micro-crawl for competitor: ${domain}...`, "info", {
				source: "competitive",
			} as any);
			...
			try {
				...
				addLog(`Competitor profile complete: ${domain}`, "success", {
					source: "competitive",
				} as any);
				...
				const enrichedProfile: CompetitorProfile = JSON.parse(
				...
				void (async () => {
					try {
						...
						const merged = CompetitorProfileBuilder.mergeEnrichment(
							profile,
							enrichedProfile,
						);
						...
						console.warn(
							"[SeoCrawlerContext] Browser enrichment failed:",
							enrichmentErr,
						);
					}
				})();
				...
				addLog(
					`Competitor crawl failed for ${domain}: ${err.message}`,
					"error",
					{ source: "competitive" } as any,
				);
				setCompetitiveState((prev) => ({
					...prev,
					isCrawlingCompetitor: null,
	...
	const removeCompetitor = useCallback(
		(domain: string) => {
			setCompetitiveState((prev) => {
				...
				const newActive = prev.activeCompetitorDomains.filter(
					(d) => d !== domain,
				);
				return {
					...prev,
			...
			crawlDb.competitorProfiles
				.where("_key")
				.startsWith(`${activeProject?.id}::${domain}`)
				.delete();
	...
	const recrawlCompetitor = useCallback(
		async (domain: string) => {
	...
	const generateCompetitiveBrief = useCallback(async () => {
		...
		if (!ownProfile || competitorProfiles.size === 0) {
			addLog(
				"Need your own crawl + at least 1 competitor to generate a brief.",
				"warn",
			);
			return;
		}
		...
		try {
			...
			addLog("Competitive brief generated successfully.", "success", {
				source: "competitive",
			} as any);
		} catch (err: any) {
	...
	useEffect(() => {
		...
		(async () => {
			...
			if (localProfiles.length > 0) {
				...
				// Fallback: try cloud
				const cloudProfiles = await loadCompetitorProfilesCloud(
					activeProject.id,
				);
	...
	const value = useMemo(
		() => ({
			...
			bulkAIAnalyzeCategory,
			tier4Results,
			runTier4Checks,
			showAddCompetitorInput,
			setShowAddCompetitorInput,
			crawlingCompetitorDomain,
			setCrawlingCompetitorDomain,
