...
export const BrandMentionsView = () => {
	...
	useEffect(() => {
		const loadMentions = async () => {
			...
			setLoading(true);
			const data = await seedBrandMentions(
				activeProject.id,
				activeProject.name,
			);
			...
			<div className="space-y-6 animate-in fade-in duration-500">
				...
				<div className="p-12 text-center bg-[#0F0F0F] rounded-3xl border border-white/5 border-dashed">
					...
					<p className="text-gray-500 max-w-md mx-auto">
						Once your brand gets mentioned across the web, they'll show up here.
						We check news sites, blogs, forums, and social media.
					</p>
				</div>
			</div>
		);
		...
		<div className="space-y-6 animate-in fade-in duration-500">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				{mentions.map((m: any) => (
					<div
						key={m.id}
						className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-5 flex flex-col hover:border-white/10 transition-all"
					>
