				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
					<div>
						...
						<p className="text-gray-300 max-w-xl mb-4">
							Your dedicated team is working on your growth. Track deliverables,
							usage, and strategy here.
						</p>
					...
					<div className="flex items-center gap-4">
						...
						<img
							...
							className="w-12 h-12 rounded-full border-2 border-brand-card bg-white"
							alt="AM"
						/>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
				<div className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-6">
					...
					<div className="w-full h-2 bg-[#222] rounded-full overflow-hidden mb-4">
						<div
							className="h-full bg-brand-red shadow-[0_0_10px_rgba(245,54,78,0.5)]"
							style={{ width: "62%" }}
						></div>
					</div>
					...
					<div className="space-y-4">
						<div className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.03] border border-white/5">
							...
							<div className="flex-1">
								<p className="text-sm font-bold text-white line-through text-opacity-50">
									Competitor Gap Analysis
								</p>
							</div>
							<span className="text-xs font-bold text-brand-green bg-brand-green/10 px-2 py-1 rounded">
								Completed
							</span>
						</div>
						<div className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.03] border border-white/5">
				...
				<div className="flex flex-col md:flex-row gap-6 relative">
					...
					{[1, 2, 3].map((step) => (
						<div key={step} className="flex-1 relative z-10">
							<div
								className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mb-4 border-4 border-brand-card ${step === 1 ? "bg-white text-black" : "bg-[#222] text-gray-500"}`}
							>
								{step}
							</div>
							...
							<p className="text-xs text-gray-500 leading-relaxed">
								{step === 1
									? "Fixing technical errors and establishing baseline content."
									: step === 2
										? "Aggressive content expansion and link velocity."
