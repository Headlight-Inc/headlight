...
export default function CompetitiveScoreWidget({ projectId }: Props) {
	...
	useEffect(() => {
		(async () => {
			...
			if (!ownProfile || compProfiles.length === 0) {
				setData(null);
				return;
			}
			...
			const avgCompScore = Math.round(
				compProfiles.reduce((s, p) => s + (p.overallSeoScore || 0), 0) /
					compProfiles.length,
			);
			...
			const allScores = [
				yourScore,
				...compProfiles.map((p) => p.overallSeoScore || 0),
			].sort((a, b) => b - a);
			...
			const topOpp =
				compProfiles.sort(
					(a, b) =>
						(b.opportunityAgainstThem || 0) - (a.opportunityAgainstThem || 0),
				)[0]?.domain || null;
		...
		return (
			<div className="rounded-2xl border border-white/5 bg-[#0F0F0F] p-5">
				...
				<p className="text-[12px] text-[#666]">
					Add competitors in the crawler to see your competitive score.
				</p>
			</div>
		);
	}
			...
			<div className="flex items-end gap-4 mb-4">
				...
				<div className="mb-1">
					<div className="text-[18px] font-bold text-[#888]">
						#{data.rank}
						<span className="text-[12px] text-[#555]">/{data.total}</span>
					</div>
				...
				{data.topThreat && (
					<div className="flex items-center gap-2 rounded-lg bg-red-500/5 border border-red-500/10 px-3 py-2">
						...
						<span className="text-[11px] text-red-300">
							Top threat:{" "}
							<span className="font-medium text-red-200">{data.topThreat}</span>
						</span>
					</div>
				)}
				{data.topOpportunity && (
