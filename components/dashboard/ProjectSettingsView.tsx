...
const GoogleIntegrationCard = ({ project }: { project: any }) => {
	...
	const handleConnect = async () => {
		if (!siteUrl.trim()) {
			alert(
				"Please enter your exact GSC Property URL (e.g., sc-domain:example.com or https://example.com/)",
			);
			return;
		}
		setStatus("loading");
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-[10px] text-gray-500">
							{status === "connected" ? "Connected via OAuth" : "Not Connected"}
						</p>
					</div>
				</div>
				{status === "loading" ? (
					...
					<button
						...
						className="px-3 py-1.5 rounded-lg text-[10px] font-bold border transition-colors text-brand-red border-brand-red/20 hover:bg-brand-red/10 disabled:opacity-50 disabled:cursor-not-allowed"
					>
						Connect
					</button>
				)}
...
const BillingSection = () => {
	...
	const openPortal = async () => {
		...
		try {
			...
			alert(
				err?.message ||
					"Could not open billing portal. Make sure Stripe is configured.",
			);
		}
		setIsLoading(false);
	};
	...
	return (
		<div className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-6">
			<h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
				<CreditCard size={18} className="text-brand-red" /> Billing &
				Subscription
			</h3>
			<div className="flex items-center justify-between p-4 bg-[#151515] rounded-xl border border-white/5">
				<div>
					...
					<span
						className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${statusColor}`}
					>
						{status === "active"
							? "Active"
							: status === "trialing"
	...
	if (!activeProject) {
		return (
			<div className="p-10 text-center text-gray-500">
				Please select or add a project first.
			</div>
		);
	}
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
					...
					<div className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							{["Google Analytics 4", "WordPress", "Slack"].map((tool, i) => (
								<div
									key={i}
									className="bg-[#151515] rounded-xl border border-white/5 p-4 flex items-center justify-between group hover:border-white/20 transition-all"
								>
									<div className="flex items-center gap-3">
										<div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center text-gray-400">
											<Plug size={18} />
				...
				<div className="lg:col-span-1 space-y-6">
					<div className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-6">
						...
						<div className="space-y-4">
							<div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
								...
								<span className="text-xs font-bold text-white bg-black px-2 py-1 rounded border border-white/10">
									Weekly
								</span>
							</div>
							<div className="flex justify-between items-center p-3 bg-white/5 rounded-lg">
							...
							<div className="flex items-center gap-3 mt-4">
								<input
									...
									checked
									readOnly
									className="rounded bg-black border-white/10 text-brand-red focus:ring-0"
								/>
					...
					<div className="bg-red-900/10 rounded-2xl border border-red-500/20 p-6">
						<h3 className="text-sm font-bold text-red-500 mb-2 uppercase tracking-wide flex items-center gap-2">
							<Trash2 size={14} /> Danger Zone
						</h3>
						<p className="text-xs text-gray-400 mb-4">
							Deleting a project cannot be undone. All ranking history will be
							lost.
						</p>
						<button
