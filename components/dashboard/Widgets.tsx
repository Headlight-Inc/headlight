...
export const KPICard: React.FC<any> = ({
	...
	<div className="bg-[#0F0F0F] p-5 rounded-2xl border border-white/5 hover:border-white/10 hover:bg-white/[0.04] transition-all group relative overflow-hidden h-[140px] flex flex-col justify-between shadow-lg">
		...
		<div className="flex justify-between items-start relative z-10">
			...
			<div
				className={`p-2 rounded-lg bg-white/5 text-gray-400 group-hover:text-white transition-colors group-hover:shadow-[0_0_15px_rgba(255,255,255,0.1)]`}
			>
				{icon}
			</div>
		</div>
		...
		<div className="relative h-10 w-full flex items-end justify-between mt-2">
			<div className="flex items-center gap-1.5 mb-1">
				<span
					className={`flex items-center text-[10px] font-bold px-1.5 py-0.5 rounded ${trendGood ? "text-brand-green bg-brand-green/10 border border-brand-green/20" : "text-red-400 bg-red-400/10 border border-red-400/20"}`}
				>
					{trendGood ? (
...
export const StrategicCard: React.FC<any> = ({
	title,
	value,
	sub,
	desc,
	...
	<div
		className={`p-6 rounded-3xl border backdrop-blur-md bg-[#0F0F0F] transition-all hover:bg-white/[0.04] group hover:-translate-y-1 duration-300 ${
			severity === "critical"
				? "border-red-500/20 hover:border-red-500/40 hover:shadow-[0_0_20px_rgba(239,68,68,0.1)]"
				: severity === "warning"
					? "border-orange-500/20 hover:border-orange-500/40 hover:shadow-[0_0_20px_rgba(249,115,22,0.1)]"
	...
	>
		<div className="flex justify-between items-start mb-4">
			...
			{severity === "critical" && (
				<span className="px-2 py-1 rounded text-[10px] font-bold bg-red-500 text-white uppercase tracking-wide shadow-glow-sm">
					Critical
				</span>
			)}
		</div>
...
export const DataCard = ({ title, value, status, icon }: any) => (
	<div className="bg-[#0F0F0F] rounded-2xl border border-white/5 p-5 flex flex-col justify-between hover:border-white/20 transition-all group">
		<div className="flex justify-between items-start mb-2">
			...
			<div
				className={`p-2 rounded-lg ${status === "good" ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
			>
				{icon}
			</div>
		</div>
...
export const TechCard: React.FC<any> = ({
	title,
	metric,
	status,
	desc,
	...
	<div className="bg-[#0F0F0F] rounded-3xl border border-white/5 p-6 relative overflow-hidden group hover:border-white/20 transition-all">
		<div
			className={`absolute top-0 right-0 w-32 h-32 opacity-10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 transition-opacity duration-500 ${color === "red" ? "bg-red-500 group-hover:opacity-20" : "bg-brand-green group-hover:opacity-20"}`}
		></div>
		<div className="flex items-center gap-3 mb-4 relative z-10">
			<div
				className={`p-2 rounded-lg ${color === "red" ? "bg-red-500/10 text-red-500" : "bg-brand-green/10 text-brand-green"}`}
			>
				{icon}
			</div>
			<div>
		...
		<div className="w-full h-1.5 bg-[#111] rounded-full overflow-hidden mt-4 relative z-10">
			<div
				className={`h-full rounded-full ${color === "red" ? "bg-red-500 w-[80%]" : "bg-brand-green w-[20%]"}`}
			></div>
		</div>
	</div>
);
...
export const TechnicalRow: React.FC<any> = ({ label, status, msg }) => (
	<div className="flex items-center justify-between group hover:bg-white/[0.02] p-2 rounded-lg -mx-2 transition-colors cursor-default">
		...
		<div className="flex items-center gap-3">
			...
			{status === "good" && (
				<div className="w-6 h-6 rounded-full bg-brand-green/10 flex items-center justify-center text-brand-green border border-brand-green/20 shadow-[0_0_8px_rgba(34,197,94,0.2)]">
					<Check size={12} strokeWidth={3} />
				</div>
			)}
			{status === "warning" && (
				<div className="w-6 h-6 rounded-full bg-orange-500/10 flex items-center justify-center text-orange-500 border border-orange-500/20 shadow-[0_0_8px_rgba(249,115,22,0.2)]">
					<AlertTriangle size={12} />
				</div>
			)}
			{status === "error" && (
				<div className="w-6 h-6 rounded-full bg-red-500/10 flex items-center justify-center text-red-500 border border-red-500/20 shadow-[0_0_8px_rgba(239,68,68,0.2)]">
					<X size={12} />
				</div>
			)}
		</div>
...
export const FilterButton = ({
	...
	const activeClass =
		color === "red"
			? "bg-red-50 text-white border-red-500 shadow-glow-sm"
			: color === "orange"
				? "bg-orange-50 text-white border-orange-500 shadow-glow-sm"
				: color === "blue"
					? "bg-blue-50 text-white border-blue-500 shadow-glow-sm"
					: color === "green"
						? "bg-green-50 text-white border-green-500 shadow-glow-sm"
						: "bg-white text-black border-white shadow-lg";

	const inactiveClass =
		"bg-white/[0.03] text-gray-400 border-white/10 hover:bg-white/10 hover:text-white";
	...
	return (
		...
		>
			{label}{" "}
			<span
				className={`px-1.5 py-0.5 rounded text-[10px] ${active ? "bg-black/20 text-white" : "bg-white/10 text-gray-500"}`}
			>
				{count}
			</span>
		</button>
...
export const LinkOpportunity: React.FC<any> = ({
	source,
	target,
	anchor,
	impact,
	...
	<div className="p-4 bg-white/[0.03] rounded-2xl border border-white/5 flex flex-col gap-4 hover:border-white/10 transition-colors group">
		...
		<div className="flex items-center gap-3 text-xs relative">
			...
			<div className="flex-1 p-3 bg-black/40 rounded-xl border border-white/5 text-gray-400 group-hover:border-white/10 transition-colors">
				<span className="text-gray-600 block text-[9px] uppercase font-bold mb-1">
					Source Page
				</span>
				<div className="truncate font-medium">{source}</div>
			</div>
			<div className="z-10 bg-[#111] p-1 rounded-full border border-white/10 text-gray-500">
				<ArrowRight size={12} />
			</div>
			<div className="flex-1 p-3 bg-black/40 rounded-xl border border-white/5 text-white group-hover:border-white/10 transition-colors">
				<span className="text-brand-green block text-[9px] uppercase font-bold mb-1">
					Target Page
				</span>
		...
		<div className="text-xs text-gray-500 px-1 flex items-center gap-2">
			Suggested Text:{" "}
			<span className="text-white font-mono bg-white/10 px-2 py-0.5 rounded border border-white/5">
				"{anchor}"
			</span>
		</div>
	</div>
