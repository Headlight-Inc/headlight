...
export const OnboardingWizard = ({
	...
	const finishOnboarding = async () => {
		...
		const targetId =
			createdProjectId || (projects.length > 0 ? projects[0].id : null);
		if (targetId) {
			...
			<div className="bg-[#0F0F0F] rounded-3xl border border-white/5 p-10 max-w-xl w-full relative overflow-hidden shadow-2xl">
				...
				<div className="text-center mb-10 mt-4">
					...
					<p className="text-sm text-gray-500 mt-2">
						{step === 1 &&
							"Let's set up your first website to track and optimize."}
						{step === 2 && "What searches do you want to rank for on Google?"}
						{step === 3 &&
							"Optional: Connect Google to sync real clicks and impressions."}
					</p>
				</div>
