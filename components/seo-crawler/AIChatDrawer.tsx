...
export default function AIChatDrawer({
	...
	const { pages, stats, healthScore, config, selectedPage, setSelectedPage } =
		useSeoCrawler();
	...
	const handleSubmit = async (e: React.FormEvent | string) => {
		...
		try {
			...
			const res = await fetch(`${apiBase}/api/ai/chat`, {
				...
				body: JSON.stringify({
					prompt: context,
					systemPrompt: "You are an SEO AI assistant. Be concise.",
				}),
			});
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
				{messages.length === 0 ? (
					<div className="flex flex-col items-center justify-center h-full text-center space-y-6">
						...
						<div>
							<p className="text-[#ccc] text-[13px] font-medium mb-1">
								How can I help with your SEO?
							</p>
							<p className="text-[#666] text-[11px]">
								Ask questions about your crawl data, request analyses, or find
								specific issues.
							</p>
						</div>
					...
					messages.map((msg, i) => (
						...
						>
							<div
								className={`max-w-[85%] rounded-lg p-3 text-[12px] leading-relaxed ${
									msg.role === "user"
