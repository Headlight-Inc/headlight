...
export default function AuditModeSelector({
	...
	const [selectedIndustry, setSelectedIndustry] =
		useState<IndustryFilter>(currentIndustry);
	...
	const activeCheckCount = useMemo(() => {
		return getActiveChecks({ modes: selectedModes, industry: selectedIndustry })
			.length;
		...
		<div className="fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
			<div className="w-full max-w-[680px] max-h-[85vh] bg-[#111] border border-[#2a2a2a] rounded-xl shadow-2xl overflow-hidden flex flex-col">
				<div className="px-5 py-4 border-b border-[#222] flex items-center justify-between">
					<div>
						<h3 className="text-[14px] text-white font-bold">
							Audit Mode & Industry
						</h3>
						<p className="text-[11px] text-[#777] mt-0.5">
							{activeCheckCount} checks active
						</p>
					</div>
					<button
						onClick={onClose}
				...
				<div className="flex-1 overflow-y-auto custom-scrollbar px-5 py-4 space-y-6">
					<section>
						<h4 className="text-[10px] text-[#666] uppercase tracking-widest font-bold mb-2">
							Select Audit Mode
						</h4>
						<p className="text-[10px] text-[#555] mb-3">
							Click a mode. Use shift-click to combine modes.
						</p>
								...
								return (
									...
									>
										...
										<div className="flex-1 min-w-0">
											<div
												className={`text-[12px] font-semibold ${selected ? "text-white" : "text-[#ddd]"}`}
											>
												{mode.label}
											</div>
					...
					<section>
						...
						<p className="text-[10px] text-[#555] mb-3">
							Adds vertical-specific checks on top of selected mode(s).
						</p>
							...
							{INDUSTRY_FILTERS.map((industry) => {
								...
								return (
									...
									>
										...
										<div className="flex-1 min-w-0">
											<div
												className={`text-[11px] font-semibold truncate ${selected ? "text-white" : "text-[#ddd]"}`}
											>
												{industry.label}
											</div>
					...
					{showSavePreset ? (
						<div className="bg-[#0f0f0f] border border-[#222] rounded-lg px-3 py-2.5 flex items-center gap-2">
							...
							<button
								onClick={handleSave}
								className="px-2.5 py-1 text-[11px] font-semibold bg-[#F5364E] text-white rounded hover:bg-[#df3248] transition-colors"
							>
								Save
							</button>
							<button
				...
				<div className="px-5 py-4 border-t border-[#222] bg-[#0d0d0d] flex items-center justify-between">
					...
					<div className="flex items-center gap-2.5">
						<button
							onClick={onClose}
							className="px-3 py-1.5 text-[12px] text-[#888] hover:text-white transition-colors"
						>
							Cancel
						</button>
						<button
							onClick={handleApply}
							className="px-4 py-1.5 text-[12px] font-bold bg-[#F5364E] text-white rounded hover:bg-[#df3248] transition-colors"
						>
							Apply Mode
						</button>
					</div>
