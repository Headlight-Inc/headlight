...
const METRICS: MetricConfig[] = [
	...
	},
	{
		...
		format: (v) =>
			v > 1e6 ? `${(v / 1e6).toFixed(1)}MB` : `${Math.round(v / 1024)}KB`,
	},
	{
		key: "score",
		label: "Score",
					...
					<thead>
						<tr>
							...
							{METRICS.map((m) => (
								<th
									key={m.key}
									className="text-center text-[#888] font-medium py-1 px-3 whitespace-nowrap"
								>
									{m.label}
								</th>
							))}
								...
								{METRICS.map((m) => {
									...
									return (
										<td
											...
											style={{
												background: getCellColor(val, m.thresholds, m.invert),
											}}
										>
											{showValues ? (
