...
import {
	useSeoCrawler,
	getHashRouteSearchParams,
} from "../../contexts/SeoCrawlerContext";
...
export default function CrawlerModals() {
	...
	const configuredCrawlerApiUrl = (import.meta as any).env
		?.VITE_CRAWLER_API_URL;
	const crawlerApiUrl =
		...
		<>
					...
					<div className="relative w-full max-w-lg bg-[#111] border border-[#333] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
						...
						<div className="p-5">
							<p className="text-[12px] text-gray-500 mb-3">
								Paste one URL per line. We will scan each one individually.
							</p>
							<textarea
								value={listUrls}
			...
			{showAutoFixModal && (
				<div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
					<div
						className="absolute inset-0 bg-black/80 backdrop-blur-sm"
						onClick={() => !isFixing && setShowAutoFixModal(false)}
					></div>
					<div className="relative w-full max-w-4xl max-h-[85vh] flex flex-col bg-[#111] border border-[#333] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
							...
							{autoFixItems?.length === 0 ? (
								...
								<div className="space-y-3">
									{autoFixItems?.map((item: any, idx: number) => (
										<div
											key={idx}
											className="bg-[#111] border border-[#222] rounded-lg p-4 transition-all"
										>
											<div className="flex items-start justify-between mb-2">
												<div className="flex-1 min-w-0 pr-4">
													<div className="text-[12px] text-blue-400 truncate mb-1">
														{item.url}
													</div>
													<div className="text-[14px] font-bold text-[#e0e0e0] truncate">
														{item.title || "Untitled Page"}
													</div>
													<div className="text-[11px] text-[#666] mt-1 flex items-center gap-2">
														<span className="bg-[#222] px-1.5 py-0.5 rounded">
															H1: {item.h1_1 || "None"}
														</span>
												...
												<div className="shrink-0 flex flex-col items-end">
													{item.fixStatus === "pending" && (
														<span className="px-2 py-1 bg-[#222] text-[#888] rounded text-[10px] font-bold uppercase tracking-wider">
															Queued
														</span>
													)}
													{item.fixStatus === "generating" && (
														<span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
															<div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />{" "}
															Generating
														</span>
													)}
													{item.fixStatus === "done" && (
														<span className="px-2 py-1 bg-green-500/20 text-green-400 rounded text-[10px] font-bold uppercase tracking-wider flex items-center gap-1">
											...
											{item.fixStatus !== "pending" && (
												<div className="mt-3 pt-3 border-t border-[#222]">
													<label className="text-[10px] text-[#555] uppercase tracking-widest font-bold mb-1 block">
													...
													{item.fixStatus === "generating" ? (
														...
														<div className="relative">
															...
															/>
															<div
																className={`absolute bottom-2 right-2 text-[10px] font-mono ${(item.generatedMeta?.length || 0) > 155 ? "text-red-400" : "text-[#666]"}`}
															>
						...
						<div className="px-6 py-4 border-t border-[#222] bg-[#141414] flex justify-between items-center">
								...
								{isFixing && (
									<div>
										...
										<div className="w-full h-1.5 bg-[#222] rounded-full overflow-hidden">
											<div
												className="h-full bg-[#F5364E] transition-all duration-300"
												style={{ width: `${autoFixProgress}%` }}
											></div>
										</div>
									</div>
								)}
							...
							<div className="flex gap-3 shrink-0">
								...
								>
									{autoFixItems?.some((i: any) => i.fixStatus === "done")
										? "Close"
										: "Cancel"}
								</button>

								{autoFixItems?.length > 0 &&
									!autoFixItems.every((i: any) => i.fixStatus === "done") && (
										<button
											onClick={async () => {
												setIsFixing(true);
												...
												const updatedPages = pages.map((p) => {
													const fixedItem = autoFixItems.find(
														(item: any) => item.url === p.url,
													);
													if (fixedItem && fixedItem.fixStatus === "done") {
														return {
															...p,
															metaDesc: fixedItem.generatedMeta || p.metaDesc,
															metaDescLength: (fixedItem.generatedMeta || "")
																.length,
															fixStatus: "applied",
														};
													}
												...
												crawlDb.pages.bulkPut(updatedPages).catch((err) => {
													console.error(
														"[CrawlDB] Failed to save auto-fix updates:",
														err,
													);
												});
											}}
