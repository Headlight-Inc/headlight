		...
		if (navigator.storage && navigator.storage.estimate) {
			navigator.storage.estimate().then((estimate) => {
				setStorageStats({
					...
					limit: Math.round(
						(estimate.quota || 500 * 1024 * 1024) / (1024 * 1024),
					),
				});
			});
		}
	...
	return (
		<div className="space-y-6">
			<SettingsSection
				...
				description="Current usage of IndexedDB and LocalStorage."
			>
				<div className="p-4 bg-[#111] border border-[#222] rounded-lg space-y-3">
					<div className="flex justify-between items-center text-[12px]">
						<span className="text-[#888] flex items-center gap-2">
							<HardDrive size={14} /> Capacity
						</span>
						<span className="text-white font-bold">
							{storageStats.used}MB / {storageStats.limit}MB
						</span>
					</div>
					...
					<p className="text-[10px] text-[#666]">
						Data is stored locally in your browser to ensure privacy and speed.
					</p>
				</div>
			</SettingsSection>
					...
					<div className="flex gap-3">
						...
						<div>
							...
							<p className="text-[10px] text-[#888]">
								This will permanently delete all locally stored crawl history
								and configurations.
							</p>
						</div>
					</div>
