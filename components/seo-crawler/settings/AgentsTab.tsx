	...
	if (isLoading) {
		return (
			<div className="flex flex-col items-center justify-center py-20 gap-4">
				...
				<p className="text-[#888] text-[12px] font-medium">
					Summoning AI Agents...
				</p>
			</div>
		);
			...
			{!selectedAgent ? (
                                    ...
                                    ${agent.trigger === "cron" ? "bg-[#36A0F5]/10 text-[#36A0F5]" : "bg-[#F536E0]/10 text-[#F536E0]"}`}
										...
										<p className="text-[#666] text-[12px] line-clamp-2 mb-3">
											{agent.id === "issue-fixer" &&
												"Analyzes crawl results and generates technical fixes."}
											{agent.id === "content-monitor" &&
												"Monitors traffic and flags content decay or stale pages."}
											{agent.id === "rank-guard" &&
												"Tracks ranking drops and correlates them with site changes."}
											{agent.id === "link-watcher" &&
												"Monitors internal authority and link opportunities."}
											{agent.id === "traffic-analyst" &&
												"Analyzes GA4 anomalies and site-wide traffic health."}
											{agent.id === "competitor-scout" &&
												"Scouts competitor strategies and content moves."}
											{agent.id === "outreach-bot" &&
												"Drafts personalized outreach for high-potential pages."}
										</p>
				...
				<div className="space-y-6">
						...
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-[12px] text-[#666]">
									Viewing latest 50 autonomous runs
								</p>
							</div>
						</div>
					</div>
					...
					<div className="space-y-2">
						{isRunsLoading ? (
							<div className="py-12 flex justify-center text-[#444]">
								Loading history...
							</div>
						) : runs.length === 0 ? (
							<div className="py-20 border-2 border-dashed border-[#111] rounded-2xl flex flex-col items-center justify-center text-center">
								...
								<button
									onClick={() => runNow(selectedAgent.id)}
									className="mt-4 text-[#F5364E] text-[12px] font-bold"
								>
							...
							runs.map((run) => (
								<div
									key={run.id}
									className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-xl p-4 flex items-center justify-between gap-4"
								>
									<div className="flex items-center gap-4 flex-1">
										<div
											className={`w-2 h-2 rounded-full ${run.status === "success" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]"}`}
										/>
										<div className="flex-1">
											...
											<div className="flex items-center gap-4 text-[10px] text-[#555]">
												...
												<span>
													{Math.round(run.durationMs / 1000)}s duration
												</span>
											</div>
										</div>
									</div>
									<div className="flex items-center gap-6 text-[11px]">
										<div className="text-center">
											...
											<p className="text-[#444] uppercase tracking-tighter">
												Findings
											</p>
										</div>
										<div className="text-center">
