...
export default function SettingsPanel() {
	...
	const renderTab = () => {
		switch (settingsTab) {
			case "general":
				return <GeneralTab config={config} setConfig={setConfig} />;
			case "performance":
				return <PerformanceTab config={config} setConfig={setConfig} />;
			case "integrations":
				return <IntegrationsTab />;
			case "ai":
				return <AITab config={config} setConfig={setConfig} />;
			case "rules":
				return <RulesTab config={config} setConfig={setConfig} />;
			case "extraction":
				return <ExtractionTab config={config} setConfig={setConfig} />;
			case "scheduling":
				return <SchedulingTab config={config} setConfig={setConfig} />;
			case "storage":
				return <StorageTab config={config} setConfig={setConfig} />;
			case "api":
				...
				<div className="w-[220px] bg-[#111] border-r border-[#222] flex flex-col">
                  ...
                  ${
							...
							>
								<span
									className={
										settingsTab === tab.id ? "text-[#F5364E]" : "text-[#666]"
									}
								>
									{tab.icon}
								</span>
