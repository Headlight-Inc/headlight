...
export default function RulesTab({ config, setConfig }: TabProps) {
	...
	const addExtractionRule = () => {
		...
		updateConfig("customExtractionRules", [
			...(config.customExtractionRules || []),
			newRule,
		]);
	};
	...
	const removeExtractionRule = (id: string) => {
		updateConfig(
			...
			(config.customExtractionRules || []).filter((r) => r.id !== id),
		);
	};

	const updateExtractionRule = (
		id: string,
		updates: Partial<CustomExtractionRule>,
	) => {
		updateConfig(
			"customExtractionRules",
			(config.customExtractionRules || []).map((r) =>
				r.id === id ? { ...r, ...updates } : r,
			),
		);
	};
	...
	return (
		<div className="space-y-6">
			<SettingsSection
				title="Crawl Boundaries"
				description="Define which URLs are allowed and which are forbidden."
			>
				<SettingsTextarea
			...
			<SettingsSection
				...
				description="Credentials for password-protected areas."
			>
				<SettingsSelect
					label="Auth Type"
				...
				{config.authType === "basic" && (
					<div className="grid grid-cols-2 gap-4">
						<SettingsInput
							...
							onChange={(val) => updateConfig("authUser", val)}
						/>
						<SettingsInput
							label="Password"
							value={config.authPass}
							onChange={(val) => updateConfig("authPass", val)}
							type="password"
						/>
					</div>
				)}
				{config.authType === "bearer" && (
					<SettingsInput
						label="Token"
						value={config.authBearerToken}
						onChange={(val) => updateConfig("authBearerToken", val)}
					/>
				)}
				{config.authType === "cookie" && (
					<SettingsTextarea
						label="Raw Cookie String"
						value={config.customCookies}
						onChange={(val) => updateConfig("customCookies", val)}
						rows={2}
					/>
				)}
			</SettingsSection>
			...
			<SettingsSection
				...
				description="Define custom pass/fail conditions for pages."
			>
				<div className="space-y-3">
					{customExtractionRules.map((rule) => (
						<div
							key={rule.id}
							className="p-4 bg-[#111] border border-[#222] rounded-lg space-y-3 relative group"
						>
							<button
							...
							<div className="grid grid-cols-2 gap-4">
								<SettingsInput
									...
									value={rule.name}
									onChange={(val) =>
										updateExtractionRule(rule.id, { name: val })
									}
								/>
								<SettingsInput
									label="Pages (Glob)"
									value={rule.pages}
									onChange={(val) =>
										updateExtractionRule(rule.id, { pages: val })
									}
								/>
							</div>
							<div className="grid grid-cols-3 gap-2">
								<div className="col-span-2">
									<SettingsInput
										...
										value={rule.selector}
										onChange={(val) =>
											updateExtractionRule(rule.id, { selector: val })
										}
									/>
								</div>
								<SettingsSelect
									...
									onChange={(val) =>
										updateExtractionRule(rule.id, { condition: val })
									}
									options={[
