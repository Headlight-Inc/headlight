...
export default function ExtractionTab({ config, setConfig }: TabProps) {
	...
	const addExtractor = () => {
		...
		updateConfig("customFieldExtractors", [
			...(config.customFieldExtractors || []),
			newExtractor,
		]);
	};
	...
	const removeExtractor = (id: string) => {
		updateConfig(
			...
			(config.customFieldExtractors || []).filter((r) => r.id !== id),
		);
	};

	const updateExtractor = (
		id: string,
		updates: Partial<CustomFieldExtractor>,
	) => {
		updateConfig(
			"customFieldExtractors",
			(config.customFieldExtractors || []).map((r) =>
				r.id === id ? { ...r, ...updates } : r,
			),
		);
	};
	...
	return (
		<div className="space-y-6">
			<SettingsSection
				...
				description="How the crawler processes pages."
			>
				<SettingsToggle
			...
			<SettingsSection
				...
				description="Simple global extraction rules."
			>
				<SettingsInput
			...
			<SettingsSection
				title="Custom Field Extractors"
				description="Map CSS selectors to custom named columns in your data."
			>
				<div className="space-y-3">
					{customFieldExtractors.map((extractor) => (
						<div
							key={extractor.id}
							className="p-4 bg-[#111] border border-[#222] rounded-lg space-y-3 relative"
						>
							<button
							...
							<div className="grid grid-cols-2 gap-4">
								<SettingsInput
									...
									onChange={(val) =>
										updateExtractor(extractor.id, { name: val })
									}
								/>
								<SettingsSelect
									label="Extract Type"
									...
									onChange={(val) =>
										updateExtractor(extractor.id, { extractType: val })
									}
									options={[
							...
							<div className="grid grid-cols-2 gap-4">
								<SettingsInput
									label="CSS Selector"
									value={extractor.cssSelector}
									onChange={(val) =>
										updateExtractor(extractor.id, { cssSelector: val })
									}
								/>
								{extractor.extractType === "attribute" && (
									<SettingsInput
										...
										onChange={(val) =>
											updateExtractor(extractor.id, { attributeName: val })
										}
										placeholder="href, data-price, etc"
									/>
								)}
								<SettingsInput
									...
									onChange={(val) =>
										updateExtractor(extractor.id, { regex: val })
									}
									placeholder="Filter result with regex..."
								/>
							</div>
						</div>
					))}
