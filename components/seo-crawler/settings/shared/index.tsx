	...
	return (
		<div className="mb-8 last:mb-0">
			<div className="mb-4">
				...
				{description && (
					<p className="text-[11px] text-[#666] mt-1">{description}</p>
				)}
			</div>
...
export function SettingsToggle({
	...
	return (
		<label
			className="flex items-center justify-between p-3 bg-[#111] border border-[#222] rounded-lg cursor-pointer hover:border-[#333] transition-colors"
		...
		>
			...
			<div
				className={`w-8 h-4 rounded-full transition-colors relative shrink-0 ${checked ? "bg-[#F5364E]" : "bg-[#333]"}`}
			>
				<div
					className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${checked ? "right-0.5" : "left-0.5"}`}
				/>
			</div>
		</label>
	);
	...
	return (
		<div className="space-y-1.5">
			<div className="flex justify-between">
				...
				<span className="text-[11px] font-bold text-[#F5364E]">
					{value}
					{unit}
				</span>
			</div>
