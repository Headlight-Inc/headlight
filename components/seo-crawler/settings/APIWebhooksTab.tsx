...
export default function APIWebhooksTab() {
	...
	useEffect(() => {
		loadData().catch((error) => {
			console.error(
				"[APIWebhooksTab] Failed to load project API resources:",
				error,
			);
		});
	}, [projectId]);
	...
	return (
		<div className="space-y-6">
			<SettingsSection
				...
				description={
					projectId
						? `API access for project ${activeProject?.name || projectId}.`
			...
			>
				<div className="grid grid-cols-2 gap-4">
					...
					<SettingsSelect
						...
						options={API_SCOPE_OPTIONS.map((scope) => ({
							label: scope,
							value: scope,
						}))}
					/>
				...
				<button
					onClick={() =>
						createApiKey().catch((error) => window.alert(error.message))
					}
					disabled={!projectId}
				...
				{generatedToken && (
					<div className="p-3 bg-[#111] border border-[#222] rounded-lg space-y-2">
						<div className="text-[11px] font-bold text-white flex items-center gap-2">
							<KeyRound size={14} className="text-[#F5364E]" /> Copy this key
							now
						</div>
						<div className="text-[11px] font-mono break-all text-emerald-300">
							{generatedToken}
						</div>
						<button
				...
				<div className="space-y-2">
					{apiKeys.map((key) => (
						...
						>
							...
							<button
								onClick={() =>
									ApiKeyService.revoke(projectId, key.id)
										.then(loadData)
			...
			<SettingsSection
				title="Webhooks"
				description="Receive crawl and issue events on your own endpoint."
			>
				<div className="grid grid-cols-2 gap-4">
					<SettingsInput
						label="Webhook Name"
					...
					<SettingsSelect
						...
						options={WEBHOOK_EVENT_OPTIONS.map((eventName) => ({
							label: eventName,
							value: eventName,
						}))}
					/>
				...
				<button
					onClick={() =>
						createWebhook().catch((error) => window.alert(error.message))
					}
				...
				<div className="space-y-2">
					{webhooks.map((hook) => (
						<div
							key={hook.id}
							className="p-3 bg-[#111] border border-[#222] rounded-lg space-y-2"
						>
							<div className="flex items-center justify-between">
								<div className="text-[12px] font-medium text-white flex items-center gap-2">
									<WebhookIcon size={13} className="text-blue-400" />{" "}
									{hook.name}
								</div>
								<button
									onClick={() =>
										WebhookService.remove(projectId, hook.id)
											.then(loadData)
											.catch((error) => window.alert(error.message))
									}
									className="p-2 rounded border border-red-500/20 text-red-400 hover:bg-red-500/10"
							...
							<div className="text-[10px] text-[#777] break-all">
								{hook.url}
							</div>
							<div className="text-[10px] text-[#777]">
