...
export default function AITab({ config, setConfig }: TabProps) {
	...
	const aiCustomKeys = config.aiCustomKeys || {
		openai: "",
		anthropic: "",
		gemini: "",
		cohere: "",
	...
	const tasks = [
		{
			...
			desc: "Generate concise executive summaries for every page.",
		},
		{
			id: "keywords",
			label: "Keyword Extraction",
			icon: <Search size={14} />,
			desc: "Identify primary and secondary semantic keywords.",
		},
		{
			id: "intent",
			label: "Search Intent Analysis",
			icon: <Zap size={14} />,
			desc: "Classify intent as Informational, Transactional, etc.",
		},
		{
			id: "quality",
			label: "Content Quality Audit",
			icon: <BarChart3 size={14} />,
			desc: "Score content for readability, depth, and relevance.",
		},
		{
			id: "eeat",
			label: "E-E-A-T Assessment",
			icon: <ShieldCheck size={14} />,
			desc: "Evaluate Experience, Expertise, Authoritativeness, and Trust.",
		},
		{
			id: "fixSuggestions",
			label: "Fix Suggestions",
			icon: <Wand2 size={14} />,
			desc: "Generate actionable steps to fix detected SEO issues.",
		},
		{
			id: "metaRewrite",
			label: "Meta Tag Optimization",
			icon: <Layout size={14} />,
			desc: "Automatically draft optimized Title and Meta descriptions.",
		},
		{
			id: "schemaGenerate",
			label: "Schema.org Generation",
			icon: <Code size={14} />,
			desc: "Generate JSON-LD structured data for the page content.",
		},
		{
			id: "altTextGenerate",
			label: "Image Alt Generation",
			icon: <Globe size={14} />,
			desc: "Create descriptive alt text for images missing it.",
		},
	];
				...
				<div className="flex items-center gap-3 mb-2">
					...
					<div>
						...
						<p className="text-[11px] text-[#aaa]">
							Powered by Gemini 1.5 & Cloudflare Workers AI
						</p>
					</div>
			...
			{config.aiEnabled && (
				<>
					<SettingsSection
						...
						description="Select which AI models to run during the crawl."
					>
                    ...
                    ${aiTasks[task.id as keyof AITaskToggles] ? "bg-[#111] border-[#F5364E]/30 shadow-[0_0_15px_rgba(245,54,78,0.05)]" : "bg-[#0a0a0a] border-[#222] grayscale opacity-60 hover:grayscale-0 hover:opacity-100"}`}
									onClick={() =>
										updateAiTask(
											task.id as keyof AITaskToggles,
											!aiTasks[task.id as keyof AITaskToggles],
										)
									}
								>
									<div
					...
					<SettingsSection
						title="Custom Provider Keys (Optional)"
						description="Use your own API keys for higher rate limits."
					>
