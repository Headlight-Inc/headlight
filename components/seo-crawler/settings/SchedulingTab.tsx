...
export default function SchedulingTab({ config, setConfig }: TabProps) {
	...
	return (
		<div className="space-y-6">
			<SettingsSection
				title="Auto-Recrawl"
				description="Schedule this project to scan automatically."
			>
				<SettingsToggle
			...
			<SettingsSection
				...
				description="Compare current results with the previous scan."
			>
				<SettingsToggle
			...
			>
				<SettingsToggle
					...
					onChange={(val) => updateConfig("alertOnScoreDrop", val)}
				/>
				<SettingsToggle
					label="On New 404 Errors"
					checked={config.alertOnNew404s}
					onChange={(val) => updateConfig("alertOnNew404s", val)}
				/>
				<SettingsToggle
					label="On Critical New Issues"
					checked={config.alertOnNewIssues}
					onChange={(val) => updateConfig("alertOnNewIssues", val)}
				/>
			</SettingsSection>
			...
			<SettingsSection title="Notification Channels">
				<div className="grid grid-cols-2 gap-3">
					<SettingsToggle
						...
						onChange={(val) => updateAlertChannel("email", val)}
					/>
					<SettingsToggle
						label="In-App Notification"
						checked={alertChannels.inApp}
						onChange={(val) => updateAlertChannel("inApp", val)}
					/>
					<SettingsToggle
						label="Slack"
						checked={alertChannels.slack}
						onChange={(val) => updateAlertChannel("slack", val)}
					/>
					<SettingsToggle
						label="Webhook"
						checked={alertChannels.webhook}
						onChange={(val) => updateAlertChannel("webhook", val)}
					/>
				</div>
				{alertChannels.slack && (
					<div className="mt-3">
