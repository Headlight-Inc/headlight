...
export default function ExportDialog({ onClose }: ExportDialogProps) {
	...
	const counts = {
		...
		issues: filteredPages.filter((page) => getPageIssues(page).length > 0)
			.length,
	...
	const exportOptions = {
		...
		selectedUrls,
		visibleColumns: columnOptions.includeVisibleColumns
			? visibleColumns
			: undefined,
		columnOptions,
		pdfOptions,
	...
	const handleExportClick = async () => {
		try {
			...
			if (format === "csv") {
				downloadBlob(
					exportCSV(
						scope === "filtered" ? filteredPages : pages,
						scope,
						exportOptions,
					),
					`headlight_export_${Date.now()}.csv`,
				);
			} else if (format === "json") {
				downloadBlob(
					exportJSON(
					...
					`headlight_export_${Date.now()}.json`,
				);
				...
				const sections = [
					...
					`- Competitor avg blog velocity: ${
						comps.length > 0
							? Math.round(
									comps.reduce(
										(sum, c) => sum + Number(c.blogPostsPerMonth || 0),
										0,
									) / comps.length,
								)
							: 0
				...
				downloadBlob(
					blob,
					`competitive-report-${new Date().toISOString().split("T")[0]}.txt`,
				);
			}

			addLog(`Export complete (${formatLabels[format]}).`, "success", {
				source: "system",
			});
		} catch (error) {
			addLog((error as Error).message || "Export failed.", "error", {
				source: "system",
			});
		} finally {
		...
		<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-4">
			...
			<div className="relative w-full max-w-[520px] rounded-3xl border border-[#232329] bg-[#111] shadow-[0_30px_90px_rgba(0,0,0,0.55)]">
				<div className="flex items-center justify-between border-b border-[#202025] px-5 py-4">
					<div>
						<div className="text-[10px] font-bold uppercase tracking-[0.26em] text-[#666]">
							Export
						</div>
					...
					<button
						onClick={onClose}
						className="rounded-lg border border-[#2e2e34] p-2 text-[#888] hover:text-white"
					>
						<X size={16} />
					</button>
				</div>
				...
				<div className="max-h-[80vh] space-y-5 overflow-y-auto px-5 py-5">
					<section>
						<div className="mb-2 text-[10px] font-bold uppercase tracking-[0.24em] text-[#666]">
							Format
						</div>
						<div className="space-y-2">
							{(Object.keys(formatLabels) as ExportFormat[]).map((option) => (
								<label
									key={option}
									className="flex cursor-pointer items-center justify-between rounded-2xl border border-[#242428] bg-[#101013] px-4 py-3"
								>
									<div>
										...
										<div className="text-[11px] text-[#777]">
											...
											{option === "json" &&
												"Structured dataset for development workflows"}
											{option === "pdf" &&
												"Visual report with summary and recommendations"}
											{option === "google-sheets" &&
												"Publish directly into a live spreadsheet"}
											{option === "excel" && "Workbook export for analysts"}
											{option === "competitive-report" &&
												"Competitive analysis summary report"}
										</div>
									</div>
									<input
										type="radio"
						...
						<div className="space-y-2">
							{[
								...
								>
									<div className="text-[12px] font-semibold text-white">
										{option.label}{" "}
										<span className="text-[#666]">({option.count})</span>
									</div>
									<input
										type="radio"
										name="scope"
										checked={scope === option.id}
										onChange={() => setScope(option.id as ExportScope)}
										className="h-4 w-4 accent-[#F5364E]"
									/>
								</label>
							))}
					...
					<section>
						...
						<div className="space-y-2 rounded-2xl border border-[#242428] bg-[#101013] p-4">
							{[
								{
									key: "includeVisibleColumns",
									label: "Include all visible columns",
								},
								{ key: "includeAiAnalysis", label: "Include AI analysis" },
								{
									key: "includeIntegrations",
									label: "Include GSC / GA4 / authority data",
								},
								...
								>
									...
									<input
										type="checkbox"
										checked={Boolean(
											columnOptions[option.key as keyof typeof columnOptions],
										)}
										onChange={(event) =>
						...
						<section>
							...
							<div className="space-y-2 rounded-2xl border border-[#242428] bg-[#101013] p-4">
								{[
									...
									{
										key: "issueBreakdownCharts",
										label: "Issue breakdown charts",
									},
									{ key: "pageLevelDetails", label: "Page-level details" },
									{ key: "recommendations", label: "Recommendations" },
									{
										key: "comparisonWithPreviousCrawl",
										label: "Comparison with previous crawl",
									},
									...
									>
										...
										<input
											type="checkbox"
											checked={Boolean(
												pdfOptions[option.key as keyof typeof pdfOptions],
											)}
											onChange={(event) =>
								...
								<label className="mt-3 flex items-center justify-between rounded-xl border border-[#232329] bg-[#0d0d10] px-3 py-2 text-[12px] text-white">
									...
									<input
										...
										className="max-w-[180px] text-[11px] text-[#888]"
									/>
								</label>
							</div>
						</section>
				...
				<div className="flex items-center justify-end gap-3 border-t border-[#202025] px-5 py-4">
					<button
						onClick={onClose}
						className="rounded-xl border border-[#2e2e34] px-4 py-3 text-[12px] font-semibold text-[#ccc] hover:border-[#444] hover:text-white"
					>
						Cancel
					</button>
					<button
