...
import MigrationPlannerService, {
	type MigrationMapping,
} from "../../services/MigrationPlannerService";
...
export default function AuditSidebar({ embedded = false }: AuditSidebarProps) {
	...
	React.useEffect(() => {
		if (
			activeSidebarSections &&
			activeSidebarSections.length > 0 &&
			!activeSidebarSections.includes(activeAuditTab)
		) {
	...
	const [monitorChanges, setMonitorChanges] = useState<
		Array<{ url: string; changes: string[] }>
	>([]);
	...
	const totalIssueCount = useMemo(() => {
		return issueGroups.reduce((sum, group) => {
			return (
				sum +
				group.issues.reduce((groupSum, issue) => groupSum + issue.count, 0)
			);
		}, 0);
	}, [issueGroups]);
	...
	const allTabs = useMemo(
		() => [
			...
			{ id: "wqa_quality", label: "Quality" },
			{
				id: "wqa_actions",
				label: "Actions",
				count: strategicOpportunities.length,
			},
		...
		[
			...
			tasks?.length,
			monitorChanges?.length,
			migrationMappings?.length,
			crawlHistory?.length,
			logs?.length,
		],
	);
	...
	const handleLogAnalysisUpload = async (
		...
		try {
			...
			addLog(
				`Imported log file ${file.name} and applied bot crawl metrics to ${result.pages.length} pages.`,
				"success",
				{ source: "analysis" },
			);
		} catch (error: any) {
			addLog(`Log analysis failed: ${error.message}`, "error", {
				source: "analysis",
			});
		} finally {
			event.target.value = "";
	...
	const createMonitorSnapshot = () => {
		...
		setMonitorChanges([]);
		addLog("Saved current crawl snapshot for change monitoring.", "success", {
			source: "analysis",
		});
	};
	...
	const runMonitorDiff = () => {
		const projectId = pages[0]?.projectId || pages[0]?.sessionId || "default";
		const changes = ChangeMonitorService.detectChanges(projectId, pages).map(
			(item) => ({
				url: item.url,
		...
		addLog(
			`Detected ${changes.length} monitored page changes.`,
			changes.length ? "warn" : "success",
			{ source: "analysis" },
		);
	};
	...
	const generateMigrationPlan = async () => {
		...
		const unmapped = mappings
			.filter((m) => m.matchType === "unmapped")
			.map((m) => m.sourceUrl);
		if (unmapped.length > 0) {
			const aiResults = await MigrationPlannerService.generateAIMappings(
				unmapped,
				targets,
			);
		...
		return (
			...
			>
				<div className="flex flex-col items-center gap-2">
					...
					{stats?.broken > 0 && (
						<div
							className="w-1.5 h-1.5 rounded-full bg-[#F5364E]"
							title={`${stats.broken} Broken Pages`}
						/>
					)}
					{isCrawling && (
						<div
					...
					{visibleTabs.map((tab) => (
						...
						>
							...
							{tab.count !== undefined && tab.count > 0 && (
								<span
									className={`px-1.5 py-0.5 rounded-full text-[9px] font-mono ${tab.id === "issues" ? "bg-red-500/20 text-red-400" : "bg-[#222] text-[#888]"}`}
								>
									{tab.count}
								</span>
							)}
					...
					<>
						...
						{activeAuditTab === "issues" && (
							<div className="space-y-6 animate-in fade-in duration-200">
								...
								{pages.length === 0 ? (
									<div className="text-[11px] text-[#666] text-center py-8">
										Scan a website to view issues.
									</div>
								) : (
									<div className="space-y-4">
										...
										{issueGroups.length === 0 && (
											<div className="text-[11px] text-green-400 flex items-center gap-2 justify-center py-4">
												<CheckCircle2 size={16} /> No issues found across all
												categories!
											</div>
										)}
									</div>
						...
						{(activeAuditTab === "opportunities" ||
							activeAuditTab === "wqa_actions") && (
							<div className="space-y-4 animate-in fade-in duration-200">
								{strategicOpportunities.length === 0 ? (
									<div className="text-[11px] text-[#666] text-center py-8">
										Run a crawl with enriched data to surface prioritized
										opportunities.
									</div>
								) : (
									strategicOpportunities.map((opportunity) => (
										...
										>
											<div className="flex items-start justify-between gap-3">
												...
												<div className="text-[10px] font-mono text-[#F5364E] shrink-0">
													{opportunity.opportunityScore}
												</div>
											</div>
											<div className="text-[10px] text-[#666] mt-2 leading-relaxed">
							...
							<div className="space-y-6 animate-in fade-in duration-200">
								<div className="bg-gradient-to-br from-amber-900/20 to-orange-900/10 border border-orange-500/30 rounded-lg p-4 relative overflow-hidden">
									...
									<p className="text-[12px] text-gray-300 leading-relaxed">
										Generative Engine Optimization (GEO) focuses on making your
										content discoverable and citeable by AI models like ChatGPT,
								...
								<div className="space-y-3">
									<h4 className="text-[11px] font-bold text-[#555] mb-3 uppercase tracking-widest border-b border-[#222] pb-1">
										AI Crawler Access
									</h4>
										...
										{(() => {
											...
											return bots.map((bot) => {
												...
												return (
													<div
														key={bot.key}
														className="flex items-center justify-between p-2 bg-[#141414] border border-[#222] rounded text-[10px]"
													>
														<span className="text-gray-400 font-medium truncate pr-1">
															{bot.label}
														</span>
								...
								<div className="space-y-4">
									<h4 className="text-[11px] font-bold text-[#555] uppercase tracking-widest border-b border-[#222] pb-1">
										Site-wide GEO Signals
									</h4>
								...
								{/* llms.txt Status */}
								<div
									className={`p-3 rounded-lg border flex items-center justify-between ${robotsTxt?.hasLlmsTxt ? "bg-green-500/5 border-green-500/20" : "bg-[#1a1a1a] border-[#222]"}`}
								>
									<div className="flex items-center gap-3">
										<div
											className={`w-8 h-8 rounded-full flex items-center justify-center ${robotsTxt?.hasLlmsTxt ? "bg-green-500/20 text-green-400" : "bg-[#222] text-[#444]"}`}
										>
											<FileText size={16} />
										</div>
										<div>
											...
											<div className="text-[10px] text-[#666]">
												{robotsTxt?.hasLlmsTxt
													? "Present and discovered"
													: "Not found at root"}
											</div>
										</div>
									</div>
									{robotsTxt?.hasLlmsTxt ? (
										<span className="text-[9px] bg-green-500 text-black font-bold px-1.5 py-0.5 rounded uppercase">
											Optimized
										</span>
									) : (
										<button className="text-[9px] text-orange-400 hover:underline">
											How to add?
										</button>
									)}
								</div>
							...
							<div className="space-y-6 animate-in fade-in duration-200">
								...
								<div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/10 border border-indigo-500/30 rounded-lg p-4 relative overflow-hidden">
									...
									<h4 className="text-[11px] font-bold text-indigo-400 mb-3 uppercase tracking-widest flex items-center gap-2">
										<Sparkles size={14} fill="currentColor" /> Executive AI
										Narrative
									</h4>
									{aiNarrative ? (
										...
										<div className="text-[11px] text-[#666] text-center py-4 border border-white/5 bg-black/20 rounded">
											{isAnalyzingAI
												...
												: "Run AI Analysis to generate site narrative."}
										</div>
									)}
								</div>
								...
								<div className="space-y-3">
									...
									{(() => {
										...
										pages.forEach((p) => {
											if (p.topicCluster)
												clusters[p.topicCluster] =
													(clusters[p.topicCluster] || 0) + 1;
										});
										...
										if (sortedClusters.length === 0)
											return (
												<div className="text-[11px] text-[#444] text-center py-4 italic">
										...
										return (
											<div className="grid grid-cols-1 gap-2">
												{sortedClusters.slice(0, 6).map(([cluster, count]) => (
													<div
														key={cluster}
														className="flex items-center justify-between p-2 bg-[#141414] border border-[#222] rounded text-[11px]"
													>
														<span className="text-gray-300 font-medium truncate pr-2">
															{cluster}
														</span>
														<span className="text-gray-500 font-mono text-[10px] bg-black px-1.5 py-0.5 rounded">
															{count} pages
														</span>
													</div>
												))}
								...
								<div className="space-y-3">
									...
									{(() => {
										...
										const counts = stages.map(
											(s) => pages.filter((p) => p.funnelStage === s).length,
										);
								...
								<div className="bg-[#1a1a1a] border border-[#222] rounded-lg p-4">
									<h4 className="text-[11px] font-bold text-emerald-400 mb-3 uppercase tracking-widest flex items-center justify-between">
										Batch AI Actions
										<span className="text-[9px] text-[#555] normal-case tracking-normal font-normal">
											Coming Soon
										</span>
									</h4>
									<div className="space-y-2 opacity-50">
										<button
											disabled
											className="w-full flex items-center justify-between p-2.5 bg-[#111] border border-[#222] rounded text-[11px] text-gray-500 cursor-not-allowed"
										>
							...
							<div className="space-y-4 animate-in fade-in duration-200">
								<div className="p-4 rounded-lg border border-cyan-500/20 bg-cyan-500/5">
									...
									<div className="mt-2 text-[11px] text-[#9aa3ad] leading-relaxed">
										Save a crawl snapshot, then compare future crawls against it
										for title, content, status, canonical, robots, and schema
										changes.
									</div>
								</div>
								...
								<div className="space-y-2">
									{monitorChanges.length === 0 ? (
										<div className="p-3 rounded border border-[#222] bg-[#0a0a0a] text-[11px] text-[#666]">
											No detected changes yet. Save a baseline, then compare
										...
										monitorChanges.slice(0, 50).map((item) => (
											<div
												key={item.url}
												className="p-3 rounded border border-[#222] bg-[#0a0a0a]"
											>
						...
						{activeAuditTab === "migration" && (
							<div className="space-y-4 animate-in fade-in duration-200">
								<div className="p-4 rounded-lg border border-amber-500/20 bg-amber-500/5">
									...
									<div className="mt-2 text-[11px] text-[#9aa3ad] leading-relaxed">
										Paste target URLs from the destination site to generate
										redirect mappings from the current crawl.
									</div>
								</div>
								<textarea
									...
									<button
										onClick={() =>
											generateMigrationPlan().catch((error) =>
												addLog(error.message, "error", { source: "analysis" }),
											)
										}
										className="px-3 py-2 rounded border border-[#333] bg-[#111] text-[11px] font-bold text-white"
									>
										...
										<>
											<button
												onClick={() =>
													downloadText(
														"migration-plan.csv",
														MigrationPlannerService.exportCsv(
															migrationMappings,
														),
													)
												}
											...
											<button
												onClick={() =>
													downloadText(
														"migration-plan.htaccess",
														MigrationPlannerService.exportHtaccess(
															migrationMappings,
														),
													)
												}
						...
						{(activeAuditTab === "history" ||
							activeAuditTab === "wqa_history") && (
							<div className="space-y-4 animate-in fade-in duration-200">
								<div className="flex items-center justify-between">
									<p className="text-[12px] text-[#888]">
										Your past crawl sessions stored on this device.
									</p>
									{crawlHistory.length > 0 && (
										<span className="text-[10px] text-[#555] font-mono">
											{crawlHistory.length} sessions
										</span>
									)}
								</div>
								...
								{isLoadingHistory ? (
									...
									<div className="flex flex-col items-center justify-center py-12 text-[#666] gap-3">
										...
										<p className="text-[12px] text-center">
											No scan history yet. Start a crawl and it&apos;ll be saved
											here automatically.
										</p>
									</div>
								) : (
													...
													<div className="flex items-start justify-between mb-1.5">
														<div className="flex-1 min-w-0">
															...
															<div className="text-[10px] text-[#555] mt-1 flex items-center gap-2">
																...
																<span
																	className={`font-bold ${
																		session.healthGrade === "A"
																			? "text-green-400"
																			: session.healthGrade === "B"
																				? "text-blue-400"
																				: session.healthGrade === "C"
																					? "text-yellow-400"
																					: "text-red-400"
																	}`}
																>
															...
															<div className="text-[9px] text-[#444] mt-1 flex items-center gap-2 font-mono">
																<span>
																	{session.runtime?.crawled ??
																		session.totalPages}{" "}
																	crawled
																</span>
																<span className="text-[#2d2d2d]">•</span>
																<span>
																	{session.runtime?.queued ?? 0} queued
																</span>
																<span className="text-[#2d2d2d]">•</span>
																<span>
																	checkpoint{" "}
							...
							<div className="h-full flex flex-col animate-in fade-in duration-200">
								...
								<div className="mb-3 p-3 bg-[#0d0d0d] border border-[#222] rounded-lg space-y-3">
									<div className="flex items-center justify-between gap-3">
										<div>
											...
											<div className="text-[10px] text-[#666]">
												Upload access logs to map Googlebot and AI crawler
												activity back to crawled pages.
											</div>
										</div>
										<label className="px-3 py-1.5 rounded border border-[#333] text-[11px] text-white flex items-center gap-2 cursor-pointer">
											...
											<input
												type="file"
												accept=".log,.txt,.csv,.jsonl,.gz"
												className="hidden"
												onChange={handleLogAnalysisUpload}
											/>
										</label>
									</div>
									{logAnalysis && (
								...
								<div className="flex items-center gap-2 mb-2">
									<div className="relative flex-1">
										<Search
											className="absolute left-2 top-1/2 -translate-y-1/2 text-[#555]"
											size={10}
										/>
										<input
											type="text"
								...
								<div className="flex-1 bg-[#0a0a0a] border border-[#222] rounded p-3 font-mono text-[10px] text-[#888] overflow-y-auto space-y-1.5 shadow-inner custom-scrollbar">
									{(() => {
										...
										return filtered.map((L: any, i: number) => (
											<div
												key={i}
												className={`leading-relaxed break-all py-0.5 ${
													L.type === "error"
														? "text-red-400"
														: L.type === "success"
															? "text-green-400"
											...
											>
												<div className="flex items-start gap-1.5">
													<span
														className="opacity-40 shrink-0 font-mono"
														style={{ fontSize: "9px" }}
													>
													...
													{L.source && (
														<span
															className={`shrink-0 px-1 py-0 rounded font-bold leading-relaxed ${
																L.source === "error" || L.type === "error"
												...
												{L.url && (
													<div className="ml-7 mt-0.5 text-[9px] text-blue-400/60 truncate">
														{L.url}
													</div>
												)}
												{L.detail && L.detail !== L.msg && (
													<div className="ml-7 mt-0.5 text-[9px] text-[#555] break-all">
														{L.detail}
													</div>
												)}
											</div>
							...
							<div className="space-y-4 animate-in fade-in duration-200">
								...
								{!sitemapData ? (
									...
									<div className="space-y-4">
										...
										<div className="space-y-2">
											<div className="text-[10px] text-[#555] uppercase tracking-widest font-bold">
												Sitemap Sources
											</div>
											{sitemapData.sources.map((s, i) => (
												<div
													key={i}
													className="text-[10px] text-[#888] truncate bg-[#0a0a0a] p-2 rounded border border-[#222] font-mono"
												>
													{s}
												</div>
											))}
									...
									<div className="space-y-6">
										...
										<div className="space-y-2">
											...
											{[
												{
													label: "In Sitemap & Crawled",
													count: sitemapCrawledCount,
													color: "bg-green-500",
												},
												{
													label: "In Sitemap but Missing",
													count: Math.max(
														0,
														sitemapData.totalUrls - sitemapCrawledCount,
													),
													color: "bg-red-500",
												},
												{
													label: "Crawled but Not in Sitemap",
													count: pages.filter(
														(p) =>
															!p.inSitemap && !p.isImage && !p.isCss && !p.isJs,
													).length,
													color: "bg-orange-400",
												},
											].map((stat) => (
												...
												>
													<div
														className={`w-1.5 h-1.5 rounded-full ${stat.color} shrink-0`}
													></div>
