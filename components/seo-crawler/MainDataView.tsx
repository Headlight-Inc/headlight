...
export default function AuditPane() {
	...
	const [mapScope, setMapScope] = useState<
		"all" | "issues" | "deep" | "important" | "focus"
	>("all");
	const [isMapWorkspaceOpen, setIsMapWorkspaceOpen] = useState(false);
	const [collapsedMapNodeIds, setCollapsedMapNodeIds] = useState<Set<string>>(
		new Set(),
	);
	...
	const graphWidth = Math.max(
		640,
		graphDimensions.width || graphContainerRef.current?.clientWidth || 0,
	);
	const graphHeight = Math.max(
		420,
		graphDimensions.height || graphContainerRef.current?.clientHeight || 0,
	);
	const workspaceGraphWidth =
		typeof window !== "undefined"
			? Math.max(960, window.innerWidth - 48)
			: graphWidth;
	const workspaceGraphHeight =
		typeof window !== "undefined"
			? Math.max(640, window.innerHeight - 48)
			: graphHeight;
	const mapSummary = useMemo(() => {
		const hiddenNodeCount =
			pages.length > 2000
				? graphData.nodes.filter(
						(node: any) => !(node.internalPageRank > 10 || node.crawlDepth < 3),
					).length
				: 0;
		const sections = new Set(
			graphData.nodes.map((node: any) => node.sectionKey || "Other"),
		);
		const maxDepth = graphData.nodes.reduce(
			(max: number, node: any) => Math.max(max, Number(node.crawlDepth || 0)),
			0,
		);
	...
	const rootMapNodeId = useMemo(() => {
		return (
			graphData.nodes.find((node: any) => node.crawlDepth === 0)?.id ||
	...
	const wqaStats = useMemo(
		() =>
			wqaState.siteStats ||
			(pages.length ? computeWqaSiteStats(pages, effectiveIndustry) : null),
		[wqaState.siteStats, pages, effectiveIndustry],
	);
	const wqaActionGroups = useMemo(
		() =>
			wqaState.actionGroups.length
				? wqaState.actionGroups
	...
	const activeMapNode =
		hoveredMapNode ||
		(selectedMapNodeId
			? graphData.nodes.find((node: any) => node.id === selectedMapNodeId)
			: null) ||
		null;
	...
	const mapGraphData = useMemo(() => {
		...
		const allowedNodeIds = new Set(
			graphData.nodes
				.filter((node: any) => {
					if (mapScope === "issues")
						return node.status >= 300 || node.id === rootMapNodeId;
					if (mapScope === "deep")
						return node.crawlDepth >= 3 || node.id === rootMapNodeId;
					if (mapScope === "important")
						return (
							node.id === rootMapNodeId ||
							node.inlinks >= 5 ||
							node.internalPageRank >= 10
						);
	...
	const mapIntelligence = useMemo(() => {
		...
		const dominantSection = Array.from(sectionCounts.entries()).sort(
			(a, b) => b[1] - a[1],
		)[0];
		const structuralShare = mapGraphData.links.length
			? Math.round(
					(mapGraphData.links.filter((link: any) => link.isStructural).length /
						mapGraphData.links.length) *
						100,
				)
			: 0;
	...
	const architectureChildrenByNode = useMemo(() => {
		...
		(mapGraphData.links || []).forEach((link: any) => {
			...
			const targetId =
				typeof link.target === "string" ? link.target : link.target?.id;
			const sourceNode = mapGraphData.nodes.find(
				(node: any) => node.id === sourceId,
			);
			const targetNode = mapGraphData.nodes.find(
				(node: any) => node.id === targetId,
			);
			if (!sourceId || !targetId || !sourceNode || !targetNode) return;
			if (
				Number(targetNode.crawlDepth || 0) !==
				Number(sourceNode.crawlDepth || 0) + 1
			)
				return;
	...
	const architectureLayout = useMemo(() => {
		const nodes = (mapGraphData.nodes || []).filter(
			(node: any) => !hiddenNodeIds.has(node.id),
		);
		...
		const laneKeys = Array.from(
			new Set(
				nodes.map((node: any) =>
					Math.min(Number(node.crawlDepth || 0), maxVisibleDepth),
				),
			),
		)
			.map(Number)
		...
		const maxLaneSpacing =
			minLaneSpacing + (architectureProfile.useCompactCards ? 76 : 90);
		const totalInnerWidth = Math.max(cardWidth, graphWidth - sidePadding * 2);
		const computedLaneSpacing =
			laneKeys.length > 1
				? Math.max(
						minLaneSpacing,
						Math.min(
							maxLaneSpacing,
							(totalInnerWidth - cardWidth) / (laneKeys.length - 1),
						),
					)
				: 0;
		...
		const contentWidth =
			laneKeys.length > 1
				...
				: cardWidth;
		const extraHorizontalOffset = Math.max(
			0,
			(graphWidth - sidePadding * 2 - contentWidth) / 2,
		);
		...
		laneBuckets.forEach((sections) => {
			sections.forEach((laneNodes) => {
				laneNodes.sort((a, b) => {
					const scoreA =
						Number(a.linkEquity || 0) * 10 + Number(a.gscClicks || 0) * 1.5;
					const scoreB =
						Number(b.linkEquity || 0) * 10 + Number(b.gscClicks || 0) * 1.5;
		...
		const laneStride =
			cardHeight + (architectureProfile.useCompactCards ? 10 : 12);
		...
		const positionedNodeMap = new Map(
			positionedNodes.map((node: any) => [node.id, node]),
		);
		const positionedLinks = links
			.map((link: any) => {
				...
				return {
					...
					sourceId,
					targetId,
					isCrossLink: Boolean(link.isCrossLink),
					isStructural: Boolean(link.isStructural),
				};
			})
			.filter(Boolean);
		...
		graphWidth,
		graphHeight,
		architectureProfile.useCompactCards,
	...
	const visibleArchitectureLinks = useMemo(() => {
		...
		if (activeMapNodeId) {
			return links.filter((link: any) => {
				const laneDelta = Math.abs(
					(link.target?.laneDepth ?? 0) - (link.source?.laneDepth ?? 0),
				);
				return (
					link.sourceId === activeMapNodeId ||
					link.targetId === activeMapNodeId ||
					laneDelta === 1
				);
			});
		}
		...
		const treeLinkSet = new Set(
			treeLinks.map(
				(link: any, index: number) =>
					`${link.sourceId}->${link.targetId}:${index}`,
			),
		);
		const crossLinks = links.filter(
			(link: any, index: number) =>
				!treeLinkSet.has(`${link.sourceId}->${link.targetId}:${index}`),
		);
		...
		return [
			...treeLinks.slice(0, treeBudget),
			...crossLinks.slice(0, crossBudget),
		];
	}, [
		architectureLayout,
		activeMapNodeId,
		architectureProfile.showAmbientLinks,
	]);
	useEffect(() => {
		if (!dragState) return;
		...
		const handlePointerMove = (e: MouseEvent) => {
			if (dragState.type === "pan") {
				...
				if (!canvas) return;
				canvas.scrollLeft =
					dragState.scrollLeft - (e.clientX - dragState.startX);
	...
	useEffect(() => {
		...
		mapMode,
		SpriteText,
		mapPerformanceProfile.showLabels,
	]);
	...
	useEffect(() => {
		...
		if (mapMode === "2d") {
			...
			const timeoutId = window.setTimeout(() => {
				const targetNode =
					(centerMapNodeId &&
						architectureLayout.nodes.find(
							(node: any) => node.id === centerMapNodeId,
						)) ||
					architectureLayout.nodes.find(
						(node: any) => node.id === rootMapNodeId,
					) ||
		...
		const timeoutId = window.setTimeout(() => {
			try {
				...
				const rootNode =
					mapGraphData.nodes.find((node: any) => node.id === centerMapNodeId) ||
	...
	const renderMapView = (workspace = false) => {
		...
		return (
			<div
				className={
					workspace
						? "fixed inset-3 z-50 overflow-hidden rounded-[28px] border border-[#2a2a2a] bg-[#090909] shadow-[0_30px_100px_rgba(0,0,0,0.55)] flex flex-col"
						: "absolute inset-0 bg-[#0d0d0d] overflow-hidden flex flex-col"
				}
			>
				{workspace && (
				...
				<div className="relative flex-1 overflow-hidden">
							...
							{mapMode === "2d" ? (
								...
								<ForceGraph3D
									...
									linkVisibility={(link: any) => {
										...
										const targetId =
											typeof link.target === "string"
												...
												: link.target?.id;
										return (
											sourceId === activeMapNodeId ||
											targetId === activeMapNodeId ||
											link.isStructural
										);
									}}
									linkColor={(link: any) => {
									...
									linkDirectionalParticles={(link: any) =>
										mapPerformanceProfile.showDirectionalParticles &&
										(link.isStructural ||
											activeMapNodeId ===
												(typeof link.source === "string"
													? link.source
									...
									linkDirectionalParticleColor={(link: any) => {
										const isFocused =
											...
											((typeof link.source === "string"
												? link.source
												: link.source?.id) === activeMapNodeId ||
												(typeof link.target === "string"
													? link.target
													: link.target?.id) === activeMapNodeId);
									...
									nodeThreeObject={(node: any) => {
										...
										const pillarMaterial = new THREE.MeshPhongMaterial({
											color,
											emissive: color,
											emissiveIntensity: isSelected ? 1.15 : 0.28,
											shininess: 80,
											transparent: true,
											opacity: 0.95,
										});
					...
					{mapMode === "2d" && (
						...
						>
							<svg
								width={architectureLayout.width}
								height={architectureLayout.height}
								className="block"
								onMouseDown={(e) => {
									const target = e.target as Element | null;
									if (target?.closest('[data-map-node-interactive="true"]'))
										return;
									const canvas = architectureCanvasRef.current;
									if (!canvas) return;
									setDragState({
							...
							>
								...
								{architectureLayout.lanes.map((lane: any) => (
									<g key={lane.id}>
										<g transform={`translate(${lane.x}, 60)`}>
											<text
												fill="#8a8a8a"
												fontSize="9"
												fontWeight="900"
								...
								{architectureLayout.sectionGuides.map((section: any) => (
									<g key={`${section.depth}-${section.sectionKey}`}>
										<text
											x={section.x + 4}
											y={section.y + 12}
											fill="#636363"
											...
											letterSpacing="1.1"
										>
											{String(section.sectionKey).toUpperCase()}
										</text>
									</g>
								))}
								{visibleArchitectureLinks.map((link: any, index: number) => {
									const isFocused =
										...
										(link.sourceId === activeMapNodeId ||
											link.targetId === activeMapNodeId);
									const laneDelta = Math.abs(
										(link.target?.laneDepth ?? 0) -
											(link.source?.laneDepth ?? 0),
									);
									...
									const controlOffset =
										deltaX > 0
											? Math.max(28, Math.min(140, deltaX * 0.42))
											: 36;
										...
										<path
											...
											stroke={
												isFocused
													? "rgba(245,54,78,0.42)"
													: isTreeLink
														? "rgba(232,236,245,0.30)"
														: "rgba(116,156,255,0.18)"
											}
											strokeWidth={isFocused ? 2 : isTreeLink ? 1.5 : 1}
											strokeDasharray={isTreeLink ? "none" : "3 5"}
											strokeLinecap="round"
								...
								{architectureLayout.nodes.map((node: any) => {
									...
									const isNearby = activeMapNodeId
										? focusedNeighborhoodIds.has(node.id)
										: false;
									const muted = activeMapNodeId ? !isNearby : false;
									const displayName =
										String(node.name || "home")
											.split("/")
											.pop()
											?.slice(0, 24) || "HOME";
									const priorityRaw = String(
										node.strategicPriority || "",
									).toLowerCase();
									const inferredPriority =
										node.issueCount > 0 || node.status >= 400
											? "H"
											: node.inlinks >= 5 || node.internalPageRank >= 10
												? "M"
												: "L";
									const priorityBadge = priorityRaw.startsWith("c")
										? "C"
										: priorityRaw.startsWith("h")
											? "H"
											: priorityRaw.startsWith("m")
												? "M"
												: priorityRaw.startsWith("l")
													? "L"
													: inferredPriority;
									const intentBadge =
										node.searchIntent === "Transactional"
											? "T"
											: node.searchIntent === "Commercial"
												? "C"
									...
									const childCount =
										architectureChildrenByNode.get(node.id)?.length || 0;
									...
									return (
										...
										>
											...
											<text
												...
												fontWeight="700"
												letterSpacing="0.7"
											>{`${node.status || "-"} • ${node.inlinks || 0} inlinks${isRoot ? " • root" : ""}`}</text>
											<g transform={`translate(14, ${node.cardHeight - 16})`}>
												<rect
													width="22"
													height="10"
													rx="5"
												...
												<text
													...
													fontSize="6.4"
													fontWeight="800"
												>{`D${node.crawlDepth ?? 0}`}</text>
											</g>
											<g transform={`translate(40, ${node.cardHeight - 16})`}>
												<rect
													width="22"
													height="10"
													rx="5"
												...
												<text
													...
													fontSize="6.4"
													fontWeight="800"
												>{`P${priorityBadge}`}</text>
											</g>
											<g transform={`translate(66, ${node.cardHeight - 16})`}>
												<rect
													width="22"
													height="10"
													rx="5"
													fill="rgba(88,86,214,0.14)"
												/>
												<text
													x="11"
													y="7"
											...
											{node.issueCount > 0 && (
												<g transform={`translate(${node.cardWidth - 54}, 11)`}>
													<rect
														width="40"
														height="12"
														rx="6"
														fill="rgba(245,54,78,0.08)"
													/>
													<text
														x="20"
														y="8"
														...
														fontSize="6.5"
														fontWeight="800"
													>{`${node.issueCount} ISSUE`}</text>
												</g>
											)}
											{node.val > 8 && (
												<g
													transform={`translate(${node.cardWidth - 18}, ${node.cardHeight - 16})`}
												>
													<circle r="7" fill="rgba(214,178,75,0.08)" />
													<text
														y="2.5"
														textAnchor="middle"
														fill="#b99641"
											...
											{childCount > 0 && (
												<g
													transform={`translate(${node.cardWidth - 18}, 18)`}
													data-map-node-interactive="true"
													onClick={(e) => {
														e.stopPropagation();
														setCollapsedMapNodeIds((prev) => {
					...
					<div className="absolute bottom-4 left-4 max-w-[360px] bg-[#101010]/90 border border-[#252525] rounded-2xl px-3 py-2.5 shadow-[0_10px_30px_rgba(0,0,0,0.25)] backdrop-blur pointer-events-auto">
						{activeMapNode ? (
							<div className="space-y-2">
								<div className="min-w-0">
									<div className="text-[11px] font-semibold text-white truncate">
										{activeMapNode.fullUrl || activeMapNode.name}
									</div>
								...
								<div className="flex items-center gap-2">
									<button
										onClick={() => {
											if (!selectedMapNodeId && activeMapNode) {
												const found = pages.find(
													(page: any) => page.url === activeMapNode.id,
												);
												if (found) setSelectedPage(found);
											}
											setMapScope("focus");
										}}
										className="px-2.5 py-1.5 rounded-lg bg-[#171717] border border-[#2a2a2a] text-[#d0d0d0] text-[10px] font-medium hover:text-white hover:border-[#3a3a3a] transition-colors"
									>
										Focus
									</button>
								</div>
							...
							<div className="text-[10px] text-[#6a6a6a] leading-relaxed">
								Hover a page to inspect it. Drag the surface to pan and use +/-
								to fold branches.
							</div>
						)}
					</div>
					<div className="absolute top-4 right-4 bottom-4 flex flex-col items-end justify-between pointer-events-none">
						<div className="pointer-events-auto flex flex-col gap-3 items-end">
							<div className="flex flex-col gap-1.5 bg-[#101010]/94 border border-[#232323] rounded-2xl p-2.5 shadow-[0_10px_30px_rgba(0,0,0,0.24)] backdrop-blur">
								<button
									...
									className={`px-3 py-2 text-[11px] rounded-xl transition-colors text-left min-w-[132px] ${mapMode === "2d" ? "bg-[#F5364E] text-white" : "text-[#8a8a8a] hover:text-white hover:bg-[#151515]"}`}
								>
									Architecture
								</button>
								<button
									onClick={() => setMapMode("3d")}
									className={`px-3 py-2 text-[11px] rounded-xl transition-colors text-left min-w-[132px] ${mapMode === "3d" ? "bg-[#F5364E] text-white" : "text-[#8a8a8a] hover:text-white hover:bg-[#151515]"}`}
								>
									3D View
								</button>
								<button
									onClick={() => setIsMapWorkspaceOpen((prev) => !prev)}
									className="px-3 py-2 text-[11px] rounded-xl transition-colors text-left min-w-[132px] text-[#8a8a8a] hover:text-white hover:bg-[#151515] flex items-center gap-2"
								>
							...
							<div className="flex flex-col gap-1 bg-[#101010]/94 border border-[#232323] rounded-2xl p-2 shadow-[0_10px_30px_rgba(0,0,0,0.2)] backdrop-blur">
								{[
									...
									<button
										key={option.id}
										onClick={() => setMapScope(option.id as any)}
										className={`px-3 py-2 rounded-xl text-[10px] text-left transition-colors min-w-[132px] ${mapScope === option.id ? "bg-[#171717] text-white" : "text-[#757575] hover:text-white hover:bg-[#141414]"}`}
									>
										{option.label}
									</button>
								))}
							...
							<button
								onClick={() => {
									if (mapMode === "2d") {
										architectureCanvasRef.current?.scrollTo({
											left: 0,
											top: 0,
											behavior: "smooth",
										});
								...
								className="pointer-events-auto w-[48px] h-[48px] flex items-center justify-center rounded-2xl bg-[#101010]/94 border border-[#232323] text-[#777] hover:text-white hover:bg-[#151515] shadow-[0_10px_30px_rgba(0,0,0,0.18)] backdrop-blur transition-colors"
						...
						<div className="pointer-events-auto inline-flex items-center gap-3 bg-[#101010]/88 border border-[#232323] rounded-2xl px-3 py-2 text-[10px] text-[#8a8a8a] shadow-[0_10px_30px_rgba(0,0,0,0.18)] backdrop-blur">
							<span className="font-mono text-[#d0d0d0]">
								{mapGraphData.nodes.length} nodes
							</span>
							<span className="font-mono text-[#7a7a7a]">
								{mapSummary.sectionCount} sections
							</span>
							<span className="font-mono text-[#7a7a7a]">
								depth {mapSummary.maxDepth}
							</span>
						</div>
					</div>
				</div>
		...
		<TableVirtuoso
			...
			components={{
				Table: (props) => (
					<table
						{...props}
						className="w-max border-collapse text-left text-[12px] whitespace-nowrap table-fixed"
					/>
				),
				TableHead: React.forwardRef((props, ref) => (
					<thead
						{...props}
						ref={ref}
						className="sticky top-0 bg-[#0d0d0d]/95 backdrop-blur-md z-[60] shadow-[0_1px_0_#333]"
					/>
				)),
				TableRow: (props) => {
				...
				TableBody: React.forwardRef((props, ref) => (
					<tbody
						{...props}
						ref={ref}
						className="bg-[#0d0d0d] font-sans tracking-tight text-[12px] text-[#ccc]"
					/>
				)),
			}}
			fixedHeaderContent={() => (
				<tr className="text-[#999] font-medium text-[11px]">
					...
					<th className="py-2 px-2 w-[40px] border-r border-b border-[#222] sticky left-[30px] bg-[#111] z-[70] font-mono text-center">
						#
					</th>
						...
						return (
							...
							>
								<div className="flex items-center justify-between pointer-events-none">
									...
									<div
										className="flex items-center shrink-0 pointer-events-auto cursor-pointer"
										onClick={() => handleSort(col.key)}
									>
										{sortConfig?.key === col.key ? (
											sortConfig.direction === "asc" ? (
												<ChevronUp size={10} className="text-[#F5364E]" />
											) : (
												<ChevronDown size={10} className="text-[#F5364E]" />
											)
										) : (
											<ChevronDown
												size={10}
												className="text-[#333] opacity-0 group-hover:opacity-100"
											/>
										)}
									</div>
								</div>
								...
								<div
									onMouseDown={(e) => {
										...
										setResizingCol({
											key: col.key,
											startX: e.clientX,
											startWidth: parseInt(String(width)),
										});
									}}
			...
			itemContent={(index, page) => {
				...
				return (
					<>
						<td
							className={`py-1.5 px-1.5 text-center border-r border-[#222] sticky left-0 z-10 ${isSelected ? "bg-[#1e2333]" : "bg-[#111]"}`}
						>
							<input
								type="checkbox"
						...
						<td
							className={`py-1.5 px-2 text-center text-[#666] border-r border-[#222] sticky left-[30px] z-10 font-mono text-[11px] ${isSelected ? "bg-[#1e2333]" : "bg-[#111]"}`}
						>
							{index + 1}
						</td>
						{displayColumns.map((col, idx) => {
							...
							if (col.key === "indexable") {
								...
								cellClass =
									"text-blue-400 cursor-pointer overflow-hidden text-ellipsis";
							} else if (
								col.key.toLowerCase().includes("size") ||
								col.key.toLowerCase().includes("bytes") ||
								col.key === "co2Mg"
							) {
								...
								[
									"page_category_confidence",
									"nap_match_with_homepage",
									"nap_has_distinct_address",
									"titleLength",
									"titlePixelWidth",
									"metaDescLength",
									"metaDescPixelWidth",
									...
									"engagementScore",
									"authorityComputedScore",
									"businessComputedScore",
									"redirectChainLength",
									"domNodeCount",
									"renderBlockingCss",
									"renderBlockingJs",
									...
									"minFontSize",
									"sitemapBrokenUrls",
									"sitemapLastmodMismatchCount",
									"sitemapValidationSampleSize",
									"urlLength",
									"anchorTextDiversity",
								].includes(col.key)
							) {
								...
								if (col.key === "titleLength" && typeof rawVal === "number") {
									...
									displayElement = (
										<div className="flex items-center justify-end gap-2 group/cell">
											<div className="w-12 h-1 bg-[#222] rounded-full overflow-hidden opacity-30 group-hover/cell:opacity-100 transition-opacity">
												<div
								...
								col.key === "commentCount" ||
								col.key === "taskCount"
							) {
								const count = Number(rawVal) || 0;
								const Icon =
									col.key === "commentCount" ? MessageSquare : CheckSquare;
								displayElement =
									count > 0 ? (
										<button
											onClick={(e) => {
								...
								col.key === "topicCluster" ||
								col.key === "funnelStage"
							) {
								displayElement = rawVal ? (
									<span className="bg-[#222] text-[#ccc] px-2 py-0.5 rounded-full text-[10px]">
										{rawVal}
									</span>
								) : (
									"-"
								...
								col.key.startsWith("gsc") &&
								typeof rawVal === "number"
							) {
								cellClass =
									"font-mono text-[11px] text-right pr-4 text-white font-bold";
								if (col.key === "gscCtr")
									displayElement = `${(rawVal * 100).toFixed(1)}%`;
								else if (col.key === "gscPosition")
									displayElement = rawVal.toFixed(1);
								else displayElement = rawVal.toLocaleString();
							} else if (
								(col.key === "mainKwPosition" ||
									col.key === "bestKwPosition") &&
								typeof rawVal === "number"
							) {
								cellClass =
									"font-mono text-[11px] text-right pr-4 text-white font-bold";
								...
								(col.key === "mainKwVolume" || col.key === "bestKwVolume") &&
								typeof rawVal === "number"
							) {
								const sourceKey =
									col.key === "mainKwVolume"
										? "mainKwVolumeSource"
										: "bestKwVolumeSource";
								...
								(col.key === "mainKeyword" || col.key === "bestKeyword") &&
								rawVal
							) {
								const sourceKey =
									col.key === "mainKeyword"
								...
								col.key.startsWith("ga4") &&
								typeof rawVal === "number"
							) {
								cellClass =
									"font-mono text-[11px] text-right pr-4 text-white font-bold";
								if (col.key === "ga4BounceRate")
									displayElement = `${(rawVal * 100).toFixed(1)}%`;
								else if (col.key === "ga4ConversionRate")
									displayElement = `${(rawVal * 100).toFixed(1)}%`;
								else if (
									col.key === "ga4AvgSessionDuration" ||
									col.key === "ga4EngagementTimePerPage"
								)
									displayElement = rawVal.toFixed(1);
								else displayElement = rawVal.toLocaleString();
							} else if (col.key === "searchIntent") {
								const intentColor =
									rawVal === "Transactional"
								...
								displayElement = (
									<span
										className={`px-2 py-0.5 rounded-full text-[10px] ${intentColor}`}
									>
										{rawVal || "Info"}
									</span>
								);
								...
								const actionStyle =
									ACTION_COLORS[String(rawVal)] || ACTION_COLORS.Monitor;
								...
								displayElement = (
									<span
										className={`${isHighPri ? "text-red-400 font-bold" : "text-[#666]"}`}
									>
										{rawVal || "Low"}
									</span>
								);
							...
							return (
								<td
									key={idx}
									className={`py-1.5 px-3 truncate border-r border-[#222] ${cellClass} ${col.key === "url" ? "sticky left-[70px] z-10 shadow-[1px_0_0_#222] " + (isSelected ? "bg-[#1e2333]" : "bg-[#111]") : ""}`}
									style={{
	...
	return (
		<main className="flex-1 flex flex-col min-h-0 min-w-0 bg-[#0a0a0a] relative">
			...
			<div
				className="flex-1 min-h-0 bg-[#0a0a0a] relative overflow-hidden"
					...
					<div className="absolute inset-0 flex flex-col items-center justify-center p-8 bg-[#0a0a0a]">
						<div className="relative mb-8 group">
							...
							<svg
								...
								className="relative z-10 text-white/20 drop-shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:text-white/40 transition-all duration-700"
							>
								<circle
									cx="9"
									cy="16"
								...
								<path
									...
									strokeWidth="2.5"
									strokeLinecap="round"
									className="opacity-0 animate-[fadeIn_0.6s_ease-out_forwards_0.2s]"
								/>
								<path
									d="M17 16H31"
									stroke="currentColor"
									strokeWidth="2.5"
									strokeLinecap="round"
									className="opacity-0 animate-[fadeIn_0.6s_ease-out_forwards_0.4s]"
								/>
								<path
									d="M17 21H27"
									stroke="currentColor"
									strokeWidth="2.5"
									strokeLinecap="round"
									className="opacity-0 animate-[fadeIn_0.6s_ease-out_forwards_0.6s]"
								/>
							</svg>
						</div>
						<h2 className="text-[14px] font-medium text-[#555] mb-1 tracking-tight">
							No data to display
						</h2>
						<p className="text-[12px] text-[#333]">
							Enter a web address at the top and click Start Scan.
						</p>
					...
					<div className="bg-[#111] border border-orange-500/30 rounded-lg shadow-[0_0_30px_rgba(249,115,22,0.15)] p-4 flex items-center gap-4 min-w-[400px]">
						...
						<div className="flex-1">
							...
							<p className="text-[11px] text-[#888]">
								You've reached the 100-page limit for this scan.
							</p>
						</div>
						<div className="flex items-center gap-2">
							<button
							...
							<button className="px-4 py-1.5 bg-orange-500 hover:bg-orange-600 text-white text-[11px] font-bold rounded flex items-center gap-2 transition-all shadow-lg shadow-orange-500/20">
								<Sparkles size={12} />
								Upgrade
							</button>
						</div>
			...
			{headerContextMenu && (
				<ColumnHeaderContextMenu
					...
					onPinRight={() => {}}
					onResizeToFit={(key) => {
						const sampleValues = filteredPages
							.slice(0, 100)
							.map((page) => String(page?.[key] ?? ""));
						const maxLength = Math.max(
							headerContextMenu.col.label.length,
							...sampleValues.map((value) => value.length),
						);
						const computedWidth = Math.min(
							520,
							Math.max(80, maxLength * 7 + 32),
						);
