...
export default function CompBriefTab() {
	...
	const hasBrief = Boolean(
		...
		!brief.executiveSummary.includes("Unable to generate"),
	);
	...
	return (
		<div className={SIDEBAR_SCROLL}>
			<div className="flex items-center justify-between">
				<div className="text-[11px] font-semibold uppercase tracking-wider text-[#555]">
					AI Competitive Brief
				</div>
				<div className="flex items-center gap-2">
					...
					>
						<RefreshCw
							size={10}
							className={isGeneratingBrief ? "animate-spin" : ""}
						/>
						{isGeneratingBrief
							? "Generating..."
							: hasBrief
					...
					{hasBrief && (
						<button
							onClick={() => {
								const analyses =
									(brief as any)?.perCompetitor ||
									(brief as any)?.competitorAnalyses ||
									[];
			...
			{isGeneratingBrief && (
				<div className="rounded-xl border border-[#1a1a1e] bg-[#0d0d0f] p-6 text-center">
					<RefreshCw
						size={20}
						className="mx-auto mb-2 animate-spin text-[#F5364E]"
					/>
									...
									>
										...
										<div className="min-w-0 flex-1">
											<div className="flex items-center gap-2">
												...
												<span
													className={`text-[11px] ${checkedActions.has(i) ? "line-through text-[#666]" : "text-[#ccc]"}`}
												>
													{action.action}
												</span>
											</div>
						...
						<div className={CARD}>
							...
							<div className="grid grid-cols-2 gap-2">
								<div className="rounded-lg border border-green-500/10 bg-green-500/5 p-3">
									<div className="mb-1.5 text-[9px] font-bold uppercase text-green-400">
										Strengths
									</div>
								...
								<div className="rounded-lg border border-red-500/10 bg-red-500/5 p-3">
									<div className="mb-1.5 text-[9px] font-bold uppercase text-red-400">
										Weaknesses
									</div>
									<div className="space-y-1">
								...
								<div className="rounded-lg border border-blue-500/10 bg-blue-500/5 p-3">
									<div className="mb-1.5 text-[9px] font-bold uppercase text-blue-400">
										Opportunities
									</div>
									<div className="space-y-1 text-[10px] text-[#ccc]">
										{activeComps.filter(
											(c) => Number(c.techHealthScore || 0) < 60,
										).length > 0 && (
										...
										{activeComps.filter((c) => Number(c.avgGeoScore || 0) < 50)
											.length > 0 && (
								...
								<div className="rounded-lg border border-orange-500/10 bg-orange-500/5 p-3">
									<div className="mb-1.5 text-[9px] font-bold uppercase text-orange-400">
										Threats
									</div>
									<div className="space-y-1 text-[10px] text-[#ccc]">
										{activeComps
