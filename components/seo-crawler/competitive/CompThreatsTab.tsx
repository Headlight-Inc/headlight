...
import {
	getCompetitorAlerts,
	getRankShiftAlerts,
} from "../../../services/CrawlerBridgeService";
...
export default function CompThreatsTab() {
	...
	useEffect(() => {
		...
		getCompetitorAlerts(activeProject.id)
			.then(setCompAlerts)
			.catch(() => setCompAlerts([]));
		getRankShiftAlerts(activeProject.id)
			.then(setRankAlerts)
			.catch(() => setRankAlerts([]));
	}, [activeProject?.id]);
	...
	const heatmapData = useMemo(
		() =>
			activeComps.map((comp) => ({
				...
				speed:
					comp.siteSpeedScore != null
						? Math.max(0, 100 - comp.siteSpeedScore)
						: null,
	...
	if (activeComps.length === 0) {
		return (
			<EmptyState
				...
				submessage="Add competitors to see threat analysis."
			/>
		);
	}
	...
	return (
		<div className={SIDEBAR_SCROLL}>
			...
			<div className={CARD}>
				...
				<div className="overflow-x-auto">
					<table className="w-full">
						<thead>
							<tr>
								<th className="pb-2 pr-2 text-left text-[9px] text-[#555]">
									Competitor
								</th>
								<th className="px-1 pb-2 text-center text-[9px] text-[#555]">
									Content
								</th>
								<th className="px-1 pb-2 text-center text-[9px] text-[#555]">
									Authority
						...
						<tbody>
							{heatmapData.map((row) => (
								<tr key={row.domain}>
									<td className="max-w-[100px] truncate py-1 pr-2 text-[10px] text-[#aaa]">
										{row.domain}
									</td>
									{(
										["content", "authority", "speed", "innovation"] as const
									).map((dim) => {
										const val = row[dim];
										return (
											<td key={dim} className="px-1 py-1">
												{val != null ? (
													<div
														className={`rounded py-1 text-center text-[10px] font-bold ${threatColor(val)}`}
													>
														{val}
													</div>
												) : (
													<div className="rounded py-1 text-center text-[10px] text-[#333]">
														—
													</div>
												)}
											</td>
			...
			{activeComps.map((comp) => {
				...
				compare(
					"Content Velocity",
					comp.blogPostsPerMonth,
					ownProfile?.blogPostsPerMonth,
					"/mo",
				);
				compare(
					"Referring Domains",
					comp.referringDomains,
					ownProfile?.referringDomains,
				);
				compare(
					"Indexable Pages",
					comp.totalIndexablePages,
					ownProfile?.totalIndexablePages,
				);
				compare(
					"Tech Health",
					comp.techHealthScore,
					ownProfile?.techHealthScore,
					"/100",
				);
				compare("GEO Score", comp.avgGeoScore, ownProfile?.avgGeoScore, "/100");
				compare(
					"Schema Coverage",
					comp.schemaCoveragePct,
					ownProfile?.schemaCoveragePct,
					"%",
				);
				compare(
					"Link Velocity",
					comp.linkVelocity60d,
					ownProfile?.linkVelocity60d,
				);
				...
				return (
					<div key={comp.domain} className={CARD}>
						...
						{threats.length > 0 && (
							<div className="mb-3">
								<div className="mb-1.5 text-[10px] font-bold uppercase text-red-400/70">
						...
						{weaknesses.length > 0 && (
							<div>
								<div className="mb-1.5 text-[10px] font-bold uppercase text-green-400/70">
									Where they're weak
								</div>
				...
				<div className="space-y-2">
					{compAlerts.length === 0 ? (
						...
						compAlerts.slice(0, 5).map((alert) => (
							<div
								key={alert.id}
								className="rounded border border-[#1a1a1e] bg-[#0a0a0a] p-2.5"
							>
