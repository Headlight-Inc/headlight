...
export default function CompTrendsTab() {
	...
	if (!ownProfile && activeComps.length === 0) {
		return (
			<EmptyState
				message="No competitive data yet."
				submessage="Run a crawl and add competitors to see trends."
			/>
		);
	}
	...
	return (
		<div className={SIDEBAR_SCROLL}>
			{pinnedKeywords.length > 0 && (
				<div className={CARD}>
					<div className={SECTION_HEADER_WITH_MARGIN}>
						Watched Keywords ({pinnedKeywords.length})
					</div>
			...
			<div className={CARD}>
				<div className={SECTION_HEADER_WITH_MARGIN}>
					{TREND_METRICS.find((m) => m.key === selectedMetric)?.label} - All
					Sites
				</div>
				{(() => {
					...
					activeComps.forEach((comp) => {
						allValues.push({
							domain: comp.domain,
							value: Number(comp[selectedMetric] || 0),
							isOwn: false,
						});
					});
			...
			<div className={CARD}>
				...
				<div className="space-y-1">
					{[
						{
							label: "Traffic Trend",
							key: "trafficTrend30d" as const,
							unit: "%",
						},
						{
							label: "Content Velocity",
							key: "contentVelocityTrend" as const,
							unit: "%",
						},
						{
							label: "Link Velocity (60d)",
							key: "linkVelocity60d" as const,
							unit: "",
						},
						{
							label: "New Pages (30d)",
							key: "recentNewPages" as const,
							unit: "",
						},
					].map((row) => {
						...
						return (
							<div key={row.key} className="flex items-center py-1 text-[10px]">
								...
								<span
									className={`w-[50px] text-right font-mono ${youWin ? "text-green-400" : "text-red-400"}`}
								>
									{yours > 0 ? "+" : ""}
									{yours}
									{row.unit}
								</span>
								<span className="mx-1.5 w-[16px] text-center text-[#333]">
									vs
								</span>
