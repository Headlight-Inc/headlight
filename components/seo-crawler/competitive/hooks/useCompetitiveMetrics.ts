...
export function useCompetitiveMetrics() {
	...
	const { advantages, vulnerabilities } = useMemo(() => {
		if (!ownProfile || activeComps.length === 0) {
			return {
				advantages: [] as MetricDelta[],
				vulnerabilities: [] as MetricDelta[],
			};
		}
		...
		check(
			"Referring Domains",
			Number(ownProfile.referringDomains || 0),
			avgOf("referringDomains"),
		);
		check(
			"Indexable Pages",
			Number(ownProfile.totalIndexablePages || 0),
			avgOf("totalIndexablePages"),
		);
		check(
			"Tech Health",
			Number(ownProfile.techHealthScore || 0),
			avgOf("techHealthScore"),
			"%",
		);
		check(
			"CWV Pass Rate",
			Number(ownProfile.cwvPassRate || 0),
			avgOf("cwvPassRate"),
			"%",
		);
		check(
			"GEO Score",
			Number(ownProfile.avgGeoScore || 0),
			avgOf("avgGeoScore"),
		);
		check(
			"Blog Posts/Mo",
			Number(ownProfile.blogPostsPerMonth || 0),
			avgOf("blogPostsPerMonth"),
		);
		check(
			"Schema Coverage",
			Number(ownProfile.schemaCoveragePct || 0),
			avgOf("schemaCoveragePct"),
			"%",
		);
		check(
			"Social Followers",
			Number(ownProfile.socialTotalFollowers || 0),
			avgOf("socialTotalFollowers"),
		);
		check(
			"Content Freshness",
			Number(ownProfile.contentFreshnessScore || 0),
			avgOf("contentFreshnessScore"),
			"%",
		);
		check(
			"Trust Signals",
			Number(ownProfile.trustSignalScore || 0),
			avgOf("trustSignalScore"),
		);
		check(
			"Site Speed",
			Number(ownProfile.siteSpeedScore || 0),
			avgOf("siteSpeedScore"),
		);
		check(
			"Crawlability",
			Number(ownProfile.crawlabilityScore || 0),
			avgOf("crawlabilityScore"),
		);

		const parseAbsDelta = (value: string) =>
			Math.abs(Number(value.replace(/[^0-9.-]/g, "")));

		return {
			advantages: adv
				.sort((a, b) => parseAbsDelta(b.delta) - parseAbsDelta(a.delta))
				.slice(0, 4),
			vulnerabilities: vul
				.sort((a, b) => parseAbsDelta(b.delta) - parseAbsDelta(a.delta))
				.slice(0, 4),
