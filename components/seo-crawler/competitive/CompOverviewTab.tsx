...
import { CARD, CARD_HIGHLIGHT, KEY_NUMBER, SECTION_HEADER_WITH_MARGIN, SIDEBAR_SCROLL } from './shared/styles';
...
function dimensionScore(profile: CompetitorProfile | null, key: string, max: number) {
  if (!profile) return 0;
    ...
    return DIMENSIONS.map((dim) => {
      ...
      const compAvg =
        activeComps.length > 0
          ? Math.round(activeComps.reduce((sum, c) => sum + dimensionScore(c, dim.key, dim.max), 0) / activeComps.length)
          : 0;
  ...
  if (!ownProfile && activeComps.length === 0) {
    return (
      <div className={SIDEBAR_SCROLL}>
        <EmptyState icon={<Users size={24} />} message="No competitive data yet. Add competitors and run a crawl." />
      </div>
    );
  }
    ...
    <div className={SIDEBAR_SCROLL}>
            ...
            return (
              <div key={dim.label}>
                <div className="mb-1 flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-bold ${winning ? 'text-green-400' : 'text-red-400'}`}>
                    {dim.yours} vs {dim.compAvg}
                  </span>
                </div>
                <div className="flex gap-1">
                  <div className="h-[6px] flex-1 overflow-hidden rounded-full bg-[#1a1a1e]">
                    <div className="h-full rounded-full bg-[#F5364E]" style={{ width: `${dim.yours}%` }} />
                  </div>
                  <div className="h-[6px] flex-1 overflow-hidden rounded-full bg-[#1a1a1e]">
                    <div className="h-full rounded-full bg-[#555]" style={{ width: `${dim.compAvg}%` }} />
                  </div>
                </div>
      ...
      <div className={CARD}>
        ...
        <div className="space-y-2">
          {ownProfile && (
            <div className="flex items-center gap-2">
              <span className="w-[80px] truncate text-[10px] text-[#F5364E]">You</span>
              <div className="h-[8px] flex-1 overflow-hidden rounded-full bg-[#1a1a1e]">
                <div
                  className="h-full rounded-full bg-[#F5364E]"
                  style={{ width: `${Math.min(100, ((ownProfile.blogPostsPerMonth || 0) / 20) * 100)}%` }}
                />
              </div>
              <span className="w-[40px] text-right font-mono text-[10px] text-white">{ownProfile.blogPostsPerMonth || 0}/mo</span>
            </div>
          )}
          {activeComps.map((comp, i) => (
            <div key={comp.domain} className="flex items-center gap-2">
              <span className="w-[80px] truncate text-[10px] text-[#888]">{comp.domain}</span>
              <div className="h-[8px] flex-1 overflow-hidden rounded-full bg-[#1a1a1e]">
                <div
                  className="h-full rounded-full"
                  style={{
                    width: `${Math.min(100, ((comp.blogPostsPerMonth || 0) / 20) * 100)}%`,
                    backgroundColor: ['#6366f1', '#06b6d4', '#f59e0b', '#10b981', '#ec4899'][i % 5],
                  }}
                />
              </div>
              <span className="w-[40px] text-right font-mono text-[10px] text-[#888]">{comp.blogPostsPerMonth || 0}/mo</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {vulnerabilities.length > 0 && (
        <div className={CARD_HIGHLIGHT}>
          <div className="mb-1 text-[10px] font-bold uppercase tracking-wider text-[#F5364E]">#1 Priority</div>
      ...
      <div className={CARD_HIGHLIGHT}>
        {!actionPlan ? (
          ...
          <>
            <div className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-[#F5364E]">30-Day Priority</div>
            <p className="text-[12px] leading-relaxed text-[#ccc]">{actionPlan}</p>
            <button onClick={() => setActionPlan(null)} className="mt-2 text-[10px] text-[#555] hover:text-[#888]">
              Dismiss
            </button>
          </>
        )}
