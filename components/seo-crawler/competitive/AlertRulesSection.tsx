...
export default function AlertRulesSection() {
	...
	const [newOperator, setNewOperator] =
		useState<AlertRule["operator"]>("increases_by");
		...
		<div className="rounded-xl border border-[#222] bg-[#0d0d0f] p-4">
			<div className="mb-3 flex items-center justify-between">
				...
				<button
					...
					className="flex items-center gap-1 text-[10px] font-bold text-[#F5364E] hover:text-white"
				>
					<Plus size={10} />
					Add Rule
				</button>
			...
			{adding && (
				<div className="mb-3 space-y-2 rounded-lg border border-[#333] bg-[#111] p-3">
					<div className="text-[10px] text-[#888]">
						Alert me when any competitor's...
					</div>
					<select
						value={newMetric}
					...
					<select
						value={newOperator}
						onChange={(e) =>
							setNewOperator(e.target.value as AlertRule["operator"])
						}
						className="w-full rounded border border-[#333] bg-[#0a0a0a] px-2 py-1.5 text-[11px] text-white"
					>
					...
					<div className="flex gap-2">
						<button
							onClick={addRule}
							className="rounded bg-[#F5364E]/10 px-3 py-1.5 text-[10px] font-bold text-[#F5364E] hover:bg-[#F5364E]/20"
						>
							Save Rule
						</button>
						<button
			...
			{rules.length === 0 && !adding && (
				<div className="py-4 text-center text-[11px] text-[#444]">
					No alert rules set. Add rules to get notified about competitor
					movements.
				</div>
			)}
			...
			{rules.map((rule) => {
				const metricLabel =
					METRIC_OPTIONS.find((o) => o.value === rule.metric)?.label ||
					rule.metric;
				const operatorLabel =
				...
				return (
					<div
						key={rule.id}
						className={`mb-1.5 flex items-center gap-2 rounded-lg px-3 py-2 ${rule.enabled ? "bg-[#111]" : "bg-[#0a0a0a] opacity-50"}`}
					>
						<button
							onClick={() => toggleRule(rule.id)}
							className={`h-3 w-3 rounded-full border ${rule.enabled ? "border-green-500 bg-green-500" : "border-[#555]"}`}
						/>
						<span className="flex-1 text-[10px] text-[#aaa]">
							{metricLabel} {operatorLabel}{" "}
