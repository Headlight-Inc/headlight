...
function findClosestPage(pages: any[], targetTitle: string): any | null {
	...
	for (const page of pages) {
		...
		const pageWords = new Set(
			pageTitle.split(/\s+/).filter((w: string) => w.length > 3),
		);
		let overlap = 0;
...
export default function PageCompareDrawer({
	competitorUrl,
	competitorTitle,
	onClose,
}: Props) {
	...
	const yourPage = useMemo(
		() => findClosestPage(pages, competitorTitle),
		[pages, competitorTitle],
	);
	...
	const rows = [
		...
		{ label: "H1", yours: yourPage?.h1_1 || "—", theirs: "—" },
		{
			...
			yours: yourPage?.metaDesc?.substring(0, 80) || "—",
			theirs: "—",
		},
		{
			label: "Schema Types",
			yours: yourPage?.schemaTypes?.join(", ") || "None",
			theirs: "—",
		},
		...
		{
			label: "Load Time",
			yours: yourPage?.loadTime ? `${yourPage.loadTime}ms` : "—",
			theirs: "—",
		},
	];
					...
					<table className="w-full">
						<thead>
							<tr className="border-b border-[#222]">
								<th className="w-[120px] pb-2 text-left text-[10px] font-bold uppercase text-[#666]">
									Metric
								</th>
								<th className="pb-2 text-left text-[10px] font-bold uppercase text-[#F5364E]">
									Your Page
								</th>
								<th className="pb-2 text-left text-[10px] font-bold uppercase text-[#6366f1]">
									Their Page
								</th>
							</tr>
						</thead>
						<tbody>
							{rows.map((row) => (
								<tr key={row.label} className="border-b border-[#111]">
									<td className="py-2.5 text-[11px] text-[#666]">
										{row.label}
									</td>
									<td className="max-w-[200px] truncate py-2.5 text-[11px] text-[#ccc]">
										{row.yours ?? "—"}
									</td>
									<td className="max-w-[200px] truncate py-2.5 text-[11px] text-[#ccc]">
										{row.theirs ?? "—"}
