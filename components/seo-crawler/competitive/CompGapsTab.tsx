...
export default function CompGapsTab() {
	...
	const linkGaps = useMemo(() => {
		...
		const compAvgRD =
			activeComps.reduce((sum, c) => sum + Number(c.referringDomains || 0), 0) /
			activeComps.length;
		const gap = Math.round(compAvgRD - yourRD);
		const yourUR = Number(ownProfile.urlRating || 0);
		const compAvgUR =
			activeComps.reduce((sum, c) => sum + Number(c.urlRating || 0), 0) /
			activeComps.length;
	...
	const intentDistribution = useMemo(() => {
		...
		const yourTotal =
			Object.values(yourIntents).reduce((a, b) => a + b, 0) || 1;
		const theirTotal =
			Object.values(theirIntents).reduce((a, b) => a + b, 0) || 1;

		return ["informational", "commercial", "transactional", "navigational"].map(
			(intent) => ({
				intent,
							...
							<div className="mt-2 grid grid-cols-3 gap-2">
								<div className="text-center">
									<div className="font-mono text-[18px] font-black text-white">
										{contentGaps.commonKeywords.length}
									</div>
								...
								<div className="text-center">
									...
									<div className="text-[9px] uppercase text-[#666]">
										Your Unique
									</div>
								</div>
								<div className="text-center">
									<div className="font-mono text-[18px] font-black text-red-400">
										{keywordGaps.length}
									</div>
									<div className="text-[9px] uppercase text-red-400/60">
										Gaps
									</div>
								</div>
							</div>
						...
						<div className={CARD}>
							...
							<div className="custom-scrollbar max-h-[300px] space-y-1 overflow-y-auto">
								{keywordGaps.slice(0, 15).map((gap, i) => (
									<div
										key={`${gap.keyword}-${i}`}
										className="flex items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-[#111]"
									>
										<button
										...
										{gap.intent && (
											<span className="rounded bg-[#111] px-1.5 py-0.5 text-[8px] font-bold uppercase text-[#888]">
												{gap.intent.slice(0, 4)}
											</span>
										)}
										{gap.volume != null && (
										...
										{gap.position != null && (
											<span className="font-mono text-[10px] text-[#F5364E]">
												#{gap.position}
											</span>
										)}
									</div>
								...
								{intentDistribution.map((item) => (
									<div key={item.intent}>
										<div className="mb-1 flex items-center justify-between">
											<span className="text-[11px] capitalize text-[#888]">
												{item.intent}
											</span>
											<span className="text-[10px] text-[#555]">
												You {item.yourPct}% · Them {item.theirPct}%
											</span>
										</div>
				...
				{subTab === "content" && (
					<>
								...
								{[
									{
										...
										avg: avgOf("totalIndexablePages"),
									},
									{
										label: "Posts/Month",
										yours: ownProfile?.blogPostsPerMonth,
										avg: avgOf("blogPostsPerMonth"),
									},
									{
										label: "Avg Words/Page",
										yours:
											ownProfile?.avgContentLength ||
											ownProfile?.avgWordsPerArticle,
										avg:
											avgOf("avgContentLength") || avgOf("avgWordsPerArticle"),
									},
									{
										label: "Topic Breadth",
										yours: ownProfile?.topicCoverageBreadth,
										avg: avgOf("topicCoverageBreadth"),
									},
									{
										label: "Content Freshness",
										yours: ownProfile?.contentFreshnessScore,
										avg: avgOf("contentFreshnessScore"),
										unit: "%",
									},
									{
										label: "Schema Coverage",
										yours: ownProfile?.schemaCoveragePct,
										avg: avgOf("schemaCoveragePct"),
										unit: "%",
									},
									{
										label: "FAQ/How-To Pages",
										yours: ownProfile?.faqHowToCount,
										avg: avgOf("faqHowToCount"),
									},
								].map((row) => {
									...
									return (
										...
										>
											...
											<span className="w-[60px] text-right font-mono text-white">
												{yours.toLocaleString()}
												{row.unit || ""}
											</span>
											<span className="w-[60px] text-right font-mono text-[#666]">
												{avg.toLocaleString()}
												{row.unit || ""}
											</span>
											<span
												className={`w-[24px] text-right text-[10px] ${winning ? "text-green-400" : "text-red-400"}`}
											>
												{winning ? "✓" : "✗"}
											</span>
										</div>
						...
						<div className={CARD}>
							...
							<GapList
								items={contentGaps.uniqueKeywords
									.slice(0, 10)
									.map((kw) => ({ keyword: kw }))}
						...
						{linkGaps && (
							<>
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											...
											<span className="font-mono text-[14px] font-bold text-white">
												{linkGaps.yourRD.toLocaleString()}
											</span>
										</div>
										<div className="flex items-center justify-between">
											<span className="text-[11px] text-[#888]">
											...
											<span className="font-mono text-[14px] font-bold text-white">
												{linkGaps.compAvgRD.toLocaleString()}
											</span>
										</div>
										...
										<div className="flex items-center justify-between">
											...
											<span
												className={`font-mono text-[16px] font-black ${linkGaps.gap > 0 ? "text-red-400" : "text-green-400"}`}
											>
												{linkGaps.gap > 0 ? "+" : ""}
								...
								<div className={CARD}>
									...
									<div className="flex items-center justify-between">
										<span className="text-[11px] text-[#888]">
											Competitor Avg new RD
										</span>
										<span className="font-mono text-[14px] font-bold text-white">
											+
											{Math.round(
												activeComps.reduce(
													(sum, c) => sum + Number(c.linkVelocity60d || 0),
													0,
								...
								<div className={CARD}>
									<div className={SECTION_HEADER_WITH_MARGIN}>
										URL Rating Comparison
									</div>
									<div className="flex items-center gap-3">
										<div className="flex-1 text-center">
											<div className="font-mono text-[20px] font-black text-white">
												{linkGaps.yourUR}
											</div>
											<div className="text-[9px] uppercase text-[#666]">
												You
										...
										<div className="flex-1 text-center">
											<div
												className={`font-mono text-[20px] font-black ${linkGaps.compAvgUR > linkGaps.yourUR ? "text-red-400" : "text-green-400"}`}
											>
												{linkGaps.compAvgUR}
											</div>
											<div className="text-[9px] uppercase text-[#666]">
