...
export default function SiteExplorer({ embedded = false }: SiteExplorerProps) {
	...
	const [contextMenu, setContextMenu] = useState<CategoryMenuState | null>(
		null,
	);
	...
	const allCategories = useMemo(() => {
		const baseCategories =
			prioritizeByIssues && pages.length > 0
				? prioritizedCategories
				: CATEGORIES;
		const shouldScopeByAuditFilter = !(
			auditFilter.modes.includes("full") && auditFilter.industry === "all"
		);
		const scopedBaseCategories = shouldScopeByAuditFilter
			? baseCategories.filter(
					(category) =>
						activeCheckCategories.has(category.id) ||
						category.id === "internal",
				)
			: baseCategories;

		const showAiInsightsCategory =
			hasAiInsights &&
			(!shouldScopeByAuditFilter || activeCheckCategories.has("ai-insights"));
		const showAiClusterCategory =
			dynamicClusters.length > 0 &&
			(!shouldScopeByAuditFilter || activeCheckCategories.has("ai-clusters"));
	...
	const handleCategorySelect = useCallback(
		(event: React.MouseEvent, group: string, sub: string) => {
			setActiveMacro(null);

			if (event.shiftKey) {
				setActiveCategories((previous) => {
					const withoutDefault = previous.filter(
						(entry) => !(entry.group === "internal" && entry.sub === "All"),
					);
					const exists = withoutDefault.some(
						(entry) => entry.group === group && entry.sub === sub,
					);
					if (exists) {
						const next = withoutDefault.filter(
							(entry) => !(entry.group === group && entry.sub === sub),
						);
		...
		>
			...
			<div className="px-2 py-2 border-b border-[#1a1a1a] shrink-0 space-y-2">
				...
				<div className="relative">
					<Search
						className="absolute left-2 top-1/2 -translate-y-1/2 text-[#444]"
						size={11}
					/>
					<input
						type="text"
			...
			<div className="flex-1 overflow-y-auto custom-scrollbar py-1 px-1">
				{filteredCategories.map((category) => {
					...
					const catCounts = (categoryCounts[category.id] || {}) as Record<
						string,
						number
					>;
					const totalCatCount =
								...
								<div className="ml-[18px] pl-3 my-1 space-y-0.5 border-l border-[#222]">
									{category.sub.map((subItem: string) => {
										const isActive = activeCategories.some(
											(s) => s.group === category.id && s.sub === subItem,
										);
										...
										return (
											<button
												key={subItem}
												onClick={(e) =>
													handleCategorySelect(e, category.id, subItem)
												}
												onContextMenu={(e) => {
													e.preventDefault();
													setContextMenu({
