			...
			{(title || action) && (
				<div className="flex items-center justify-between mb-3">
					<div className="min-w-0">
						{title && (
							<div className="text-[11px] font-bold uppercase tracking-widest text-white truncate">
								{title}
							</div>
						)}
						{subtitle && (
							<div className="text-[10px] text-[#666] truncate">{subtitle}</div>
						)}
					</div>
					{action}
				</div>
...
export default function WqaReportsView() {
	...
	const industry =
		wqaState?.effectiveIndustry || wqaState?.industry || "general";
	const stats = useMemo(
		() => computeWqaSiteStats(pages, industry),
		[pages, industry],
	);
	...
	const searchBands = [
		...
		{
			band: "Striking",
			count: facets.searchStatuses.striking,
			color: TONE.warn,
		},
	...
	const trafficTrend = [
		{
			state: "Growing",
			count: facets.trafficStatuses.growing,
			color: TONE.good,
		},
		{ state: "Stable", count: facets.trafficStatuses.stable, color: TONE.info },
		{
			state: "Declining",
			count: facets.trafficStatuses.declining,
			color: TONE.bad,
		},
	...
	const funnelData = Object.entries(facets.funnelStages || {}).map(
		([stage, count]) => ({ stage, count: Number(count) }),
	);
	...
	const ctrScatter = pages
		...
		.slice(0, 300)
		.map((p: any) => ({
			pos: Number(p.gscPosition || 0),
			ctr: Number(p.gscCtr || 0) * 100,
			impr: Number(p.gscImpressions || 0),
		}));
	...
	const topActions = [...actions]
		...
		.map((a) => ({
			label: a.action,
			impact: Math.round(a.totalEstimatedImpact),
			pages: a.pageCount,
			category: a.category,
		}));
	...
	const indexabilityData = [
		{
			state: "Indexed",
			count: facets.indexabilities.indexed,
			color: TONE.good,
		},
		{
			state: "Blocked",
			count: facets.indexabilities.blocked,
			color: TONE.warn,
		},
		{
			state: "Redirect",
			count: facets.indexabilities.redirect,
			color: TONE.info,
		},
		...
		return (
			<div className="flex-1 flex flex-col bg-[#070707]">
				<EmptyViewState
					...
					subtitle="Run a crawl or clear filters to see charts."
				/>
			</div>
		);
	}
				...
				<div className="wqa-reports-grid grid grid-cols-12 gap-3 auto-rows-[minmax(160px,auto)]">
					...
					<Card col={2} title="Pages" subtitle="Current filter">
						<Kpi
							...
							tone="white"
							hint={`${stats.indexedPages.toLocaleString()} indexed`}
						/>
					</Card>
					<Card col={2} title="Search" subtitle="GSC totals">
						<Kpi
							...
							tone="info"
							hint={`${stats.totalClicks.toLocaleString()} clicks`}
						/>
					</Card>
					...
					<Card col={2} title="Avg position" subtitle="GSC position">
						<Kpi
							...
							tone="warn"
							hint={`${stats.pagesInStrikingDistance} striking`}
						/>
					</Card>
					<Card col={2} title="Decay risk" subtitle="Pages at risk">
						<Kpi
							label="Pages"
							value={stats.decayRiskCount}
							tone="bad"
							hint={`${stats.pagesLosingTraffic} losing traffic`}
						/>
					</Card>
					<Card col={2} title="Health" subtitle="Site composite">
						<Kpi
							label="Avg score"
							value={Math.round(stats.avgHealthScore)}
							tone={
					...
					<Card
						col={6}
						title="Quality radar"
						subtitle="Six composite dimensions"
					>
					...
					<Card
						col={6}
						title="Value tiers"
						subtitle="Distribution of business value"
					>
						<ResponsiveContainer width="100%" height={220}>
							<BarChart
								data={valueTierData}
					...
					<Card col={6} title="Search position bands">
						<ResponsiveContainer width="100%" height={220}>
							...
							>
								...
								>
									{searchBands.map((b, i) => (
										<Bar key={i} dataKey="count" fill={b.color} />
									))}
								</Bar>
							</BarChart>
						</ResponsiveContainer>
					...
					<Card
						...
						subtitle="Treemap sized by page count"
					>
					...
					<Card
						...
						subtitle="Each page (capped at 300)"
					>
						<ResponsiveContainer width="100%" height={260}>
							<LineChart
								data={ctrScatter.slice().sort((a, b) => a.pos - b.pos)}
							...
							>
								...
								<Line
									...
									stroke={ACCENT}
									strokeWidth={0}
									dot={{ r: 2, stroke: ACCENT, fill: ACCENT }}
					...
					<Card
						col={9}
						title="Top actions by estimated impact"
						subtitle="Ranked across technical, content, industry"
					>
						<ResponsiveContainer width="100%" height={220}>
							<BarChart
								data={topActions}
					...
					<Card
						col={12}
						title="Site-level coverage"
						subtitle="Percentages across crawled pages"
					>
					...
					<Card col={6} title="Commercial signals">
						<div className="grid grid-cols-2 gap-4">
							...
							<Kpi
								label="Goal comps."
								value={stats.totalGoalCompletions.toLocaleString()}
								tone="violet"
							/>
							<Kpi
								label="Subscribers"
					...
					<Card col={6} title="Risk signals">
						<div className="grid grid-cols-2 gap-4">
							<Kpi
								...
								value={stats.orphanPagesWithValue}
								tone="warn"
								hint="Inlinks=0 but traffic > 0"
							/>
							<Kpi
								label="Cannibalizing"
								value={stats.cannibalizationCount}
								tone="bad"
								hint="Competing URLs on same query"
							/>
							<Kpi
								label="Zero impressions"
								value={stats.pagesWithZeroImpressions}
								tone="bad"
								hint="Indexed but invisible"
							/>
							<Kpi
								label="Est. impact"
								value={Math.round(stats.totalEstimatedImpact).toLocaleString()}
								tone="good"
								hint="Sum across actions"
							/>
						</div>
					</Card>
					...
					<Card
						col={12}
						title="Clicks distribution by position"
						subtitle="Area curve, pages aggregated per integer position"
					>
						<ResponsiveContainer width="100%" height={220}>
							<AreaChart
								data={Array.from({ length: 30 }, (_, i) => {
									const pos = i + 1;
									const bucket = pages.filter(
										(p: any) => Math.round(Number(p.gscPosition || 0)) === pos,
									);
									return {
										pos,
										clicks: bucket.reduce(
											(s: number, p: any) => s + Number(p.gscClicks || 0),
											0,
										),
									};
								})}
