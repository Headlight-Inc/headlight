...
const PALETTE = ['#F5364E', '#3b82f6', '#22c55e', '#f59e0b', '#a855f7', '#06b6d4', '#ec4899', '#84cc16', '#eab308', '#64748b'];

export default function MapClusterMode({ pages }: Props) {
    const { segments, rows } = useMemo(() => {
        const buckets: Record<string, { count: number; impressions: number; clicks: number; sessions: number }> = {};
        for (const p of pages) {
            const cat = String(p.pageCategory || 'other');
            if (!buckets[cat]) buckets[cat] = { count: 0, impressions: 0, clicks: 0, sessions: 0 };
            buckets[cat].count++;
        ...
        return {
            segments: entries.map(([label, v], i) => ({ label: formatCat(label), value: v.count, color: PALETTE[i % PALETTE.length] })),
            rows: entries.map(([label, v], i) => ({ label: formatCat(label), ...v, color: PALETTE[i % PALETTE.length] })),
        };
    }, [pages]);
        ...
        <div className="absolute inset-0 grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-0">
            ...
            <div className="overflow-auto custom-scrollbar p-6">
                <table className="w-full text-[12px]">
                    ...
                    <tbody>
                        {rows.map(r => (
                            <tr key={r.label} className="border-b border-[#121212] hover:bg-[#0d0d0d]">
                                <td className="py-2">
                                ...
                                <td className="py-2 text-right font-mono text-white">{r.count.toLocaleString()}</td>
                                <td className="py-2 text-right font-mono text-[#888]">{Math.round((r.count / totalPages) * 100)}%</td>
                                <td className="py-2 text-right font-mono text-[#888]">{r.impressions.toLocaleString()}</td>
                                <td className="py-2 text-right font-mono text-[#888]">{r.clicks.toLocaleString()}</td>
                                <td className="py-2 text-right font-mono text-[#888]">{r.sessions.toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
