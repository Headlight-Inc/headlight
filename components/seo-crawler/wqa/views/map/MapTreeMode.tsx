		...
		return (
			<ul
				className={
					depth === 0
						? "list-none pl-0"
						: "list-none pl-4 border-l border-[#1a1a1a]"
				}
			>
				{kids.map((k) => {
					const bad = k.issues / Math.max(1, k.count);
					const tone =
						bad > 0.2 ? "#ef4444" : bad > 0.05 ? "#f59e0b" : "#22c55e";
					return (
						<li key={k.path} className="py-0.5">
							...
							>
								...
								<span className="text-[10px] text-[#555] ml-auto pl-2 shrink-0">
									...
									{k.issues > 0 && (
										<span className="text-red-400"> · {k.issues} issues</span>
									)}
								</span>
							</button>
	...
	return (
		<div className="absolute inset-0 overflow-auto custom-scrollbar p-6">
			<div className="max-w-3xl">
				<div className="text-[10px] uppercase tracking-widest text-[#555] mb-2">
					Root
				</div>
				<div className="flex items-center gap-2 mb-3">
					...
					<span className="text-[10px] text-[#555] ml-auto">
						{tree.count.toLocaleString()} pages
					</span>
				</div>
				{render(tree)}
			</div>
