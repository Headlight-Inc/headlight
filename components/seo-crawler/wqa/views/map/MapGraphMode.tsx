...
export default function MapGraphMode({ pages }: Props) {
	...
	useEffect(() => {
		...
		(async () => {
			...
			const { default: ForceGraph2D } =
				await import("react-force-graph-2d").catch(() => ({
					default: null as any,
				}));
		...
		<div ref={ref} className="absolute inset-0 bg-[#070707]">
			{ForceGraph && graph.nodes.length > 0 && (
				<ForceGraph
					...
					linkColor={(l: any) =>
						l.isStructural ? "rgba(245,54,78,0.35)" : "rgba(255,255,255,0.08)"
					}
					linkWidth={(l: any) => (l.isStructural ? 0.8 : 0.3)}
					nodeLabel={(n: any) =>
						`${n.title || n.name} — depth ${n.crawlDepth}, in ${n.inlinks}`
					}
					onNodeClick={(n: any) => {
