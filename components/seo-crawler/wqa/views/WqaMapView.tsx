...
const SUBMODES: Array<{ id: WqaMapSubmode; label: string; Icon: React.ElementType; hint: string }> = [
    { id: 'tree',    label: 'Tree',    Icon: GitBranch, hint: 'URL hierarchy by path' },
    { id: 'cluster', label: 'Cluster', Icon: Layers,    hint: 'Grouped by page category' },
...
export default function WqaMapView() {
    ...
    const body = useMemo(() => {
        ...
        switch (submode) {
            case 'cluster': return <MapClusterMode pages={filteredPages} />;
            case 'graph':   return <MapGraphMode   pages={filteredPages} />;
            case 'tree':
            default:        return <MapTreeMode    pages={filteredPages} />;
        }
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-2.5 h-[24px] text-[11px] font-medium rounded flex items-center gap-1.5 transition-colors ${
                                        active ? 'bg-[#1a1a1a] text-white' : 'text-[#888] hover:text-[#ddd] hover:bg-[#141414]'
                                    }`}
                                >
