...
export default function WqaTechTab() {
    ...
    const techTallies = useMemo(() => {
        ...
        let httpInsecure = 0, sslIssues = 0, missingHsts = 0;
        for (const p of (pages || [])) {
        ...
        return { broken, redirects, nonIndexable, canonMissing, slow, httpInsecure, sslIssues, missingHsts };
    }, [pages]);
    ...
    return (
        <div className="space-y-4">
            ...
            <Card>
                ...
                <Row label="Non-indexable"   value={fmtInt(techTallies.nonIndexable)} tone={techTallies.nonIndexable > 0 ? 'warn' : 'neutral'} />
                <Row label="Broken (4xx/5xx)" value={fmtInt(techTallies.broken)}    tone={techTallies.broken > 0 ? 'bad' : 'good'} />
                <Row label="Redirects"       value={fmtInt(techTallies.redirects)}  tone={techTallies.redirects > 0 ? 'warn' : 'neutral'} />
                <Row label="Missing canonical" value={fmtInt(techTallies.canonMissing)} tone={techTallies.canonMissing > 0 ? 'warn' : 'neutral'} />
            </Card>
            ...
            <Card>
                ...
                <div className="mt-3">
                    <Row label="Slow (>1.5s TTFB)" value={fmtInt(techTallies.slow)} tone={techTallies.slow > 0 ? 'warn' : 'good'} />
                </div>
            </Card>

            <Card>
                <SectionTitle title="Security" />
                <Row label="HTTP pages"    value={fmtInt(techTallies.httpInsecure)} tone={techTallies.httpInsecure > 0 ? 'bad' : 'good'} />
                <Row label="SSL issues"    value={fmtInt(techTallies.sslIssues)}    tone={techTallies.sslIssues > 0 ? 'bad' : 'good'} />
                <Row label="Missing HSTS"  value={fmtInt(techTallies.missingHsts)}  tone={techTallies.missingHsts > 0 ? 'warn' : 'good'} />
            </Card>
                ...
                {industry === 'news' && (
                    <>
                        <Row label="News sitemap" value={fmtPct(stats.newsSitemapCoverage, 1)}
                             hint="Required for Google News" tone={stats.newsSitemapCoverage > 0 ? 'good' : 'warn'} />
