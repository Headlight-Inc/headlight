...
export default function WqaOverviewTab() {
	const { pages, wqaState, filteredWqaPagesExport, setWqaFilter, wqaFilter } =
		useSeoCrawler();
	...
	const indexabilityMix = [
		...
		{
			label: "Blocked",
			value: Math.max(0, stats.htmlPages - stats.indexedPages),
			color: "#ef4444",
		},
	];
		...
		<div className="space-y-4">
			...
			<Card>
				<div className="flex items-start justify-between">
					<div>
						...
						<div className="flex items-baseline gap-2 mt-0.5">
							<div
								className={`text-[40px] font-black leading-none ${scoreTone(score) === "good" ? "text-green-400" : scoreTone(score) === "warn" ? "text-orange-400" : "text-red-400"}`}
							>
								{score}
							</div>
					...
					<div className="text-right text-[10px] text-[#888] space-y-1">
						<div className="flex items-center gap-1 justify-end">
							<Globe size={10} />
							{fmtInt(stats.totalPages)} pages
						</div>
						<div className="flex items-center gap-1 justify-end">
							<Languages size={10} />
							{wqaState.detectedLanguage || "—"}
						</div>
						<div className="flex items-center gap-1 justify-end">
							<Server size={10} />
				...
				<div className="mt-3 flex flex-wrap gap-1">
					...
					{wqaState.isMultiLanguage && (
						<Chip label="Multi-language" tone="neutral" />
					)}
					{wqaState.isLowIndustryConfidence && (
						<Chip label="Low confidence" tone="warn" />
					)}
				</div>
			</Card>
			...
			<div>
				...
				<div className="grid grid-cols-2 gap-1.5">
					<StatTile
						label="Impressions"
						value={fmtInt(stats.totalImpressions)}
					/>
			...
			<Card>
				<SectionTitle
					...
					hint={`${fmtInt(stats.totalPages)} pages`}
				/>
			...
			<div>
				...
				<div className="grid grid-cols-2 gap-1.5">
					<button
						onClick={() =>
							setWqaFilter({ ...wqaFilter, trafficStatus: "declining" })
						}
						className="text-left"
					>
						<StatTile
							label="Losing traffic"
							value={fmtInt(stats.pagesLosingTraffic)}
							tone={stats.pagesLosingTraffic > 0 ? "bad" : "neutral"}
						/>
					</button>
					<button
					...
					<StatTile
						...
						value={fmtInt(stats.orphanPagesWithValue)}
						tone={stats.orphanPagesWithValue > 0 ? "warn" : "neutral"}
					/>
					<StatTile
						label="Cannibalization"
						value={fmtInt(stats.cannibalizationCount)}
						tone={stats.cannibalizationCount > 0 ? "warn" : "neutral"}
					/>
					<button
						onClick={() =>
					...
					>
						<StatTile
							...
							tone={stats.pagesInStrikingDistance > 0 ? "accent" : "neutral"}
						/>
					</button>
					<StatTile
						label="Decay risk"
						value={fmtInt(stats.decayRiskCount)}
						tone={stats.decayRiskCount > 0 ? "warn" : "neutral"}
					/>
				</div>
			</div>
			...
			{categorySunburst.length > 0 && (
				<Card>
					<SectionTitle
						...
						hint={`${Object.keys(stats.pagesByCategory).length} types`}
					/>
						...
						<div className="flex-1 space-y-0.5">
							{categorySunburst.slice(0, 6).map((c) => (
								<button
									key={c.label}
									onClick={() =>
										setWqaFilter({ ...wqaFilter, pageCategory: c.label })
									}
									className="w-full"
								>
			...
			<Card>
				...
				<Row
					label="Visible pages"
					value={`${fmtInt(filteredWqaPagesExport.length)} / ${fmtInt(stats.totalPages)}`}
				/>
				<div className="mt-2">
					<Bar
						pct={
							stats.totalPages > 0
								? (filteredWqaPagesExport.length / stats.totalPages) * 100
								: 0
						}
					/>
				</div>
