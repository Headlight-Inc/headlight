...
export default function WqaSidebarRouter({
	embedded = false,
}: {
	embedded?: boolean;
}) {
	...
	const Body = (() => {
		switch (wqaSidebarTab) {
			case "wqa_actions":
				return <WqaActionsTab />;
			case "wqa_search":
				return <WqaSearchTab />;
			case "wqa_content":
				return <WqaContentTab />;
			case "wqa_tech":
				return <WqaTechTab />;
			case "wqa_overview":
			default:
				return <WqaOverviewTab />;
		}
	})();
	...
	return (
		...
		>
			...
			<div className="flex-1 overflow-y-auto custom-scrollbar p-3">{Body}</div>
		</aside>
	);
}
