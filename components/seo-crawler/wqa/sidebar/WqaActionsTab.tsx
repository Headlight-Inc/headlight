...
const CATEGORY_META: Record<
	"technical" | "content" | "industry",
	{ label: string; Icon: React.ElementType; tone: "accent" | "good" | "warn" }
> = {
...
export default function WqaActionsTab() {
	...
	const [filterCat, setFilterCat] = useState<
		"all" | "technical" | "content" | "industry"
	>("all");
	...
	const sorted = [...filteredGroups].sort(
		(a, b) => b.totalEstimatedImpact - a.totalEstimatedImpact,
	);
	...
	const totals = useMemo(
		() => ({
			technical: groups
				...
				.reduce((s, g) => s + g.pageCount, 0),
			content: groups
				.filter((g) => g.category === "content")
				.reduce((s, g) => s + g.pageCount, 0),
			industry: groups
				.filter((g) => g.category === "industry")
				.reduce((s, g) => s + g.pageCount, 0),
	...
	return (
		<div className="space-y-4">
			<div className="grid grid-cols-2 gap-1.5">
				<StatTile
					label="Est. impact"
					value={fmtInt(totals.impact)}
					sub="monthly clicks"
					tone="accent"
				/>
				<StatTile
					label="Pages w/ action"
					value={fmtInt(totals.technical + totals.content + totals.industry)}
				/>
				<StatTile
					label="Technical"
					value={fmtInt(totals.technical)}
					tone="accent"
				/>
			...
			<div className="flex items-center gap-1 flex-wrap">
				...
				<Chip
					label="Technical"
					tone="accent"
					active={filterCat === "technical"}
					onClick={() => setFilterCat("technical")}
				/>
				<Chip
					label="Content"
					tone="warn"
					active={filterCat === "content"}
					onClick={() => setFilterCat("content")}
				/>
				<Chip
					label="Industry"
					tone="good"
					active={filterCat === "industry"}
					onClick={() => setFilterCat("industry")}
				/>
			</div>
					...
					sorted.map((group) => {
						...
						const isActiveFilter =
							(group.category === "technical" &&
								wqaFilter.technicalAction === group.action) ||
							(group.category === "content" &&
								wqaFilter.contentAction === group.action);
							...
							<Card key={`${group.category}:${group.action}`}>
								<div className="flex items-start justify-between gap-2">
									<div className="min-w-0 flex-1">
										...
										{group.reason && (
											<div className="text-[10px] text-[#777] mt-0.5 line-clamp-2">
												{group.reason}
											</div>
										)}
									</div>
									<button
										onClick={() => {
											if (group.category === "technical")
												setWqaFilter({
													...wqaFilter,
													technicalAction: isActiveFilter
														? "all"
														: group.action,
												});
											else if (group.category === "content")
												setWqaFilter({
													...wqaFilter,
													contentAction: isActiveFilter ? "all" : group.action,
												});
										}}
								...
								<div className="mt-2 grid grid-cols-2 gap-2">
									...
									<div>
										...
										<div className="text-[14px] font-mono font-black text-[#F5364E]">
											{fmtInt(group.totalEstimatedImpact)}
										</div>
									</div>
								</div>
								...
								<div className="mt-2">
									<Bar
										pct={(group.totalEstimatedImpact / maxImpact) * 100}
										tone={
											meta.tone === "accent"
												? "accent"
												: meta.tone === "warn"
