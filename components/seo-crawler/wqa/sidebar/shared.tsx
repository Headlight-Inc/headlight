...
export const SectionTitle = ({
	...
	<div className="flex items-baseline justify-between mb-2">
		<h4 className="text-[10px] font-black uppercase tracking-widest text-[#888]">
			{title}
		</h4>
...
export const Card = ({
	...
	<div
		className={`bg-[#0d0d0d] border border-[#1a1a1a] rounded-lg ${pad ? "p-3" : ""}`}
	>
		{children}
	</div>
);
...
export const StatTile = ({
	...
	return (
		<div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded px-2.5 py-2">
			...
			<div
				className={`text-[16px] font-black leading-tight mt-0.5 ${toneClass}`}
			>
				{value}
			</div>
			{sub && (
				<div className="text-[9px] text-[#666] mt-0.5 truncate">{sub}</div>
			)}
		</div>
	);
};
...
export const Row = ({
	...
	return (
		<div className="flex items-center justify-between py-1.5 border-b border-[#161616] last:border-b-0">
			...
			<div className={`text-[12px] font-mono font-bold shrink-0 ml-2 ${color}`}>
				{value}
			</div>
		</div>
	);
