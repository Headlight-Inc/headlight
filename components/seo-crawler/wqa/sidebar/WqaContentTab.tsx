...
export default function WqaContentTab() {
    ...
    const ageMix = [
        { label: 'Fresh',   value: wqaFacets.contentAges.fresh  || 0, color: '#22c55e' },
        { label: 'Aging',   value: wqaFacets.contentAges.aging  || 0, color: '#3b82f6' },
        { label: 'Stale',   value: wqaFacets.contentAges.stale  || 0, color: '#f59e0b' },
        { label: 'No date', value: wqaFacets.contentAges.nodate || 0, color: '#444' },
    ];

    const funnelMix = Object.entries(wqaFacets.funnelStages || {}).map(([label, value], i) => ({
        label, value,
        ...
        <div className="space-y-4">
            ...
            <Card>
                ...
                {stats.decayRiskCount > 0 && (
                    <button onClick={() => setWqaFilter({ ...wqaFilter, contentAge: 'stale' })} className="w-full mt-3">
                        <Row label="Decay risk pages" value={fmtInt(stats.decayRiskCount)} hint="Age + traffic loss + position drop" tone="warn" />
                    </button>
                )}
            </Card>
