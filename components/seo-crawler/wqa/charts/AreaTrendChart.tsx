			...
			{hoveredIdx !== null && (
				<div
					...
					style={{
						left: Math.min(
							width - 100,
							Math.max(0, points1[hoveredIdx].x - 60),
						),
						bottom: height - points1[hoveredIdx].y + 20,
					}}
				>
					<div className="text-[10px] font-bold text-white mb-1 border-b border-[#222] pb-0.5">
					...
					<div className="space-y-0.5">
						<div className="flex justify-between gap-4 text-[9px]">
							...
							<span className="font-mono font-bold text-white">
								{formatValue(data[hoveredIdx].value1)}
							</span>
						</div>
						{label2 && (
							<div className="flex justify-between gap-4 text-[9px]">
								<span className="text-[#888]">{label2}</span>
								<span className="font-mono font-bold text-[#F5364E]">
									{formatValue(data[hoveredIdx].value2 || 0)}
								</span>
							</div>
						)}
					</div>
