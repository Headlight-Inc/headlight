		...
		<div>
			<div className="grid grid-cols-1 lg:grid-cols-[1.1fr_1.4fr_1fr] gap-4 mb-5">
				...
				<div className="bg-[#0a0a0a] border border-[#222] rounded p-3 min-w-0 overflow-hidden flex flex-col">
					...
					<div className="flex flex-wrap gap-2 mt-2">
						...
						{Number(page?.wordCount || 0) > 0 && (
							<StatusBadge
								status="info"
								label={`${formatNumber(page?.wordCount)} words`}
							/>
						)}
						{page?.visibleDate && (
							<StatusBadge
								status="info"
								label={`Pub ${String(page.visibleDate).slice(0, 10)}`}
							/>
						)}
						{page?.lastModified && (
							<StatusBadge
								status="info"
								label={`Upd ${String(page.lastModified).slice(0, 10)}`}
							/>
						)}
					</div>
					<div className="grid grid-cols-2 sm:grid-cols-2 gap-x-3 gap-y-2 mt-3 pt-3 border-t border-[#1a1a1a]">
						<PageField
							label="Author"
							value={page?.author || page?.wpAuthorName}
						/>
						<PageField label="Topic" value={page?.topicCluster} />
						<PageField
							label="Section"
							value={page?.wpCategory || page?.section}
						/>
				...
				<div className="bg-[#0a0a0a] border border-[#222] rounded p-3 min-w-0">
					...
					<div className="space-y-2">
						...
						{!page?.technicalAction &&
							!page?.contentAction &&
							!page?.industryAction && (
								<div className="text-[12px] text-[#666] italic">
				...
				<div className="bg-[#0a0a0a] border border-[#222] rounded p-3 min-w-0">
					...
					<div className="grid grid-cols-2 gap-2">
						...
						<MetricCard
							label="Health"
							value={formatNumber(page?.healthScore)}
						/>
						...
						<MetricCard
							...
							color={
								Number(page?.sessionsDeltaPct || 0) < 0
									? "text-red-400"
									: "text-green-400"
							}
						/>
						<MetricCard
							label="Position"
							value={formatNumber(page?.gscPosition, {
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
				...
				<TrendCard
					...
					values={trend28d(page, "gscImpressions")}
				/>
				<TrendCard label="CTR 28d" values={trend28d(page, "gscCtr")} />
			</div>

			{Array.isArray(page?.secondaryActions) &&
				page.secondaryActions.length > 0 && (
					<div>
...
function PageField({ label, value }: { label: string; value: any }) {
	...
	return (
		<div className="min-w-0">
			<div className="text-[9px] uppercase tracking-wider text-[#555] mb-0.5">
				{label}
			</div>
			<div className="text-[11px] text-[#ccc] truncate" title={val}>
				{val}
			</div>
		</div>
	);
