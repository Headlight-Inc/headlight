...
export default function SearchTab({ page }: { page: any }) {
	...
	const topQueries = Array.isArray(page?.gscTopQueries)
		? page.gscTopQueries
		: [];
	const siblings = Array.isArray(page?.cannibalizationSiblings)
		? page.cannibalizationSiblings
		: [];
	const strikingKw = Array.isArray(page?.strikingDistanceKeywords)
		? page.strikingDistanceKeywords
		: [];

	if (!hasGsc) {
		return (
			<div className="bg-[#0a0a0a] border border-[#222] rounded p-5 text-center">
				<div className="text-[14px] text-white font-semibold mb-2">
					Google Search Console not connected
				</div>
				<div className="text-[12px] text-[#666]">
					Connect GSC in Integrations to enable search-performance actions.
				</div>
			</div>
		);
	}
		...
		<div>
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
				...
				<MetricCard
					...
					value={formatNumber(page?.gscImpressions)}
				/>
				<MetricCard
					label="CTR"
					value={formatPercent(page?.gscCtr, 100)}
					sub={`Gap ${formatPercent(ctrGap, 100)}`}
				/>
				<MetricCard
					label="Avg position"
					value={formatNumber(page?.gscPosition, { maximumFractionDigits: 1 })}
				/>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
				...
				<SparkCard
					label="Impressions 28d"
					values={seriesOf(page, "gscImpressions")}
				/>
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 gap-4 xl:gap-8 mb-5">
				<div>
					...
					<DataRow
						label="Main position"
						value={formatNumber(page?.mainKwPosition, {
							maximumFractionDigits: 1,
						})}
					/>
					<DataRow
						label="Main volume"
						value={formatNumber(page?.mainKwVolume)}
					/>
					<DataRow label="Best KW" value={page?.bestKeyword} />
					<DataRow
						label="Best position"
						value={formatNumber(page?.bestKwPosition, {
							maximumFractionDigits: 1,
						})}
					/>
				</div>
				<div>
					...
					<div className="flex flex-wrap gap-2 mb-2">
						<StatusBadge
							...
							label={
								ctrGap < 0
									? `Underperforming CTR ${formatPercent(Math.abs(ctrGap), 100)}`
									: "CTR on benchmark"
							}
						/>
						<StatusBadge
						...
						<StatusBadge
							...
							label={
								page?.isCannibalized ? "Cannibalized" : "No cannibalization"
							}
						/>
					</div>
					<DataRow
						...
						value={
							Number(page?.gscPosition || 0) <= 3
								? "Top 3"
								: Number(page?.gscPosition || 0) <= 10
									? "Page 1"
									: Number(page?.gscPosition || 0) <= 20
										? "Page 2 (striking)"
										: "Beyond page 2"
						}
					/>
					<DataRow
						label="Expected CTR"
						value={formatPercent(page?.expectedCtr, 100)}
					/>
				</div>
			</div>
					...
					<table className="w-full text-[11px] font-mono">
						...
						<tbody>
							{topQueries.map((q: any, i: number) => (
								...
								>
									<td className="px-3 py-1.5 text-[#ddd]">
										{q?.query || q?.keyword || "—"}
									</td>
									<td className="px-3 py-1.5 text-[#bbb] text-right">
									...
									<td className="px-3 py-1.5 text-[#bbb] text-right">
										{formatPercent(q?.ctr, 100)}
									</td>
									<td className="px-3 py-1.5 text-[#bbb] text-right">
										{formatNumber(q?.position, { maximumFractionDigits: 1 })}
									</td>
									<td
										className={`px-3 py-1.5 text-right ${Number(q?.positionDelta || 0) < 0 ? "text-green-400" : Number(q?.positionDelta || 0) > 0 ? "text-red-400" : "text-[#666]"}`}
									>
										{q?.positionDelta
											? formatNumber(q.positionDelta, {
													maximumFractionDigits: 1,
												})
											: "—"}
									</td>
				...
				<>
					...
					<div className="flex flex-wrap gap-2 mb-5">
						{strikingKw.map((k: any, i: number) => (
							<span
								key={i}
								className="px-2 py-1 rounded border border-[#222] bg-[#0a0a0a] text-[11px] text-[#ccc]"
							>
								{k?.query}{" "}
								<span className="text-[#666] font-mono ml-1">
									pos {formatNumber(k?.position, { maximumFractionDigits: 1 })}
								</span>
							</span>
						))}
					</div>
							...
							<tbody>
								{siblings.map((s: any, i: number) => (
									<tr
										...
										className="border-b border-[#1a1a1a] hover:bg-[#111]"
									>
										<td className="px-3 py-1.5 text-blue-400 break-all">
											{s?.url}
										</td>
										<td className="px-3 py-1.5 text-[#bbb] text-right">
											pos{" "}
											{formatNumber(s?.position, { maximumFractionDigits: 1 })}
										</td>
										<td className="px-3 py-1.5 text-[#bbb] text-right">
											cl {formatNumber(s?.clicks)}
										</td>
									</tr>
								))}
							</tbody>
