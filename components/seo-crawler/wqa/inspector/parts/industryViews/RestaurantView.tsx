	...
	return (
		<div>
			...
			<div className="flex flex-wrap gap-2 mb-5">
				<StatusBadge
					status={schemaTypes.includes("Menu") ? "pass" : "warn"}
					label="Menu schema"
				/>
				<StatusBadge
					status={sig.hasReservationLink ? "pass" : "info"}
					label="Reservation link"
				/>
				<StatusBadge
					status={sig.hasHoursMarkup ? "pass" : "warn"}
					label="Hours markup"
				/>
				<StatusBadge
					status={sig.hasFoodImages ? "pass" : "info"}
					label="Food images present"
				/>
			</div>
