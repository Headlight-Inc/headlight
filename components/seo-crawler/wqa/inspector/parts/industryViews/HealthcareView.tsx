	...
	return (
		<div>
			...
			<div className="flex flex-wrap gap-2 mb-5">
				<StatusBadge
					status={sig.hasMedicalAuthor ? "pass" : "fail"}
					label="Medical author"
				/>
				<StatusBadge
					status={sig.hasMedicalReviewer ? "pass" : "warn"}
					label="Medical reviewer"
				/>
				<StatusBadge
					status={sig.hasMedicalDisclaimer ? "pass" : "warn"}
					label="Medical disclaimer"
				/>
				<StatusBadge
					status={sig.hasCitations ? "pass" : "info"}
					label={`Citations ${sig.citationCount ?? ""}`}
				/>
			</div>
