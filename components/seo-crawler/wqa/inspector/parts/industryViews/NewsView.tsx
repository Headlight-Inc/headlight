	...
	return (
		<div>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
				<MetricCard
					label="Byline"
					value={sig.hasAuthorAttribution ? "Yes" : "No"}
					color={sig.hasAuthorAttribution ? "text-green-400" : "text-red-400"}
				/>
				<MetricCard
					label="Pub date"
				...
				<MetricCard
					label="News sitemap"
					value={page?.inNewsSitemap ? "Yes" : "No"}
					color={page?.inNewsSitemap ? "text-green-400" : "text-red-400"}
				/>
			</div>
			...
			<div className="flex flex-wrap gap-2 mb-5">
				...
				/>
				<StatusBadge
					status={sig.hasPublishDateMarkup ? "pass" : "warn"}
					label="Publish date markup"
				/>
				<StatusBadge
					status={sig.hasUpdateDateMarkup ? "pass" : "info"}
					label="Update date markup"
				/>
				<StatusBadge
					status={page?.amphtml ? "pass" : "info"}
					label={page?.amphtml ? "Has AMP" : "No AMP"}
				/>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
				...
				<DataRow
					label="Tags"
					value={Array.isArray(sig.tags) ? sig.tags.join(", ") : sig.tags}
				/>
