	...
	return (
		<div>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
				<MetricCard
					label="Price visible"
					value={sig.priceVisible ? "Yes" : "No"}
					color={sig.priceVisible ? "text-green-400" : "text-red-400"}
				/>
				<MetricCard
					label="Stock"
					value={sig.outOfStock ? "Out" : sig.hasStockStatus ? "In" : "—"}
					color={sig.outOfStock ? "text-red-400" : "text-green-400"}
				/>
				<MetricCard
					label="Reviews"
					value={formatNumber(sig.reviewCount)}
					sub={sig.aggregateRating ? `★ ${sig.aggregateRating}` : ""}
				/>
			...
			<div className="flex flex-wrap gap-2 mb-5">
				<StatusBadge
					status={
						(page?.schemaTypes || []).includes("Product") ? "pass" : "fail"
					}
					label="Product schema"
				/>
				<StatusBadge
					status={(page?.schemaTypes || []).includes("Offer") ? "pass" : "warn"}
					label="Offer schema"
				/>
				<StatusBadge
					status={
						(page?.schemaTypes || []).includes("AggregateRating")
							? "pass"
							: "info"
					}
				...
				/>
				<StatusBadge
					status={sig.inProductFeed ? "pass" : "warn"}
					label={sig.inProductFeed ? "In product feed" : "Missing from feed"}
				/>
			</div>
