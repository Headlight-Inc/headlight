...
export default function SuggestedRewriteBlock({
	...
	const tone = (n: number) =>
		!limit ? "#888" : n === 0 ? "#ef4444" : n > limit ? "#f59e0b" : "#22c55e";
	return (
		<div className="bg-[#0a0a0a] border border-[#222] rounded p-3 mb-3">
			<div className="flex items-center justify-between mb-2">
				...
				<div className="flex items-center gap-3 text-[10px] font-mono">
					<span style={{ color: tone(curLen) }}>
						{curLen}
						{limit ? `/${limit}` : ""}
					</span>
					{suggestion && <span className="text-[#666]">→</span>}
					{suggestion && (
						<span style={{ color: tone(sugLen) }}>
							{sugLen}
							{limit ? `/${limit}` : ""}
						</span>
					)}
			...
			<div className="text-[12px] text-[#ccc] break-words">
				{current || <span className="italic text-[#666]">Missing</span>}
			</div>
			{suggestion && (
