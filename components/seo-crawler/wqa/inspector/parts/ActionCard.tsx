	...
	const priBucket = !priority
		...
		: priority <= 3
			...
			: priority <= 6
				...
				: "Low";
	const priTone =
		priBucket === "High" ? "fail" : priBucket === "Mid" ? "warn" : "info";

	return (
		<div
			className={`rounded border px-3 py-2.5 ${
				primary
					? "border-[#F5364E]/40 bg-[#F5364E]/5"
					: CAT_COLOR[category || ""] || "border-[#222] bg-[#0a0a0a]"
			}`}
		>
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					...
					{reason && (
						<div className="text-[11px] text-[#aaa] mt-0.5">{reason}</div>
					)}
					<FactorChips factors={factors} />
				</div>
				<div className="flex flex-col items-end gap-1 shrink-0">
					{priBucket && (
						<StatusBadge status={priTone as any} label={`P-${priBucket}`} />
					)}
					{effort && (
					...
					{Number.isFinite(Number(estimatedImpact)) &&
						Number(estimatedImpact) > 0 && (
							<span className="text-[10px] text-[#8ad]">
								+{formatNumber(estimatedImpact)} cl/mo
							</span>
						)}
