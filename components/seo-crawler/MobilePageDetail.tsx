...
export default function MobilePageDetail({
	page,
	onClose,
}: MobilePageDetailProps) {
	const {
		setAutoFixItems,
		setShowAutoFixModal,
		...
		if (activeTab === "perf") {
			return (
				<div className="grid grid-cols-2 gap-3">
					<Metric
						label="LCP"
						value={page.lcp ? `${(Number(page.lcp) / 1000).toFixed(2)}s` : "--"}
					/>
					<Metric label="CLS" value={page.cls ?? "--"} />
					<Metric label="INP" value={page.inp ? `${page.inp}ms` : "--"} />
					<Metric
						label="Response"
						value={page.loadTime ? `${page.loadTime}ms` : "--"}
					/>
					<Metric
						label="Page size"
						value={
							page.sizeBytes
								? `${Math.round(Number(page.sizeBytes) / 1024)} KB`
								: "--"
						}
					/>
		...
		if (activeTab === "ai") {
			return (
				<div className="space-y-3">
					...
					<Metric
						label="Topic cluster"
						value={page.topicCluster || "Not clustered"}
					/>
					<Metric
						label="Priority"
						value={page.strategicPriority || "Not scored"}
					/>
					<Metric
						label="Recommended action"
						value={page.recommendedAction || "No recommendation yet"}
					/>
				</div>
			);
		}
	...
	return (
		<div className="fixed inset-0 z-[75] flex flex-col bg-[#09090b]">
			<div className="border-b border-[#202025] px-4 py-3">
				<button
					onClick={onClose}
					className="inline-flex items-center gap-2 text-[12px] font-semibold text-[#ddd]"
				>
					<ArrowLeft size={16} />
					Back
				</button>
				<div className="mt-3">
					...
					<div className="mt-1 truncate text-[12px] text-[#6d93ff]">
						{page.url}
					</div>
				</div>
			</div>
					...
					<div className="mt-3 flex flex-wrap gap-2">
						{issues.length === 0 ? (
							...
							issues.map((issue) => (
								<span
									key={issue.id}
									className="rounded-full border border-[#2f2f36] bg-[#17171b] px-3 py-1 text-[11px] font-semibold text-[#ddd]"
								>
									{issue.label}
								</span>
							))
