...
function ScatterTooltip({ active, payload }: any) {
	...
	return (
		<div className="rounded-lg border border-[#333] bg-[#111] px-3 py-2 shadow-xl">
			<div className="mb-1 max-w-[280px] truncate text-[11px] font-bold text-white">
				{d.keyword}
			</div>
			<div className="space-y-0.5 text-[10px] text-[#999]">
				...
				<div className="flex justify-between gap-4">
					...
					<span className="font-mono text-white">
						{Number(d.impressions).toLocaleString()}
					</span>
				</div>
				<div className="flex justify-between gap-4">
					<span>Clicks</span>
					<span className="font-mono text-white">
						{Number(d.clicks).toLocaleString()}
					</span>
				</div>
				<div className="flex justify-between gap-4">
					<span>Opportunity</span>
					<span
						className={`font-mono font-bold ${d.opportunity > 60 ? "text-[#F5364E]" : d.opportunity > 30 ? "text-yellow-400" : "text-blue-400"}`}
					>
						{d.opportunity}/100
					</span>
				</div>
			</div>
			<div className="mt-1 max-w-[280px] truncate text-[9px] text-[#555]">
				{d.url}
			</div>
		</div>
	);
...
export default function KeywordLandscapeView() {
	...
	const scatterData = useMemo(() => {
		return analysisPages
			.filter(
				(p: any) => (p.gscPosition || 0) > 0 && (p.gscImpressions || 0) > 0,
			)
			.map((p: any) => ({
				url: p.url,
	...
	const filteredGaps = useMemo(() => {
		...
		if (intentFilter !== "all") {
			gaps = gaps.filter(
				(g: any) =>
					(g.intent || "informational").toLowerCase() === intentFilter,
			);
		}
	...
	const stats = useMemo(() => {
		const quickWins = scatterData.filter(
			(d) => d.position >= 4 && d.position <= 20 && d.impressions > 100,
		).length;
		...
		return {
			...
			top3Count,
			highOpp,
			gapCount: allKeywordGaps.length,
		};
		...
		<div className="custom-scrollbar flex h-full flex-col overflow-y-auto bg-[#0a0a0a]">
			<div className="grid grid-cols-5 gap-3 border-b border-[#1a1a1e] px-5 py-4">
				{[
					...
					<div
						key={s.label}
						className="rounded-xl border border-[#1a1a1e] bg-[#111] px-4 py-3"
					>
						<div className={`font-mono text-[18px] font-black ${s.color}`}>
							{s.value.toLocaleString()}
						</div>
						<div className="mt-0.5 text-[10px] uppercase tracking-wider text-[#555]">
							{s.label}
						</div>
					</div>
				))}
			...
			<div className="border-b border-[#1a1a1e] px-5 py-4">
				<div className="mb-3 flex items-center justify-between">
					<div>
						...
						<p className="mt-0.5 text-[10px] text-[#555]">
							Each dot is a ranking keyword. Size = clicks. Color = opportunity
							score.
						</p>
					</div>
						...
						<div className="flex h-[320px] items-center justify-center p-4">
							<div className={EMPTY_STATE_BOX}>
								<p className={EMPTY_STATE_TEXT}>
									No ranking keyword data. Connect Google Search Console to
									populate this chart.
								</p>
							</div>
						</div>
								...
								<Scatter data={scatterData} isAnimationActive={false}>
									{scatterData.map((entry, i) => (
										<Cell
											key={`kw-${i}`}
											fill={
												entry.opportunity > 60
													? "#F5364E"
													: entry.opportunity > 30
														? "#F59E0B"
														: "#3B82F6"
											}
											fillOpacity={0.75}
				...
				<div className="mt-2 flex items-center gap-6 text-[9px] text-[#555]">
					<span>
						<span className="text-green-500">|</span> Position 3 — Top results
					</span>
					<span>
						<span className="text-yellow-500">|</span> Position 10 — First page
					</span>
					<span>
						<span className="text-red-500">|</span> Position 20 — Second page
					</span>
				</div>
			</div>
							...
							<table className="w-full">
								<thead className="sticky top-0 z-10 bg-[#111]">
									<tr className="border-b border-[#1a1a1e]">
										{[
											{
												key: "keyword" as GapSortKey,
												label: "Keyword",
												align: "text-left",
												width: "",
											},
											{
												key: "intent" as GapSortKey,
												label: "Intent",
												align: "text-left",
												width: "w-[100px]",
											},
											{
												key: "volume" as GapSortKey,
												label: "Est. Volume",
												align: "text-right",
												width: "w-[100px]",
											},
											{
												key: "position" as GapSortKey,
												label: "Comp. Position",
												align: "text-right",
												width: "w-[110px]",
											},
											{
												key: "difficulty" as GapSortKey,
												label: "Difficulty",
												align: "text-center",
												width: "w-[90px]",
											},
										...
										return (
											...
											>
												<td className="px-3 py-2">
													<div className="flex items-center gap-2">
														<span className="text-[11px] font-medium text-[#ddd]">
															{gap.keyword}
														</span>
														{gap.volume > 500 && (
															<ArrowUpRight
																size={10}
																className="shrink-0 text-[#F5364E]"
															/>
														)}
													</div>
													{gap.source && (
														<div className="mt-0.5 text-[9px] text-[#444]">
															from {gap.source}
														</div>
													)}
												</td>
												<td className="px-3 py-2">
													<span
														className={`rounded border px-1.5 py-0.5 text-[9px] font-bold uppercase ${INTENT_STYLES[intent] || INTENT_STYLES.informational}`}
													>
														{intent}
													</span>
												</td>
												<td className="px-3 py-2 text-right font-mono text-[11px] text-[#ccc]">
													{gap.volume != null
														? Number(gap.volume).toLocaleString()
														: "—"}
												</td>
												<td className="px-3 py-2 text-right font-mono text-[11px] text-[#ccc]">
												...
												<td className="px-3 py-2 text-center">
													<span
														className={`text-[10px] font-bold ${DIFFICULTY_STYLES[difficulty] || "text-[#888]"}`}
													>
														{difficulty}
													</span>
												</td>
