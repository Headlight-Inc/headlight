...
export default function CompetitorMatrixGrid() {
	...
	const [pageCompareTarget, setPageCompareTarget] = useState<{
		url: string;
		title: string;
	} | null>(null);
	...
	const groupedRows = useMemo(() => {
		...
		return [...grouped.entries()]
			...
			.map(([category, rows]) => ({
				category,
				rows: [...rows].sort(
					(a, b) => rank(category, a.label) - rank(category, b.label),
				),
			}));
	}, []);
					...
					<tr className="bg-[#0d0d0f] border-b border-[#222]">
						...
						{allProfiles.map((profile, i) => (
							...
							>
								...
								{i === 0 && <span className={YOU_BADGE}>YOU</span>}
								...
								<div className="mt-1">
									<span
										className={`inline-flex rounded-full px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide ${
