...
function ChartCard({
	...
	return (
		<div
			className={`rounded-xl border border-[#1a1a1e] bg-[#0d0d0f] p-4 ${className}`}
		>
			<div className="mb-3">
				<div className="text-[11px] font-semibold text-[#888]">{title}</div>
				{subtitle && (
					<div className="mt-0.5 text-[9px] text-[#555]">{subtitle}</div>
				)}
			</div>
			{children}
		</div>
...
function GaugeMeter({
	...
	const color =
		clampedValue >= 70 ? "#22c55e" : clampedValue >= 40 ? "#eab308" : "#ef4444";
	return (
		<div className="text-center">
			<div className="relative mx-auto h-16 w-16">
				...
				<div className="absolute inset-0 flex items-center justify-center">
					<span className="font-mono text-[14px] font-black text-white">
						{clampedValue}
					</span>
				</div>
			</div>
			<div className="mt-1 max-w-[80px] truncate text-[9px] text-[#666]">
				{label}
			</div>
		</div>
	);
...
function ChartTooltip({ active, payload, label }: any) {
	...
	return (
		<div className="rounded-lg border border-[#333] bg-[#111] px-3 py-2 shadow-xl">
			{label && (
				<div className="mb-1 text-[10px] font-bold text-white">{label}</div>
			)}
...
export default function CompetitorChartsView() {
	...
	const radarData = useMemo(() => {
		const dimensions = [
			{
				key: "Search",
				fn: (p: CompetitorProfile) => normalize(p.overallSeoScore),
			},
			{
				key: "Content",
				fn: (p: CompetitorProfile) => normalize(p.totalIndexablePages, 2000),
			},
			{
				key: "Authority",
				fn: (p: CompetitorProfile) => normalize(p.urlRating),
			},
			{
				key: "Tech",
				fn: (p: CompetitorProfile) => normalize(p.techHealthScore),
			},
			{
				key: "UX",
				fn: (p: CompetitorProfile) => normalize(p.trustSignalScore),
			},
			{
				key: "Social",
			...
			{
				key: "AI Ready",
				fn: (p: CompetitorProfile) => normalize(p.avgGeoScore),
			},
			{
				key: "Freshness",
				fn: (p: CompetitorProfile) => normalize(p.contentFreshnessScore),
			},
		];
	...
	const keywordDistData = useMemo(
		() =>
			allProfiles.map((p) => {
				...
				const top20 = Math.max(
					0,
					Number(p.keywordsInTop20 || 0) - top3 - top10,
				);
				const rest = Math.max(
					0,
					Number(p.totalRankingKeywords || 0) - top3 - top10 - top20,
				);
				return {
					domain: p.domain,
					"Top 3": top3,
	...
	const sovData = useMemo(() => {
		const total = allProfiles.reduce(
			(sum, p) => sum + Number(p.totalRankingKeywords || 0),
			0,
		);
		if (total === 0) return [];
	...
	const techStackData = useMemo(() => {
		...
		return Array.from(allTech).map((tech) => {
			...
			allProfiles.forEach((p) => {
				row[p.domain] =
					(p.techStackSignals || []).includes(tech) || p.cmsType === tech;
			});
			return row;
		});
	}, [allProfiles]);
		...
		<div className="custom-scrollbar h-full overflow-y-auto bg-[#0a0a0a] p-5 space-y-5">
			<div className="mb-5 flex flex-wrap items-center gap-4">
				{profileLegend.map((p) => (
					<div key={p.domain} className="flex items-center gap-1.5">
						...
						/>
						<span
							className={`text-[10px] font-bold ${p.isOwn ? "text-white" : "text-[#888]"}`}
						>
			...
			<div className="space-y-6">
				<Section title="Big Picture">
					<div className="grid grid-cols-2 gap-3">
						<ChartCard
							...
							subtitle="8-dimension comparison across all competitors"
						>
						...
						<ChartCard
							...
							subtitle="All competitors ranked by composite score"
						>
							<ResponsiveContainer width="100%" height={300}>
								<BarChart
									data={scoreRanking}
				...
				<Section title="Search Visibility">
					<div className="grid grid-cols-3 gap-3">
						<ChartCard
							...
							subtitle="Keywords by ranking position"
						>
						...
						<ChartCard
							...
							subtitle="Estimated monthly organic visits"
						>
							<ResponsiveContainer width="100%" height={220}>
						...
						<ChartCard
							...
							subtitle="Keyword universe split by site"
						>
							{sovData.length === 0 ? (
								<div className="flex h-[220px] items-center justify-center text-[11px] text-[#555]">
									No keyword data
								</div>
							) : (
								<ResponsiveContainer width="100%" height={220}>
				...
				<Section title="Content & Authority">
					<div className="grid grid-cols-2 gap-3">
						<ChartCard
							title="Content Volume vs Freshness"
							subtitle="X = pages, Y = freshness %, bubble size = traffic"
						>
						...
						<ChartCard
							...
							subtitle="X = referring domains, Y = traffic, bubble size = authority"
						>
							<ResponsiveContainer width="100%" height={260}>
								<ScatterChart margin={{ bottom: 8, right: 16 }}>
									<CartesianGrid strokeDasharray="3 3" stroke="#1a1a1e" />
												...
												<div className="rounded-lg border border-[#333] bg-[#111] px-3 py-2 shadow-xl">
													...
													<div className="mt-1 space-y-0.5 text-[9px] text-[#999]">
														<div>
															Referring Domains:{" "}
															<span className="font-mono text-white">
																{d.referringDomains.toLocaleString()}
															</span>
														</div>
														<div>
															Traffic:{" "}
				...
				<Section title="Technical & AI Readiness">
					<div className="grid grid-cols-2 gap-3">
						<ChartCard
							...
							subtitle="Composite health score per site"
						>
							<div className="flex flex-wrap items-center justify-center gap-6 py-4">
								{allProfiles.map((p) => (
									<GaugeMeter
										key={p.domain}
										value={Number(p.techHealthScore || 0)}
										label={p.domain}
									/>
								))}
							</div>
						...
						<ChartCard
							title="AI Readiness"
							subtitle="GEO score, citation, passage & snippet readiness"
						>
							<ResponsiveContainer width="100%" height={220}>
								<BarChart
									data={aiData}
				...
				<Section title="Social Presence">
					<ChartCard
						title="Social Following by Platform"
						subtitle="Follower counts across major platforms"
					>
						<ResponsiveContainer width="100%" height={220}>
				...
				{techStackData.length > 0 && (
					<Section title="Tech Stack">
						<ChartCard
							...
							subtitle="Frameworks, CMS, libraries, and tools per competitor"
						>
							<div className="overflow-x-auto">
								<table className="w-full">
									<thead>
										<tr className="border-b border-[#1a1a1e]">
											<th className="px-3 py-2 text-left text-[9px] font-bold uppercase tracking-widest text-[#555]">
												Technology
											</th>
											{allProfiles.map((p, i) => (
												<th
									...
									<tbody>
										{techStackData.map((row) => (
											<tr
												key={row.tech}
												className="border-b border-[#111] transition-colors hover:bg-white/[0.01]"
											>
												<td className="px-3 py-1.5 text-[10px] text-[#ccc]">
													{row.tech}
												</td>
												{allProfiles.map((p) => (
													<td
