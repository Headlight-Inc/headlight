  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2 border-b border-[#1e1e22]">
        <div className="text-[11px] font-bold text-white truncate">{domain}</div>
        <div className="text-[10px] text-[#666]">{profile?.businessName || 'Unknown business'}</div>
      </div>
      ...
      {actions.map((item, i) =>
        'divider' in item ? (
          ...
          >
            <span className={item.danger ? 'text-red-400' : 'text-[#555]'}>{item.icon}</span>
            <span className="flex-1 text-left">{item.label}</span>
            {item.hint && <span className="text-[10px] text-[#555]">{item.hint}</span>}
          </button>
        )
      )}
    </div>
