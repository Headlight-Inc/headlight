...
export default function AddCompetitorModal({ isOpen, onClose }: Props) {
	const { addCompetitorAndCrawl, pages, crawlingCompetitorDomain, ownProfile } =
		useSeoCrawler();
	...
	const [discovered, setDiscovered] = useState<
		Array<{ domain: string; source: string; confidence: string }>
	>([]);
		...
		<div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-[2px]">
			<div className="w-full max-w-lg rounded-2xl border border-[#242428] bg-[#0d0d0f] shadow-2xl">
				...
				<div className="flex items-center justify-between border-b border-[#1e1e22] px-5 py-4">
					...
					<button
						onClick={onClose}
						className="p-1 rounded hover:bg-[#222] text-[#666] hover:text-white transition"
					>
						<X size={16} />
					</button>
				</div>
				...
				<div className="p-5 space-y-5">
					...
					>
						...
						{discovering
							? "Analyzing your link neighborhood..."
							: "Auto-Discover from Crawl Data"}
					</button>
										...
										>
											...
											<div className="min-w-0 flex-1">
												...
												<div className="mt-0.5 flex items-center gap-2">
													...
													<span
														className={`rounded px-1.5 py-0.5 text-[8px] font-bold uppercase ${confidenceColor}`}
													>
														{comp.confidence}
													</span>
												</div>
