...
const ChartTooltip = ({ active, payload, label }: any) => {
	...
	return (
		<div className="bg-[#111]/95 backdrop-blur-md border border-[#333] shadow-[0_8px_30px_rgba(0,0,0,0.5)] rounded-md p-3 min-w-[130px] flex flex-col z-[100] outline outline-1 outline-[#000]">
			{label && (
				<span className="text-[10px] font-bold text-[#888] uppercase tracking-wider pb-1.5 mb-2 border-b border-[#222]">
					{label}
				</span>
			)}
			<div className="flex flex-col gap-2">
				{payload.map((entry: any, index: number) => {
					const displayName =
						entry.name === "value" ? entry.payload.name : entry.name;
					const dotColor =
						entry.payload.fill || entry.payload.color || entry.color || "#ccc";
					return (
						<div
							key={index}
						...
						>
							...
							<span className="text-[11px] font-mono text-white flex items-center justify-end">
...
export default function ComparisonView({ onClose }: ComparisonViewProps) {
	...
	const [leftSessionId, setLeftSessionId] = useState<string | null>(
		compareSessionId,
	);
	const [rightSessionId, setRightSessionId] = useState<string | null>(
		currentSessionId,
	);
	...
	const [selectedNewIssues, setSelectedNewIssues] = useState<Set<string>>(
		new Set(),
	);
	const [selectedFixedIssues, setSelectedFixedIssues] = useState<Set<string>>(
		new Set(),
	);
	...
	useEffect(() => {
		...
		setLeftSessionId(
			(current) => current || compareSessionId || previous || latest,
		);
		setRightSessionId(
			(current) => current || currentSessionId || latest || previous,
		);
	...
	const exportDiff = () => {
		...
		downloadBlob(
			exportComparisonCSV(diffResult),
			`compare_${leftSessionId}_${rightSessionId}.csv`,
		);
	};
	...
	const unifiedData = useMemo(() => {
		...
		(diffResult.added || []).forEach((p: any) => {
			const item = getOrInit(p.url);
			item.types.add("new");
			item.pageData = p;
		});
		(diffResult.removed || []).forEach((p: any) => {
			const item = getOrInit(p.url);
			item.types.add("missing");
			item.pageData = p;
		});
		(diffResult.changed || []).forEach((p: any) => {
			const item = getOrInit(p.url);
			item.types.add("modified");
			item.fieldChanges = p.fieldChanges || [];
		});
		(diffResult.issuesFixed || []).forEach((p: any) => {
			const item = getOrInit(p.url);
			item.types.add("fixed");
			item.issuesFixed = p.issues || [];
		});
		(diffResult.newIssues || []).forEach((p: any) => {
			const item = getOrInit(p.url);
			item.types.add("new_issues");
			item.newIssues = p.issues || [];
		});

		const getDelta = (item: any, fld: string) => {
			const match = item.fieldChanges.find((f: any) => f.field === fld);
			return match
				? Number(match.newValue || 0) - Number(match.oldValue || 0)
				: 0;
		};
		...
		return Array.from(map.values()).map((item) => {
			return {
				...item,
				primaryType: item.types.has("missing")
					? "missing"
					: item.types.has("new")
						? "new"
						: item.types.has("new_issues")
							? "new_issues"
							: item.types.has("modified")
								? "modified"
								: "fixed",
	...
	const availableNewIssues = useMemo(() => {
		...
		unifiedData.forEach((row) =>
			row.newIssues.forEach((iss: any) =>
				issues.set(iss.label, (issues.get(iss.label) || 0) + 1),
			),
		);
	...
	const availableFixedIssues = useMemo(() => {
		const issues = new Map<string, number>();
		unifiedData.forEach((row) =>
			row.issuesFixed.forEach((iss: any) =>
				issues.set(iss.label, (issues.get(iss.label) || 0) + 1),
			),
		);
		return Array.from(issues.entries()).sort((a, b) => b[1] - a[1]);
	...
	const filteredData = useMemo(() => {
		let result = unifiedData.filter((item) => {
			...
			if (selectedTypes.size > 0) {
				let matchType = false;
				if (selectedTypes.has("new") && item.types.includes("new"))
					matchType = true;
				if (selectedTypes.has("missing") && item.types.includes("missing"))
					matchType = true;
				if (selectedTypes.has("modified") && item.types.includes("modified"))
					matchType = true;
				if (!matchType) return false;
			}
			if (
				selectedNewIssues.size > 0 &&
				!item.newIssues.some((iss: any) => selectedNewIssues.has(iss.label))
			)
				return false;
			if (
				selectedFixedIssues.size > 0 &&
				!item.issuesFixed.some((iss: any) => selectedFixedIssues.has(iss.label))
			)
				return false;
			return true;
		});
	...
	const totalIssuesFixed = (diffResult?.issuesFixed || []).reduce(
		(acc: number, item: any) => acc + (item.issues?.length || 0),
		0,
	);
	const totalIssuesIntroduced = (diffResult?.newIssues || []).reduce(
		(acc: number, item: any) => acc + (item.issues?.length || 0),
		0,
	);
	...
	const gridTemplateString = `${colWidths[0]}px minmax(300px, 1fr) ${colWidths
		.slice(2)
		.map((w) => `${w}px`)
		.join(" ")}`;
			...
			<header className="flex h-12 shrink-0 items-center justify-between border-b border-[#222] bg-[#0a0a0a] px-3 z-20 shadow-sm relative">
				<div className="flex items-center gap-3 shrink-0">
					<button
						...
						className="flex flex-col justify-center items-center gap-[3px] w-6 h-6 rounded hover:bg-[#1a1a1a] transition-colors"
					>
					...
					<div className="flex items-center text-[11px] bg-[#111] border border-[#222] rounded overflow-hidden h-6 ml-1">
						<select
							...
							className="bg-transparent text-[#aaa] outline-none cursor-pointer px-2 h-full hover:bg-[#1a1a1a]"
						>
						...
						<span className="text-[#555] px-2 font-bold select-none h-full flex items-center border-l border-r border-[#222]">
							vs
						</span>
						<select
							value={rightSessionId || ""}
							onChange={(e) => setRightSessionId(e.target.value)}
							className="bg-transparent text-white outline-none cursor-pointer px-2 h-full hover:bg-[#1a1a1a]"
						>
							{orderedSessions.map((s) => (
								<option key={s.id} value={s.id}>
									{new Date(s.startedAt).toLocaleDateString()}
					...
					<div className="flex items-center gap-5 text-[11px] font-bold tracking-wider">
						<div className="flex items-center gap-2">
							...
							<div className="flex items-center gap-1.5 bg-[#151515] rounded px-2 py-0.5 border border-[#222]">
								<span className="text-white font-mono">
									{Number(summary.healthScore?.new || 0).toFixed(0)}%
								</span>
								{healthDelta !== 0 && (
									<span
										className={
											healthDelta > 0
												? "text-emerald-500 font-mono flex items-center"
												: "text-[#F5364E] font-mono flex items-center"
										}
									>
										{healthDelta > 0 ? (
						...
						<div className="flex items-center gap-2">
							...
							<div className="flex items-center gap-1.5 bg-[#151515] rounded px-2 py-0.5 border border-[#222]">
								<span className="text-white font-mono">
									{formatValue(summary.totalClicks?.new || 0)}
								</span>
								{clickDelta !== 0 && (
									<span
										className={
						...
						<div className="flex gap-3 ml-2">
							...
							{totalMissing > 0 && (
								<span className="text-[#F5364E] font-mono flex items-center gap-1">
				...
				<div className="flex items-center gap-2 shrink-0 relative z-30">
					...
					<button
						...
						className="flex items-center gap-1.5 px-2 py-1 h-6 text-[11px] font-bold text-[#aaa] hover:text-white transition-colors"
					>
						<Link2 size={13} /> Share
					</button>
					<button
						onClick={exportDiff}
						className="flex items-center gap-1.5 bg-[#1a1a1a] hover:bg-[#222] border border-[#222] rounded px-3 py-1 h-6 text-[11px] font-bold text-[#eee] transition-colors"
					>
					...
					<button
						onClick={onClose}
						className="p-1 px-1.5 text-[#777] hover:bg-[#1a1a1a] rounded hover:text-white transition-colors"
					>
						<X size={16} />
					</button>
				</div>
			...
			>
				...
				<div className="flex w-full h-[260px] shrink-0">
					...
					<div className="flex-1 flex flex-col p-5">
						<span className="text-[12px] font-bold text-[#eee] mb-4">
						...
						<div className="flex-1 w-full bg-[#0d0d0d] rounded-md border border-[#222]/50 shadow-inner p-2">
							{totalIssuesIntroduced === 0 && totalIssuesFixed === 0 ? (
								<div className="flex h-full items-center justify-center text-[11px] text-[#555] font-medium">
									No issue changes detected
								</div>
							) : (
								<ResponsiveContainer width="100%" height="100%">
									<BarChart
										data={[
											{
												name: "New Issues",
												value: totalIssuesIntroduced,
												fill: "#F5364E",
											},
											{
												name: "Fixed Issues",
												value: totalIssuesFixed,
												fill: "#10B981",
											},
										].filter((d) => d.value > 0)}
						...
						<div className="flex-1 w-full flex items-center justify-center bg-[#0d0d0d] rounded-md border border-[#222]/50 shadow-inner relative">
							{totalAdded > 0 || totalMissing > 0 || totalModified > 0 ? (
								...
								<span className="text-[11px] text-[#555] font-medium">
									No additions or removals detected
								</span>
							)}
						</div>
					</div>
					...
					<div className="flex-[1.2] flex flex-col p-5 border-l border-[#222]/60">
						<span className="text-[12px] font-bold text-[#eee] mb-4">
							A/B Search Traffic Snapshot
						</span>
						<div className="flex-1 w-full bg-[#0d0d0d] rounded-md border border-[#222]/50 shadow-inner p-2">
							{(summary.totalClicks?.old || 0) === 0 &&
							(summary.totalClicks?.new || 0) === 0 ? (
								<div className="flex h-full items-center justify-center text-[11px] text-[#555] font-medium">
								...
								<ResponsiveContainer width="100%" height="100%">
									<BarChart
										data={[
											{
												period: "Historical",
												Clicks: summary.totalClicks?.old || 0,
												fill: "#444",
											},
											{
												period: "Live Crawl",
												Clicks: summary.totalClicks?.new || 0,
												fill: clickDelta >= 0 ? "#3B82F6" : "#F5364E",
											},
										]}
			...
			<div className="flex flex-1 overflow-hidden relative">
				{showSidebar && (
					<div className="w-[200px] shrink-0 border-r border-[#222] bg-[#111] flex flex-col overflow-y-auto custom-scrollbar z-10 shadow-xl">
						<div className="p-4 flex flex-col gap-6">
							<div className="flex flex-col gap-2">
								<span className="text-[11px] font-bold text-[#777] uppercase tracking-wider border-b border-[#222] pb-2">
									Page Structure
								</span>
								{[
									...
									{ id: "missing", label: "Missing", count: totalMissing },
									{
										id: "modified",
										label: "Modified Data",
										count: diffResult?.changed?.length || 0,
									},
								].map((f) => (
									<label
										key={f.id}
										onClick={() => toggleFilter(setSelectedTypes, f.id)}
										className={`flex items-center justify-between px-2 py-1.5 text-[11px] font-medium cursor-pointer rounded transition-colors ${selectedTypes.has(f.id) ? "bg-[#F5364E]/10 text-[#F5364E]" : "text-[#aaa] hover:bg-[#1a1a1a] hover:text-[#fff]"}`}
									>
										<span>{f.label}</span>
							...
							{availableNewIssues.length > 0 && (
								<div className="flex flex-col gap-2">
									...
									{availableNewIssues.slice(0, 10).map(([label, count]) => (
										<label
											key={label}
											onClick={() => toggleFilter(setSelectedNewIssues, label)}
											className={`flex items-center justify-between px-2 py-1.5 text-[11px] font-medium cursor-pointer rounded transition-colors ${selectedNewIssues.has(label) ? "bg-[#F5364E]/10 text-[#F5364E]" : "text-[#aaa] hover:bg-[#1a1a1a] hover:text-[#fff]"}`}
										>
								...
								<div className="flex flex-col gap-2">
									...
									{availableFixedIssues.slice(0, 10).map(([label, count]) => (
										<label
											key={label}
											onClick={() =>
												toggleFilter(setSelectedFixedIssues, label)
											}
											className={`flex items-center justify-between px-2 py-1.5 text-[11px] font-medium cursor-pointer rounded transition-colors ${selectedFixedIssues.has(label) ? "bg-[#F5364E]/10 text-[#F5364E]" : "text-[#aaa] hover:bg-[#1a1a1a] hover:text-[#fff]"}`}
										>
											<span className="truncate pr-2 leading-tight">
												{label}
											</span>
				...
				<div className="flex-1 flex flex-col min-w-0 bg-[#0a0a0a] overflow-x-hidden relative">
						...
						<div className="flex items-center gap-3">
							...
							{selectedUrls.size > 0 && (
								<div className="flex items-center gap-2 pl-3 border-l border-[#222]">
									<span className="text-[11px] font-bold text-[#eee]">
										{selectedUrls.size} selected
									</span>
									<button
										onClick={handleBulkTask}
										className="h-6 px-3 bg-white hover:bg-[#eaeaea] text-black rounded text-[10px] font-bold transition-colors"
									>
										Create Task
									</button>
									<button
										onClick={() => setSelectedUrls(new Set())}
										className="text-[11px] px-1 text-[#666] hover:text-[#eee] font-medium"
									>
										Clear
									</button>
								</div>
					...
					<div className="flex-none overflow-hidden border-b border-[#222] bg-[#111] w-full z-10">
						<div
							className="grid items-center h-9 text-[11px] font-bold text-[#666] select-none"
							...
							{[
								...
								>
									{col.noSort ? (
										<button
											onClick={() => {
												if (selectedUrls.size === filteredData.length)
													setSelectedUrls(new Set());
												else
													setSelectedUrls(
														new Set(filteredData.map((d) => d.url)),
													);
											}}
											className="hover:text-white"
										>
											{selectedUrls.size === filteredData.length &&
										...
										<div
											className="cursor-pointer hover:text-white flex items-center gap-1.5 w-full h-full"
											onClick={() => handleSort(col.k)}
										>
											<div
												className={`flex items-center gap-1 truncate w-full ${col.align === "right" ? "justify-end" : ""}`}
											>
												{col.l}{" "}
												{sortField === col.k && (sortDir === "asc" ? "↑" : "↓")}
											</div>
										</div>
									)}
						...
						{filteredData.length === 0 ? (
							<div className="flex flex-col items-center justify-center text-[#555] gap-3 mt-20 p-5">
								...
								<div className="text-[12px] font-medium text-[#777]">
									No comparison records found for active filters
								</div>
							</div>
						) : (
								...
								{filteredData.map((row) => {
									...
									return (
										...
										>
											...
											<div
												className={`truncate text-left pl-3 w-full font-mono ${isSelected ? "text-white font-medium" : "text-[#aaa]"}`}
												title={row.url}
											>
												{row.url}
											</div>
											...
											<div className="flex items-center pl-3 w-full">
												...
												{row.primaryType === "missing" && (
													<span className="text-[#F5364E] font-medium">
														Missing
													</span>
												)}
												{row.primaryType === "new_issues" && (
													<span className="text-orange-400 font-medium">
														Issues
													</span>
												)}
												{row.primaryType === "modified" && (
													<span className="text-[#888] font-medium">
														Modified
													</span>
												)}
												{row.primaryType === "fixed" && (
													<span className="text-emerald-500 font-medium">
														Fixed
													</span>
												)}
...
function PageDetailsSidebar({
	...
	const CATEGORIES = [
		...
		{
			title: "Authority & External",
			keys: ["urlRating", "referringDomains", "authorityScore"],
		},
		{
		...
		},
		{
			...
			keys: ["mixedContent", "insecureForms", "hstsMissing"],
		},
	];
	...
	const PropertyRow = ({
		...
		const diffRecord = (row.fieldChanges || []).find(
			(c: any) => c.field === propKey,
		);
				...
				<div className="flex flex-col gap-1.5 py-3 border-b border-[#222]">
					...
					<div className="flex flex-col gap-1.5 font-mono text-[11px] mt-1">
						<div className="flex items-center justify-between bg-[#F5364E]/10 border border-[#F5364E]/20 text-[#F5364E] rounded px-3 py-1.5">
							<span className="text-[9px] font-bold opacity-70 tracking-widest px-1.5 bg-[#F5364E]/20 rounded-sm">
								OLD
							</span>
							<span
								className="truncate max-w-[200px]"
								title={formatDetailedValue(diffRecord.oldValue, propKey)}
							>
								{formatDetailedValue(diffRecord.oldValue, propKey)}
							</span>
						</div>
						<div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded px-3 py-1.5">
							<span className="text-[9px] font-bold opacity-70 tracking-widest px-1.5 bg-emerald-500/20 rounded-sm">
								NEW
							</span>
							<span
								className="truncate max-w-[200px]"
								title={formatDetailedValue(diffRecord.newValue, propKey)}
							>
								{formatDetailedValue(diffRecord.newValue, propKey)}
							</span>
						</div>
					</div>
				</div>
		...
		return (
			<div className="flex items-center justify-between py-2 border-b border-[#1a1a1a]">
				<span className="text-[11px] font-medium text-[#777] min-w-[100px]">
					{displayLabel}
				</span>
				<span
					className="text-[11px] text-[#ccc] font-mono text-right break-all ml-4"
					title={formatDetailedValue(val, propKey)}
				>
					{formatDetailedValue(val, propKey)}
				</span>
			</div>
		);
	};
		...
		<div className="w-[360px] h-full bg-[#0c0c0c] flex flex-col shadow-[-15px_0_40px_rgba(0,0,0,0.8)]">
			<div className="flex flex-col border-b border-[#222] p-4 bg-[#0a0a0a]">
				<div className="flex justify-between items-start mb-3">
					...
					<button
						onClick={onClose}
						className="p-1 text-[#777] hover:text-white rounded hover:bg-[#1a1a1a] transition-colors"
					>
						<X size={16} />
					</button>
				</div>
				<div className="flex flex-col gap-2">
					<span
						className="text-[12px] font-mono text-white truncate block w-full"
						title={row.url}
					>
						{row.url}
					</span>
					<a
						...
						target="_blank"
						rel="noreferrer"
						className="text-[11px] font-medium text-blue-400 hover:text-blue-300 flex items-center gap-1.5 w-fit transition-colors"
					>
			...
			<div className="flex-1 overflow-y-auto custom-scrollbar p-5">
					...
					<div className="mb-8 flex flex-col gap-4">
						{row.newIssues.length > 0 && (
							<div className="flex flex-col gap-1.5">
								<span className="text-[11px] font-bold text-[#F5364E] uppercase tracking-wide">
									Issues Found
								</span>
								<div className="flex flex-col gap-1.5">
									{row.newIssues.map((iss: any) => (
										<div
											key={iss.id}
											className="flex justify-between items-center py-2 px-2.5 bg-[#F5364E]/10 rounded border border-[#F5364E]/20 text-[11px] text-[#eee]"
										>
											<span className="truncate pr-2">{iss.label}</span>
											<button
												onClick={() => void handleCreateTask(iss.label)}
												className="text-[10px] font-bold text-[#F5364E] hover:text-white transition-colors shrink-0"
											>
												Create Task
											</button>
										</div>
							...
							<div className="flex flex-col gap-1.5 pt-2">
								...
								<div className="flex flex-col gap-1.5">
									{row.issuesFixed.map((iss: any) => (
										<div
											key={iss.id}
											className="py-2 px-2.5 bg-emerald-500/10 rounded border border-emerald-500/20 text-[11px] text-[#555] line-through"
										>
											{iss.label}
										</div>
									))}
				...
				<div className="flex flex-col gap-10 pb-20">
					{CATEGORIES.map((category) => {
						const hasVisibleFields = category.keys.some(
							(k) =>
								(!showOnlyChanges ||
									(row.fieldChanges || []).some((c: any) => c.field === k)) &&
								(showAllFields ||
									currentContext[k] !== undefined ||
									(row.fieldChanges || []).some((c: any) => c.field === k)),
						);
						...
						return (
							<div key={category.title} className="flex flex-col">
								<h4 className="text-[10px] font-black text-[#555] uppercase tracking-[0.2em] mb-3 border-b border-[#1a1a1a] pb-1.5">
									{category.title}
								</h4>
								<div className="flex flex-col">
									{category.keys.map((key) => (
										<PropertyRow key={key} propKey={key} />
									))}
								</div>
							</div>
						);
					...
					{showAllFields && (
						<div className="flex flex-col pt-4 border-t border-[#222]/40">
							<h4 className="text-[10px] font-black text-blue-500/70 uppercase tracking-[0.2em] mb-3 pb-1.5">
								Uncategorized Raw Data
							</h4>
