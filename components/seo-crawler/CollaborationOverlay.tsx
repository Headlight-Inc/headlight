...
export const CollaborationOverlay: React.FC<CollaborationOverlayProps> = ({
	...
	const handleUpdateTaskStatus = async (status: TaskStatus) => {
		...
		try {
			...
			setTasks((prev) =>
				prev.map((t) => (t.id === activeTask.id ? { ...t, status } : t)),
			);
	...
	const handleCreateTask = async () => {
		...
		try {
			const newTask = await createTask(activeProject.id, {
				...
				createdBy: user.id,
				linkedIssueId:
					collabOverlayTarget.type === "issue" ? collabOverlayTarget.id : null,
			});
							...
							<div className="space-y-1.5">
								...
								<div className="flex items-center gap-2 p-2 bg-white/5 rounded-xl border border-white/5">
									{activeTask.assignee_avatar ? (
										<img
											src={activeTask.assignee_avatar}
						...
						<div className="space-y-3">
							...
							<div className="space-y-2">
								{subtasks.map((sub) => (
									<div
										key={sub.id}
										className="flex items-center gap-3 p-3 bg-white/[0.02] rounded-xl border border-white/5 group"
									>
										<input
											type="checkbox"
										...
										/>
										<span
											className={`text-sm flex-1 ${sub.completed ? "text-gray-600 line-through" : "text-gray-300"}`}
										>
											{sub.title}
										</span>
										<button
					...
					<section className="bg-white/5 border border-dashed border-white/10 rounded-2xl p-6 space-y-4">
						<div className="flex flex-col items-center text-center space-y-2 mb-2">
							...
							<p className="text-xs text-gray-500 max-w-xs">
								Assign this issue to a team member to track progress and get
								notifications.
							</p>
						</div>
								...
								<div key={comment.id} className="flex gap-4 group">
									...
									<div className="flex-1 space-y-1.5">
										...
										<div className="flex items-center gap-2 pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
											...
											<button className="text-[10px] font-bold text-gray-500 hover:text-white uppercase tracking-tighter">
												Reply
											</button>
											{!comment.resolved && (
												<button
