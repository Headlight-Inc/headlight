...
import {
	...
	exchangeGoogleCode,
} from "../../services/GoogleOAuthHelper";
import {
	parseBacklinkCsv,
	parseKeywordCsv,
...
function Toggle({
	...
	return (
		...
		>
			<div
				className={`${small ? "w-2.5 h-2.5" : "w-4 h-4"} bg-white rounded-full transition-transform ${checked ? (small ? "translate-x-3.5" : "translate-x-4") : ""}`}
			></div>
		</div>
	);
}
			...
			<div className="space-y-6">
				...
				<div className="rounded-lg border border-white/[0.06] bg-white/[0.01] p-5">
					<div className="flex items-center justify-between mb-1">
						...
						<div
							className={`w-2 h-2 rounded-full ${integrationConnections.google ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-white/10"}`}
						/>
					</div>
					<p className="text-[11px] text-white/40 mb-4">
						Unified connection for Search Console impressions and GA4 traffic
						data.
					</p>
					...
					{integrationConnections.google ? (
						...
						<button
							onClick={handleConnectGoogle}
							className="px-4 py-2 text-[10px] font-bold rounded border border-white/10 bg-white/[0.04] text-white/70 hover:bg-white/[0.08] hover:text-white"
						>
				...
				<div className="rounded-lg border border-white/[0.04] bg-white/[0.01] p-5">
					...
					<p className="text-[11px] text-white/40 mb-4">
						Local SEO performance signals and GBP insights.
					</p>
					{!integrationConnections.google ? (
				...
				<div className="rounded-lg border border-white/[0.06] bg-white/[0.01] p-5">
					<div className="flex items-center justify-between mb-1">
						...
						<div
							className={`w-2 h-2 rounded-full ${integrationConnections.google ? "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.4)]" : "bg-white/10"}`}
						/>
					</div>
					<p className="text-[11px] text-white/40 mb-4">
						Unified connection for search data and automated cloud storage.
					</p>
					...
					{integrationConnections.google ? (
						...
						<div className="flex flex-col items-center justify-center py-4 px-2 border border-dashed border-white/10 rounded-lg bg-white/[0.01]">
							...
							<p className="text-[10px] text-white/30 text-center mb-4 max-w-[200px]">
								Connect your Google account to enable automatic cloud backups.
							</p>
							<button
								onClick={handleConnectGoogle}
								className="px-4 py-2 text-[10px] font-bold rounded border border-white/10 bg-white/[0.04] text-white/70 hover:bg-white/[0.08] hover:text-white"
				...
				<div className="rounded-lg border border-white/[0.06] bg-white/[0.01] p-5">
					<div className="flex items-center justify-between mb-1">
						...
						<div
							className={`w-2 h-2 rounded-full ${integrationConnections.bingWebmaster ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-white/10"}`}
						/>
					</div>
					<p className="text-[11px] text-white/40 mb-4">
						Sync Bing search impressions and crawl errors using OAuth or API
						Key.
					</p>
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between rounded border border-emerald-500/10 bg-emerald-500/[0.02] px-3 py-2">
								<span className="text-[11px] text-white/70">
									{integrationConnections.bingWebmaster.accountLabel ||
										"Connected"}
								</span>
								<button
				...
				<div className="rounded-lg border border-white/[0.06] bg-white/[0.01] p-5">
					<div className="flex items-center justify-between mb-1">
						...
						<div
							className={`w-2 h-2 rounded-full ${mcpServers.length > 0 ? "bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.4)]" : "bg-white/10"}`}
						/>
					</div>
					<p className="text-[11px] text-white/40 mb-4">
						Connect to external AI tool providers (Linear, Amplitude, Slack).
					</p>
					...
					<div className="space-y-3">
						{mcpServers.map((server) => (
							<div
								key={server.id}
								className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/5 rounded-lg group"
							>
	...
	return (
		<div className="rounded-lg border border-white/[0.06] bg-white/[0.01] p-5">
			<div className="flex items-center justify-between mb-1">
				...
				<div
					className={`w-2 h-2 rounded-full ${connection ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]" : "bg-white/10"}`}
				/>
			</div>
			...
			{connection ? (
				<div className="flex items-center justify-between rounded border border-emerald-500/10 bg-emerald-500/[0.02] px-3 py-2">
					<div className="flex flex-col">
						...
						<span className="text-[9px] text-white/20 font-bold uppercase">
							{connection.uploadMeta?.rowCount || 0} rows parsed
						</span>
					</div>
					<button
						onClick={onRemove}
				...
				<div>
					<button
						...
						className="px-4 py-2 text-[10px] font-bold rounded border border-white/10 bg-white/[0.04] text-white/70 hover:text-white flex items-center gap-2"
					>
