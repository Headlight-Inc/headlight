...
const renderSnippet = (page: any) => (
	<div className="border border-[#2a2a2a] rounded-lg bg-[#0a0a0a] p-4">
		<div className="text-[#8ab4f8] text-[20px] leading-tight truncate">
			{page?.title || "Untitled Page"}
		</div>
		<div className="text-[#5f6368] text-[12px] mt-1 truncate">{page?.url}</div>
		<div className="text-[#bdc1c6] text-[13px] mt-2 line-clamp-3">
			{page?.metaDesc ||
				"No meta description found. Search engines may auto-generate a snippet."}
		</div>
	</div>
);
...
export default function SeoTab({ page }: { page: any }) {
	const issues = getPageIssues(page).filter((issue) =>
		/title|meta|h1|h2|canonical|index|robots|hreflang|amp|mobile|pagination|schema/i.test(
			issue.label,
		),
	);
		...
		<div>
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 gap-4 xl:gap-8">
					...
					<div className="space-y-4">
							...
							<div className="flex items-center gap-2 mb-1">
								...
								<StatusBadge
									...
									label={
										!page?.title
											? "Missing"
											: `${formatNumber(page?.titleLength)}ch / ${formatNumber(page?.titlePixelWidth)}px`
									}
								/>
							</div>
							...
							<div className="flex items-center gap-2 mb-1">
								...
								<StatusBadge
									status={
										!page?.metaDesc
											? "fail"
											: Number(page?.metaDescLength || 0) > 155
												? "warn"
												: "pass"
									}
									label={
										!page?.metaDesc
											? "Missing"
											: `${formatNumber(page?.metaDescLength)}ch`
									}
								/>
							</div>
						...
						<div>
							<div className="flex items-center gap-2 mb-1">
								...
								<StatusBadge
									status={
										!page?.h1_1 ? "fail" : page?.multipleH1s ? "warn" : "pass"
									}
									label={
										!page?.h1_1
											? "Missing"
											: page?.multipleH1s
												? "Multiple"
												: `${formatNumber(page?.h1_1Length)}ch`
									}
								/>
							</div>
							<div className="text-[12px] text-white bg-[#0a0a0a] border border-[#222] rounded p-2 break-all">
								{page?.h1_1 || (
									<span className="text-red-400 italic">No H1 tag found</span>
								)}
							</div>
						</div>
					</div>
				...
				<div>
					...
					<DataRow
						label="Indexable"
						value={page?.indexable !== false ? "Yes" : "No"}
					...
					<DataRow
						label="Canonical"
						value={
							page?.canonical ? <TruncatedUrl url={page.canonical} /> : "None"
						}
						status={
					...
					<DataRow
						label="X-Robots-Tag"
						value={page?.xRobots || page?.xRobotsTag}
					/>
					<DataRow
						label="rel=next"
						value={page?.relNextTag || page?.httpRelNext}
					/>
					<DataRow
						label="rel=prev"
						value={page?.relPrevTag || page?.httpRelPrev}
					/>
					<DataRow
						label="Meta Refresh"
						value={page?.metaRefresh}
						status={page?.metaRefresh ? "warn" : "pass"}
					/>
