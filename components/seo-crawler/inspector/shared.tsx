...
export const DataRow = ({
	...
	return (
		<div className="grid grid-cols-[120px_1fr] gap-x-3 text-[12px] py-1 min-w-0 overflow-hidden">
			...
			<span className={`${mono ? "font-mono" : ""} ${tone} break-all min-w-0`}>
...
export const SectionHeader = ({
	...
	<h4
		className={`text-[11px] font-black uppercase tracking-widest border-b border-[#222] pb-1 mb-3 flex items-center gap-2 ${color || "text-[#444]"}`}
	>
		{icon}
		{title}
	</h4>
...
export const MetricCard = ({
	...
	<div className="bg-[#0a0a0a] border border-[#222] rounded p-3">
		...
		<div className={`text-[20px] font-black mt-1 ${color || "text-white"}`}>
			{normalizeValue(value)}
		</div>
...
export const TruncatedUrl = ({ url }: { url: string }) => (
	<span
		title={url}
		className="text-blue-400 truncate text-[11px] font-mono block w-full"
	>
		{url}
	</span>
);
...
export const IssuesList = ({
	...
	const { setCollabOverlayTarget, setActiveAuditTab, setShowAuditSidebar } =
		useSeoCrawler();
