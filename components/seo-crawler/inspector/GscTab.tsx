		...
		return (
			<div className="bg-[#0a0a0a] border border-[#222] rounded p-5 text-center">
				...
				<div className="text-[12px] text-[#666]">
					Connect GSC in Integrations to populate clicks, impressions, CTR, and
					query data.
				</div>
			</div>
		);
	...
	return (
		<div>
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
				<MetricCard
					label="Clicks (30d)"
					value={formatNumber(page?.gscClicks)}
				/>
				<MetricCard
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 gap-4 xl:gap-8">
				<div>
					...
					<DataRow
						label="Best Volume"
						value={formatNumber(page?.bestKwVolume)}
					/>
				</div>
							...
							<tbody>
								{topQueries.map((query: any, index: number) => (
									...
									>
										...
										<td className="px-3 py-1.5 text-[#bbb]">
										...
										<td className="px-3 py-1.5 text-[#bbb]">
											{formatPercent(query?.ctr, 100)}
										</td>
										<td className="px-3 py-1.5 text-[#bbb]">
											{formatNumber(query?.position, {
												maximumFractionDigits: 1,
											})}
										</td>
									</tr>
