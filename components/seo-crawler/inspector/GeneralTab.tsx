	...
	return (
		<div>
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 mb-5">
				...
				<MetricCard
					label="Authority"
					value={formatNumber(page?.authorityScore, {
						maximumFractionDigits: 0,
					})}
			...
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 xl:gap-8">
				...
				<div>
					...
					<DataRow
						label="Indexability"
						value={page?.indexabilityStatus}
						status={page?.indexable === false ? "fail" : "pass"}
					/>
					<DataRow
						label="Canonical"
					...
					<DataRow
						label="Timestamp"
						value={
							page?.crawlTimestamp || page?.timestamp
								? new Date(
										page?.crawlTimestamp || page?.timestamp,
									).toLocaleString()
								: "—"
				...
				<div>
					...
					<DataRow
						label="CO2"
						value={page?.co2Mg ? `${page.co2Mg}mg` : "—"}
						mono
					/>
					<DataRow
						label="Carbon Rating"
					...
					<DataRow
						label="Cookies"
						value={formatNumber(page?.cookieCount)}
						mono
					/>
					<DataRow
						label="SSL Valid"
						...
						<table className="w-full text-[11px] font-mono">
							<tbody>
								{responseHeaders.map(([key, value]) => (
									...
									>
										<td className="px-3 py-1 text-[#F5364E] w-[210px]">
											{key}
										</td>
			...
			{redirectChain.length > 0 && (
				<div className="mt-6">
					<div className="flex items-center gap-2 mb-3">
						...
						<StatusBadge
							status={redirectChain.length > 2 ? "warn" : "info"}
							label={`${redirectChain.length} hops`}
						/>
					</div>
					<div className="space-y-2">
						{redirectChain.map((url: string, index: number) => (
							...
							>
								<span className="text-[#666] mr-2">#{index + 1}</span>
								{url}
							</div>
						))}
					</div>
