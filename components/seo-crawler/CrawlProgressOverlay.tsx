...
const formatEta = (queued: number, crawlRate: string | number) => {
	...
	if (!Number.isFinite(rate) || rate <= 0 || queued <= 0)
		return "Calculating...";
...
export default function CrawlProgressOverlay() {
	...
	const progress = useMemo(() => {
		...
		const total =
			maxPages > 0 ? maxPages : crawlRuntime.discovered || crawlRuntime.crawled;
	...
	const statusCounts = useMemo(
		() => ({
			...
			redirects: pages.filter(
				(page) => page.statusCode >= 300 && page.statusCode < 400,
			).length,
			broken: pages.filter(
				(page) => page.statusCode >= 400 && page.statusCode < 500,
			).length,
		...
		return (
			...
			>
				...
				<div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#1c1c20]">
					<div
						className="h-full rounded-full bg-gradient-to-r from-[#F5364E] via-[#ff6b7d] to-[#f8b4bd]"
						style={{ width: `${progress}%` }}
					/>
				</div>
				<div className="hidden text-[11px] text-[#9a9aa0] md:block">
					{crawlRuntime.crawled} pages · {crawlRate} p/s · {elapsedTime}
				</div>
				<div
				...
				>
					<button
						...
						className="rounded border border-[#2e2e33] px-2 py-1 text-[10px] font-bold text-[#ddd] hover:border-[#444]"
					>
						<Pause size={11} />
					</button>
					<button
						onClick={() => clearCrawlerWorkspace()}
						className="rounded border border-[#44262a] px-2 py-1 text-[10px] font-bold text-[#ff8d99] hover:border-[#6b3037]"
					>
						<Square size={11} />
					</button>
				</div>
			...
			<div className="w-full max-w-[720px] overflow-hidden rounded-2xl border border-[#27272b] bg-[#111] shadow-[0_30px_100px_rgba(0,0,0,0.6)]">
				<div className="border-b border-[#222] bg-[#151518] px-6 py-4">
					<div className="flex items-center justify-between">
						<div>
							<div className="text-[11px] font-bold uppercase tracking-[0.28em] text-[#777]">
								Live Crawl
							</div>
					...
					<div className="mt-4 h-2 overflow-hidden rounded-full bg-[#1d1d21]">
						<div
							className="h-full rounded-full bg-gradient-to-r from-[#F5364E] via-[#ff6b7d] to-[#f8b4bd] transition-all"
							style={{ width: `${progress}%` }}
						/>
					</div>
				</div>
					...
					<div className="space-y-5">
						<div className="grid grid-cols-2 gap-3">
							{[
								{
									label: "Pages crawled",
									value: `${crawlRuntime.crawled} / ~${Math.max(crawlRuntime.discovered, crawlRuntime.crawled)}`,
								},
								...
								{
									label: "Errors",
									value: (stats?.broken || 0) + statusCounts.server,
								},
								{ label: "ETA", value: eta },
							].map((metric) => (
								<div
									key={metric.label}
									className="rounded-xl border border-[#212126] bg-[#0d0d10] p-4"
								>
									<div className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#666]">
										{metric.label}
									</div>
						...
						<div className="rounded-xl border border-[#212126] bg-[#0d0d10] p-4">
							<div className="mb-3 flex items-center justify-between">
								...
								<div className="text-[10px] text-[#666]">
									{liveFeed.length} latest entries
								</div>
							</div>
							<div
								ref={feedRef}
								className="max-h-[280px] overflow-y-auto rounded-lg border border-[#1a1a1d] bg-[#09090b] p-3 font-mono text-[11px] custom-scrollbar"
							>
								{liveFeed.length === 0 ? (
									...
									liveFeed.map((entry, index) => (
										<div
											...
											className={`border-b border-[#141418] py-2 last:border-b-0 ${getLogTone(entry)}`}
										>
											<span className="mr-2 text-[#555]">
						...
						<div className="rounded-xl border border-[#212126] bg-[#0d0d10] p-4">
							...
							<div className="mt-3 grid gap-3 text-[12px] text-[#c3c3c8]">
								...
								<div className="flex items-center justify-between">
									<span>Workers</span>
									<span className="font-semibold text-white">
										{crawlRuntime.activeWorkers}/{crawlRuntime.concurrency}
									</span>
								</div>
								<div className="flex items-center justify-between">
									<span>Utilization</span>
									<span className="font-semibold text-white">
										{Math.round(Number(crawlRuntime.workerUtilization || 0))}%
									</span>
								</div>
								<div className="flex items-center justify-between">
									<span>Max depth seen</span>
									<span className="font-semibold text-white">
						...
						<div className="rounded-xl border border-[#212126] bg-[#0d0d10] p-4">
							...
							<div className="mt-3 flex flex-wrap gap-2 text-[11px] font-semibold">
								...
								<span className="rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1 text-amber-300">
									3xx: {statusCounts.redirects}
								</span>
								<span className="rounded-full border border-red-500/20 bg-red-500/10 px-3 py-1 text-red-300">
									4xx: {statusCounts.broken}
								</span>
								<span className="rounded-full border border-red-700/20 bg-red-700/10 px-3 py-1 text-red-200">
									5xx: {statusCounts.server}
								</span>
