...
function NewProjectForm({ onCreated, onCancel }: NewProjectFormProps) {
	...
	const handleCreate = async () => {
		...
		setLoading(true);
		try {
			const newProject = await projectCtx.addProject(
...
function ProjectSelector() {
	...
	const isActive =
		isCrawling ||
		crawlRuntime.stage === "crawling" ||
		crawlRuntime.stage === "connecting" ||
	...
	const progress = Math.min(
		...
		Math.max(
			0,
			(crawlRuntime.crawled / Math.max(1, crawlRuntime.discovered)) * 100,
		),
	);
	...
	const handleDelete = (e: React.MouseEvent, id: string, name: string) => {
		...
		if (
			confirm(
				`Are you sure you want to delete project "${name}"? This will permanently remove all associated crawl data.`,
			)
		) {
	...
	const handleSaveEdit = async (e: React.MouseEvent, id: string) => {
		...
		const url = editForm.url.startsWith("http")
			? editForm.url
			: `https://${editForm.url}`;
			...
			>
				...
				<span className="flex-1 text-left truncate text-[#ccc] relative z-10">
					{active ? (
						...
						<span className="text-[#555]">Select a project…</span>
					)}
				</span>
				{active?.domain && (
					<span className="text-[10px] text-[#555] truncate max-w-[100px] hidden sm:block relative z-10">
						{active.domain}
					</span>
				)}
				<ChevronDown
							...
							{projects.map((p) => (
								<div key={p.id}>
									{editingProjectId === p.id ? (
										...
										>
											<div
												className={`w-1.5 h-1.5 rounded-full shrink-0 ${p.id === active?.id ? "bg-[#F5364E]" : "bg-[#333]"}`}
											/>
											<span className="flex-1 text-[12px] font-medium truncate">
												{p.name}
											</span>
											<span className="text-[10px] text-[#444] truncate max-w-[80px] hidden sm:block">
												{p.domain}
											</span>
											...
											{p.id === active?.id && (
												<Check size={10} className="shrink-0 text-[#F5364E]" />
											)}
										</div>
									)}
								</div>
...
export default function CrawlerHeader() {
	...
	const isPausedSession =
		!isCrawling && crawlRuntime.stage === "paused" && pages.length > 0;
	const isActiveSession =
		isCrawling ||
		crawlRuntime.stage === "crawling" ||
		crawlRuntime.stage === "connecting";
	const crawlButtonLabel = isActiveSession
		? "Pause Scan"
		: isPausedSession
			? "Resume Scan"
			: "Start Scan";
		...
		<header className="h-[52px] border-b border-[#222] bg-[#141414] flex items-center px-4 justify-between shrink-0 relative z-40">
			<div className="flex items-center gap-5">
				<div className="flex items-center gap-2 relative">
					...
					<svg
						...
						className={`shrink-0 relative z-10 transition-colors duration-500 ${isCrawling ? "text-[#F5364E] drop-shadow-[0_0_12px_rgba(245,54,78,0.8)]" : "text-white drop-shadow-[0_0_8px_rgba(245,54,78,0.3)]"}`}
					>
					...
					<span className="font-bold text-white text-[15px] tracking-tight relative z-10">
						Headlight{" "}
						<span className="text-gray-500 font-medium transition-colors">
							Scanner
						</span>
					</span>
				</div>
			...
			<div className="flex items-center gap-2 ml-4">
				<button
					...
					title={
						isCrawling
							? "Pause the crawl before clearing"
							: "Clear current crawler workspace"
					}
				>
					<X size={11} /> Clear
				</button>
								...
								<div className="relative z-10 flex items-center gap-2 w-full justify-center">
									{analysisRuntime.isAnalyzing ? (
										<div className="flex items-center gap-2">
											...
											<svg
												className="w-3.5 h-3.5 -rotate-90 animate-in fade-in duration-500"
												viewBox="0 0 24 24"
											>
												<circle
												...
												<circle
													...
													strokeDashoffset={
														...
														Math.PI *
														10 *
														(1 - analysisRuntime.progress / 100)
													}
													strokeLinecap="round"
													stroke="currentColor"
													fill="transparent"
										...
										<>
											<Sparkles
												size={11}
												className={
													analysisRuntime.stage === "completed"
														? "text-green-400"
														: "text-white"
												}
												fill="currentColor"
				...
				<div className="flex items-center gap-1 ml-2 border-l border-[#333] pl-3">
					...
					<div className="relative">
						...
						{showShortcuts && (
							<div className="absolute right-0 top-full mt-1 w-[260px] bg-[#111] border border-[#333] rounded-lg shadow-2xl z-[100] p-4 animate-in fade-in slide-in-from-top-2 duration-150">
								<h4 className="text-[11px] font-bold text-[#888] uppercase tracking-wider mb-3">
									Keyboard Shortcuts
								</h4>
								<div className="space-y-2">
									{[
										...
										>
											...
											<kbd className="px-1.5 py-0.5 bg-[#222] border border-[#333] rounded text-[10px] text-[#ccc] font-mono">
												{s.keys}
											</kbd>
										</div>
									))}
					...
					<button
						...
						className={`p-1.5 rounded transition-colors ${showSettings ? "bg-[#333] text-white" : "text-[#888] hover:bg-[#222] hover:text-white"}`}
						title="Configuration"
					>
						<Settings size={14} />
					</button>
				...
				{isAuthenticated ? (
					<div
						className="flex items-center gap-2 px-2 py-1 bg-[#1a1a1a] border border-[#222] rounded ml-1"
						title={user?.email || ""}
					>
						<div className="w-5 h-5 rounded-full bg-[#F5364E]/20 flex items-center justify-center text-[9px] font-bold text-[#F5364E]">
							{(profile?.full_name || user?.email || "?")[0].toUpperCase()}
						</div>
						<span className="text-[10px] text-[#888] max-w-[80px] truncate hidden xl:block">
