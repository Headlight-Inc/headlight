	...
	return (
		<div className="space-y-6 animate-in fade-in duration-200">
			...
			<div className="bg-gradient-to-br from-[#1a1a1a] to-[#111] border border-[#222] rounded-lg p-4 relative overflow-hidden group/health w-full shadow-inner">
				<div
					className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-[40px] -mr-10 -mt-10 transition-colors duration-1000 ${
						healthScore.grade === "A"
							? "bg-green-500/10"
							: healthScore.grade === "B"
								? "bg-blue-500/10"
				...
				></div>
				<div
					className={`absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-${healthScore.grade === "A" ? "green" : healthScore.grade === "B" ? "blue" : healthScore.grade === "F" ? "red" : "yellow"}-500/50 to-transparent`}
				></div>
				{/* Live crawl progress */}
				{isCrawling && (
				...
				<div className="flex items-center gap-4 relative z-10">
					<div className="flex-1">
						<h4 className="text-[11px] font-bold text-[#888] uppercase tracking-widest mb-1">
							Site Health
						</h4>
						<div className="flex items-baseline gap-2">
							<span className="text-3xl font-black text-white">
								{pages.length === 0 ? "--" : healthScore.score}
							</span>
							<span
								className={`text-lg font-black ${healthScore.grade === "A" ? "text-green-400" : healthScore.grade === "B" ? "text-blue-400" : healthScore.grade === "C" ? "text-yellow-400" : healthScore.grade === "D" ? "text-orange-400" : healthScore.grade === "F" ? "text-red-400" : "text-[#555]"}`}
							>
								{healthScore.grade}
							</span>
					...
					<div className="bg-[#141414] p-3 rounded border border-[#222]">
						...
						<div className="text-[16px] font-mono text-white">
							{pages.length > 0
								? Math.round(
										pages.reduce(
											(acc: any, p: any) => acc + (p.loadTime || 0),
											0,
										) / pages.length,
									)
								: 0}
			...
			<div className="bg-[#1a1a1a] border border-[#222] rounded-lg p-4">
				<h4 className="text-[11px] font-bold text-[#F5364E] mb-3 uppercase tracking-widest flex items-center gap-1.5">
					Fix These First
				</h4>
				{pages.length === 0 ? (
					<div className="text-[11px] text-[#666] text-center py-4">
						Scan a website to see what needs fixing.
					</div>
				) : (
					<div className="space-y-3">
						{(() => {
							if (auditInsights.length === 0)
								return (
									<div className="text-[11px] text-green-400 flex items-center gap-2 pb-2">
										<CheckCircle2 size={12} /> Looking good! No big issues
										found.
									</div>
								);
							...
							return auditInsights.slice(0, 5).map((r: any) => (
								...
								>
									<div
										className={`mt-0.5 px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shrink-0 border ${
											r.impact === "High"
									...
									<div className="flex-1 min-w-0">
										<div className="text-[12px] text-[#eee] font-medium leading-tight group-hover/fix:text-white transition-colors">
											{r.label}
										</div>
										<div className="text-[10px] text-[#666] mt-0.5 leading-snug">
											{r.summary}
										</div>
									</div>
					...
					{[
						{
							label: "2xx Success",
							count: pages.filter(
								(p: any) => p.statusCode >= 200 && p.statusCode < 300,
							).length,
							color: "bg-green-500",
						},
						{
							label: "3xx Redirect",
							count: stats?.redirects || 0,
							color: "bg-orange-400",
						},
						{
							label: "4xx Error",
						...
						},
						{
							...
							count: pages.filter((p: any) => p.statusCode >= 500).length,
							color: "bg-red-700",
						},
					].map((stat) => (
						<div
						...
						>
							<div
								className={`w-2 h-2 rounded-full ${stat.color} shrink-0`}
							></div>
