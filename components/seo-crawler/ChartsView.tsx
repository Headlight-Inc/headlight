...
export default function ChartsView() {
	...
	const radarData = useMemo(() => {
		...
		const avgReadability =
			htmlPages.reduce(
				(s: number, p: any) => s + (Number(p.readability) || 0),
				0,
			) / n;
		const avgWordCount =
			htmlPages.reduce((s: number, p: any) => s + (p.wordCount || 0), 0) / n;
		const uniqueRate =
			(1 - htmlPages.filter((p: any) => p.exactDuplicate).length / n) * 100;
		const schemaRate =
			(htmlPages.filter((p: any) => p.schemaTypes && p.schemaTypes.length > 0)
				.length /
				n) *
			100;
		const headingRate = (htmlPages.filter((p: any) => p.h1_1).length / n) * 100;
	...
	const heatmapData = useMemo(() => {
		return pages
			.filter(
				(p: any) =>
					p.contentType?.includes("text/html") && p.statusCode === 200,
			)
