...
import type { Forecast } from '../../../../services/right-sidebar/types'
export function ForecastPill({ f }: { f: Forecast }) {
  const positive = f.deltaValue >= 0
  const good = (f.positiveIsGood ?? true) ? positive : !positive
  const cls = good ? 'border-emerald-700/50 text-emerald-300 bg-emerald-900/20'
                   : 'border-red-700/50 text-red-300 bg-red-900/20'
  return (
    <span className={`inline-flex items-center gap-1 rounded border px-1.5 py-0.5 text-[10px] tabular-nums ${cls}`}>
