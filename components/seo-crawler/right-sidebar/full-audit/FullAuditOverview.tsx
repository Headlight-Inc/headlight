...
import {
	HeroStrip, PillarCard, RecommendedActionsBlock, AlertsBlock,
	BenchmarkBlock, CompareBlock, DrillFooter, EmptyState,
	compactNum, fmtPct, scoreToTone,
} from '../_shared'

export function FullAuditOverview() {
	const { pages, setRsTab } = useSeoCrawler() as any
	...
	const alerts = [
		s.issues.errors > 0 ? { id: 'err', label: `${compactNum(s.issues.errors)} pages with 4xx or 5xx`, tone: 'bad' as const, onClick: () => drill.toCategory('codes', '404 Not Found') } : null,
		s.issues.notIndexable > 0 ? { id: 'idx', label: `${compactNum(s.issues.notIndexable)} pages not indexable`, tone: 'warn' as const, onClick: () => drill.toCategory('indexability', 'Non-Indexable') } : null,
		s.tech.cwvPass < 70 ? { id: 'cwv', label: `Only ${fmtPct(s.tech.cwvPass)} of pages pass Core Web Vitals`, tone: 'warn' as const, onClick: () => drill.toCategory('performance', 'Poor LCP') } : null,
	...
	return (
		<div className="flex flex-col gap-3 p-3">
			...
			<PillarCard pillars={s.pillars.map(p => ({ ...p, onClick: () => setRsTab?.('fullAudit', p.id === 'tech' ? 'tech' : p.id === 'links' ? 'links' : p.id === 'search' ? 'search' : p.id === 'ai' ? 'ai' : 'fixes') }))} />
