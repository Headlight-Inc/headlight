...
import {
	...
	ConnectorStatusBlock, TrendBlock, EmptyState,
...
export function FullAuditOverview() {
	...
	const alerts = [
		s.issues.errors > 0 && { id: 'err', label: `${compactNum(s.issues.errors)} pages with 4xx or 5xx`, tone: 'bad' as const, onClick: () => drill.toCategory('codes', '404 Not Found') },
		s.issues.notIndexable > 0 && { id: 'idx', label: `${compactNum(s.issues.notIndexable)} pages not indexable`, tone: 'warn' as const, onClick: () => drill.toCategory('indexability', 'Non-Indexable') },
		s.tech.cwvPass < 70 && { id: 'cwv', label: `Only ${fmtPct(s.tech.cwvPass)} of pages pass Core Web Vitals`, tone: 'warn' as const, onClick: () => drill.toCategory('performance', 'Poor LCP') },
		s.links.broken > 0 && { id: 'bln', label: `${compactNum(s.links.broken)} broken internal links`, tone: 'warn' as const, onClick: () => drill.toCategory('links', 'Broken Internal') },
		!s.connectors.gsc.connected && { id: 'gsc', label: 'Search Console not connected — search insights are limited', tone: 'info' as const, onClick: () => openSettings?.('connectors') },
		...
		<div className="flex flex-col gap-3 p-3">
			...
			{s.hasPrior && (
				<TrendBlock
					...
					hint={`Now ${s.score} · prev ${s.scorePrev}`}
				/>
			)}
		</div>
	)
