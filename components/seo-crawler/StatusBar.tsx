			...
			<div className="flex items-center gap-4">
							...
							crawlHistory?.length > 0 && (
								<>
									...
									<span className="text-[#555]">
										{crawlHistory.length} Sessions in History
									</span>
								</>
							)
						)}
				...
				{(isCrawling ||
					...
					activeViewType !== "competitor_matrix" && (
						<>
							<span className="text-[#333]">|</span>
							<span className="flex items-center gap-1 font-mono text-[#888]">
								<Clock size={11} className="text-[#444]" /> {elapsedTime}
							</span>
							<span className="text-[#333]">|</span>
							<span className="font-mono text-[#888]">{crawlRate} p/s</span>
							<span className="text-[#333]">|</span>
							<span className="flex items-center gap-1 font-mono text-[#888]">
								<Route size={11} className="text-[#444]" />{" "}
								{crawlRuntime.queued} queued
							</span>
							<span className="text-[#333]">|</span>
							<span className="flex items-center gap-1 font-mono text-[#888]">
								<Cpu size={11} className="text-[#444]" />{" "}
			...
			<div className="flex items-center gap-4 text-[#666]">
				...
				{!isAuthenticated ? (
					<span className="text-orange-400/80">
						Guest Mode ({trialPagesLimit} URL limit)
					</span>
				) : (
				...
				<span className="text-[#444] text-[10px]">
					© 2024 - {new Date().getFullYear()} Headlight SEO
				</span>
			</div>
		</div>
	);
