...
export default function CrawlerSettingsDrawer({
	isOpen,
	onClose,
}: CrawlerSettingsDrawerProps) {
	const {
		urlInput,
		setUrlInput,
		listUrls,
	...
	const onStart = async () => {
		...
		setCrawlingMode(
			localConfig.mode === "single" ? "spider" : (localConfig.mode as any),
		);
	...
	const handleCsvImport = async (event: ChangeEvent<HTMLInputElement>) => {
		...
		try {
			const text = await readFileText(file);
			const urls = parseUrlsFromText(text).filter((u) =>
				/^https?:\/\//i.test(u),
			);
	...
	const renderTab = () => {
		...
		switch (activeTab) {
			...
			case "ai":
				return <AITab {...tabProps} />;
			case "rules":
				return <RulesTab {...tabProps} />;
			case "extraction":
				return <ExtractionTab {...tabProps} />;
			case "scheduling":
				return <SchedulingTab {...tabProps} />;
			case "storage":
				return <StorageTab {...tabProps} />;
			case "api":
			...
			<div className="w-full max-w-6xl h-full bg-[#050505] border-l border-[#222] shadow-2xl flex animate-in slide-in-from-right duration-500 ease-out">
				...
				<div className="w-[220px] bg-[#111] border-r border-[#222] flex flex-col">
					<div className="h-[60px] flex items-center px-5 border-b border-[#222]">
						<h3 className="text-[14px] font-bold text-white flex items-center gap-2">
							<SettingsIcon size={16} className="text-[#F5364E]" />{" "}
							Configuration
						</h3>
					</div>
					...
					<div className="p-3 space-y-1 flex-1 overflow-y-auto">
						{SETTINGS_TABS.map((tab) => (
							...
							>
								<span
									className={
										activeTab === tab.id ? "text-[#F5364E]" : "text-[#666]"
									}
								>
									{tab.icon}
								</span>
				...
				<div className="flex-1 flex flex-col bg-[#050505]">
					...
					<div className="px-8 py-4 border-t border-[#222] bg-[#0a0a0a]/50 flex justify-between items-center">
						<button
							onClick={handleClose}
							className="text-[12px] font-bold text-[#666] hover:text-white transition-colors"
						>
							Discard Setup
						</button>
