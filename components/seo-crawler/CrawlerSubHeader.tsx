				...
				<div className="flex items-center gap-3 overflow-x-auto custom-scrollbar-hidden mr-4">
					...
					<div className="hidden md:flex items-center gap-2 mr-2">
						<div className="relative">
							<select
								value={
									auditFilter.modes.includes("full")
										? "full"
										: auditFilter.modes[0] || "full"
								}
								onChange={(e) => {
						...
						<div className="relative">
							<select
								...
								onChange={(e) => {
									applyAuditMode(
										auditFilter.modes,
										e.target.value as IndustryFilter,
									);
								}}
								className="h-[26px] pl-2 pr-7 bg-[#0a0a0a] border border-[#222] rounded text-[11px] text-[#ccc] focus:outline-none focus:border-[#555] appearance-none cursor-pointer hover:border-[#333] transition-colors"
					...
					{!isCompetitiveMode && (
						<>
							{stats.broken > 0 && (
								<button
									onClick={() => setActiveMacro("broken")}
									className={`px-2.5 py-1 rounded text-[12px] transition-colors shrink-0 ${activeMacro === "broken" ? "bg-red-950/40 text-red-400 border border-red-500/20" : "bg-transparent text-[#888] hover:bg-[#1a1a1a] hover:text-[#ccc]"}`}
								>
									Errors 4xx/5xx
								</button>
							)}
							{stats.redirects > 0 && (
								<button
									onClick={() => setActiveMacro("redirects")}
									className={`px-2.5 py-1 rounded text-[12px] transition-colors shrink-0 ${activeMacro === "redirects" ? "bg-orange-950/40 text-orange-400" : "bg-transparent text-[#888] hover:bg-[#1a1a1a] hover:text-[#ccc]"}`}
								>
									Redirects 3xx
								</button>
							)}
							{stats.slowPages > 0 && (
								<button
									onClick={() => setActiveMacro("slow")}
									className={`px-2.5 py-1 rounded text-[12px] transition-colors shrink-0 ${activeMacro === "slow" ? "bg-orange-950/40 text-orange-400" : "bg-transparent text-[#888] hover:bg-[#1a1a1a] hover:text-[#ccc]"}`}
								>
									Slow Pages
								</button>
							)}
							{stats.nonIndexable > 0 && (
								<button
									onClick={() => setActiveMacro("nonIndexable")}
									className={`px-2.5 py-1 rounded text-[12px] transition-colors shrink-0 ${activeMacro === "nonIndexable" ? "bg-blue-950/40 text-blue-400" : "bg-transparent text-[#888] hover:bg-[#1a1a1a] hover:text-[#ccc]"}`}
								>
									Non-Indexable
								</button>
							)}
					...
					{isCompetitiveMode && (
						<div className="flex items-center gap-2">
								...
								{activeCompetitors.map((comp: any) => {
									...
									return (
										<button
											...
											onClick={() => {
												const next = active
													? activeCompetitorDomains.filter(
															(domain) => domain !== comp.domain,
														)
							...
							>
								<RefreshCw
									size={12}
									className={crawlingCompetitorDomain ? "animate-spin" : ""}
								/>
								{crawlingCompetitorDomain
									? `Crawling ${crawlingCompetitorDomain}...`
									: "Refresh All"}
							</button>
						</div>
					)}
				...
				<div className="flex items-center shrink-0 gap-3">
							...
							{showColumnPicker && (
								<div className="absolute right-0 top-full mt-2 w-[500px] bg-[#111] border border-[#333] rounded-lg shadow-2xl z-[1000] p-4 animate-in fade-in slide-in-from-top-2 duration-150">
									<div className="flex justify-between items-center mb-4 pb-2 border-b border-[#222]">
										<h4 className="text-[11px] font-bold text-[#888] uppercase tracking-wider">
											Show/Hide Columns
										</h4>
										<div className="flex gap-2">
											...
											<button
												...
												className="text-[10px] text-[#555] hover:underline"
											>
												Reset
											</button>
										</div>
									...
									<div className="grid grid-cols-2 gap-x-6 gap-y-2 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
										{[
											...
											<div key={group} className="col-span-2 mb-2">
												<h5 className="text-[10px] text-[#444] font-black uppercase tracking-widest mb-2 border-l-2 border-[#F5364E] pl-2">
													{group}
												</h5>
												<div className="grid grid-cols-2 gap-2">
													{ALL_COLUMNS.filter((c) => c.group === group).map(
														(col) => (
															<label
																key={col.key}
															...
															>
																...
																/>
																<div
																	className={`w-3.5 h-3.5 rounded-sm border transition-colors flex items-center justify-center ${visibleColumns.includes(col.key) ? "bg-[#F5364E] border-[#F5364E]" : "bg-[#0a0a0a] border-[#333] group-hover:border-[#555]"}`}
																>
																...
																<span
																	className={`text-[11px] truncate ${visibleColumns.includes(col.key) ? "text-[#eee]" : "text-[#666] group-hover:text-[#aaa]"}`}
																>
																	{col.label}
																</span>
															</label>
					...
					{isGridView && (
						<button
							...
							onClick={() => {
								const missingMeta = pages.filter(
									(p: any) => !p.metaDesc || p.metaDesc.trim() === "",
								);
								setAutoFixItems(
									missingMeta.map((p: any) => ({
										...p,
										fixStatus: "pending",
										generatedMeta: "",
									})),
						...
						>
							<Tag
								size={12}
								className="group-hover:text-[#F5364E] text-[#888]"
							/>{" "}
							Auto-Fix
						</button>
					)}
					...
					<div className="relative w-48">
						...
						/>
						<input
							id="headlight-grid-search"
							type="text"
							value={searchQuery}
							onChange={(e) => setSearchQuery(e.target.value)}
							placeholder="Search... (⌘F)"
							className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded pl-7 pr-3 py-1 text-[12px] text-[#e0e0e0] focus:border-[#F5364E] focus:outline-none transition-colors"
						/>
					</div>
				</div>
			</div>
