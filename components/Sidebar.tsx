...
const HeadlightLogo = ({ collapsed }: { collapsed?: boolean }) => (
	...
	>
		<svg
			...
			className="shrink-0 text-white drop-shadow-[0_0_8px_rgba(245,54,78,0.5)]"
		>
		...
		{!collapsed && (
			<div className="flex flex-col justify-center transition-opacity duration-300">
				<span className="text-white font-extrabold text-lg tracking-wider uppercase font-heading">
					Headlight
				</span>
			</div>
		)}
...
const SidebarItem = ({
	...
	return (
		...
		>
			<div
				className={`transition-transform duration-200 ${active ? "scale-110 text-brand-red" : "group-hover:scale-110"}`}
			>
				{icon}
			</div>
			{!collapsed && (
				<span
					className={`text-sm font-medium ${active ? "text-white" : ""} truncate`}
				>
					{label}
				</span>
			)}
			{hasNotification && (
				<div
					className={`absolute w-2 h-2 rounded-full bg-brand-red border-2 border-[#0A0A0A] ${collapsed ? "top-2 right-2" : "right-3 top-1/2 -translate-y-1/2"}`}
				></div>
			)}
			{badge && !collapsed && (
				<span
					className={`ml-auto text-[9px] uppercase font-bold px-1.5 py-0.5 rounded ${
						active ? "bg-white/20 text-white" : "bg-brand-red/10 text-brand-red"
					}`}
				>
					{badge}
				</span>
...
export const Sidebar: React.FC<SidebarProps> = ({
	...
	const [newProjectIndustry, setNewProjectIndustry] =
		useState<IndustryType>("saas");
	...
	return (
		...
		>
			...
			<div
				className={`pt-4 pb-2 flex flex-col gap-2 ${isCollapsed ? "px-2 items-center" : "px-4"}`}
			>
				{/* Logo Area */}
				<div
					className={`h-10 flex items-center ${isCollapsed ? "justify-center" : ""}`}
				>
				...
				<div className="relative mt-2" ref={projectMenuRef}>
					<button
						onClick={() =>
							!isCollapsed && setIsProjectMenuOpen(!isProjectMenuOpen)
						}
						className={`w-full flex items-center gap-3 bg-[#151515] border border-white/10 rounded-xl hover:border-white/20 hover:bg-[#1A1A1A] transition-all group ${
							isCollapsed ? "p-2 justify-center h-12" : "p-3"
						}`}
					>
						...
						{!isCollapsed && (
							<div className="flex-1 text-left min-w-0">
								...
								<p className="text-[10px] text-gray-500 truncate">
									{activeProject ? activeProject.url : "Add to start"}
								</p>
							</div>
						)}
						{!isCollapsed && (
						...
						<div className="absolute top-full left-0 w-full mt-2 bg-[#1A1A1A] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
							<div className="p-2 border-b border-white/5">
								<input
									...
									className="w-full bg-black/20 text-xs text-white p-2 rounded-lg border border-white/5 focus:outline-none focus:border-brand-red/50"
								/>
							</div>
							<div className="max-h-48 overflow-y-auto scrollbar-thin">
								{projects.map((p) => (
									<button
										...
										onClick={() => {
											navigate(
												`/project/${p.id}/dashboard?view=${currentView}`,
											);
											setIsProjectMenuOpen(false);
										}}
				...
				<div className="flex flex-col gap-1">
					{!isCollapsed && (
						<div className="px-4 mb-1 animate-in fade-in duration-300">
							<span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest font-heading">
								Core
							</span>
						</div>
					)}
				...
				<div className="flex flex-col gap-1">
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=keyword_research`,
								);
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=content_predictor`,
								);
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=competitors`,
								);
				...
				<div className="flex flex-col gap-1">
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=rank_tracker`,
								);
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=mentions`,
								);
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=site_audit`,
								);
				...
				<div className="flex flex-col gap-1">
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=opportunities`,
								);
					...
					<SidebarItem
						...
						label="Agency Hub"
						active={currentView === "agency_hub"}
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=agency_hub`,
								);
					...
					<SidebarItem
						...
						onClick={() => {
							if (activeProject)
								navigate(
									`/project/${activeProject.id}/dashboard?view=automation`,
								);
			...
			<div className="p-3 bg-[#0A0A0A] border-t border-white/[0.08] flex flex-col gap-1">
				<SidebarItem
					...
					onClick={() => {
						if (activeProject)
							navigate(
								`/project/${activeProject.id}/dashboard?view=settings_project`,
							);
				...
				<SidebarItem
					...
					onClick={() => {
						if (activeProject)
							navigate(
								`/project/${activeProject.id}/dashboard?view=settings_account`,
							);
					...
					<div className="mt-3 bg-[#111] rounded-xl border border-white/5 p-3 flex flex-col gap-2">
						<div className="flex justify-between items-end">
							...
							<span className="text-xs text-white font-bold">
								4.2k <span className="text-gray-600">/ 5k</span>
							</span>
						</div>
				...
				<div
					className={`mt-2 flex items-center gap-3 px-1 py-1.5 rounded-lg hover:bg-white/5 transition-all group ${isCollapsed ? "justify-center" : ""}`}
				>
